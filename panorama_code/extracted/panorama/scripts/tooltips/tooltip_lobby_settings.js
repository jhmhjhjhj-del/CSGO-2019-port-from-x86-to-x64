...
var TooltipLobby = ( function ()
{
	...
	var _GetLobbyStatistics = function ()
	{
		...
		var MakeStatsRow = function ( statType, iconName )
		{
			...
			var nStat = ( matchmakeingStats && typeof matchmakeingStats[ statType ] === 'number' )
				? matchmakeingStats[ statType ]
				: 0;

			if( statType === 'avgSearchTimeSeconds' )
			{
				var time = FormatText.SecondsToDDHHMMSSWithSymbolSeperator( nStat );
				p.SetDialogVariable( 'stat', time );
			}
			else
				p.SetDialogVariableInt( 'stat', nStat );
