...
var MockAdapter = ( function()
{
	...
	function _BuildAllPlayersMatchDataJSO()
	{
		var allplayerdata = [];
		var oTeams = GameStateAPI.GetPlayerDataJSO();
		if ( typeof oTeams === 'string' )
		{
			try { oTeams = oTeams ? JSON.parse( oTeams ) : {}; }
			catch ( e ) { oTeams = {}; }
		}
		if ( !oTeams )
			oTeams = {};

		for ( var teamName in oTeams )
		{
			if ( !oTeams.hasOwnProperty( teamName ) )
				continue;

			var teamPlayers = oTeams[ teamName ];
			for ( var slot in teamPlayers )
			{
				if ( !teamPlayers.hasOwnProperty( slot ) )
					continue;

				var xuid = teamPlayers[ slot ];
				if ( !xuid || xuid === '0' )
					continue;

				var entindex = GameStateAPI.GetPlayerIndex( xuid );
				allplayerdata.push( {
					xuid: String( xuid ),
					name: GameStateAPI.GetPlayerName( xuid ),
					teamnumber: String( GameStateAPI.GetPlayerTeamNumber( xuid ) ),
					teamcolor: GameStateAPI.GetPlayerColor( xuid ) || "",
					isbot: GameStateAPI.IsFakePlayer( xuid ),
					entindex: String( entindex >= 0 ? entindex : 0 ),
					score: GameStateAPI.GetPlayerScore( xuid )
				} );
			}
		}

		return { allplayerdata: allplayerdata };
	}
	...
	return {
		...
		GetPlayerNameWithNoHTMLEscapes: function GetPlayerNameWithNoHTMLEscapes ( xuid ) { if ( typeof GameStateAPI.GetPlayerNameWithNoHTMLEscapes !== 'function' ) return GameStateAPI.GetPlayerName( xuid ); return _APIAccessor( GameStateAPI.GetPlayerNameWithNoHTMLEscapes( xuid ), k_GetPlayerName, xuid );},
		...
		GetPlayerStatsJSO: function _GetPlayerStatsJSO ( xuid )
		{
			var val = _APIAccessor( MatchStatsAPI.GetPlayerStatsJSO( xuid ), k_GetPlayerStatsJSO, xuid );
		...
		GetTeamNextRoundLossBonus: function _GetTeamNextRoundLossBonus ( team ) { if ( typeof GameStateAPI.GetTeamNextRoundLossBonus !== 'function' ) return 0; return _APIAccessor( GameStateAPI.GetTeamNextRoundLossBonus( team ), k_GetTeamNextRoundLossBonus );},
		...
		GetMusicIDForPlayer: function _GetMusicIDForPlayer ( xuid ) { if ( typeof GameStateAPI.GetMusicIDForPlayer === 'function' ) return _APIAccessor( GameStateAPI.GetMusicIDForPlayer( xuid ), k_GetMusicIDForPlayer, xuid ); return InventoryAPI.GetMusicIDForPlayer( xuid );},
		GetPlayerColor: function _GetPlayerColor ( xuid ) {return _APIAccessor( GameStateAPI.GetPlayerColor( xuid ), k_GetPlayerColor, xuid );},
		HasCommunicationAbuseMute: function _HasCommunicationAbuseMute ( xuid ) { if ( typeof GameStateAPI.HasCommunicationAbuseMute !== 'function' ) return false; return _APIAccessor( GameStateAPI.HasCommunicationAbuseMute( xuid ), k_HasCommunicationAbuseMute );},
		...
		GetCooperativePlayerTeamName: function _GetCooperativePlayerTeamName () { if ( typeof GameStateAPI.GetCooperativePlayerTeamName !== 'function' ) return 'CT'; return _APIAccessor( GameStateAPI.GetCooperativePlayerTeamName(), k_GetCooperativePlayerTeamName );},
		GetAllPlayersMatchDataJSO: function _GetAllPlayersMatchDataJSO ()
		{
			var data = ( typeof GameStateAPI.GetAllPlayersMatchDataJSO === 'function' ) ? GameStateAPI.GetAllPlayersMatchDataJSO() : undefined;
			if ( data && data.allplayerdata && data.allplayerdata.length > 0 )
				return _APIAccessor( data, k_GetAllPlayersMatchDataJSO );
			return _BuildAllPlayersMatchDataJSO();
		},
