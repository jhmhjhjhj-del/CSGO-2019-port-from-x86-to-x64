...
var PopupAddFriend = ( function(){
	...
	var _OnEntrySubmit = function ()
	{
		...
		{
			FriendsListAPI.RequestFriendProfileUpdateFromScript( xuid );

			var _RefreshLookupTile = function( tries )
			{
				var elTile = $.GetContextPanel().FindChildTraverse( 'JsPopupFriendTile' );
				if ( elTile && elTile.IsValid() )
				{
					friendTile.Init( elTile );
					var nm = FriendsListAPI.GetFriendName( xuid );
					if ( nm && nm !== '...' && nm !== '_' )
						return;
				}
				if ( tries > 0 )
					$.Schedule( 0.35, function() { _RefreshLookupTile( tries - 1 ); } );
			};
	...
	var _FriendsListUpdateName = function( xuid )
	{
		...
		if ( elTile && elTile.IsValid() && ( !xuid || xuid === elTile.GetAttributeString( 'xuid', '' ) ) )
		{
			friendTile.Init( elTile ); 
		}
	};
