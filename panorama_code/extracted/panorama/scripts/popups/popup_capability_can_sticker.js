...
var CapabilityCanSticker = ( function()
{
	...
	var _SetItemModel = function( toolId, itemId, isRemoveStickers )
	{
		...
		try
		{
			modelPath = ItemInfo.GetModelPathFromJSONOrAPI( itemId );
		}
		catch ( eModel ) {}
		if ( !modelPath )
			modelPath = 'img://inventory_' + itemId;
		modelPath = modelPath + '?stickers';

		elPanel.SetAsActivePreviewPanel();
		try
		{
			// Fullscreen sticker UI — 1024 composite looks soft on painted skins.
			if ( typeof elPanel.SetEconItemTextureSize === 'function' )
				elPanel.SetEconItemTextureSize( 2048 );
		}
		catch ( eSize ) {}
		try
		{
	...
	var _ShowStickerIconsToApplyOrRemove = function( isRemoveStickers, toolId, itemId )
	{
		...
		if ( isRemoveStickers )
		{
			...
			for ( var i = 0; i < slotCount; i++ )
			{
				...
				// Strip size suffixes — JS always appends `_large.png`.
				if ( imagePath.length > 6 &&
					( imagePath.substring( imagePath.length - 6 ) === '_large' ||
					  imagePath.substring( imagePath.length - 6 ) === '_small' ) )
				{
					imagePath = imagePath.substring( 0, imagePath.length - 6 );
				}

				// Do not use image path as panel id (slashes break CreatePanel).
				var elSticker = $.CreatePanel( 'Button', elStickersToRemove, 'ScrapeSlot_' + i );
				elSticker.BLoadLayoutSnippet( 'ScrapeStickerBtn' );
				var elImg = elSticker.FindChildInLayoutFile( 'ScrapeStickerImage' );
				if ( elImg )
				{
					var src = 'file://{images_econ}/' + imagePath + '_large.png';
					elImg.SetImage( src );
				}
	...
	var _OnContinue = function( elContinueBtn )
	{
		...
		$.GetContextPanel().SetAttributeString( 'selectedstickerslot', String( _GetSelectedStickerSlot() ) );
