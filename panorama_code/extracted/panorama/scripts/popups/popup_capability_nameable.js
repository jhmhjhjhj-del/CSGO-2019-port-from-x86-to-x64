...
var CapabilityNameable = ( function()
{
	...
	var _SetUpButtonStates = function( toolId, itemId, hasName, noTool )
	{
		...
		// Remove is a separate context-menu action — hide the minus in rename UI.
		m_elRemoveConfirm.SetHasClass( 'hidden', true );
		m_elTextEntry.SetFocus();
	...
	var _OnRemoveConfirm = function( itemId )
	{
		UiToolkitAPI.ShowGenericPopupOkCancel(
			$.Localize( '#popup_nameable_remove_confirm_title' ) || 'Remove Name Tag',
			$.Localize( '#tooltip_nameable_remove' ) || 'Remove the custom name from this item?',
			'',
			function()
			{
				try
	...
	{
		var text = m_elTextEntry ? ( m_elTextEntry.text || '' ) : '';
		var isValid = false;
		try
		{
			isValid = InventoryAPI.SetNameToolString( text ) === true;
		}
		catch ( e )
		{
			isValid = false;
		}
		// Offline / stub: API used to return void — never leave enabled as undefined.
		if ( typeof isValid !== 'boolean' )
			isValid = ( text.replace( /^\s+|\s+$/g, '' ).length > 0 && text.length <= 20 );

		m_elValidBtn.enabled = !!isValid;

		var onMouseOver = function( valid )
		{
			if ( !valid )
	...
	var _ClosePopup = function()
	{
		...
		// Remove-name mode hides the async bar (no name-tag tool) — still must close.
		if ( elAsyncActionBarPanel && !elAsyncActionBarPanel.BHasClass( 'hidden' ) )
		{
			InspectAsyncActionBar.OnEventToClose();
			return;
		}
		if ( elPurchase && !elPurchase.BHasClass( 'hidden' ) )
		{
