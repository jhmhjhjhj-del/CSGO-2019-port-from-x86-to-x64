...
var CapabilityDecodable = ( function()
{
	...
	var _UpdateScrollResultTile = function( numericType, type, itemId )
	{
		...
		if ( type === "crate_unlock" || type === 'graffity_unseal' )
		{
			...
			{
				// Offline: notification arrives before/during countdown — keep id, don't skip anim.
				return;
			}
			else
			{
	...
	var _ShowUnlockAnimation = function()
	{
		...
		{
			// Graffiti/spray has no loot list; offline UseTool already filled m_itemFromContainer
			// before this event — always run countdown instead of instant inspect.
			if ( _GetContainerType( m_caseId ) === 'graffiti' )
			{
				_SetUpCaseOpeningCountdown();
			}
			else if ( InventoryAPI.IsValidItemID( m_itemFromContainer ) )
			{
				_ShowInspect();
			}
			else
	...
	var _ClosePopUp = function()
	{
		...
		// Always dismiss — do not depend on which footer bar is visible (ESC / Закрыть).
