...
var InpsectPurchaseBar = ( function()
{
	...
	var _Init = function( elPanel, itemId, funcGetSettingCallback )
	{
		...
		var hasStorePrice = !!ItemInfo.GetStoreOriginalPrice( m_itemid, 1 );
		...
		var elDropdown = elPanel.FindChildInLayoutFile( 'PurchaseCountDropdown' );
		var elBtn = elPanel.FindChildInLayoutFile( 'PurchaseBtn' );

		if ( !hasStorePrice )
		{
			// Offline / no Steam store: upsell stays on PurchaseItemName (html Label).
			// Do NOT put #popup_capability_upsell on TextButton — it has no html=true.
			if ( elDropdown )
				elDropdown.visible = false;
			if ( elBtn )
			{
				elBtn.enabled = false;
				elBtn.text = '#popup_decodeable_button';
