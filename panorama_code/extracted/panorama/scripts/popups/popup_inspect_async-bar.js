...
var InspectAsyncActionBar = ( function()
{
	...
	var _PerformAsyncAction = function( funcGetSettingCallback, funcCallbackOnAction )
	{
		...
		if ( m_worktype === 'can_sticker' )
		{
			...
			var selectedStickerSlot = parseInt( funcGetSettingCallback( 'selectedstickerslot', '0' ), 10 );
			if ( isNaN( selectedStickerSlot ) )
				selectedStickerSlot = 0;

			var bIsValid = false;
			try
			{
				bIsValid = InventoryAPI.SetStickerToolSlot( m_itemid, selectedStickerSlot ) === true;
			}
			catch ( eSlot )
			{
				bIsValid = false;
