...
var MainMenuStore = ( function()
{
	...
	var _GetStoreItems = function( itemsByCategory )
	{
		...
		for ( var i = 0; i < count; i++ )
		{
			...
			var PaintIdx = StoreAPI.GetBannerEntryPaintIdx( i );
			var FauxItemId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( ItemId, PaintIdx || 0 );
			...
			else if ( ItemInfo.ItemHasCapability( FauxItemId, 'decodable' ) &&
				!InventoryAPI.IsTool( FauxItemId ) )
			{
				if ( !itemsByCategory.cases )
					itemsByCategory.cases = [];
				itemsByCategory.cases.push( FauxItemId );
			}
			else
			{
	...
	var _FillOutItemData = function( elItem, id, type )
	{
		...
		// Offline store sells the case/key itself. Do NOT preview loot[0] skin —
		// that made a case tile look like you were buying the skin.
		elImage.itemid = id;
	...
	var _SortTabs = function ()
	{
		...
		NewPostition( tabList.find(function (obj) { return obj.id === 'cases'; } ) );
	...
	var _OnActivateStoreItem = function( elItem, id, type )
	{
		...
		{
			elItem.SetPanelEvent( 'onactivate', _ShowDecodePopup.bind( undefined, id, id ) );
		}
		else
	...
	{
		// Offline: always show local wallet (retail only showed Perfect World balance).
		var balance = StoreAPI.GetAccountWalletBalance();
