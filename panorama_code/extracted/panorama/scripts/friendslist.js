...
var friendsList = (function() {
	...
	var _SetLocalPlayerAvatar = function ()
	{
		...
		if( newPanel )
		{
			var elName = newPanel.FindChildInLayoutFile( 'JsPlayerName' );
			if ( elName )
			{
				var nick = ( typeof StubLocalProfile !== 'undefined' )
					? StubLocalProfile.GetDisplayName( xuid )
					: FriendsListAPI.GetFriendName( xuid );
				if ( nick )
					elName.text = nick;
			}
			var elAvatar = newPanel.FindChildInLayoutFile( 'JsPlayerCardAvatar' );
			if ( elAvatar && typeof Avatar !== 'undefined' )
		...
		var _BindLocalAvatarClick = function( tries )
		{
			if ( !newPanel || !newPanel.IsValid() )
				return;
			var elAvatar = newPanel.FindChildInLayoutFile( 'JsPlayerCardAvatar' );
			if ( elAvatar )
			{
			...
			if ( tries > 0 )
				$.Schedule( 0.05, function() { _BindLocalAvatarClick( tries - 1 ); } );
		};
		_BindLocalAvatarClick( 24 );
	}
	...
	var _AddOpenPlayerCardAction = function ( elAvatar, xuid ) {
		...
		var openCard = function ( xuid )
		{
			...
			if ( xuid !== 0 ) {
				...
				if ( contextMenuPanel && contextMenuPanel.IsValid && contextMenuPanel.IsValid() )
					contextMenuPanel.SetAttributeString( 'xuid', xuid );
				if ( contextMenuPanel )
	...
	{
		if ( !_m_tabs || !_m_tabs.length )
			return;
		if ( tabIndex < 0 || tabIndex >= _m_tabs.length )
			return;

		if ( _m_activeTabIndex !== tabIndex )
		{
			var prev = _m_tabs[ _m_activeTabIndex ];
			if ( prev && prev.elContent )
				prev.elContent.AddClass( 'hidden' );
		}

		_m_activeTabIndex = tabIndex;
		var tab = _m_tabs[ tabIndex ];
		if ( !tab || !tab.elContent )
			return;
		tab.elContent.RemoveClass( 'hidden' );
...
(function()
{
	...
	$.RegisterForUnhandledEvent( 'PanoramaComponent_MyPersona_NameChanged', friendsList.SetLocalPlayerAvatar );
	$.RegisterForUnhandledEvent( 'StubProfile_Updated', friendsList.SetLocalPlayerAvatar );
