...
var PlayMenu = ( function()
{
	...
	var _SyncDialogsFromSessionSettings = function( settings )
	{
		...
		// Offline workshop mapgroups are workshop/<id>/<map> or …@workshop/<id>.
		// Without @workshop the old check cleared m_isWorkshop and showed official modes+maps.
		m_isWorkshop = _IsWorkshopMapGroupName( settings.game.mapgroupname );
		...
		// Do not SetSelected here — collapses an open play-type dropdown.
	...
	var m_bSuppressPlayDropDownSelect = false;

	var _UpdatePlayDropDown = function()
	{
		var el = $( '#PlayTopNavDropdown' );
		if ( !el )
			return;

		if ( m_bSuppressPlayDropDownSelect )
			return;

		var wantId = ( m_activeMapGroupSelectionPanelID === k_workshopPanelId )
			? 'PlayWorkshop'
			: ( 'Play-' + m_serverSetting );

		var cur = el.GetSelected();
		if ( cur )
		{
			// Prefer data-type — training id is PlayTraining, not Play-training.
			var curType = cur.GetAttributeString( 'data-type', '' );
			if ( m_activeMapGroupSelectionPanelID === k_workshopPanelId )
			{
				if ( curType === 'workshop' || cur.id === 'PlayWorkshop' )
					return;
			}
			else if ( curType === m_serverSetting || cur.id === wantId )
			{
				return;
			}
		}

		el.SetSelected( wantId );
	};
	...
	var _RefreshMatchmakingUi = function()
	{
		// Full enable/disable for maps + modes + GO, but never collapse play-type dropdown.
		m_bSuppressPlayDropDownSelect = true;
		try
		{
			_SyncDialogsFromSessionSettings( LobbyAPI.GetSessionSettings() );
		}
		catch ( e ) {}
		m_bSuppressPlayDropDownSelect = false;
	};
	...
	var _SessionSettingsUpdate = function( sessionState ) 
	{
		...
		{
			_RefreshMatchmakingUi();
		}
		else if ( sessionState === "searching" ||
		          sessionState === "stop" ||
		          sessionState === "gc_update" ||
		          sessionState === "accept" ||
		          sessionState === "idle" ||
	...
	var _LazyCreateWorkshopTab = function()
	{
		...
		for ( var idxMap = 0; idxMap < numMaps; ++idxMap )
		{
			...
			{
				try { mapInfo = JSON.parse( mapInfo ); } catch ( e ) { mapInfo = null; }
			}
	...
	var _PlayTopNavDropdownChanged = function()
	{
		...
		{
			// DropDown can re-fire oninputsubmit when opened with the same entry.
			// Re-applying session settings collapses the menu and spam-sets bot difficulty.
			if ( !m_isWorkshop && m_serverSetting === playType )
				return;
	...
	var _UpdateBotDifficultyButton = function()
	{
		...
		if ( !bShowBotDifficultyButton )
			return;

		var botDiff = GameInterfaceAPI.GetSettingString( 'player_botdifflast_s' );
		var curSel = elDropDown.GetSelected();
		if ( curSel && String( curSel.id ) === String( botDiff ) )
			return;
	...
	var _IsWorkshopMapGroupName = function( mapgroupname )
	{
		if ( !mapgroupname )
			return false;
		if ( mapgroupname.includes( '@workshop' ) )
			return true;
		// Offline scan format: workshop/<publishedfileid>/<map>
		return /(^|,)workshop\//.test( mapgroupname );
	};
	...
	var _SetPlayDropdownToWorkshop = function()
	{
		...
		m_serverSetting = 'listen';
		m_isWorkshop = true;
		// Always show workshop list / hide mode radios first — ApplySessionSettings
		// syncs back and used to wipe m_isWorkshop when mapgroup lacked @workshop.
