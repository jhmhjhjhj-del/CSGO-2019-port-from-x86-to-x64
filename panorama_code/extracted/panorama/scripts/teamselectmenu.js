...
var TeamSelectMenu = ( function (){
	...
	function _ShowPanel( bShow )
	{
		// CSGOShowTeamSelectMenu(false) must NOT run show logic — HidePanel / skip path
		// used to DispatchEvent(false) into this handler and recurse until PANEL_STACK overflow.
		if ( bShow === false )
			return;
		...
		// Only skip when server force-assigns (or retakes exception below).
		// Do NOT skip solely because !IsListenServerHost — that path did jointeam +
		// DispatchEvent(false) and blew Panorama context stack on AutoMM dedicated.
		try
		{
			var skipAutoAssign = false;
			var isRetakes = false;
			try
			{
				isRetakes = ( GameInterfaceAPI.GetSettingString( 'game_type' ) === '0'
					&& GameInterfaceAPI.GetSettingString( 'game_mode' ) === '4' );
			}
			catch ( eRet ) {}
			if ( !isRetakes )
			{
				try
				{
					if ( GameInterfaceAPI.GetSettingString( 'mp_force_assign_teams' ) === '1' )
						skipAutoAssign = true;
				}
				catch ( eForce ) {}
			}
			if ( skipAutoAssign )
			{
				GameInterfaceAPI.ConsoleCommand( 'jointeam 0 1' );
				// Do not DispatchEvent(CSGOShowTeamSelectMenu, false) — re-enters this handler.
				return;
			}
		}
		catch ( e ) {}
