...
var ItemContextEntires = ( function (){
	...
	var _Entries = [
		...
		},
		{
			...
			populateFilter: ['inspect', 'loadout'],
			AvailableForItem: function ( id ) {
				if ( !LoadoutAPI.IsLoadoutAllowed() )
					return false;
		...
		},
		{
			...
			OnSelected: function( id )
			{
				...
				if ( !isMusicvolumeOn )
				{
					$.DispatchEvent( 'ShowResetMusicVolumePopup', id );
				}
				else
				{
		...
		},
		{
			...
			AvailableForItem: function ( id ) {
				...
				// Cases/capsules are containers (open_package), not nametag-style Use Item.
				var toolType = InventoryAPI.GetToolType( id );
				if ( toolType === 'supply_crate' ) return false;
				if ( !InventoryAPI.IsTool( id ) ) return false;
				// GetItemAttributeValue returns "" when missing — "" is not undefined in JS.
		...
		},
		{
			...
			OnSelected: function( id )
			{
				...
				{
					// Keys pick a case; cases/capsules open decodable UI directly.
					// supply_crate is a schema "tool" but must not take the key path.
					var toolType = InventoryAPI.GetToolType( id );
					if ( toolType === 'decoder_ring' )
					{
		...
		},
		{
			...
			OnSelected:  function ( id ) {
				...
				{
					// Offline: no store — don't open empty rename UI. Tell the player.
					// Keep context menu alive until OK (callbacks bind to that panel).
					UiToolkitAPI.ShowGenericPopupOk(
						$.Localize( '#inv_context_nameable' ) || 'Переименовать',
						$.Localize( '#inv_context_nameable_no_tool' ) || 'У вас нет именного ярлыка.',
						'',
						function()
						{
							$.DispatchEvent( 'ContextMenuEvent', '' );
		...
		{
			// Separate from Rename — remove custom name without opening name-tag apply UI.
			name: 'remove_nameable',
			AvailableForItem: function ( id ) {
				try
				{
					return ItemInfo.ItemHasCapability( id, 'nameable' ) && InventoryAPI.HasCustomName( id );
				}
				catch ( e )
				{
					return false;
			...
			OnSelected: function ( id ) {
				// Do NOT close the context menu before ShowGenericPopupOkCancel —
				// Ok/Cancel callbacks are bound to that panel; closing first →
				// "Context panel for function no longer exists".
				var itemId = String( id );
				UiToolkitAPI.ShowGenericPopupOkCancel(
					$.Localize( '#SFUI_InvUse_Reset_nameable' ) || 'Remove Name',
					$.Localize( '#SFUI_InvUse_Reset_nameable_Warning' ) || 'Remove the name tag from this item?',
					'',
					function()
					{
						$.DispatchEvent( 'ContextMenuEvent', '' );
						...
						$.DispatchEvent( 'ShowAcknowledgePopup', 'nametag_remove', itemId );
					},
					function()
					{
						$.DispatchEvent( 'ContextMenuEvent', '' );
	...
	var EquipItem = function( id, team, slot )
	{
		...
		{
			UiToolkitAPI.ShowGenericPopupOk(
				$.Localize( '#SFUI_InvPanel_Loadout_Title' ),
				$.Localize( '#tooltip_loadout_disabled' ),
				'',
				function() {}
			);
			return;
