...
var PartyMenu = ( function()
{
	...
	var _UpdateMembersList = function( lobbySettings, numPlayersActuallyInParty )
	{
		...
		{
			// Keep as string — never Number(steamID) (precision loss → empty nick / wrong avatar).
			var xuid = '';
			if ( i < numPlayersActuallyInParty )
			{
				try { xuid = '' + ( PartyListAPI.GetXuidByIndex( i ) || '' ); }
				catch ( eX ) { xuid = ''; }
			}
			...
			if ( i < numPlayersActuallyInParty )
			{
				elPartyMemberCurrent = _MakeNewPartyMemberTile( "PartyMember" + i, xuid, i );
	...
	var _MakeNewPartyMemberTile = function( panelIdToLoad, xuid, memberIndex )
	{
		...
		// Stable panel id — do not use raw steamid (can break FindChild / Avatar).
		var avatarId = 'PartyAvatar' + ( memberIndex !== undefined ? memberIndex : 0 );
		var elAvatar = $.CreatePanel( "Panel", memberBtn, avatarId );
		elPartyMember.Data().avatarId = avatarId;
	...
	{
		var avatarId = elPartyMember.Data().avatarId;
		var elAvatar = avatarId
			? elPartyMember.FindChildInLayoutFile( avatarId )
			: null;
		if ( !elAvatar )
			return;
	...
	var _IsLocalPartyXuid = function( xuid )
	{
		try
		{
	...
	var _IsBadNick = function( nick )
	{
		return !nick || nick === '' || nick === 'unnamed' || nick === '...' || nick === 'Player';
	};

	var _partyNickLogLeft = 8;
	...
	var _SetPartyMemberName = function( elPartyMember, xuid )
	{
		...
		var sx = '' + xuid;
		var myXuid = '';
		try { myXuid = '' + MyPersonaAPI.GetXuid(); } catch ( eMy ) {}
		var bLocal = _IsLocalPartyXuid( sx );
		var nick = '';
		var src = 'none';
		var stubN = '', partyN = '', frN = '', cvarN = '';

		if ( bLocal )
		{
			try
			{
				if ( typeof StubLocalProfile !== 'undefined' && StubLocalProfile.GetDisplayName )
					stubN = StubLocalProfile.GetDisplayName( sx ) || '';
			}
			catch ( eStub ) {}
			if ( !_IsBadNick( stubN ) ) { nick = stubN; src = 'stub'; }
			if ( _IsBadNick( nick ) )
			{
				try
				{
					if ( typeof PartyListAPI !== 'undefined' && PartyListAPI.GetPartyMemberSetting )
						partyN = PartyListAPI.GetPartyMemberSetting( sx, 'name' ) || '';
				}
				catch ( eParty ) {}
				if ( !_IsBadNick( partyN ) ) { nick = partyN; src = 'party'; }
			}
			if ( _IsBadNick( nick ) )
			{
				try { frN = FriendsListAPI.GetFriendName( sx ) || ''; }
				catch ( eFr ) {}
				if ( !_IsBadNick( frN ) ) { nick = frN; src = 'friends'; }
			}
			if ( _IsBadNick( nick ) )
			{
				try
				{
					if ( typeof GameInterfaceAPI !== 'undefined' )
						cvarN = GameInterfaceAPI.GetSettingString( 'name' ) || '';
				}
				catch ( eName ) {}
				if ( !_IsBadNick( cvarN ) ) { nick = cvarN; src = 'cvar'; }
			}
			if ( _IsBadNick( nick ) )
			{
				nick = 'Player';
		...
		if ( _partyNickLogLeft > 0 )
		{
			_partyNickLogLeft--;
			$.Msg( '[PartyNick] xuid=' + sx + ' my=' + myXuid + ' local=' + ( bLocal ? 1 : 0 )
				+ ' src=' + src + ' nick=' + nick
				+ ' stub=' + stubN + ' party=' + partyN + ' fr=' + frN + ' cvar=' + cvarN );
		}

		elName.text = nick;
	};

	var _OnLocalNameChanged = function()
	{
		_RefreshPartyMembers();
	};
	...
	{	
		...
		try
		{
			if ( typeof StubAutoMm !== 'undefined' && StubAutoMm.GetOfficialServerWarning )
				serverWarning = StubAutoMm.GetOfficialServerWarning() || '';
		}
		catch ( e ) {}
	...
	var _ShowMatchAcceptPopUp = function( map )
	{
		try
		{
			var go = LobbyAPI.GetStubAutoMmGo();
			if ( go && go.indexOf( 'accept|' ) === 0 )
			{
				if ( typeof StubAutoMm !== 'undefined' && StubAutoMm.OnSessionUpdate )
				{
					StubAutoMm.OnSessionUpdate( 'server_reserved' );
					return;
				}
			}
		}
	...
	return {
		...
		OnLocalNameChanged: _OnLocalNameChanged
	};
} )();
...
(function()
{
	...
	$.RegisterForUnhandledEvent( 'PanoramaComponent_MyPersona_NameChanged', PartyMenu.OnLocalNameChanged );
	$.RegisterForUnhandledEvent( 'PanoramaComponent_FriendsList_NameChanged', PartyMenu.OnLocalNameChanged );
	$.RegisterForUnhandledEvent( 'StubProfile_Updated', PartyMenu.OnLocalNameChanged );
