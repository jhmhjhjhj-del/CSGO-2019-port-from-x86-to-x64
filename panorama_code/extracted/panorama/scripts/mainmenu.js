...
var MainMenu = ( function() {
	...
	var _OnVanitySettingsPressed = function()
	{
	...
	var _OnStubGrantPressed = function()
	{
		UiToolkitAPI.ShowCustomLayoutPopup(
			'',
			'file://{resources}/layout/popups/popup_stub_grant.xml'
		);
	};

	var _RigVanityHover = function( vanityPanel )
	{
		...
		elHover.SetPanelEvent( 'onmouseover', function()
		{
			if ( $( '#VanityControls' ) )
				$( '#VanityControls' ).AddClass( 'pulse-vanity-controls' );
		} );

		elHover.SetPanelEvent( 'onmouseout', function()
		{
			if ( $( '#VanityControls' ) )
	...
	var _SwitchVanity = function( team )
	{
		...
		GameInterfaceAPI.SetSettingString( 'ui_vanitysetting_itemid', '' );
		...
		for ( var i = 0; i < models.length; i++ )
		{
			...
			{
				GameInterfaceAPI.SetSettingString( 'ui_vanitysetting_model', models[i].model );
				GameInterfaceAPI.SetSettingString( 'ui_vanitysetting_loadoutslot', models[i].loadoutSlot );
				break;
			}
		}
	...
	var _OnGoToCharacterLoadoutPressed = function()
	{
		...
		var elVanityContextMenu = UiToolkitAPI.ShowCustomLayoutContextMenuParametersDismissEvent(
			'VanityPanelGoToCharacterLoadout',
			'',
		...
		{	                                                                 
			...
			{	                             
				...
				if ( arrModels.filter( function( entry )
					{
						return entry.model === loadout.model
							&& ( entry.team === loadout.team || entry.team === 'any' )
							&& entry.team !== 'any';
					} ).length <= 0 )
				{
			...
			if ( InventoryAPI.IsValidItemID( loadout.itemId ) && InventoryAPI.IsItemInfoValid( loadout.itemId ) )
			{
				// Stale vanity convar often keeps a stock/faux id after the player equipped a skin
				// in the same slot — prefer the live loadout item so 3D paints apply.
				if ( loadout.team && loadout.loadoutSlot )
				{
					var equippedId = LoadoutAPI.GetItemID( loadout.team, loadout.loadoutSlot );
					if ( InventoryAPI.IsValidItemID( equippedId ) &&
						InventoryAPI.IsItemInfoValid( equippedId ) &&
						equippedId !== loadout.itemId &&
						InventoryAPI.IsFauxItemID( loadout.itemId ) &&
						!InventoryAPI.IsFauxItemID( equippedId ) )
					{
						loadout.itemId = equippedId;
						GameInterfaceAPI.SetSettingString( 'ui_vanitysetting_itemid', loadout.itemId );
					}
				}
				break;
	...
	var _DefaultModelLoadouts = function()
	{
		...
		var loadoutsList = CharacterAnims.GetValidCharacterModels().filter( function( entry )
		{
			return entry.team === 'ct' || entry.team === 't';
		} );
		var randomEntry = Math.floor( Math.random() * loadoutsList.length );
		return Object.assign( {}, loadoutsList[ randomEntry ] );
	};
	...
	{
		var el = $( '#MainMenuNavBarOverwatch' );
		if ( el )
			el.SetHasClass( 'mainmenu-navbar__btn-small--hidden', false );
	};
	...
	return {
		...
		OnVanitySettingsPressed				: _OnVanitySettingsPressed,
		OnStubGrantPressed					: _OnStubGrantPressed,
