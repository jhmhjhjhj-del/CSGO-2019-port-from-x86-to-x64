		...
		Object.keys( oMatchEndVoteData[ "voting_options" ] ).forEach( function( key, index ) {
			...
			else
			{
				...
				else if ( type == "map" )
				{
					...
					// GetFriendlyMapName returns #SFUI_Map_* token — must Localize or UI shows the key.
					var token = '#SFUI_Map_' + internalName;
					text = $.Localize( token );
					if ( !text || text === token || text === ( 'SFUI_Map_' + internalName ) )
					{
						var fromApi = GameTypesAPI.GetFriendlyMapName( internalName );
						if ( fromApi && fromApi.length > 0 )
						{
							if ( fromApi.charAt( 0 ) === '#' )
							{
								text = $.Localize( fromApi );
								if ( !text || text === fromApi )
									text = internalName;
							}
							else
							{
