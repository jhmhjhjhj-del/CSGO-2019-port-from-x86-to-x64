...
var Loadout = ( function() {
	...
	var _FillOutItemData = function( itemId, loadoutSlot )
	{
		...
		try
		{
			var pv = InventoryAPI.GetItemAttributeValue( idForDisplay, 'set item texture prefab' );
			if ( pv ) paintKit = Math.round( Number( pv ) );
		}
		catch ( eP ) {}

		var elImage = $.GetContextPanel().FindChildInLayoutFile( 'LoadoutSingleItemImage' );
		if ( paintKit > 0 )
		{
			...
			if ( elImage )
			{
				...
				var imagePath = InventoryAPI.GetItemInventoryImage( idForDisplay );
				if ( imagePath )
					elImage.SetImage( 'file://{images_econ}/' + imagePath + '.png' );
			}
		}
		else
		{
