...
var Scoreboard = ( function()
{
	...
	function _IsQueuedMatchmakingMode_Team ()
	{
		return ( typeof GameStateAPI.IsQueuedMatchmakingMode_Team === 'function' ) ? _IsQueuedMatchmakingMode_Team() : false;
	}

	function _GetActiveQuestID ()
	{
		if ( typeof MissionsAPI !== 'undefined' && typeof MissionsAPI.GetActiveQuestID === 'function' )
			return MissionsAPI.GetActiveQuestID();
		return 0;
	}

	function _SafeRecordUIEvent ( name )
	{
		if ( typeof StoreAPI !== 'undefined' && typeof StoreAPI.RecordUIEvent === 'function' )
			StoreAPI.RecordUIEvent( name );
	}
	...
	{
		if ( _m_pendingEntUpdate === entindex )
			return;
		_m_pendingEntUpdate = entindex;
		$.Schedule( 0.05, function ()
		{
			var idx = _m_pendingEntUpdate;
			_m_pendingEntUpdate = -1;
			if ( idx >= 0 )
				_UpdatePlayerByEntIndex( idx );
		} );
	}
		...
		switch ( stat ) 
		{
			...
			case 'ping':
				fn = function( oPlayer, bSilent )
				{
					...
					var mmPing = ( typeof StubMm !== 'undefined' ) ? StubMm.PersonaFor( oPlayer.m_xuid ) : null;
					if ( mmPing )
					{
						elLabel.SetHasClass( "sb-row__cell--ping__label--bot", false );
						elLabel.text = '' + mmPing.ping;
						oPlayer.m_oStats[ stat ] = mmPing.ping;
					}
					else
					{
			...
			case 'money':
				fn = function( oPlayer, bSilent = false )
				{
					...
					var mode = MockAdapter.GetGameModeInternalName( false );
					var bFogEnemyMoney = ( mode === 'competitive' || mode === 'scrimcomp2v2' );
					if ( bFogEnemyMoney &&
						!MockAdapter.IsDemoOrHltv() &&
						!IsTeamASpecTeam( MockAdapter.GetPlayerTeamName( GetLocalPlayerId() ) ) &&
						MockAdapter.ArePlayersEnemies( oPlayer.m_xuid, GetLocalPlayerId() ) )
					{
						newStatValue = -1;
					}
			...
			case 'name':
				fn = function( oPlayer, bSilent = false )
				{
					...
					var displayName = MockAdapter.GetPlayerName( oPlayer.m_xuid );
					var mmPersona = ( typeof StubMm !== 'undefined' ) ? StubMm.PersonaFor( oPlayer.m_xuid ) : null;
					if ( !displayName && mmPersona && mmPersona.name )
						displayName = mmPersona.name;

					elNameLabel.SetDialogVariable( 'player_name', displayName );
			...
			case 'flair':
				fn = function( oPlayer, bSilent = false )
				{
					...
					// Offline InventoryAPI.GetFlairItemId ignores xuid → local medal on every row.
					if ( MockAdapter.IsFakePlayer( oPlayer.m_xuid ) )
					{
						var elAvatarBot = oPlayer.m_oElStats[ 'avatar' ];
						if ( elAvatarBot && elAvatarBot.IsValid() )
						{
							var elFlairBot = elAvatarBot.FindChildTraverse( "flair-image" );
							if ( elFlairBot && elFlairBot.IsValid() )
							{
								elFlairBot.AddClass( 'hidden' );
								elFlairBot.SetImage( '' );
							}
						}
						oPlayer.m_oStats[ stat ] = '0';
						return;
					}
					...
					if ( oPlayer.m_oStats[ stat ] !== newStatValue )
					{
						...
						var elFlairCol = oPlayer.m_oElStats[ stat ];
						if ( elFlairCol && elFlairCol.IsValid() )
							elFlairCol.AddClass( 'hidden' );

						var elAvatarPanel = oPlayer.m_oElStats[ 'avatar' ];
						if ( !elAvatarPanel || !elAvatarPanel.IsValid() )
							return;

						var elFlairImage = elAvatarPanel.FindChildTraverse( "flair-image" );
						...
						if ( !imagepath )
						{
							elFlairImage.AddClass( 'hidden' );
							elFlairImage.SetImage( '' );
							return;
						}

						elFlairImage.RemoveClass( 'hidden' );
			...
			case 'skillgroup':
				fn = function( oPlayer, bSilent = false )
				{
					...
					var mmSk = ( typeof StubMm !== 'undefined' ) ? StubMm.PersonaFor( oPlayer.m_xuid ) : null;
					if ( mmSk )
						newStatValue = mmSk.rank;

					if ( ( !newStatValue || newStatValue <= 0 ) && typeof MockAdapter.IsFakePlayer === 'function' )
					{
						try
						{
							if ( !MockAdapter.IsFakePlayer( oPlayer.m_xuid ) )
							{
								var localXuid = ( typeof MyPersonaAPI !== 'undefined' && MyPersonaAPI.GetXuid ) ? MyPersonaAPI.GetXuid() : '';
								if ( !localXuid || ( '' + oPlayer.m_xuid ) === ( '' + localXuid ) )
								{
									var mode = MockAdapter.GetGameModeInternalName( false );
									var rankType = ( mode == "scrimcomp2v2" ) ? "wingman" : "competitive";
									if ( typeof PartyListAPI !== 'undefined' && PartyListAPI.GetFriendCompetitiveRank )
										newStatValue = PartyListAPI.GetFriendCompetitiveRank( oPlayer.m_xuid, rankType );
								}
							}
						}
						catch ( eRank ) {}
			...
			case 'rank':
				fn = function( oPlayer, bSilent = false )
				{
					...
					var mmLv = ( typeof StubMm !== 'undefined' ) ? StubMm.PersonaFor( oPlayer.m_xuid ) : null;
					if ( mmLv )
						newStatValue = mmLv.level;
	...
	function _GetPlayerRowForGameMode ()
	{
		...
		if ( _IsQueuedMatchmakingMode_Team() )
		{
	...
	function _UpdateMatchInfo ()
	{
		...
		if ( elMapLabel && elMapLabel.IsValid() && elMapLabel.text == "" )
		{
			...
			else
			{
				...
				else if ( _IsQueuedMatchmakingMode_Team() )
				{
		...
		if ( elCoopStats )
		{
			let questID = _GetActiveQuestID(); 
			if ( questID > 0 )
			{
				elCoopStats.AddClass( "show-mission-desc" );
				if ( typeof MissionsAPI.ApplyQuestDialogVarsToPanelJS === 'function' )
				...
				if ( elLabel && typeof MissionsAPI.GetQuestDefinitionField === 'function' )
				{
		...
		var _UpdateCasualties = function( teamName, elRnd )
		{
			...
			if ( _m_oTeams[ teamName ] )
			{
				...
				if ( MockAdapter.GetGameModeInternalName( false ) == "scrimcomp2v2" )
					nPlayers = 2;
				else if ( MockAdapter.GetGameModeInternalName( false ) == "retakes" )
					nPlayers = 4;
	...
	function _CycleStats ()
	{
		...
		_SafeRecordUIEvent( "ScoreboardMoreStatsToggle" );
	}

	function _MuteVoice ()
	{
		var cur = GameInterfaceAPI.GetSettingString( 'voice_enable' );
		GameInterfaceAPI.SetSettingString( 'voice_enable', ( cur === '1' ) ? '0' : '1' );
	...
	{
		_SafeRecordUIEvent( "ScoreboardMuteVoiceToggle" );
	...
	function _BlockUgc ()
	{
		_SafeRecordUIEvent( "ScoreboardBlockUgcToggle" );
	...
	function _GetFreeForAllTopThreePlayers ()
	{
		...
		if ( elTeam && elTeam.IsValid() && elTeam.GetChildCount() > 0 )
		{
			var children = elTeam.Children();
			p1 = children[ 0 ] && children[ 0 ].m_xuid ? children[ 0 ].m_xuid : '0';
			p2 = children[ 1 ] && children[ 1 ].m_xuid ? children[ 1 ].m_xuid : '0';
			p3 = children[ 2 ] && children[ 2 ].m_xuid ? children[ 2 ].m_xuid : '0';
		}

		$.DispatchEvent( 'EndOfMatch_GetFreeForAllTopThreePlayers_Response', p1, p2, p3 );
		return [ p1, p2, p3 ];
	};
	...
	function _UpdateLeaderboard( strLBname )
	{
		...
		if ( typeof LeaderboardsAPI.GetLeaderboardDetailsHandleForCoop !== 'function' )
			return;

		var hPrevDetails = LeaderboardsAPI.GetLeaderboardDetailsHandleForCoop(); 
		var activeQuestID = _GetActiveQuestID();
	...
	{
		if ( typeof GameStateAPI.GetCrosshairCode !== 'function' || typeof MyPersonaAPI.BApplyCrosshairCode !== 'function' )
			return;
