...
var EOM_Podium = ( function () {
	...
	function _AddPlayerToPodium( _xuid, _position )
	{
		...
		var rank = GameStateAPI.GetPlayerXpLevel( _xuid );
		...
		var skillGroup = GameStateAPI.GetPlayerCompetitiveRanking( _xuid );
		...
		// ItemPreviewPanel + CharacterAnims flood DispatchAsyncEvent offline — avatars only.
		var bEnablePodiumModels = false;
		if ( bEnablePodiumModels )
		{
		try
		{
			...
			if ( id == 0 || ( typeof ItemInfo !== 'undefined' && ItemInfo.GetSlotSubPosition( id ) == 'c4' ) )
			{
			...
			var settings = {
				...
				itemId: id,
				loadoutSlot: ( typeof ItemInfo !== 'undefined' ) ? ItemInfo.GetSlotSubPosition( id ) : '',
				playIntroAnim: false,
				selectedWeapon: ( typeof ItemInfo !== 'undefined' ) ? ItemInfo.GetItemDefinitionName( id ) : ''
			};

			if ( typeof CharacterAnims !== 'undefined' && CharacterAnims.PlayAnimsOnPanel )
	...
	function _CalculateTeamBestStats()
	{
		...
		if ( typeof oPlayerList === 'string' )
		{
			try { oPlayerList = oPlayerList ? JSON.parse( oPlayerList ) : {}; }
			catch ( e ) { oPlayerList = {}; }
		}
		...
		for ( var teamName in oPlayerList )
		{
			...
			for ( var j in oPlayerList[ teamName ] )
			{
				...
				if ( typeof oPlayerBestStats === 'string' )
				{
					try { oPlayerBestStats = oPlayerBestStats ? JSON.parse( oPlayerBestStats ) : {}; }
					catch ( e ) { oPlayerBestStats = {}; }
				}
				if ( !oPlayerBestStats )
					oPlayerBestStats = {};
	...
	function _DisplayMe() 
	{
		...
		if ( typeof _m_oMatchEndData === 'string' )
		{
			try { _m_oMatchEndData = _m_oMatchEndData ? JSON.parse( _m_oMatchEndData ) : null; }
			catch ( e ) { _m_oMatchEndData = null; }
		}
		...
		if ( bFreeForAll )
		{
			...
			if ( _m_arrTopPlayerXuid[ 2 ] )
				_AddPlayerToPodium( _m_arrTopPlayerXuid[ 2 ], 4 );
			
			if ( _m_arrTopPlayerXuid[ 1 ] )
				_AddPlayerToPodium( _m_arrTopPlayerXuid[ 1 ], 2 );
			
			if ( _m_arrTopPlayerXuid[ 0 ] )	
				_AddPlayerToPodium( _m_arrTopPlayerXuid[ 0 ], 3 );
	...
	function _Start()
	{
		...
		if ( _DisplayMe() )
		{
			EndOfMatch.SetScoreboardVisible( false );
