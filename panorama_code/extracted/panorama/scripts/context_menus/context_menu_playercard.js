...
var contextmenuPlayerCard = ( function (){
	...
	var _Init = function ()
	{
		...
		_LoadPlayerCard( xuid );
		_GetContextMenuEnteries();

		                                                                                 
	};

	var _LoadPlayerCard = function( xuidHint )
	{
		var xuid = xuidHint || $.GetContextPanel().GetAttributeString( "xuid", "" );
		if ( !xuid || xuid === "(not found)" )
			xuid = $.GetContextPanel().GetAttributeString( "xuid", "" );
		if ( !xuid )
			return;

		var oldPanel = $.GetContextPanel().FindChildInLayoutFile( 'JsContextMenuPlayercard' );
		if ( oldPanel )
			oldPanel.DeleteAsync( 0.0 );
		...
		var _SyncPlayerCard = function( tries )
		{
			if ( !newPanel || !newPanel.IsValid || !newPanel.IsValid() )
				return;

			var cardRoot = newPanel.GetChild( 0 );
			if ( !cardRoot || !cardRoot.IsValid || !cardRoot.IsValid() )
			{
				if ( tries > 0 )
					$.Schedule( 0, function() { _SyncPlayerCard( tries - 1 ); } );
				return;
			}

			cardRoot.SetAttributeString( 'xuid', xuid );
			if ( typeof playerCard !== 'undefined' && playerCard.InitWithXuid )
				playerCard.InitWithXuid( xuid, cardRoot );
		};
		_SyncPlayerCard( 12 );
	};
			...
			AvailableForItem: function ( id ) {
				if( FriendsListAPI.IsFriendJoinable( id ))
				{
					...
					if (LobbyAPI.IsSessionActive()) {
						...
						var nMachines = party.numMachines || party.numPlayers || 0;

						for (var i = 0; i < nMachines; i++) {
							var machine = party['machine' + i];
							if ( machine && machine.player0 && id === machine.player0.xuid )
								return false;
						}
					}
			...
			OnSelected:  function ( id ) {
				if ( _IsSelf( id ) )
				{
					UiToolkitAPI.ShowCustomLayoutPopupParameters(
						'',
						'file://{resources}/layout/popups/popup_stub_profile.xml',
						''
					);
				}
				$.DispatchEvent( 'ContextMenuEvent', '' );
		...
		},
		{
			...
			AvailableForItem: function( id )
			{
				...
				if (LobbyAPI.IsSessionActive() && LobbyAPI.BIsHost()) {
					...
					for (var i = 0; i < nMachines; i++) {
						var machine = party['machine' + i];
						if ( machine && machine.player0 && id === machine.player0.xuid && !_IsSelf( id ))
							return true;
					}
