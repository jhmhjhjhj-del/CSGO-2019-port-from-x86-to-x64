...
GLMRendererInfo::GLMRendererInfo( GLMRendererInfoFields *info )
{
	...
	glGetIntegerv( GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS, &m_info.m_nMaxVertexTIU );
	glGetIntegerv( GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS, &m_info.m_nMaxCombinedTIU );

	m_info.m_nNumVertexSamplers = ( m_info.m_nMaxVertexTIU > 4 ) ? 4 : 0;
	m_info.m_nFirstVertexSampler = ( m_info.m_nMaxCombinedTIU <= m_info.m_nMaxVertexTIU ) ? 0 : 16;
	m_info.m_nTotalSamplerCount = ( m_info.m_nMaxCombinedTIU <= m_info.m_nMaxVertexTIU ) ? 16 : ( 16 + 4 );
