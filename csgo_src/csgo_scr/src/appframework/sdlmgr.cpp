...
{
	// SDL_PumpEvents gathers pending input information from devices and places it on the event queue.
	// Usually no need to call this directly as SDL_PollEvent() implicitly calls it, but if message pump 
	// isn't running (e.g. during init), then calling it explicitly can prevent SDL getting into an
	// inconsistent state - on OSX for example it was possible to get into a state where left mouse clicks
	// weren't picked up if you clicked on the initial black screen during startup. 
