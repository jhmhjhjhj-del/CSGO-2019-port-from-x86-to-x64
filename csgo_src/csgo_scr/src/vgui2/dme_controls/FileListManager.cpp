...
void CFileListManager::OnOpenContextMenu( KeyValues *pParams )
{
	...
	{
		bool bP4Connected = g_p4->IsConnectedToServer();
		...
		for ( int i = 0; i < nSelected; ++i )
		{
			...
			if ( bP4Connected )
			{
				...
				{
					++nInPerforce;
					if ( g_p4->GetFileState( pFilename ) != P4FILE_UNOPENED )
					{
						++nOpenForEdit;
					}
				}
...
void CFileListManager::OnAddToPerforce( KeyValues *pParams )
{
	...
	bool bSuccess = g_p4->OpenFilesForAdd( nFileCount, ppFileNames );
	if ( !bSuccess )
	{
		vgui::MessageBox *pError = new vgui::MessageBox( "Perforce Error!", g_p4->GetLastError(), GetParent() );
...
void CFileListManager::OnOpenForEdit( KeyValues *pParams )
{
	...
	bool bSuccess = g_p4->OpenFilesForEdit( nFileCount, ppFileNames );
	if ( !bSuccess )
	{
		vgui::MessageBox *pError = new vgui::MessageBox( "Perforce Error!", g_p4->GetLastError(), GetParent() );
		pError->SetSmallCaption( true );
...
void CFileListManager::Refresh()
{
	...
	const bool bP4Connected = g_p4 ? g_p4->IsConnectedToServer() : false;
	...
	for ( int i = 0; i < nFiles; ++i )
	{
		...
		bool bInPerforce = bP4Connected && g_p4->IsFileInPerforce( pFileName );
		bool bOpenForEdit = bInPerforce && g_p4->GetFileState( pFileName ) != P4FILE_UNOPENED;
