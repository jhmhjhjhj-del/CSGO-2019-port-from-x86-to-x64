...
bool AnimationController::ParseScriptFile(char *pMem, int length)
{
	...
	char token[512];
	pMem = ParseFile(pMem, token, sizeof( token ), NULL);
	while (token[0])
	{
		bool bAccepted = true;
		...
		while (token && token[0])
		{
			...
			if (!stricmp(token, "animate"))
			{
				...
				if (cmdAnimate.variable == m_sPosition)
				{
					...
					char token2[32];
					char *psz = ParseFile(token, token2, sizeof( token2 ), NULL);
					psz = ParseFile(psz, token2, sizeof( token2 ), NULL);
					psz = token2;
...
// 							}
			...
			char *peek = ParseFile(pMem, token, sizeof( token ), NULL);
			if ( Q_stristr( token, "[$" ) )
			{
