...
void PerforceFileList::AddItemToDirectoryList( const char *pFullPath, int nItemID, bool bIsDirectory )
{
	...
	else
	{
		...
		if ( !g_p4->GetClientSpecForDirectory( pFullPath, pClientSpec, sizeof(pClientSpec) ) )
		{
			pClientSpec[0] = 0;
		}
...
static P4File_t *FindFileInPerforceList( const char *pFileName, CUtlVector<P4File_t> &fileList, bool *pFound )
{
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		const char *pPerforceFileName = g_p4->String( fileList[i].m_sLocalFile );
...
void PerforceFileList::Refresh()
{
	...
	for ( int i = FirstItem(); i != InvalidItemID(); i = NextItem( i ) )
	{
		...
		P4File_t fileInfo;
		bool bIsFileInPerforce = g_p4->GetFileInfo( pFile, &fileInfo );
	...
	*/
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		CUtlVector<P4File_t> &fileList = g_p4->GetFileListUsingClientSpec( pDirectory, pInfo->m_ClientSpec );
		int nFileCount = fileList.Count();
		bool *pFound = (bool*)malloc( nFileCount * sizeof(bool) );
