...
#ifdef DEBUG
	float flPerfTimer = Plat_FloatTime();
#endif
...
#ifdef DEBUG
			if ( !m_CompletionList.Count() )
			{
				if ( Plat_FloatTime() - flPerfTimer > 0.05f )
				{
					bCheckSubstrings = false; // FIXME: (ususally in debug) you can get in a state 
					// where typing a long, partial command - like "give weapon_ak47" will NEVER 
					// produce any autocomplete results, but this search lags typing each console 
					// character by over a second - making the console nigh unusable. Hack for now
					// is to stop checking substrings if it's taking too long. A better fix may
					// be to wait just a bit for the user to stop typing characters before trying
					// to rebuild the autocomplete list.
				}
			}
#endif
