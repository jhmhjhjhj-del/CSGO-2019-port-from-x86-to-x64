...
{
	bool bIsInPerforce = g_p4->IsFileInPerforce( pFullPath );
	bool bIsOpened = ( g_p4->GetFileState( pFullPath ) != P4FILE_UNOPENED );
	switch( m_Action )
	{
...
void CPerforceFileListFrame::AddFile( const char *pFullPath )
{
	...
	P4FileState_t state = g_p4->GetFileState( pFullPath );
...
void CPerforceFileListFrame::AddFile( const char *pRelativePath, const char *pPathId )
{
	...
	{
		g_p4->GetOpenedFileListInPath( pPathId, m_OpenedFiles );
	...
	for ( k = 0; k < nOpenedFileCount; ++k )
	{
		...
		const char *pLocalFile = g_p4->String( m_OpenedFiles[k].m_sLocalFile );
...
bool CPerforceFileListFrame::PerformOperation( )
{
	...
	{
	case PERFORCE_ACTION_FILE_ADD:
		bSuccess = g_p4->OpenFilesForAdd( nFileCount, ppFileNames );
		break;

	case PERFORCE_ACTION_FILE_EDIT:
		bSuccess = g_p4->OpenFilesForEdit( nFileCount, ppFileNames );
		break;

	case PERFORCE_ACTION_FILE_DELETE:
		bSuccess = g_p4->OpenFilesForDelete( nFileCount, ppFileNames );
		break;

	case PERFORCE_ACTION_FILE_REVERT:
		bSuccess = g_p4->RevertFiles( nFileCount, ppFileNames );
		break;
	...
	case PERFORCE_ACTION_FILE_SUBMIT:
		{ 
			...
			{
				bSuccess = g_p4->SubmitFiles( nFileCount, ppFileNames, pDescription );
			}
		}
		break;
	}

	const char *pErrorString = g_p4->GetLastError();
	if ( !bSuccess )
	{
