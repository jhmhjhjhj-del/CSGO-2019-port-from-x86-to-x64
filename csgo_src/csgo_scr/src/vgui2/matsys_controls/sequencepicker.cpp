...
void CSequencePicker::SetSequenceLayer( int nLayerIndex, int nSequenceIndex, float flWeight )
{
	...
	// added extra members to MDLSequenceLayer_t (m_bLoop, m_flStartTime), setting appropriately for old behavior in CMDL::SetupBones()
	m_SequenceLayers[ nLayerIndex ].m_bLoop = true;
	m_SequenceLayers[ nLayerIndex ].m_flStartTime = -1.0f;
