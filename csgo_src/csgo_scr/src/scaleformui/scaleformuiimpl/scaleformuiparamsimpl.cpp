...
double ScaleformUIImpl::Params_GetArgAsNumber( SFPARAMS params, int index )
{
	...
	SF_PARAMS_ASSERT( nIndex < pParams->ArgCount, "SF param out of range!" );
	if ( nIndex >= pParams->ArgCount )
		return 0.0;

	SF_PARAMS_ASSERT( pParams->pArgs[index].IsNumber(), "SF param is not a number!" );
	if ( !pParams->pArgs[nIndex].IsNumber() )
		return 0.0;

	return ( double ) pParams->pArgs[nIndex].GetNumber();
}
...
bool ScaleformUIImpl::Params_GetArgAsBool( SFPARAMS params, int index )
{
	...
	SF_PARAMS_ASSERT( pParams->pArgs[nIndex].IsBool(), "SF param is not a boolean!" );

	// NOTE: .GetBool() here is an invalid cast if IsBool() returns false.  I am not changing
	//       this to check and return false because existing code may rely on non-boolean
	//       objects having c-like boolean behavior.
	//
	//       that is, null/0 => false, non-null/non-zero => true.

	return pParams->pArgs[nIndex].GetBool();
}
...
const char* ScaleformUIImpl::Params_GetArgAsString( SFPARAMS params, int index )
{
	...
	SF_PARAMS_ASSERT( pParams->pArgs[nIndex].IsString(), "SF param is not a string!" );
	if ( !pParams->pArgs[nIndex].IsString() )
		return "";

	return pParams->pArgs[nIndex].GetString();
}
...
const wchar_t* ScaleformUIImpl::Params_GetArgAsStringW( SFPARAMS params, int index )
{
	...
	SF_PARAMS_ASSERT( pParams->pArgs[nIndex].IsStringW(), "SF param is not a StringW!" );
	if ( !pParams->pArgs[nIndex].IsStringW() )
		return L"";

	return pParams->pArgs[nIndex].GetStringW();
}
...
void ScaleformUIImpl::Params_SetResult( SFPARAMS params, const char* value, bool bMakeNewValue /* = true */ )
{
	...
	{
		FromSFPARAMS( params )->pRetVal->SetString( value );
	}
}
...
void ScaleformUIImpl::Params_SetResult( SFPARAMS params, const wchar_t* value, bool bMakeNewValue /* = true */ )
{
	...
	{
		FromSFPARAMS( params )->pRetVal->SetStringW( value );
	}
}
