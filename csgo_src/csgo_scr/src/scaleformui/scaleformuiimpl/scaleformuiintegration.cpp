...
Image*  CScaleformImageCreator::LoadProtocolImage(const ImageCreateInfo& info, const SF::String& url)
{
	...
	{
		// PERFECT WORLD HACK: no avatars for Chinese Government certification!
//		static bool s_bPerfectWorld = !!CommandLine()->FindParm( "-perfectworld" );
//		if ( s_bPerfectWorld )
//		{
//			return ( ( ScaleformUIImpl* ) m_pScaleformUI )->CreateImageFromFile( "resource/flash/econ/avatars/pwavatar.png", info, 184, 184 );
//		}
// ^^ this code is commented out August 2017. Perfect World now has a tool to ban user avatars that are offensive and
// we can just enable the avatars display in the game.
	...
	else if ( char const *szStringTablesImg = StringAfterPrefix( url, "img://stringtables:" ) )
	{
		// Parse width and height attributes
		uint width = 0, height = 0;
		if ( szStringTablesImg[ 0 ] == '(' )
		{
			width = Q_atoi( szStringTablesImg + 1 );
			szStringTablesImg = strchr( szStringTablesImg, 'x' );
			if ( !szStringTablesImg )
				return NULL;
			height = Q_atoi( szStringTablesImg + 1 );
			szStringTablesImg = strstr( szStringTablesImg, "):" );
			if ( !szStringTablesImg )
				return NULL;
			szStringTablesImg += 2;
		}

		if ( !szStringTablesImg || !szStringTablesImg[ 0 ] )
			return NULL;
