...
ifeq ($(OS),Linux)
	CXXFLAGS = $(CFLAGS) $(WARN_CXX_FLAGS) -std=gnu++0x -fpermissive
else
	CXXFLAGS = $(CFLAGS) $(WARN_CXX_FLAGS) -std=gnu++11 -stdlib=libc++ -Wno-c++11-narrowing -Wno-dangling-else
endif
...
ifneq ($(findstring MAKEFILE_FORCE_C_COMPILE,$(DEFINES)),)
	FORCE_COMPILE_AS_FLAGS = -x c
	COMPILE_AS_FLAGS=$(CFLAGS)
else
	FORCE_COMPILE_AS_FLAGS =
	COMPILE_AS_FLAGS=$(CXXFLAGS)
endif
...
ifeq ($(BUILDING_MULTI_ARCH),1)
	SINGLE_ARCH_COMPILE_FLAGS=$(subst -arch x86_64,,$(COMPILE_AS_FLAGS))
	COMPILE_FILE = \
		$(QUIET_PREFIX) \
		echo "---- $(lastword $(subst /, ,$<)) as MULTIARCH----";\
		mkdir -p $(OBJ_DIR) && \
		$(CXX) $(FORCE_COMPILE_AS_FLAGS) $(SINGLE_ARCH_COMPILE_FLAGS) $(GENDEP_CXXFLAGS) -o $@ -c $< && \
		$(CXX) $(FORCE_COMPILE_AS_FLAGS) $(COMPILE_AS_FLAGS) -o $@ -c $<
else
	COMPILE_FILE = \
		$(QUIET_PREFIX) \
		echo "---- $(lastword $(subst /, ,$<)) ----";\
		mkdir -p $(OBJ_DIR) && \
		$(CXX) $(FORCE_COMPILE_AS_FLAGS) $(COMPILE_AS_FLAGS) $(GENDEP_CXXFLAGS) -o $@ -c $<
endif
