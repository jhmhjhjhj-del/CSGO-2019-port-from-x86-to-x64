...
//	nCount++;
	...
	m_CheckinListCtrl.SetItemText( nIndex, 2, g_p4->String( FileInfo->m_sName ) );
	m_CheckinListCtrl.SetItemText( nIndex, 3, g_p4->String( FileInfo->m_sPath ) );
...
BOOL CMapDocCheckin::OnInitDialog()
{
	...
	g_p4->GetOpenedFileList( FileList, true );
	...
	while( pos != NULL )
	{
		...
		if ( pMapDoc )
		{
			...
			{
				if ( g_p4->GetFileInfo( pMapDoc->GetPathName(), &FileInfo ) == true )
				{
	...
	if ( nFileCount > 0 )
	{
		...
		for( int i = 0; i < m_CheckinListCtrl.GetItemCount(); i++ )
		{
			if ( m_CheckinListCtrl.GetItemState( i, LVIS_STATEIMAGEMASK ) == INDEXTOSTATEIMAGEMASK( LVIS_SELECTED ) )
			{
				...
				const char *pszFileName = g_p4->String( m_FileList[ i ] );
		...
		if ( g_p4->SubmitFiles( nFileCount, ppFileNames, Description ) == false )
		{
			m_CheckInStatusControl.SetWindowText( "Checkin FAILED!" );
			m_SubmitButtonControl.EnableWindow( TRUE );

			sprintf( temp, "Could not check in map(s): %s", g_p4->GetLastError() );
			AfxMessageBox( temp, MB_ICONHAND | MB_OK );
...
void CMapDocStatus::UpdateMapList( bool RedoList )
{
	...
	while( pos != NULL )
	{
		...
		if ( g_p4 && Options.general.bEnablePerforceIntegration == TRUE && g_p4->GetFileInfo( pMapDoc->GetPathName(), &FileInfo ) == true )
		{
			switch( FileInfo.m_eOpenState )
			{	
			...
			m_FileListCtrl.SetItemText( nIndex, 3, g_p4->String( FileInfo.m_sName ) );
			m_FileListCtrl.SetItemText( nIndex, 4, g_p4->String( FileInfo.m_sPath ) );
		}
		else
		{
			if ( pMapDoc->IsReadOnly() )
...
{
	if ( !g_p4 || Options.general.bEnablePerforceIntegration == FALSE )
	{
		return;
	}
	...
	if ( nFileCount > 0 )
	{
		...
		if ( g_p4->OpenFilesForEdit( nFileCount, ppFileNames ) == false )
		{
			SetControls( false, "Checkout FAILED." );

			sprintf( temp, "Could not check out map(s): %s", g_p4->GetLastError() );
			AfxMessageBox( temp, MB_ICONHAND | MB_OK );
...
void CMapDocStatus::OnBnClickedAdd()
{
	...
	if ( nFileCount > 0 )
	{
		...
		if ( g_p4->OpenFilesForAdd( nFileCount, ppFileNames ) == false )
		{
			SetControls( false, "Adding FAILED." );
			sprintf( temp, "Could not add map(s): %s", g_p4->GetLastError() );
			AfxMessageBox( temp, MB_ICONHAND | MB_OK );
