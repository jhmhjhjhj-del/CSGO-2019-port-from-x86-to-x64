...
bool CRender::DrawCollisionWalkable( MDLHandle_t mdlHandle, const VMatrix &mViewMatrix )
{
	...
	if ( vertCount )
	{
		...
		pCamera->GetViewForward( vecCameraForward );

		Color colorFlat( 100, 100, 100, 255 );
		...
		{
			Vector vec0;
			mViewMatrix.V3Mul( outVerts[j], vec0 );
			Vector vec1;
			mViewMatrix.V3Mul( outVerts[j + 1], vec1 );
			Vector vec2;
			mViewMatrix.V3Mul( outVerts[j + 2], vec2 );
		
			Vector vecSideA = (vec1 - vec0).Normalized();
			Vector vecSideB = (vec2 - vec0).Normalized();
			Vector vecTriNormal = CrossProduct( vecSideB, vecSideA ).Normalized();


			float flDotWalkable = DotProduct( vecTriNormal, Vector( 0, 0, 1 ) );
			if ( flDotWalkable > 0.7f )
			{
				colorFlat[0] = 200;
				colorFlat[1] = 200;
				colorFlat[2] = 200;
			...
			float flLighting = 0.5f + 0.5f * abs( DotProduct( vecTriNormal, vecCameraForward ) );
			colorFlat[0] *= flLighting;
			colorFlat[1] *= flLighting;
			colorFlat[2] *= flLighting;
		
			meshBuilder.Position3fv( vec0.Base() );
			meshBuilder.Color4ubv( (byte*)&colorFlat );
			meshBuilder.TexCoord2f( 0, 0, 0 );
			meshBuilder.AdvanceVertex();
