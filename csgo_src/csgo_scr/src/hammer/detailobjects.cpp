...
void DetailObjects::ParseDetailObjectFile( KeyValues& keyValues )
{
	...
	for( pIter = keyValues.GetFirstSubKey(); pIter; pIter = pIter->GetNextKey() )
	{
		...
		s_DetailObjectDict[i].m_AssociatedMaterial = pIter->GetString( "associated_material", "detail/detailsprites" );
...
void DetailObjects::AddDetailSpriteToFace( const char* pMaterialName, const Vector &vecOrigin, const QAngle &vecAngles, DetailModel_t const& model, float flScale )
{	
	CSpriteModel	*pSpriteModel = new CSpriteModel;
	m_DetailSprites.AddToTail(pSpriteModel);

	pSpriteModel->LoadSprite( pMaterialName );
...
void DetailObjects::PlaceDetail( const char* pMaterialName, DetailModel_t const& model, const Vector& pt, const Vector& normal )
{
	...
	default:
		{
			...
			AddDetailSpriteToFace( pMaterialName, pt, angles, model, flScale );
		}
		break;
	}
}
...
void DetailObjects::EmitDetailObjectsOnFace( CMapFace *pMapFace, DetailObject_t& detail )
{
	...
	for (int i = 1; i < nPoints-1; i++ )
	{
		...
		for (int i = 0; i < numSamples; ++i )
		{
			...
			PlaceDetail( detail.m_AssociatedMaterial.String(), detail.m_Groups[group].m_Models[model], pt, normal );
		}
	}
}
...
void	DetailObjects::Render3D(CRender3D *pRender)
{
	...
	if ( sprites )
	{
		...
		for ( int i = 0; i < sprites; i++ )
		{
			...
			if ( IsPointInBox( viewPoint, Mins, Maxs ) )
			{
				pSprite->DrawSprite3DForVertexShader( pRender, color );
			}
		}
		pRender->PopRenderMode();
	}
