...
BEGIN_MESSAGE_MAP(CMapDoc, CDocument)
	...
	ON_COMMAND( ID_COLLISION_WALKABLE, OnCollisionWalkable )
	ON_UPDATE_COMMAND_UI( ID_COLLISION_WALKABLE, OnUpdateCollisionWalkable )
...
bool CMapDoc::CheckOut( )
{
	...
	if ( g_p4->GetFileInfo( GetPathName(), &FileInfo ) == true )
	{
	...
	if ( !g_p4->OpenFileForEdit( GetPathName() ) )
	{
		return false;
	}
...
bool CMapDoc::CheckOutBsp( )
{
	...
	if ( g_p4->GetFileInfo( bspPath.Get(), &FileInfo ) == false )
		return false;
	...
	// Check out the file
	if ( !g_p4->OpenFileForEdit( bspPath.Get() ) )
	{
...
bool CMapDoc::GetBspFileStatus( unsigned char &FileStatus  )
{
	...
	if ( g_p4 != NULL && g_p4->GetFileInfo( bspPath, &FileInfo ) == true )
	{
...
bool CMapDoc::AddToVersionControl( )
{
	...
	if ( g_p4->OpenFileForAdd( GetPathName() ) == false )
	{
		return false;
	}
...
bool CMapDoc::SyncToHeadRevision( )
{
	...
	if ( g_p4->SyncFile( GetPathName() ) == false )
	{
		return false;
	}
...
bool CMapDoc::SyncBspToHeadRevision()
{
	...
	if ( g_p4->SyncFile( bspPath ) == false )
	{
		return false;
	}
...
bool CMapDoc::Revert( )
{
	...
	if ( g_p4->RevertFile( GetPathName() ) == false )
	{
		return false;
	}
...
void CMapDoc::CheckFileStatus( )
{
	...
	m_bCheckedOut = false;
	m_bIsVersionControlled = false;
	if ( g_p4 != NULL && Options.general.bEnablePerforceIntegration == TRUE )
	{
		if ( g_p4->GetFileInfo( GetPathName(), &FileInfo ) == true )
		{
...
void CMapDoc::OnFileVersionControlAdd( void )
{
	...
	if ( AddToVersionControl() == false && g_p4 )
	{
		char temp[ 2048 ];

		sprintf( temp, "Could not add file to version control: %s", g_p4->GetLastError() );
...
{
	if ( g_p4 == NULL || m_bIsVersionControlled == true || Options.general.bEnablePerforceIntegration == FALSE )
	{
		pCmdUI->Enable( false );
		return;
	}
...
void CMapDoc::OnFileVersionControlCheckOut( void )
{
	...
	if ( CheckOut() == false )
	{
		...
		sprintf( temp, "Could not check out file: %s", g_p4->GetLastError() );
		AfxMessageBox( temp, MB_ICONHAND | MB_OK );
	}
...
void CMapDoc::OnFileVersionControlCheckOutBsp( void )
{
	...
	if ( CheckOutBsp() == false )
	{
		...
		sprintf( temp.Get(), "Could not check out file: %s", g_p4->GetLastError() );
		AfxMessageBox( temp, MB_ICONHAND | MB_OK );
	}
}
...
void CMapDoc::OnUpdateVersionControlCheckOut(CCmdUI *pCmdUI)
{
	...
	if ( g_p4 == NULL || m_bCheckedOut == true || m_bIsVersionControlled == false || Options.general.bEnablePerforceIntegration == FALSE )
	{
		pCmdUI->Enable( false );
		return;
	}
...
{
	if ( g_p4 == NULL || BspOkToCheckOut() == false || Options.general.bEnablePerforceIntegration == FALSE )
	{
		pCmdUI->Enable( false );
		return;
	}
...
{
	if ( g_p4 == NULL || m_bCheckedOut == false || Options.general.bEnablePerforceIntegration == FALSE )
	{
		pCmdUI->Enable( false );
		return;
	}
...
{
	if ( g_p4 == NULL || Options.general.bEnablePerforceIntegration == FALSE )
	{
		pCmdUI->Enable( false );
		return;
	}
...
void CMapDoc::OnUpdateCollisionWalkable( CCmdUI *pCmdUI )
{
	pCmdUI->SetCheck( Options.general.bShowCollisionWalkable ? 1 : 0 );
	pCmdUI->Enable( !GetMainWnd()->IsShellSessionActive() );
}

void CMapDoc::OnCollisionWalkable( void )
{
	Options.general.bShowCollisionWalkable = !Options.general.bShowCollisionWalkable;
	UpdateAllViews( MAPVIEW_UPDATE_TOOL );
}
		...
		for ( int i = 0; i < pMapSolid->GetFaceCount(); i++ )
		{
			...
			if ( strstr( buf, "tools/tools" ) )
			{
				...
				if ( strstr( buf, "tools/tools" ) && strstr( buf, "clip" ) )
				{
					...
					else if ( strstr( buf, "droneclip" ) )
					{
						AddChildGroupToAutoVisGroup( pObject, "Drone", "Clips" );
						continue;
					}
				}	
...
void CMapDoc::OnInstancingCheckOutManifest( )
{
	...
	if ( pManifest->CheckOut() == false && g_p4 )
	{
		char temp[ 2048 ];

		sprintf( temp, "Could not check out manifest: %s", g_p4->GetLastError() );
		AfxMessageBox( temp, MB_ICONHAND | MB_OK );
	}
}
...
void CMapDoc::OnInstancingAddManifest( )
{
	...
	if ( pManifest->AddToVersionControl() == false && g_p4 )
	{
		char temp[ 2048 ];
