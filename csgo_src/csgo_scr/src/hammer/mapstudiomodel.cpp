...
void CMapStudioModel::CalcBounds(BOOL bFullUpdate)
{
	...
	if (m_pStudioModel != NULL)
	{
		...
		{
			Mins *= m_pStudioModel->GetScale();
			Maxs *= m_pStudioModel->GetScale();
			m_CullBox.bmins *= m_pStudioModel->GetScale();
			m_CullBox.bmaxs *= m_pStudioModel->GetScale();
		}
...
void CMapStudioModel::PushUniformScaleToModel()
{
	...
	if ( pEntity != NULL )
	{
		...
		{
			float flScale = atof( pszUniformScale );
			if ( flScale <= 0 )
			{
				flScale = 1.0f;
			}
			m_pStudioModel->SetScale( flScale );
		}
	}
}

void CMapStudioModel::OnUndoRedo()
{
	BaseClass::OnUndoRedo();
	PushUniformScaleToModel();
}
...
void CMapStudioModel::OnParentKeyChanged(const char* szKey, const char* szValue)
{
	...
	else if ( !stricmp( szKey, "uniformscale" ) )
	{
		float flScale = atof( szValue );
