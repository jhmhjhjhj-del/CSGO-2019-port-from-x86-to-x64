...
bool CManifest::AddToVersionControl( )
{
	...
	if ( !g_p4->OpenFileForAdd( GetPathName() ) )
	{
		return false;
	}
...
void CManifest::CheckFileStatus( void )
{
	...
	m_bCheckedOut = false;
	m_bIsVersionControlled = false;
	if ( g_p4 != NULL && g_p4->GetFileInfo( GetPathName(), &FileInfo ) == true )
	{
	...
	for( int i = 0; i < GetNumMaps(); i++ )
	{
		...
		if ( g_p4 != NULL && g_p4->GetFileInfo( pManifestMap->m_AbsoluteMapFileName, &FileInfo ) == true )
		{
