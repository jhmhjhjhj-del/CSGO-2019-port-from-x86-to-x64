...
BOOL CSyncFileDialog::OnInitDialog()
{
	...
	m_FileNameControl.SetWindowText( g_p4->String( m_pFileInfo->m_sLocalFile ) );
...
void CheckForFileSync( const char *pszFileName, bool bClearRepeat )
{
	...
	if ( g_p4 && Options.general.bEnablePerforceIntegration == TRUE )
	{
		P4File_t	FileInfo;

		if ( g_p4->GetFileInfo( pszFileName, &FileInfo ) == true )
		{
			if ( FileInfo.m_iHeadRevision != FileInfo.m_iHaveRevision )
			{
				...
				{
					if ( g_p4->SyncFile( pszFileName ) == false )
					{
