...
void CManifestListBox::OnVersionControlCheckOut()
{
	...
	if ( !g_p4->OpenFileForEdit( m_pTrackerManifestMap->m_AbsoluteMapFileName ) )
	{
		char temp[ 2048 ];
		
		sprintf( temp, "Could not check out map: %s", g_p4->GetLastError() );
...
void CManifestListBox::OnVersionControlAdd()
{
	...
	if ( !g_p4->OpenFileForAdd( m_pTrackerManifestMap->m_AbsoluteMapFileName ) )
	{
		char temp[ 2048 ];

		sprintf( temp, "Could not add map: %s", g_p4->GetLastError() );
		AfxMessageBox( temp, MB_ICONHAND | MB_OK );
	}
	else
	{
...
BOOL CManifestCheckin::OnInitDialog()
{
	...
	if ( activeDoc && activeDoc->GetManifest() )
	{
		...
		{
			if ( g_p4->GetFileInfo( pManifest->GetPathName(), &FileInfo ) == true )
			{
		...
		for( int i = 0; i < pManifest->GetNumMaps(); i++ )
		{
			...
			{
				if ( g_p4->GetFileInfo( pManifestMap->m_AbsoluteMapFileName, &FileInfo ) == true )
				{
					int nIndex = m_CheckinListCtrl.InsertItem( nCount, "" );
					nCount++;
