	...
	: m_FileTracker( this ), m_FileWhitelist( NULL ),m_FileTracker2( this )
{
	...
	m_bEnableFileWriteProtection = false;
}
...
bool CZipPackFile::FindFile( const char *pFilename, int &nIndex, int64 &nOffset, int &nLength )
{
	...
	if ( !V_RemoveDotSlashes( szCleanName ) )
	{
		return false;
	}
...
bool CZipPackFile::Prepare( int64 fileLen, int64 nFileOfs )
{
	...
	size_t cbFilename( MIN( zipFileHeader.fileNameLength, V_ARRAYSIZE( filename ) - 1 ) );

	zipDirBuff.Get( filename, cbFilename );
...
//-----------------------------------------------------------------------------
// We've been asked to write to a file. Is that file in a location we own?
//-----------------------------------------------------------------------------
bool CBaseFileSystem::IsFileInWritableLocation( const char *pPath ) const
{
	AUTO_LOCK( m_SearchPathsMutex );

	for ( const auto &searchPath : m_SearchPaths )
	{
		const char* pPossibleParentPath = searchPath.GetPathString();
		if ( StringHasPrefixCaseSensitive( pPath, pPossibleParentPath ) )
			return true;
	}

	if ( !m_bEnableFileWriteProtection )
	{
		Msg( "MSG_FILEWRITE - Filesystem was asked to write to '%s', but we don't own that location. Allowing.\n", pPath );
		return true;
	}

	Warning( "WARN_FILEWRITE - Asked to write to '%s', but we don't own that location. Disallowing.\n", pPath );
	return false;
}
...
FileHandle_t CBaseFileSystem::OpenForWrite( const char *pFileName, const char *pOptions, const char *pathID )
{
	...
	{
		V_strcpy_safe( szFullFilename, pFileName );
	}
	else
	{
		ComputeFullWritePath( szFullFilename, sizeof( szFullFilename ), pFileName, pathID );
	}

	if ( !V_RemoveDotSlashes( szFullFilename ) )
	{
		Assert( !"Someone is attempting to get us to write above the root. That's bad." );
		return ( FileHandle_t )0;
	}

	if ( !IsFileInWritableLocation( szFullFilename ) )
	{
		Assert( !"Someone is attempting to ask us to write to a location we don't own. No." );
		return ( FileHandle_t )0;
	}
	
	int64 size;
	FILE *fp = Trace_FOpen( szFullFilename, pOptions, 0, &size );
	if ( !fp )
	{
		return ( FileHandle_t )0;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseFileSystem::Truncate( FileHandle_t file )
{
	VPROF_BUDGET( "CBaseFileSystem::Truncate", VPROF_BUDGETGROUP_OTHER_FILESYSTEM );

	AUTOBLOCKREPORTER_FH( Truncate, this, true, file, FILESYSTEM_BLOCKING_SYNCHRONOUS, FileBlockingItem::FB_ACCESS_WRITE );

	CFileHandle *fh = ( CFileHandle * )file;
	if ( !fh )
	{
		FileSystemWarning( FILESYSTEM_WARNING, "FS:  Tried to Truncate NULL file handle!\n" );
		return false;
	}
	return fh->Truncate();
...
{
	m_bEnableFileWriteProtection = bEnable;
}

bool CBaseFileSystem::GetWriteProtectionEnable() const
{
	return m_bEnableFileWriteProtection;
}
...
bool CFileHandle::Truncate()
{
	Assert( IsValid() );

	if ( !m_pFile )
	{
		m_fs->FileSystemWarning( FILESYSTEM_WARNING, "FS:  Tried to Truncate NULL file pointer inside valid file handle!\n" );
		return false;
	}

	bool success = m_fs->FS_truncate( m_pFile );
	m_fs->Trace_FWrite( 0, m_pFile );
