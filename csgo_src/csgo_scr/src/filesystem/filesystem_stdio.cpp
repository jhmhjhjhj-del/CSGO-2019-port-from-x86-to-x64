...
protected:
	...
	virtual bool FS_truncate( FILE* fp );
...
public:
	...
	virtual long FS_ftell();
	virtual int FS_feof();
	virtual bool FS_truncate() { return false; }
	virtual size_t FS_fread( void *dest, size_t destSize, size_t size);
...
// Purpose: low-level filesystem wrapper
//-----------------------------------------------------------------------------
bool CFileSystem_Stdio::FS_truncate( FILE *fp )
{
	if ( ShouldFailIo() )
		return 0;

	CStdFilesystemFile *pFile = ( ( CStdFilesystemFile * )fp );
	return pFile->FS_truncate();
}
...
CStdioFile *CStdioFile::FS_fopen( const char *filename, const char *options, int64 *size )
{
	...
	if (pFile && size)
	{
		...
		if (rt == -1)
		{
			...
			if (eOSType == k_eWin2000 || eOSType == k_eWinXP || eOSType == k_eWin2003)
			{
				rt = WindowsXPStatShim(filename, &buf);
			}
		}
#endif // defined(_WIN32) && defined(FILESYSTEM_MSVC2015_STAT_BUG_WORKAROUND)		
...
// Purpose: low-level filesystem wrapper
//-----------------------------------------------------------------------------
bool CStdioFile::FS_truncate()
{
	// Make sure no buffers are dirty before we truncate file size.
	// (we use raw file descriptor io here which interacts poorly with
	// FILE* buffering, this should make it OK)
	int64 pos = ftell( m_pFile );
	fflush( m_pFile );

#if defined(WIN32)
	errno_t err = _chsize_s( _fileno( m_pFile ), pos );
	return ( err == 0 );
#elif defined(POSIX) || defined(PS3)
	int err = ftruncate( fileno( m_pFile ), pos );
	return ( err == 0 );
#endif
}
