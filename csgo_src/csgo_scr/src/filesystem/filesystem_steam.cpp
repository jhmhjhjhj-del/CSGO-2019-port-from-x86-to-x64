...
// Purpose: low-level filesystem wrapper
//-----------------------------------------------------------------------------
bool CFileSystem_Steam::FS_truncate( FILE *fp )
{
#ifdef POSIX
	CSteamFile *steamFile = ( CSteamFile * )fp;
	fp = GetFileHandle( steamFile );
	if ( BWriteable( steamFile ) )
	{
		...
		int64 pos = ftell( fp );
		fflush( m_pFile );
		int err = ftruncate( fileno( fp ), pos );
		return ( err == 0 );
	}
	else
	{
#endif
		TSteamError steamError = { eSteamErrorUnknown, eNoDetailedErrorAvailable, 0, "FS_truncate not implemented for steam files" };
		CheckError( ( SteamHandle_t )fp, steamError );
		return false;
#ifdef POSIX
	}
