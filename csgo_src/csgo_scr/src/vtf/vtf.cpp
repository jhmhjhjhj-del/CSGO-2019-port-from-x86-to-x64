...
bool CVTFTexture::Init( int nWidth, int nHeight, int nDepth, ImageFormat fmt, int iFlags, int iFrameCount, int nForceMipCount )
{
	...
	if ( ( fmt == IMAGE_FORMAT_DXT1 ) || ( fmt == IMAGE_FORMAT_DXT3 ) || ( fmt == IMAGE_FORMAT_DXT5 ) ||
		 ( fmt == IMAGE_FORMAT_DXT1_RUNTIME ) || ( fmt == IMAGE_FORMAT_DXT5_RUNTIME ) || (fmt == IMAGE_FORMAT_DXT3_RUNTIME) )
	{
...
bool CVTFTexture::Serialize( CUtlBuffer &buf )
{
	...
	else if ( m_Format == IMAGE_FORMAT_DXT3_RUNTIME )
	{
		header.imageFormat = IMAGE_FORMAT_DXT3;
	}
...
//-----------------------------------------------------------------------------
void CVTFTexture::ConvertImageFormat( ImageFormat fmt, bool bNormalToDUDV, bool bNormalToDXT5GA, bool bYCoCg )
{
	if ( !m_pImageData )
	{
		return;
		...
		for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame)
		{
			for (int iFace = 0; iFace < m_nFaceCount; ++iFace)
			{
				...
				for ( int z = 0; z < nMipDepth; ++z, pSrcData += nSrcFaceStride, pDstData += nDstFaceStride )
				{
					...
					else if ( bYCoCg && !(m_Options.flags0 & VtfProcessingOptions::OPT_NOCOMPRESS) )
					{
						if ( ( m_Format == IMAGE_FORMAT_RGB888 ) ||
							 ( m_Format == IMAGE_FORMAT_BGR888 ) )
						{
							ImageLoader::ConvertRGBToDXT5YCoCg( pSrcData, m_Format, pDstData, nMipWidth, nMipHeight );
						}
					}
					else
					{
