...
bool CAudioXAudio2::Init( const audio_device_init_params_t &params, int nDeviceIndex )
{
    ...
    // setup the format structure
	{
		...
		if ( params.m_bOverrideSpeakerConfig )
		{
			...
			if ( nChannels > deviceDetails.OutputFormat.Format.nChannels && nChannels == 4 )
			{
				//don't allow more channels than the device supports if quad output is selected as this can cause problems.
