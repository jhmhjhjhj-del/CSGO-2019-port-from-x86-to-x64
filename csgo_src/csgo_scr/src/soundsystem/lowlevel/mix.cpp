...
// convert a mono source from signed 16-bit PCM to signed unit scale float
void ConvertShortToFloat( float *pFloatOut, const short *pMonoInput, uint nCount, float flScale )
{
	fltx4 *pOutput = reinterpret_cast<fltx4 *>( pFloatOut );
	const shortx8 *pInput = reinterpret_cast<const shortx8 *>( pMonoInput );
	fltx4 fl4Scale = ReplicateX4( flScale );
	// output 8 values per iteration of the loop so divide by 8
	nCount /= 8;
	for ( uint i = 0; i < nCount; i++ )
	{
		shortx8 samples = LoadUnalignedShortSIMD( pInput );
		pInput++;
		fltx4 lo = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractLowAsInt32( samples ) ), fl4Scale );
		fltx4 hi = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractHighAsInt32( samples ) ), fl4Scale );
		StoreAlignedSIMD( (float *)pOutput, lo );
		pOutput++;
		StoreAlignedSIMD( (float *)pOutput, hi );
		pOutput++;
...
void SumStereoShortToFloat( float *pFloatOut, const short *pStereoInput, uint nCount, float flScale )
{
	fltx4 *pOutput = reinterpret_cast<fltx4 *>( pFloatOut );
	const shortx8 *pInput = reinterpret_cast<const shortx8 *>( pStereoInput );
	fltx4 fl4Scale = ReplicateX4( flScale );
	// output 4 values per iteration of the loop so divide by 4
	nCount /= 4;
	for ( uint i = 0; i < nCount; i++ )
	{
		shortx8 samples = LoadUnalignedShortSIMD( pInput );
		pInput++;
		// convert these at half scale because we will add them together
		fltx4 lo = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractLowAsInt32( samples ) ), fl4Scale );
		fltx4 hi = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractHighAsInt32( samples ) ), fl4Scale );

		fltx4 left = _mm_shuffle_ps( lo, hi, MM_SHUFFLE_REV( 0, 2, 0, 2 ) );
		fltx4 right = _mm_shuffle_ps( lo, hi, MM_SHUFFLE_REV( 1, 3, 1, 3 ) );
		StoreAlignedSIMD( (float *)pOutput, AddSIMD( left, right ) );
		pOutput++;
	}
}

void ConvertFloatToShort( short *pOutput, const float *pFloatInput, uint nCount, float flScale )
{
	fltx4 fl4Scale = ReplicateX4( flScale );
	const fltx4 *pPhononData = reinterpret_cast<const fltx4 *>( pFloatInput );
	nCount /= 8;
	for ( uint n = 0; n < nCount; ++n )
	{
		fltx4 fl4Samples0 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4Scale );
		pPhononData++;
		fltx4 fl4Samples1 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4Scale );
		pPhononData++;

		shortx8 nSamples0 = RoundtFloatToInt32( fl4Samples0 );
		shortx8 nSamples1 = RoundtFloatToInt32( fl4Samples1 );
		shortx8 nSamplesOut = PackInt32x2ToShortx8( nSamples0, nSamples1 );
		StoreUnalignedSIMD( pOutput, nSamplesOut );
		pOutput += 8;
	}
}

void AddStereoFloatToStereoShort( short *pOutput, float *pFloatInput, float flFloatScale, short *pShortInput, float flShortScale, uint nCount )
{
	const shortx8 *pInput = reinterpret_cast<const shortx8 *>( pShortInput );
	const fltx4 *pPhononData = reinterpret_cast<const fltx4 *>( pFloatInput );
	fltx4 fl4FloatScale = ReplicateX4( flFloatScale );
	fltx4 fl4ShortScale = ReplicateX4( flShortScale );
	nCount /= 8;
	for ( uint n = 0; n < nCount; n++ )
	{
		fltx4 fl4Samples0 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4FloatScale );
		pPhononData++;
		fltx4 fl4Samples1 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4FloatScale );
		pPhononData++;
		...
		pInput++;
		// convert these at half scale because we will add them together
		fltx4 fl4InputSamples0 = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractLowAsInt32( samples ) ), fl4ShortScale );
		fltx4 fl4InputSamples1 = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractHighAsInt32( samples ) ), fl4ShortScale );

		shortx8 nSamples0 = RoundtFloatToInt32( AddSIMD( fl4Samples0, fl4InputSamples0 ) );
		shortx8 nSamples1 = RoundtFloatToInt32( AddSIMD( fl4Samples1, fl4InputSamples1 ) );
		shortx8 nSamplesOut = PackInt32x2ToShortx8( nSamples0, nSamples1 );
		StoreUnalignedSIMD( pOutput, nSamplesOut );
		pOutput += 8;
	}
...
void AddStereoFloatToMonoShort( short *pOutput, float *pFloatInput, float flFloatScale, short *pShortInput, float flShortScale, uint nCount )
{
	const shortx8 *pInput = reinterpret_cast<const shortx8 *>( pShortInput );
	const fltx4 *pPhononData = reinterpret_cast<const fltx4 *>( pFloatInput );
	fltx4 fl4FloatScale = ReplicateX4( flFloatScale );
	...
	for ( uint n = 0; n < nCount; n++ )
	{
		// read 8 samples of input data
		shortx8 samples = LoadUnalignedShortSIMD( pInput );
		pInput++;

		// extract and scale 8 float samples
		fltx4 fl4SamplesA0 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4FloatScale );
		pPhononData++;
		fltx4 fl4SamplesA1 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4FloatScale );
		pPhononData++;

		// extract and scale first 4 mono samples
		fltx4 fl4SamplesBMono = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractLowAsInt32( samples ) ), fl4ShortScale );
		// duplicate each sample making 8 samples total
		fltx4 fl4SamplesB0 = _mm_shuffle_ps( fl4SamplesBMono, fl4SamplesBMono, MM_SHUFFLE_REV( 0, 0, 1, 1 ) );
		fltx4 fl4SamplesB1 = _mm_shuffle_ps( fl4SamplesBMono, fl4SamplesBMono, MM_SHUFFLE_REV( 2, 2, 3, 3 ) );

		// now add them together and write out in 16-bit
		shortx8 nSamples0 = RoundtFloatToInt32( AddSIMD( fl4SamplesA0, fl4SamplesB0 ) );
		shortx8 nSamples1 = RoundtFloatToInt32( AddSIMD( fl4SamplesA1, fl4SamplesB1 ) );
		shortx8 nSamplesOut = PackInt32x2ToShortx8( nSamples0, nSamples1 );
		StoreUnalignedSIMD( pOutput, nSamplesOut );
		pOutput += 8;

		// read second half of float data
		fl4SamplesA0 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4FloatScale );
		pPhononData++;
		fl4SamplesA1 = MulSIMD( LoadAlignedSIMD( pPhononData ), fl4FloatScale );
		pPhononData++;

		// extract and scale high 4 mono samples
		fltx4 fl4InputSamples1 = MulSIMD( SignedIntConvertToFltSIMD( SignedExtractHighAsInt32( samples ) ), fl4ShortScale );
		fl4SamplesB0 = _mm_shuffle_ps( fl4InputSamples1, fl4InputSamples1, MM_SHUFFLE_REV( 0, 0, 1, 1 ) );
		fl4SamplesB1 = _mm_shuffle_ps( fl4InputSamples1, fl4InputSamples1, MM_SHUFFLE_REV( 2, 2, 3, 3 ) );

		// now add them together and write out in 16-bit
		nSamples0 = RoundtFloatToInt32( AddSIMD( fl4SamplesA0, fl4SamplesB0 ) );
		nSamples1 = RoundtFloatToInt32( AddSIMD( fl4SamplesA1, fl4SamplesB1 ) );
		nSamplesOut = PackInt32x2ToShortx8( nSamples0, nSamples1 );
		StoreUnalignedSIMD( pOutput, nSamplesOut );
		pOutput += 8;
	}
}
