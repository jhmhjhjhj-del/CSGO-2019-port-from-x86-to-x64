...
public:
	...
	void		PlaySoundAtOSLevel( const char *filename );
...
void Plat_PlaySound( const char* );

void CSoundSystem::PlaySoundAtOSLevel( const char* filename )
{
	char relpath[1024] = "sound/";
	Q_strncat( relpath, filename, sizeof(relpath));

	char fullpath[1024];
	g_pFullFileSystem->RelativePathToFullPath( relpath, "GAME", fullpath, sizeof(fullpath));

	Plat_PlaySound( fullpath );
}