...
// Note: Must match the definition in game/shared/shareddefs.h
enum DamageHitGroup_t
{
	HITGROUP_GENERIC = 0,
	HITGROUP_HEAD,
	HITGROUP_CHEST,
				...
				int boneMask )
{
	...
	{
		BoneQuaternionAligned q3;
		float weight = aBonesSlerpAlignWeights[i];
	...
	{
		float weight = aBonesSlerpNoAlignWeights[i];
	...
	matrix3x4_t **hitboxbones, int fContentsMask, const Vector &vecOrigin, float flScale, trace_t &tr )
{
	...
	enum EHitGroupType_t
	{
		...
		k_EHitGroupType_Neck, // same as chest but doesn't block head
	...
	for ( int i = 0; i < set->numhitboxes; i++ )
	{
		...
		case HITGROUP_HEAD:
			...
			break;
		case HITGROUP_NECK:
			pHitGroupResult = &arrHitGroupResults[k_EHitGroupType_Neck];
			break;
		case HITGROUP_CHEST:
...
bool Studio_AnimPosition( mstudioanimdesc_t *panim, float flCycle, Vector &vecPos, QAngle &vecAngle )
{
	...
	for (int i = 0; i < panim->nummovements; i++)
	{
		...
		if ( pMove->endframe >= flFrame )
		{
			...
			if (iLoops != 0)
			{
				mstudiomovement_t *pLoopMove = panim->pMovement( panim->nummovements - 1 );
				vecPos = vecPos + iLoops * pLoopMove->position;
				vecAngle.y = vecAngle.y + iLoops * pLoopMove->angle;
			}
			return true;
		}
		else
