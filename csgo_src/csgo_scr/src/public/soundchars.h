...
#define CHAR_HRTF_FORCE		'&'		// as one of 1st 2 chars in name, indicates wav should use HRTF spatialization for all entities (including owners).
#define CHAR_HRTF			'~'		// as one of 1st 2 chars in name, indicates wav should use HRTF spatialization for non-owners.
#define CHAR_HRTF_BLEND		'`'		// as one of 1st 2 chars in name, indicates wav should use HRTF spatialization for non-owners, blended with stereo for sounds sufficiently close.
...
#define CHAR_MUSIC			'%'		// as one of 1st 2 chars in name, indicates main menu music
...
inline bool IsSoundChar(char c)
{
	...
	b = b || ( c == CHAR_DOPPLER || c == CHAR_DIRECTIONAL || c == CHAR_DISTVARIANT || c == CHAR_HRTF || c == CHAR_HRTF_BLEND || c == CHAR_HRTF_FORCE || c == CHAR_SPATIALSTEREO || c == CHAR_FAST_PITCH );
	b = b || ( c == CHAR_SUBTITLED || c == CHAR_MUSIC );
