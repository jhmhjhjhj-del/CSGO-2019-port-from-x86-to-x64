...
//
// Purpose: Insert this file into all projects that need certain CRT
//			functionality (not relating to memory) overridden.
//
// Overridden functionality:
//
// 1. FLS allocation.
// 
// The Win32 C runtime allocates several fiber-local-storage (FLS) entries
// during CRT init, which is a process-global scarce resource (128 entries
// per process at the time of this writing).  However, this storage is
// re-allocated for every active CRT, which means that a process with many
// DLLS loaded that each statically link the CRT are at risk of running out
// of entries.  When this happens, the CRT fails to initialize and the DLL
// does not load.  As you can imagine, random DLL loads failing is a big
// problem!
//
// The CRT workaround for systems without FLS is to use TLS (thread-local
// storage) instead.  We implement that workaround here.
//
// There is a risk, however; if the CRT or any of our code actually tries
// to use FLS on multiple fibers, it could fail because those fibers don't
// necessarily have their own thread, and therefore might share the same
// TLS.
//
// Currently, this doesn't seem to happen (we don't use fibers, and this
// workaround has shipped to customers and the CRT doesn't appear to be
// using fibers behind our back).  But a future version of the CRT that
// suddenly requires this functionality will break our code.
//
// 2. CRT asserts.  We redirect windows CRT asserts to AssertMsg().
//
// 3. CRT usages of Event API and critical sections?  (REI: I've
// asked Daniel J why we are overriding these so I can update this
// documentation.)
//
// $NoKeywords: $
//===========================================================================//

#if !(defined( _PREFAST_ ) && defined( _WIN32 ))	// Disable this file for the win32 analyze build to work around a build error
        ...
        )
{
	...
	V_WStringToUTF8( _Message, Msg8, sizeof(Msg8) );
	V_WStringToUTF8( _File, File8, sizeof(File8) );
	AssertMsg( false, "CRT assertion failed: %s(%u): %s",
			   File8, _Line, Msg8 );
}

#endif // #if defined( _WIN32 )

#endif // #if defined( USE_VCRT )

#if !defined( NO_CRTOVERRIDE ) && !defined( USE_VCRT )

#if defined( _MSC_VER ) && ( _MSC_VER >= 1900 ) && !defined( _DLL )

// If we're on Windows and in VS2015 or newer, we want to override the FLS
// CRT-local wrappers with our own version that forces the fallback behavior
// that is used on platforms that don't have FLS. (Except in DLL CRT builds
// where we don't need to do this and probably can't.)
...
#if (WINVER < 0x602) // Old windows SDK, but we are defining some additional symbols for the CRT
#include <intrin.h>  
...
// Needed for decltype() declaration of EventSetInformation_pft
ULONG
EVNTAPI
EventSetInformation(
...
// Redefine _ASSERTE to point at our assert, otherwise we fail to link in debug due to
// missing symbols from crtdebug.h
#undef _ASSERTE
#if defined( USE_VCRT )
#define _ASSERTE(expr) Assert(msg)
#else
...
{
    void* const encoded_nullptr = __crt_fast_encode_pointer<void * const>(nullptr);
...
#endif // #if defined( _MSC_VER ) && ( _MSC_VER >= 1900 ) && !defined( _DLL )

#endif // #if !defined( NO_CRTOVERRIDE ) && !defined( USE_VCRT )

#endif // #if !(defined( _PREFAST_ ) && defined( _WIN32 ))	// Disable this file for the win32 analyze build to work around a build error