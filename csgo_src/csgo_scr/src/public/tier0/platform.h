...
// Partner depot defaults to no GC — but offline stub needs server-side SOCache
// so unusual knives get the correct model via InventoryUpdated / GiveNamedItem.
#if !defined( NO_STEAM_GAMECOORDINATOR ) && !defined( CSGO_OFFLINE_GC_INVENTORY )
	#define NO_STEAM_GAMECOORDINATOR
#endif
...
{
	return ::new( pMemory ) T( Forward<Args>(args)... );
}
