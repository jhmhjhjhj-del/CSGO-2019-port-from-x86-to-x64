...
//-----------------------------------------------------------------------------
//
// Static/global-data-compatible single-initialization functionality.
// Makes it easy to protect a code block to only run once without
// needing an explicit lock or anything other than a default-zero-init
// static/global int32.
//
//-----------------------------------------------------------------------------

// Need-init must be zero for convenient use with
// static/global zero-initialization.
#define THREAD_INIT_ONCE_NEED_INIT 0
#define THREAD_INIT_ONCE_IN_INIT   1
#define THREAD_INIT_ONCE_INIT_DONE 2

typedef int32 ThreadInitOnce_t;

// If ThreadInitOnce returns true then the caller must do the
// initialization (and other callers will block waiting).
// Once finished with initialization the initializing code should
// call ThreadFinishInitOnce.
// If ThreadInitOnce returns false the initialization is done
// and the caller is free to proceed.
PLATFORM_INTERFACE bool ThreadInitOnce( ThreadInitOnce_t *pMarker );
PLATFORM_INTERFACE void ThreadFinishInitOnce( ThreadInitOnce_t *pMarker );
// Variant which calls the given function pointer for initialization.
// Fatal-errors if the initialization fails.
PLATFORM_INTERFACE void ThreadInitOnceCall( ThreadInitOnce_t *pMarker, bool( *InitFunc )(ThreadInitOnce_t*) );
...
#if defined( _PS3 ) || defined( PLATFORM_POSIX )
	bool Wait( uint32 dwTimeout = TT_INFINITE );
#endif
