...
public:
	...
	/// 
	/// Initializing the client library automatically initiates ping measurement,
	/// and ping times tend to be relatively stable, so you probably will never
	/// need to call this.  It's not called automatically when you initialize a
	/// dedicated server.  However, if you use any function that needs the data
	/// (such as listening for P2P connections or creating a relayed connection),
	/// we will begin gathering the data at that time.
	///
	/// Here are two times when you might need to call it:
	///
	/// - You really do need to force an immediate refresh of the data.  (For
	///   example, in response to a specific user input to refresh this information.)
	/// - On a dedicated server, when you know that you will need P2P
	///   functionality.
	///
	/// Don't call this function "just in case", before making a connection.
	/// The library will automatically refresh the information if it's too
	/// old as needed.
	///
	...
	/// Convert a ping location into a format suitable for sending over the wire.
