...
#define BONE_SCREEN_ALIGN_BILLBOARD	0x40	// bone aligns to the screen normal, not camera center
#define BONE_KEEP_UPRIGHT			0x80
...
struct mstudiobbox_t
{
	...
	float volume() const
	{
		if ( flCapsuleRadius > 0.0f )
		{
			// capsule
			float sphereVol = ( ( 4.0f / 3.0f ) * M_PI_F ) * ( flCapsuleRadius * flCapsuleRadius * flCapsuleRadius );
			float cylHeight = ( bbmax - bbmin ).Length();
			float cylVol = M_PI_F * ( flCapsuleRadius * flCapsuleRadius * cylHeight );
			return ( sphereVol + cylVol );
		}
		else
		{
			// box
			Vector sz = bbmax - bbmin;
			return Max( sz.x, 0.0f ) * Max( sz.y, 0.0f ) * Max( sz.z, 0.0f );
		}
	}
