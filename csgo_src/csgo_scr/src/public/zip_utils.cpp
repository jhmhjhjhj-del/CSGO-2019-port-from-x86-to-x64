...
//-----------------------------------------------------------------------------
class CBufferStream : public IZipWriteStream
{
public:
	CBufferStream( CUtlBuffer& buff ) : IZipWriteStream(), m_buff( &buff ) {}
...
//-----------------------------------------------------------------------------
class CFileStream : public IZipWriteStream
{
public:
	CFileStream( FILE *fout ) : IZipWriteStream(), m_file( fout ), m_hFile( INVALID_HANDLE_VALUE ) {}
	CFileStream( HANDLE hOutFile ) : IZipWriteStream(), m_file( NULL ), m_hFile( hOutFile ) {}
...
public:
	...
	// Write the zip to a custom stream
	// (set by file's previous alignment, or a call to ForceAlignment)
	void			SaveToCustomStream( IZipWriteStream& stream );
...
private:
	...
	void			SaveDirectory( IZipWriteStream& stream );
...
void CZipFile::ParseFromBuffer( void *buffer, int bufferlength )
{
	...
	for( int64_t offset = static_cast< int64_t >( fileLen - sizeof( ZIP_EndOfCentralDirRecord ) ) ; offset >= 0LL; --offset )
	{
		buf.SeekGet( CUtlBuffer::SEEK_HEAD, static_cast<unsigned int>( offset ) );
		buf.GetObjects( &rec );
		if ( rec.signature == PKID( 5, 6 ) )
		{
			...
			if ( rec.commentLength )
			{
				...
				int commentLength = MIN( rec.commentLength, sizeof( commentString ) - 1 );
	...
	for ( i = 0; i < rec.nCentralDirectoryEntries_Total; i++ )
	{
		...
		size_t fileNameLength = MIN( zipFileHeader.fileNameLength, V_ARRAYSIZE( tmpString ) - 1 );
		buf.Get( tmpString, fileNameLength );
		tmpString[ fileNameLength ] = '\0';
...
HANDLE CZipFile::ParseFromDisk( const char *pFilename )
{
	...
	for ( int64_t offset = static_cast< int64_t >( fileLen - sizeof( ZIP_EndOfCentralDirRecord ) ); offset >= 0LL; offset-- )
	{
		CWin32File::FileSeek( hFile, static_cast<unsigned int>( offset ), FILE_BEGIN );
...
//-----------------------------------------------------------------------------
// Purpose: Store data out to a custom stream
//-----------------------------------------------------------------------------
void CZipFile::SaveToCustomStream( IZipWriteStream& stream )
{
	SaveDirectory( stream );
}
...
//-----------------------------------------------------------------------------
void CZipFile::SaveDirectory( IZipWriteStream& stream )
{
...
public:
	...
	// Writes out zip file to a custom 'write stream' - uses current alignment size 
	// (set by file's previous alignment, or a call to ForceAlignment)
	virtual void			SaveToCustomStream( IZipWriteStream& stream );
...
void CZip::SaveToCustomStream( IZipWriteStream& stream )
{
	m_ZipFile.SaveToCustomStream( stream );
}
