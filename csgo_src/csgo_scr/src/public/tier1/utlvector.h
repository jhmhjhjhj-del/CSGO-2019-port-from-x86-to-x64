...
// VC++ does not like us using (*p).IsUtlVector in some cases, throwing out analyze warnings. Pass through
// this identify ref functions to convince it that (*p) is just a regular reference and can be used without warning us.
template <typename T> FORCEINLINE_TEMPLATE T& AnalyzeVecRef(T& ref) { return ref; }

#define FOR_EACH_VEC( vecName, iteratorName ) \
	for ( int iteratorName = 0; AnalyzeVecRef(vecName).IsUtlVector && iteratorName < AnalyzeVecRef(vecName).Count(); iteratorName++ )
#define FOR_EACH_VEC_BACK( vecName, iteratorName ) \
	for ( int iteratorName = AnalyzeVecRef(vecName).Count()-1; AnalyzeVecRef(vecName).IsUtlVector && iteratorName >= 0; iteratorName-- )
...
public:
	...
	int AddToTail( const T& src )
	{
		...
		::CopyConstruct( &m_pData->m_Elements[iNew], src );
		m_pData->m_Size++;
		return iNew;
	}
	...
	int AddToTail( T&& src )
	{
		int iNew = Count();
		EnsureCapacity( Count() + 1 );
		::MoveConstruct( &m_pData->m_Elements[iNew], Move( src ) );
		m_pData->m_Size++;
		return iNew;
	}
...
//-----------------------------------------------------------------------------
template< typename T, class A >
void CUtlVector<T, A>::Shuffle( IUniformRandomStream* pSteam /*= NULL*/ )
{
...
inline int CUtlVector<T, A>::AddToHead( T&& src )
{
	// Can't insert something that's in the list... reallocation may hose us
	Assert( ( Base() == NULL ) || ( &src < Base() ) || ( &src >= ( Base() + Count() ) ) );
	return InsertBefore( 0, Move( src ) );
}
...
inline int CUtlVector<T, A>::AddToTail( T&& src )
{
	// Can't insert something that's in the list... reallocation may hose us
	Assert( ( Base() == NULL ) || ( &src < Base() ) || ( &src >= ( Base() + Count() ) ) );
	return InsertBefore( m_Size, Move( src ) );
}
...
inline int CUtlVector<T, A>::InsertAfter( int elem, T&& src )
{
	// Can't insert something that's in the list... reallocation may hose us
	Assert( ( Base() == NULL ) || ( &src < Base() ) || ( &src >= ( Base() + Count() ) ) );
	return InsertBefore( elem + 1, Move( src ) );
}
