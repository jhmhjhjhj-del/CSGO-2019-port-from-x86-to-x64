...
// be encoded
//
// CS:GO only: We also encode the characters \ # $ for simplification of CS:GO loc code
//
// Returns false if there was not enough room in pDest to encode the entire source string, otherwise true
// If pnDstRequiredSize is present, it will contain the number of characters written to pDest on success and
// the number of characters required for successful encoding on failure.
bool V_BasicHtmlEntityEncode( OUT_Z_CAP( nDestChars ) char *pDest, int nDestChars, IN_CAP( nInChars ) char const *pIn, int nInChars, int* pnDstRequiredChars = nullptr, bool bPreserveWhitespace = false );
bool V_BasicHtmlEntityEncode( OUT_Z_CAP( nDestChars ) wchar_t *pDest, int nDestChars, IN_CAP( nInChars ) wchar_t const *pIn, int nInChars, int* pnDstRequiredChars = nullptr, bool bPreserveWhitespace = false );
