...
{
	KeyValues *m_pNode;
	KeyValues *m_pLastSubNode;
	uint m_unSubNodes;
};

struct KVJSONParserContext_t
{
	CUtlVector< KVJSONParserStackEntry_t > m_vecStack;
	// This is a CUtlBuffer and not a CUtlString for performance reasons;
	// CUtlString releases its memory when cleared, which we do not want.
	CUtlBuffer m_bufNextChildName;
};
...
public:
	...
	// Assign keyvalues from a JSON blob
	// Used for pasing JSON data into KV data

	static inline KeyValues *KVJSONParserCreateNewSubNode( KVJSONParserContext_t *ctx, bool bPush );
	static inline int JSONParserCallback( void* void_ctx, int type, const JSON_value* value );
	static inline KeyValues *FromJSON( CUtlBuffer &inputBuffer );
...
//	void WriteConvertedString( CUtlBuffer &buffer, const char *pszString );
	...
	void RecursiveLoadFromBuffer( char const *resourceName, CKeyValuesTokenReader &buf, GetSymbolProc_t pfnEvaluateSymbolProc, bool bForceTypeString = false );
...
inline KeyValues *KeyValues::KVJSONParserCreateNewSubNode( KVJSONParserContext_t *ctx, bool bPush )
{
	KeyValues *pNewSubNode;

	{
		KVJSONParserStackEntry_t& tail = ctx->m_vecStack[ ctx->m_vecStack.Count() - 1 ];
		CNumStr numStr;
		const char *pszName = ctx->m_bufNextChildName.TellPut() ? (const char*)ctx->m_bufNextChildName.Base() : NULL;
		if ( !pszName || !pszName[ 0 ] )
		{
			numStr.SetUint32( tail.m_unSubNodes );
			pszName = numStr;
		}
		pNewSubNode = tail.m_pLastSubNode ?
			tail.m_pLastSubNode->CreatePeerKey( pszName ) :
			tail.m_pNode->CreateKey( pszName );
		tail.m_pLastSubNode = pNewSubNode;
		tail.m_unSubNodes++;
	}

	if ( bPush )
	{
		KVJSONParserStackEntry_t& newtail = ctx->m_vecStack[ ctx->m_vecStack.AddToTail() ];
		newtail.m_pNode = pNewSubNode;
		newtail.m_pLastSubNode = NULL;
		newtail.m_unSubNodes = 0;
	}

	ctx->m_bufNextChildName.Clear();

	return pNewSubNode;
}


inline int KeyValues::JSONParserCallback( void* void_ctx, int type, const JSON_value* value )
{
	KVJSONParserContext_t *ctx = (KVJSONParserContext_t *)void_ctx;
	switch ( type )
	{
	case JSON_T_ARRAY_BEGIN:
		if ( ctx->m_vecStack.Count() == 0 )
		{
			// this might be the start of the root object itself
			KVJSONParserStackEntry_t& tail = ctx->m_vecStack[ ctx->m_vecStack.AddToTail() ];
			tail.m_pNode = new KeyValues( (const char*)ctx->m_bufNextChildName.Base() );
			tail.m_pLastSubNode = NULL;
			tail.m_unSubNodes = 0;
			ctx->m_bufNextChildName.Clear();
		}
		else
		{
			KVJSONParserCreateNewSubNode( ctx, true );
		}
		break;

	case JSON_T_ARRAY_END:
		// We may get called for the root node, don't remove it though, so we can actually return it at the end.
		if ( ctx->m_vecStack.Count() > 1 )
		{
			ctx->m_vecStack.Remove( ctx->m_vecStack.Count() - 1 );
		}
		break;
	...
	case JSON_T_KEY:
		ctx->m_bufNextChildName.Clear();
		ctx->m_bufNextChildName.Put( value->vu.str.value, (int)value->vu.str.length + 1 ); // include null terminator
		break;
	...
	case JSON_T_STRING:
	{
		...
		KeyValues *pCreatedNode = KVJSONParserCreateNewSubNode( ctx, false );
		...
		case JSON_T_INTEGER:
			...
			if ( value->vu.integer_value <= INT_MAX && value->vu.integer_value >= INT_MIN )
			{
				pCreatedNode->SetInt( NULL, (int32)value->vu.integer_value );
			}
			else
			{
				pCreatedNode->SetUint64( NULL, (uint64)value->vu.integer_value );
			}
			break;

		case JSON_T_FLOAT:
			pCreatedNode->SetFloat( NULL, value->vu.float_value );
			break;

		case JSON_T_NULL:
			pCreatedNode->SetPtr( NULL, NULL );
			break;

		case JSON_T_TRUE:
			pCreatedNode->SetInt( NULL, 1 );
			break;

		case JSON_T_FALSE:
			pCreatedNode->SetInt( NULL, 0 );
			break;

		case JSON_T_STRING:
			pCreatedNode->SetString( NULL, value->vu.str.value );
			break;
...
inline KeyValues *KeyValues::FromJSON( CUtlBuffer &inputBuffer )
{
	...
	int nBytesRemaining = inputBuffer.GetBytesRemaining();
	const char *pchInput = (const char *)inputBuffer.PeekGet();
	...
	bool bWrapContent = pchInput[ 0 ] == '\"';

	KVJSONParserContext_t context;
	JSON_config config;
	init_JSON_config( &config );
	config.depth = 19;
	config.callback = &KeyValues::JSONParserCallback;
	...
	bool bSuccess = ( JSON_parser_string( jc, pchInput, nBytesRemaining ) != 0 );

	if ( bWrapContent )
		JSON_parser_char( jc, '}' );

	bSuccess = bSuccess && JSON_parser_done( jc );
	...
	{
		Assert( context.m_vecStack.Count() <= 1 );
		if ( context.m_vecStack.Count() == 0 )
		{
			bSuccess = false;
		}
		else
		{
			KeyValues *pWrapRoot = context.m_vecStack[ context.m_vecStack.Count() - 1 ].m_pNode;
			KeyValues *pRealRoot = pWrapRoot->GetFirstValue();
			if ( !pRealRoot )
			{
				bSuccess = false;
			}
			...
			{
				if ( pRealRoot->GetNextKey() )
				{
					bSuccess = false;
				}
				else
				{
					pWrapRoot->RemoveSubKey( pRealRoot );
					pWrapRoot->deleteThis();
					context.m_vecStack[ context.m_vecStack.Count() - 1 ].m_pNode = pRealRoot;
				}
			}
		}
	}

	if ( bSuccess && context.m_vecStack.Count() == 1 )
	{
		return context.m_vecStack[ 0 ].m_pNode;
	}
	else
	{
		if ( context.m_vecStack.Count() )
			context.m_vecStack[ 0 ].m_pNode->deleteThis();
