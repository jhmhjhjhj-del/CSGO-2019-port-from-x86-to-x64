...
public:
	...
	// Makes sure we've got at least this much memory without
	// crossing blocks.  Size cannot be larger than any block.
	// Returns the item index to use as the starting position,
	// which may be different than the hinted position if
	// the hinted position does not have sufficient contiguous space.
...
protected:
	...
	int MinorIndex( int i ) const { return (int)( i & m_nIndexMask ); }
	void ChangeSize( int nBlocks );
	int NumElementsInBlock() const { return (int)( m_nIndexMask + 1 ); }

	T** m_pMemory;
	int m_nBlocks;
	uint32 m_nIndexMask : 27;
	uint32 m_nIndexShift : 5;
};
...
void CUtlBlockMemory<T,I>::Init( int nGrowSize /* = 0 */, int nInitSize /* = 0 */ )
{
	...
	Assert( m_nIndexMask + 1 == ( 1U << m_nIndexShift ) );
...
inline int CUtlBlockMemory<T,I>::EnsureContiguousCapacity( int start, int num )
{
	...
	{
		// The whole range falls within one existing block, use
		// the suggested start.
		return start;
	}
