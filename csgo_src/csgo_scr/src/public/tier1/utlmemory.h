...
public:
	CUtlMemoryFixedGrowable( int nGrowSize = 0, int nInitSize = SIZE ) : BaseClass( m_pFixedMemory.Base(), SIZE ) 
	{
		// Compile-time check that alignment values are sane
		ValidateAlignment<T>();

		// Check that memory allocation is aligned properly.  If heap allocating and ALIGNMENT
		// > heap max alignment, you will need to use a custom allocator to allocate this object
		// aligned.
		Assert( ( (uintptr_t)m_pFixedMemory.Base() ) % VALIGNOF_TEMPLATE_SAFE( T ) == 0 );
...
private:
	...
	AlignedByteArray_t< SIZE, T > m_pFixedMemory;
};
...
{
public:
	static const int ALIGNMENT = MAX( VALIGNOF_TEMPLATE_SAFE( T ), nAlignment );

	// constructor, destructor
	explicit CUtlMemoryFixed( int nGrowSize = 0, int nInitSize = 0 ) 
	{
		// Compile-time check that alignment values are sane
		ValidateAlignmentExplicit<T, ALIGNMENT>();

		// Check that memory allocation is aligned properly.  If heap allocating and ALIGNMENT
		// > heap max alignment, you will need to use a custom allocator to allocate this object
		// aligned.
		Assert( ( ( uintptr_t )m_pFixedMemory.Base() ) % ALIGNMENT == 0 );
	...
	T* Base()												{ return m_pFixedMemory.Base(); }
	const T* Base() const									{ return m_pFixedMemory.Base(); }
...
private:
	AlignedByteArrayExplicit_t< SIZE, T, ALIGNMENT > m_pFixedMemory;
};
