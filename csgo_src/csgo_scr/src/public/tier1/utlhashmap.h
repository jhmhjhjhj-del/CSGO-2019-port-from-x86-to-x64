...
// utlhashmap HashFunctor specialization for CUtlStrings
template<>
...
void CUtlHashMap<K,T,L,H>::Swap( CUtlHashMap<K,T,L,H> &that )
{
	...
	std::swap( m_bitsMigratedBuckets, that.m_bitsMigratedBuckets );
	m_memNodes.Swap( that.m_memNodes );
	std::swap( m_iNodeFreeListHead, that.m_iNodeFreeListHead );
	std::swap( m_cElements, that.m_cElements );
	std::swap( m_nMaxElement, that.m_nMaxElement );
	std::swap( m_nMinRehashedBucket, that.m_nMinRehashedBucket );
	std::swap( m_nMaxRehashedBucket, that.m_nMaxRehashedBucket );
}
