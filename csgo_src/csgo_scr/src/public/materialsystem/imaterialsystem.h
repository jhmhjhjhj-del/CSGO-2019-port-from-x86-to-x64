...
// Panorama Slots, analagous to Scaleform slots. Describe the callsites in a frame where panorama is invoked (runframe and renderframe).
...
struct FlashlightState_t
{
	...
	bool m_bIgnoreNonHDRPath; // when true, ignore 2.0 flashlight scale when HDR not enabled (only used by panorama 3d panels for now)
...
//-----------------------------------------------------------------------------
// Custom buffer types that can be passed from client into shader dynamic state
//-----------------------------------------------------------------------------

enum ClientRenderStateBuffer_t
{
	CLIENT_RENDER_STATE_BUFFER_PROXIMITY_PLAYERS,	// array of Vector4D (x,y,z,distance) parameters, sorted from nearest to farthest
	CLIENT_RENDER_STATE_BUFFER_DANGERZONE_INFO,		// single Vector4D (x,y,z) position (w) radius
	CLIENT_RENDER_STATE_BUFFER_WORLD_2D_MINS_MAXS,	// (x,y) (z,w) client world top-down mins/maxs
};

#define CLIENT_RENDER_STATE_BUFFER_SET( pRenderContext, eBufferType, TVariableType, pArray, numElements ) \
	pRenderContext->SetClientRenderStateBufferState( eBufferType, static_cast< TVariableType const * >( pArray ), ( numElements ) * sizeof( TVariableType ) )

#define CLIENT_RENDER_STATE_BUFFER_GET( pShaderAPI, eBufferType, TVariableType, pArray, numElements ) \
	TVariableType const *pArray = NULL; \
	int numElements = 0; \
	{ \
		void const *pvDataBufferGetClientRenderStateBufferState = NULL; \
		int cbDataSizeGetClientRenderStateBufferState = 0; \
		if ( pShaderAPI->GetClientRenderStateBufferState( eBufferType, pvDataBufferGetClientRenderStateBufferState, cbDataSizeGetClientRenderStateBufferState ) && \
			( cbDataSizeGetClientRenderStateBufferState > 0 ) && \
			!( cbDataSizeGetClientRenderStateBufferState % sizeof( TVariableType ) ) ) \
		{ \
			pArray = reinterpret_cast< TVariableType const * >( pvDataBufferGetClientRenderStateBufferState ); \
			numElements = cbDataSizeGetClientRenderStateBufferState / sizeof( TVariableType ); \
		} \
	}
...
// #ifdef PANORAMA_ENABLE_
	virtual ITexture *			CreatePanoramaAlphaTexture( const char *pTextureName, const char *pTextureGroupName, int w, int h, int nFlags ) = 0;
	virtual bool				UpdatePanoramaAlphaTexture( ITexture *pTexture, int xOffset, int yOffset, int width, int height, void *pImageData ) = 0;
	virtual void*				GetPanormaTexturePtr( ITexture *pTexture ) = 0;
	virtual void				ResetPanoramaRenderState( ) = 0;
	virtual void *GetOSVertexShader( const char* pszName, int nIndex ) = 0;
	virtual void *GetOSPixelShader( const char*  pszName, int nIndex ) = 0;
	virtual void GetShaderApiDynamicState( uint32 **ppRenderStates, SamplerStateCopy_t ** ppSamplerStates, bool *srgbWrite ) = 0;
...
public:
	...
	virtual void				ForceCommitScissorRect() = 0;
...
// #endif
	...
	virtual void SetClientRenderStateBufferState( ClientRenderStateBuffer_t eBuffer, void const *pvBufferBase, int numBufferBytes ) = 0;
	
	virtual void DrawScreenSpaceRectangle( IMaterial *pMaterial, float flDestX, float flDestY, float flW, float flH, float flU0, float flV0, float flU1, float flV1, void *pClientRenderable = NULL )=0;
};
