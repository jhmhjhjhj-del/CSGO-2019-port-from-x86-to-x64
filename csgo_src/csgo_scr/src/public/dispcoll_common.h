...
public:
	...
	unsigned short		m_uiFlags:8;			// Uses 8-bits - maybe look into merging it with something?
...
//=============================================================================
//
// Initialization data for displacement Collision Tree
//
struct FDispCollTreeParams
{
	FDispCollTreeParams()
		: bSurfacePropAlphaSpecified( false )
		, fSurfacePropAlphaFilter( 0.5f )
	{}

	bool bSurfacePropAlphaSpecified;
	float fSurfacePropAlphaFilter;
};
...
{
public:
	static const int kMaxSurfacePropTypes = 5;

	// Creation/Destruction.
	CDispCollTree();
	~CDispCollTree();
	virtual bool Create( CCoreDispInfo *pDisp, const FDispCollTreeParams& params );
	...
	bool BCollisionFacesAreAllValid( void ) const;
...
//	inline void SetTriFlags( short iTri, unsigned short nFlags )	{ m_aTris[iTri].m_uiFlags = nFlags; }
	...
	inline void SetSurfaceProps( int iProp, short nSurfProp )		{ Assert( ( iProp >= 0 ) && ( iProp < kMaxSurfacePropTypes ) ); m_nSurfaceProps[iProp] = nSurfProp; }
...
protected:
	...
	bool AABBTree_Create( CCoreDispInfo *pDisp, const FDispCollTreeParams& params );
	void AABBTree_CopyDispData( CCoreDispInfo *pDisp, const FDispCollTreeParams& params );
...
protected:
	...
	short							m_nSurfaceProps[kMaxSurfacePropTypes];	// Surface properties (save off from texdata for impact responses)
