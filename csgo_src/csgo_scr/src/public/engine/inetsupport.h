...
public:
	...
	// Send an ad-hoc network packet to an arbitrary network address,
	// not associated with any particular netchan
	virtual int SendPacketToNsAddress( int sock,  const ns_address &to, const void *data, int length ) = 0;

	virtual ISteamNetworkingUtils *GetSteamNetworkingUtils() = 0;
	virtual ISteamNetworkingSockets *GetSteamNetworkingSockets() = 0;
	virtual ISteamNetworkingSockets *GetSteamNetworkingSocketsGameServer() = 0;

	virtual CRC64_t BuildServerInstanceCRCToken() = 0;

	// Describe SteamID of the remote network address (useful for old-style P2P connections via 0.0.0.1:1 on port=ISteamSocketMgr::STEAM_CNX_PORT
	virtual CSteamID GetSteamIdForRemoteNetworkAddress( ns_address &netAdrRemote ) = 0;
};
