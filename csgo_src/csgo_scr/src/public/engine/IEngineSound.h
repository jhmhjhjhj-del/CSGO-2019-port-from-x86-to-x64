...
public:
	...
	virtual int EmitSound( IRecipientFilter& filter, int iEntIndex, int iChannel, const char *pSoundEntry, HSOUNDSCRIPTHASH nSoundEntryHash, const char *pSample, 
		...
		const Vector *pOrigin = NULL, const Vector *pDirection = NULL, CUtlVector< Vector >* pUtlVecOrigins = NULL, bool bUpdatePositions = true, float soundtime = 0.0f, int speakerentity = -1, const CSoundParameters* params=nullptr ) = 0;
...
//	virtual EntChannel_t	CreateEntChannel() = 0;
	...
	// Sets a sound's origin point by guid. Returns true if successful.
	virtual bool	SetOriginByGuid( int guid, const Vector& origin ) = 0;
