...
public:
	...
	virtual void					GetIlluminationPoint( const model_t *model, IClientRenderable *pRenderable, Vector const& origin, 
										QAngle const& angles, float scale, Vector* pLightingCenter ) = 0;
	...
	virtual vcollide_t				*GetVCollide( const model_t *model, float scale ) const = 0;
	virtual vcollide_t				*GetVCollide( int modelindex, float scale ) const = 0;
};
