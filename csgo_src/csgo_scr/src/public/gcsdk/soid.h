...
const uint32 k_SOID_Type_LbsUpdate = 7;
const uint32 k_SOID_Type_EconSettlement = 8;
const uint32 k_SOID_Type_PlayerProfile = 9;
const uint32 k_SOID_Type_EventWatchEms = 10;
...
inline SOID_t GetSOIDForEconSettlement( uint32 eSettlementType )
{
	return SOID_t( k_SOID_Type_EconSettlement, eSettlementType );
}

inline SOID_t GetSOIDForEventWatchEms( uint32 eSettlementType )
{
	return SOID_t( k_SOID_Type_EventWatchEms, eSettlementType );
}

inline SOID_t GetSOIDForLbsUpdate( AccountID_t unAccountID, int iLbsLbId )
{
	return SOID_t( k_SOID_Type_LbsUpdate, ( uint64( unAccountID ) << 32 ) | uint64( uint32( iLbsLbId ) ) );
}
