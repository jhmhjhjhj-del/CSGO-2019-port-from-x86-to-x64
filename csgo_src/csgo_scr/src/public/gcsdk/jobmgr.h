...
public:
	...
	void SetThreadPoolSize( uint cThreads, bool bPreCreateThreads );
	...
	bool BResumeSQLJob( CGCSQLQueryGroup* pQueryGroup );
	...
	void DumpSQLInflight( int numTopRunningQueries );
#endif
	...
	// Yields the current job the specified other job finishes
	bool BYieldingWaitForJob( CJob &jobCur, JobID_t nJobIDToWaitFor );
	...
	// Work item.  Work items can be an actual CWorkitem object that does work in another thread.
	// Or it can just be a generic flag that says, "I'm waiting one something, and some system has me in a table
	// and will wake me up when it's done.)
	bool BYieldingWaitForWorkItem( CJob &job, const char *pszWorkItemName, JobID_t nJobID );
	...
	// When we are waiting on a a "virtual work item" and don't have a real CWorkItem,
	// set the job ID of the job that is actually doing the work that we are waiting
	// on.  (E.g. a single job is doing a bunch of batched work.)  This enables better
	// reporting and analysis.  This must be called from 
	void SetWaitForWorkItemJobOwner( JobID_t nJobIDWaiting, JobID_t nJobIDDoingWorkItem );
	...
	// Wakes up all jobs that were waiting on the given job to finish
	void WakeJobsWaitingOnJob( CJob &job );
