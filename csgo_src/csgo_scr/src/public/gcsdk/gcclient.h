...
public:
	...
	/// Allow a message to be sent outside of session handshake
	void AddAllowedConnectionlessMessage( uint32 unMsg ) { m_rbAllowedConnectionlessMessages.InsertIfNotFound( unMsg ); }
	...
	/// Steam datagram routing, for servers.  Sent in the HELLO message
	void SetServerSteamdatagramRouting( const std::string &routingBlob ) { m_sSteamdatagramRoutingBlob = routingBlob; }
...
protected:
	...
	// Messages that can be sent "connectionless" without establishing a GC session
	CUtlRBTree< uint32, short, CDefLess< uint32 > > m_rbAllowedConnectionlessMessages;
	...
	std::string m_sSteamdatagramRoutingBlob;
