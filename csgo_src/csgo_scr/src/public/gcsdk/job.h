...
public:
	...
	bool BYieldingWaitForWorkItem( const char *pszWorkItemName = NULL, JobID_t nJobID = k_GIDNil );

	// Abstracts how threaded work is submitted to a processor.
	bool BYieldingWaitForThreadFuncWorkItem( CWorkItem *pItem );
	bool BYieldingWaitForThreadFuncWorkItems( CWorkItem **workItems, int nCount );

	// Creates a default threaded work item and waits for it. (Takes ownership of functor pointer.)
	bool BYieldingWaitForThreadFunc( CFunctor *jobFunctor );

	// Creates default threaded work items and waits for them all. (Takes ownership of functor pointers.)
	bool BYieldingWaitForThreadFuncs( CFunctor **jobFunctors, int nCount );
	...
	// Set a job name, for debugging.
	// NOTE: This string must be statically allocated!
	void SetJobName( const char *pDebugName ) { m_pchJobName = pDebugName ? pDebugName : ""; }
...
private:
	...
	// Do linked list work, set the job we are waiting on
	void SetWaitingForJob( CJob *pJobToWaitFor );
	...
	JobID_t m_jobIDWaitingOn;  			// The job we're waiting on.  This is used if we're actually waiting on a job, but also if we're waiting on a work item, and we know the job that is doing the work.
	CJob *m_pFirstJobWaitingOnThis;		// Are any jobs waiting on this job to finish?  If so, this is the head of the linked list.  (m_pJobWaitingOnNext is the next link)
	CJob *m_pJobWaitingOnNext;       	// Next job in linked list of jobs waiting on the same thing.
