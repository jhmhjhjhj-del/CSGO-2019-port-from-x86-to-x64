...
//-----------------------------------------------------------------------------
abstract_class IZipWriteStream
{
public:
	virtual void Put( const void* pMem, int size ) = 0;
	virtual unsigned int Tell( void ) = 0;					// Note that Tell must return a relative offset
															// (relative to when we started writing the zip file to the stream)
};
...
public:
	...
	// (set by file's previous alignment, or a call to ForceAlignment)
	virtual void			SaveToCustomStream( IZipWriteStream& stream ) = 0;
