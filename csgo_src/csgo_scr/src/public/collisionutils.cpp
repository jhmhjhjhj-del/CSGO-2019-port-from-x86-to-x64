	...
	const Vector &vecOBBMins, const Vector &vecOBBMaxs, float flTolerance, CBaseTrace *pTrace )
{
	...
	// Compute a bounding sphere around the bloated OBB (broadphase reject)
	{
		Vector vecTranslatedMins = VectorTransform( vecOBBMins, matOBBToWorld );
		Vector vecTranslatedMaxs = VectorTransform( vecOBBMaxs, matOBBToWorld );

		Vector vecOBBCenter;
		VectorAdd( vecTranslatedMins, vecTranslatedMaxs, vecOBBCenter );
		vecOBBCenter *= 0.5f;

		Vector vecOBBHalfDiagonal;
		VectorSubtract( vecTranslatedMaxs, vecTranslatedMins, vecOBBHalfDiagonal );
