...
#define _FIELD_AMBIGUOUS(klass,name,fieldtype,count,flags,mapname,tolerance)		{ fieldtype, #name, offsetof(classNameTypedef, klass::name), count, flags, mapname, NULL, NULL, NULL, sizeof( ((classNameTypedef *)0)->klass::name ), NULL, 0, tolerance }
...
#define DEFINE_PRED_ARRAY_AMBIGUOUS(klass,name,fieldtype,count,flags) _FIELD_AMBIGUOUS(klass, name, fieldtype, count, flags, NULL, 0.0f )
