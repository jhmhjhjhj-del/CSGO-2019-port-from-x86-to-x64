...
struct MDLSquenceLayer_t
{
	...
	float	m_flStartTime;		// set to -ve for old behavior (uses main queued sequence time). Time can be in the future, allowing a delayed start/blend into (weight is set to 0 and ignored if time < startTime)
	bool	m_bLoop;			// set to true to indicate a looping layer (old behavior assumes true).
};
...
public:
	...
	void EnableJiggleBones();
	void DisableJiggleBones();
...
public:
	...
	void	SetLocalTransform( MDLHandle_t handle, matrix3x4_t &matLocalTransform );
...
protected:
	...
	CUtlVector<MDLSquenceLayer_t> m_aSequenceLayers;
};
