...
/*
this guard is useful to hunt down threading issues with m_fStateFlags 
...
public:
	...
	int operator &= ( int x ) { 
		if ( m_nValue & FL_EDICT_LOCK_PVS )DebuggerBreak();
		return m_nValue &= x; 
	}
	int operator |= ( int x ) {
		if ( m_nValue & FL_EDICT_LOCK_PVS )DebuggerBreak();
	...
	int operator | ( int x ) const { return m_nValue | x; }
	int operator & ( int x ) const { return m_nValue & x; }
};
*/
...
public:
	...
	// NOTE: YOU CAN'T CHANGE THE LAYOUT OR SIZE OF CBASEEDICT AND REMAIN COMPATIBLE WITH HL2_VC6!!!!!
	// the upper bits must be 0, they are used for spin-locking the edict for very short operations
	int	m_fStateFlags;	
...
{
	ThreadInterlockedAndNot( &m_fStateFlags, FL_EDICT_CHANGED | FL_FULL_EDICT_CHANGED );
	//interlocked equivalent of : m_fStateFlags &= ~(FL_EDICT_CHANGED | FL_FULL_EDICT_CHANGED);
