...
struct ClientReplayEventParams_t
{
	ClientReplayEventParams_t( int nEventType )
	{
		...
		m_bForceUseTheseSettings = false; // by default some settings can be established from the player
	}
	int m_nEventType;
...
public:
	...
	virtual void		GetAreaBits( EnginePVSContext_t *pEnginePVSContext, int area, unsigned char *bits, int buflen ) = 0;
	...
	virtual void		SetAreaPortalStates( EnginePVSContext_t *pEnginePVSContext, const int *portalNumbers, const bool *isOpen, int nPortals ) = 0;
	...
	virtual void StopClientsHltvReplay( ) = 0;
	virtual void SetFixupTarget( int edict ) = 0;

	virtual float GetGameTimescale() = 0; // Just the getter for SetGameTimescale; use GetTimescale() to get the actual scale with all modifiers applied
};
...
public:
	...
	virtual void			PreparePVSInfo_Parallel( const unsigned short *pEdictIndices, int nEdicts, volatile uint32 *pCounter ) = 0;
};
...
public:
	...
	virtual void			ClientSetupVisibility( EnginePVSContext_t *pEnginePVSContext, edict_t *pViewEntity, edict_t *pClient, unsigned char *pvs, int pvssize ) = 0;
