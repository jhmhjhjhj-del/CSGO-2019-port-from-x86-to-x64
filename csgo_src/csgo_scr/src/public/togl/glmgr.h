...
class GLMContext
{
	...
	protected:
		...
		GLMTexSampler					m_samplers[ GLM_MAX_TOTAL_SAMPLER_COUNT ];
		
		uint8							m_nDirtySamplerFlags[ GLM_MAX_TOTAL_SAMPLER_COUNT ];	// 0 if the sampler is dirty, 1 if not
		uint32							m_nNumDirtySamplers;							// # of unique dirty sampler indices in m_nDirtySamplers
		uint8							m_nDirtySamplers[ GLM_MAX_TOTAL_SAMPLER_COUNT + 1 ];	// dirty sampler indices

		int GetTotalSamplerCount()
		{
			return m_caps.m_nTotalSamplerCount;
		}
...
{
	Assert( sampler < m_caps.m_nTotalSamplerCount );
