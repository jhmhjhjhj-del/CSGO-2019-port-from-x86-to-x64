...
#define RS_CAPTURE_MODE_GAME ( 1<<0 )
#define RS_CAPTURE_MODE_PANORAMA ( 1<<1 )

struct RsShadowValue_t
{
	uint32 nCaptureMode;
	uint32 value;
};
...
struct TOGL_CLASS IDirect3DDevice9 : public IUnknown
{
	...
	FORCEINLINE HRESULT TOGLMETHODCALLTYPE GetPixelShader( IDirect3DPixelShader9** ppShader );
	...
	FORCEINLINE HRESULT TOGLMETHODCALLTYPE GetPixelShaderConstantF( UINT StartRegister, float* pConstantData, UINT Vector4fCount );
	...
	FORCEINLINE HRESULT TOGLMETHODCALLTYPE GetVertexShader( IDirect3DVertexShader9** ppShader );
	...
	FORCEINLINE HRESULT TOGLMETHODCALLTYPE GetVertexDeclaration( IDirect3DVertexDeclaration9** ppDecl );
	...
	FORCEINLINE HRESULT TOGLMETHODCALLTYPE GetStreamSource( UINT StreamNumber, IDirect3DVertexBuffer9** ppStreamData, UINT *pOffsetInBytes, UINT* pStride );
	...
	FORCEINLINE HRESULT TOGLMETHODCALLTYPE GetIndices( IDirect3DIndexBuffer9** ppIndexData );
	...
	HRESULT TOGLMETHODCALLTYPE GetScissorRect( RECT* pRect );
...
private:
	...
	IDirect3DBaseTexture9		*m_textures[ GLM_MAX_TOTAL_SAMPLER_COUNT ];				// set by SetTexture... NULL if stage inactive
...
public:
	...
	int				m_nCaptureMode;		// RS_CAPTURE_MODE_GAME/PANORAMA
	RsShadowValue_t m_RsShadow[ 210 + 1 ];
	RsShadowValue_t m_SamplerStateShadow[ 16 + 4 ][ 13 + 1 ]; // 16+4 --> Max Pixel & vertex samplers

	int TOGLMETHODCALLTYPE GetTotalSamplerCount();

	FORCEINLINE HRESULT TOGLMETHODCALLTYPE SetCaptureMode( int nCaptureMode ) {
		m_nCaptureMode = nCaptureMode;
		return S_OK;
	}
};
...
{
	if ( m_nCaptureMode & RS_CAPTURE_MODE_GAME )
	{
		m_SamplerStateShadow[Sampler][ Type ].value = Value;
	}
	m_SamplerStateShadow[ Sampler ][ Type ].nCaptureMode |= m_nCaptureMode;
	...
	Assert( Sampler < GetTotalSamplerCount() );
	...
	DWORD MinFilter, DWORD MagFilter, DWORD MipFilter )
{
	if ( m_nCaptureMode & RS_CAPTURE_MODE_GAME )
	{
		m_SamplerStateShadow[Sampler][ D3DSAMP_ADDRESSU ].value = AddressU;
		m_SamplerStateShadow[Sampler][ D3DSAMP_ADDRESSV ].value = AddressV;
		m_SamplerStateShadow[Sampler][ D3DSAMP_ADDRESSW ].value = AddressW;
		m_SamplerStateShadow[Sampler][ D3DSAMP_MAGFILTER ].value = MagFilter;
		m_SamplerStateShadow[Sampler][ D3DSAMP_MINFILTER ].value = MinFilter;
		m_SamplerStateShadow[Sampler][ D3DSAMP_MIPFILTER ].value = MipFilter;
	}
	m_SamplerStateShadow[Sampler][ D3DSAMP_ADDRESSU ].nCaptureMode |= m_nCaptureMode;
	m_SamplerStateShadow[Sampler][ D3DSAMP_ADDRESSV ].nCaptureMode |= m_nCaptureMode;
	m_SamplerStateShadow[Sampler][ D3DSAMP_ADDRESSW ].nCaptureMode |= m_nCaptureMode;
	m_SamplerStateShadow[Sampler][ D3DSAMP_MAGFILTER ].nCaptureMode |= m_nCaptureMode;
	m_SamplerStateShadow[Sampler][ D3DSAMP_MINFILTER ].nCaptureMode |= m_nCaptureMode;
	m_SamplerStateShadow[Sampler][ D3DSAMP_MIPFILTER ].nCaptureMode |= m_nCaptureMode;
...
#else
	Assert( GetCurrentOwnerThreadId() == ThreadGetCurrentId() );
	Assert( Stage < GetTotalSamplerCount() );
...
FORCEINLINE HRESULT TOGLMETHODCALLTYPE IDirect3DDevice9::GetTexture( DWORD Stage, IDirect3DBaseTexture9** ppTexture )
{
...
FORCEINLINE HRESULT TOGLMETHODCALLTYPE IDirect3DDevice9::SetRenderStateInline( D3DRENDERSTATETYPE State, DWORD Value )
{
	...
	if ( m_nCaptureMode & RS_CAPTURE_MODE_GAME )
	{
		m_RsShadow[ State ].value = Value;
	}
	m_RsShadow[ State ].nCaptureMode |= m_nCaptureMode;
...
FORCEINLINE HRESULT TOGLMETHODCALLTYPE IDirect3DDevice9::GetIndices( IDirect3DIndexBuffer9** ppIndexData )
{
	Assert( GetCurrentOwnerThreadId() == ThreadGetCurrentId() );
	*ppIndexData = m_indices.m_idxBuffer;
	if ( *ppIndexData ) (*ppIndexData)->AddRef();
	return S_OK;
}

FORCEINLINE HRESULT TOGLMETHODCALLTYPE IDirect3DDevice9::GetStreamSource( UINT StreamNumber, IDirect3DVertexBuffer9** ppStreamData, UINT *pOffsetInBytes, UINT* pStride )
{
	*ppStreamData = m_streams[ StreamNumber ].m_vtxBuffer;
	*pOffsetInBytes = m_streams[ StreamNumber ].m_offset;
	*pStride = m_streams[ StreamNumber ].m_stride;
...
FORCEINLINE HRESULT TOGLMETHODCALLTYPE IDirect3DDevice9::GetPixelShaderConstantF( UINT StartRegister, float* pConstantData, UINT Vector4fCount )
{
	memcpy( pConstantData, &m_ctx->m_programParamsF[ 1 ].m_values[ StartRegister ][ 0 ], (4 * sizeof( float )) * Vector4fCount );
	return S_OK;
}
...
FORCEINLINE HRESULT TOGLMETHODCALLTYPE IDirect3DDevice9::GetPixelShader( IDirect3DPixelShader9** ppShader )
{
	Assert( GetCurrentOwnerThreadId() == ThreadGetCurrentId() );

	*ppShader = m_pixelShader;
...
FORCEINLINE HRESULT IDirect3DDevice9::GetVertexDeclaration( IDirect3DVertexDeclaration9** ppDecl )
{
	*ppDecl = m_pVertDecl;
