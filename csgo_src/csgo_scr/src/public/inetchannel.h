...
// Reasons for connection closing.  In the Source engine, we really don't have any code
// that cares right now, we use the string for that.  We'll just try to classify them
// as "unusual" or "unusual", so that in our backend analytics we can tell how often
// "unusual" thingsa re happening.
enum EConnectionEnd
{
	k_EConnectionEnd_UsualGeneric = k_ESteamNetConnectionEnd_App_Min,
	k_EConnectionEnd_UnusualGeneric = k_ESteamNetConnectionEnd_AppException_Min
};
...
public:
	...
	virtual void	Shutdown( int nReason, const char *pszReason ) = 0;
	...
	virtual HSteamNetConnection GetSteamNetConnection() const = 0;
};
