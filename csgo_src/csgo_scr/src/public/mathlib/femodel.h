...
public:
	...
	float GetError( const VectorAligned *pPos )const;
public:
	...
	uint16 m_nExtraIterations;
	uint16 m_nFollowSimCtrl; // 0.0 in m_flFollowSimCtrl means there's no bone; otherwise m_nFollowSimCtrl is the Ctrl to hook up to and move all dynamic nodes when animation changes it
	...
	float32 m_flFollowSimCtrl;

	float32 m_flWindage;
	float32 m_flWindDrag;

	float32 m_flTeleportDeltaSq;
