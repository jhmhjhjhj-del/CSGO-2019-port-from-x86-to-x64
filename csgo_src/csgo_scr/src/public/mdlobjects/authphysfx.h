...
public:
	...
	int FindNodeIndex( const char *pName )const;
	...
	void Swap( CAuthPhysFx &other )
	{
		...
		::Swap( m_FollowSimCtrl, other.m_FollowSimCtrl );
		::Swap( m_flFollowSimCtrl, other.m_flFollowSimCtrl );
		::Swap( m_flWindage, other.m_flWindage );
		::Swap( m_flWindDrag, other.m_flWindDrag );
		::Swap( m_flTeleportDistanceMultiplier, other.m_flTeleportDistanceMultiplier );
	...
	void Append( const CAuthPhysFx &other )
	{
		...
		if ( m_nMergePriority < other.m_nMergePriority )
		{
			...
			m_FollowSimCtrl = other.m_FollowSimCtrl;
			m_flFollowSimCtrl = other.m_flFollowSimCtrl;
			m_flWindage = other.m_flWindage;
			m_flWindDrag = other.m_flWindage;
			m_flTeleportDistanceMultiplier = other.m_flTeleportDistanceMultiplier;
	...
	void InitDefaults()
	{
		...
		m_flFollowSimCtrl = 0.0f;
		m_flWindage = 1.0f;
		m_flTeleportDistanceMultiplier = 1.0f;
		m_flWindDrag = 0;
	...
	{
		float flSumLen = 0, flSumCount = 0;

		for ( int i = 0; i < m_Rods.Count(); ++i )
		{
			flSumLen += ( m_Nodes[ m_Rods[ i ].m_nNodes[ 0 ] ].m_Transform.m_vPosition - m_Nodes[ m_Rods[ i ].m_nNodes[ 1 ] ].m_Transform.m_vPosition ).Length();
			flSumCount++;
		}

		for ( int i = 0; i < m_Quads.Count(); ++i )
		{
			const uint *nNode = m_Quads[ i ].m_nNodes;
			Vector a = CrossProduct( m_Nodes[ nNode[ 0 ] ].m_Transform.m_vPosition - m_Nodes[ nNode[ 2 ] ].m_Transform.m_vPosition, m_Nodes[ nNode[ 1 ] ].m_Transform.m_vPosition - m_Nodes[ nNode[ 3 ] ].m_Transform.m_vPosition );
			flSumLen += sqrtf( a.Length() ); // geometric mean of the length of a diagonal
			flSumCount += 1.0f;
		}

		if ( flSumCount > 0 )
		{
			return flSumLen / flSumCount;
		}
		else
		{
			return 0.0f;
...
public:
	...
	float m_flFollowSimCtrl;
	float m_flWindage;
	float m_flWindDrag;
	float m_flTeleportDistanceMultiplier;
