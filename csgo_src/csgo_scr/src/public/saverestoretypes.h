...
public:
	...
	void BuildEntityHash()
	{
		...
		{
			entitytable_t *pBuildTable = GetEntityInfo( i );
			m_EntityToIndex.Insert(  CHashElement( pBuildTable->hEnt.Get(), i ) );
		}
#endif
	}
	...
	int	GetEntityIndex( const CBaseEntity *pEntity )
	{
		...
		if ( pEntity )
		{
			...
			else
			{
				...
				for ( int i = 0; i < nEntities; i++ )
				{
					entitytable_t *pLocalTable = GetEntityInfo( i );
					if ( pLocalTable->hEnt == pEntity )
						return pLocalTable->id;
				}
			}
		}
#endif
