...
public:
    ...
    SocketErrorCode_t	PollClientConnectionState( bool *isConnected, timeval &timeout = g_zeroTimeout );	// (Client-only) Checks the status of endpoint 0 to see if it is connected successfully to a server.
    ...
    SocketErrorCode_t	CanReadFromEndpoint( int endpointIndex, bool *canRead, timeval &timeout = g_zeroTimeout );	// Gets a value indicating whether the endpoint can be successfully read from.

    SocketErrorCode_t	CanWriteToEndpoint( int endpointIndex, bool *canWrite, timeval &timeout = g_zeroTimeout );	// Gets a value indicating whether the endpoint can be successfully written to.

    SocketErrorCode_t	CanReadFromListeningSocket( bool *canRead, timeval &timeout = g_zeroTimeout );	// Gets a value indicating whether an incoming connection request has been received on the listening socket.
