...
public:
	...
	// What is the game timescale (including all time-scaling factors)
...
// For non-split screen games this will always be zero
	...
	// Set the local timescale applied to the game (only one of many factors that contribute to timescale)
	...
	virtual float GetGameTimescale() = 0; // This is just the getter for SetGameTimescale; use GetTimescale() if you want the actual Timescale to apply

	// more demo playback stuff
	virtual int DemoFindNextImportantTick( const char* szEventName, int nStartTick = -1 ) = 0;
	virtual int DemoFindPrevImportantTick( const char* szEventName, int nStartTick = -1 ) = 0;

	// Exception info upload from outside of engine.dll
	virtual void ExecuteExceptionHandler( const char *szStringVal ) = 0;

	//Interface to control player voice volume.
	virtual float GetPlayerVoiceVolume(XUID player_id) = 0;
	virtual void SetPlayerVoiceVolume(XUID player_id, float fVol) = 0;
};
...
public:
	...
	// HUD alternate ticks
	virtual bool			HudShouldPaintThisFrame() = 0;
	...
	// Let keybindings controls from one of the options screens process input
	virtual bool			IsBindWidgetCapturingInput( ) = 0;
	virtual bool			HandleBindWidgetInputCapture( const InputEvent_t &inputEvent ) = 0;
#endif
	...
	// Inventory access
	virtual void DetermineSubscriptionKvToAdvertise( KeyValues *pSettings, const char *szReason ) = 0;
...
// #if defined( INCLUDE_SCALEFORM_ )
	virtual IScaleformSlotInitController * GetScaleformSlotInitController() { return nullptr; }
// #endif
	...
	virtual void OnHltvFixupOperatorStatus( const CSVCMsg_HltvFixupOperatorStatus &msg ) = 0;
	virtual const char * AllocateCensoredString( char const *msg ) = 0;

	virtual IAvatarImageMgr *GetAvatarImageMgr() = 0;

	// Allows client.dll to eavesdrop on the input system key processing asynchronously, mostly needed for +voicerecord handler in menus
	virtual void IN_PreKeyEvent( int eventcode, ButtonCode_t keynum, const char *pszCurrentBinding ) = 0;

	virtual bool PlayerDeathInfo(IClientEntity* e, float* timeSinceDeath) = 0;

	virtual bool PlayerOnSameTeamAsLocalPlayer(IClientEntity* e) = 0;
};
