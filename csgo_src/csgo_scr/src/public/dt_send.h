...
// -------------------------------------------------------------------------------------------------------------- //
// SendTable extensions
// -------------------------------------------------------------------------------------------------------------- //

class ISendTableExtension
{
public:
	virtual void Initialize( SendTable *pSendTable, const SendProp**ppProps, int numProps ) {} // calleda fter m_pPrecalc is initialized
	virtual void Terminate() {}
	// return the props (their indices) which should be treated as if they are always changing; MUST return true if pOut is non-empty or would be non-empty if it was not nullptr
	virtual void GetUncompressibleProps( CUtlVectorFixedGrowable< int, 8 > *pOut ) {}
};
...
// The following enum must be synchronized with static const char *s_pWellKnownPropNames[] in src\engine\dt_send_eng.cpp

enum SendTableWellKnownPropNamesEnum_t
{
	SPWKN_ANG_ROTATION,
	SPWKN_VEC_ORIGIN,
	SPWKN_VEC_ORIGIN_2,
	SPWKN_VEC_ORIGIN_SECOND,
	SPWKN_VEC_ORIGIN_2_SECOND,
	SPWKN_OBSERVER_MODE,
	SPWKN_CAMERAMAN_SCOREBOARD,
	SPWKN_EYE_ANGLES,
	SPWKN_EYE_ANGLE_0,
	SPWKN_EYE_ANGLE_1,
	SPWKN_OBSERVER_TARGET,
	SPWKN_VIEW_OFFSET_ENTITYBASE,//"view_ofs"
	SPWKN_VIEW_OFFSET_0,
	SPWKN_VIEW_OFFSET_1,
	SPWKN_VIEW_OFFSET_2,
	//SPWKN_CELLBITS, // m_cellbits
	//SPWKN_CELLX, // m_cellX
	//SPWKN_CELLY, // m_cellY
	//SPWKN_CELLZ, // m_cellZ
	SPWKN_COUNT
...
public:
	...
	void		SetExtension( ISendTableExtension *pExtension );
public:
	...
	ISendTableExtension* m_pExtension;
	int			m_nWellKnownProps[ SPWKN_COUNT ]; // initially, all indices are garbage and are initialized on first use
protected:
	bool		m_bInitialized : 1;
	bool		m_bHasBeenWritten : 1;		
	bool		m_bHasPropsEncodedAgainstCurrentTickCount : 1; // m_flSimulationTime and m_flAnimTime, e.g.
	bool		m_bWellKnownPropsInitialized : 1;
	friend void SendTable_InitWellKnownProps( SendTable * pTable );
};
...
inline int CountEnumBits( tEnum countValue )
{
	uint32 count = ( uint32 )countValue;
