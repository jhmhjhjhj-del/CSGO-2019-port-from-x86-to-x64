...
ConVar r_staticlight_streams( "r_staticlight_streams", "1", FCVAR_HIDDEN ); // NOTE: Must be kept in sync with engine/l_studio.cpp
...
int ComputeBytesToRead( int lhs, size_t structSize, const char* mapName )
{
	int64 destVal = int64( lhs ) * structSize;
	int32 retVal = int32( destVal );
	if ( destVal != retVal )
	{
		Assert( !"Found a malicious map. Attempted integer overflow bug is present, we're about to shutdown." );
		Sys_Error( "Couldn't load corrupted map %s", mapName );
	}
...
void Mod_LoadLighting( bool bLoadHDR )
{
	...
	if ( !lh.LumpSize() )
	{
		Msg( "Mod_LoadLighting: empty lump hdr=%d\n", bLoadHDR ? 1 : 0 );
		lh.GetMap()->lightdata = NULL;
		return;
	}
	Assert ( lh.LumpVersion() != 0 );

	Msg( "Mod_LoadLighting: alloc %d bytes hdr=%d\n", lh.UncompressedLumpSize(), bLoadHDR ? 1 : 0 );
	...
	if ( !lh.GetMap()->lightdata )
	{
		...
		Warning( "Mod_LoadLighting: AllocateLightingData FAILED size=%d\n", lh.UncompressedLumpSize() );
		return;
	}
	...
	case LUMP_OCCLUSION_VERSION:
		{
			...
			if (b->numoccluders)
			{
				int nSize = ComputeBytesToRead( b->numoccluders, sizeof(doccluderdata_t), lh.GetLoadName() );
			...
			{
				int nSize = ComputeBytesToRead( b->numoccluderpolys, sizeof(doccluderpolydata_t), lh.GetLoadName() );
			...
			{
				int nSize = ComputeBytesToRead( b->numoccludervertindices, sizeof(int), lh.GetLoadName() );
...
void *Hunk_AllocNameAlignedClear_( int size, int alignment, const char *pHunkName )
{
	...
	// x64: unsigned long truncates pointers above 4GB → corrupt aligned allocs in Mod_LoadFaces
	pMem = (void *)( ( ( ( uintptr_t )pMem ) + (alignment-1) ) & ~( (uintptr_t)alignment - 1 ) );
...
void Mod_LoadNodes( void )
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		if ( in->firstface + in->numfaces > s_nMapSurfacesLoaded )
			Host_Error( "Map %s is corrupted and cannot be loaded.", lh.GetMapPathName() );
...
{
	int nModelCount = m_Models.Count();
	for ( int nModelIndex=0 ; nModelIndex < nModelCount ; nModelIndex++ )
	{
		model_t	*pModel = m_Models[nModelIndex].modelpointer;
		...
		else
		{
			if ( g_pMDLCache->IsDataLoaded( pModel->studio, MDLCACHE_STUDIOHWDATA ) )
			{
				...
				if ( pStudioHdr )
				{
					...
					for ( int nMaterial=0; nMaterial < nMaterials; nMaterial++ )
					{
						if ( pMaterials[nMaterial] && pMaterials[nMaterial]->WasReloadedFromWhitelist() )
						{
...
{
	++g_nMapLoadCount;
	Msg( "Map_LoadModelGuts: begin %s\n", mod->szPathName ? mod->szPathName : "?" );
...

	{
		...
		Msg( "Map_LoadModelGuts: Mod_LoadTexinfo done\n" );
		Host_KeepAliveDuringMapLoad();
	}
	...
	// Until BSP version 19, this must occur after loading texinfo
	{
		...
		Msg( "Map_LoadModelGuts: Mod_LoadLighting begin\n" );
		...
		Msg( "Map_LoadModelGuts: Mod_LoadLighting done\n" );
		Host_KeepAliveDuringMapLoad();
	}

	{
		...
		Mod_LoadPrimitives();
		Msg( "Map_LoadModelGuts: Mod_LoadPrimitives done\n" );
		Host_KeepAliveDuringMapLoad();
	}

	{
		...
		Msg( "Map_LoadModelGuts: Mod_LoadPrimVerts begin\n" );
		Mod_LoadPrimVerts();
		Msg( "Map_LoadModelGuts: Mod_LoadPrimVerts done\n" );
		Host_KeepAliveDuringMapLoad();
	}

	{
		...
		Msg( "Map_LoadModelGuts: Mod_LoadPrimIndices begin\n" );
		Mod_LoadPrimIndices();
		Msg( "Map_LoadModelGuts: Mod_LoadPrimIndices done\n" );
		Host_KeepAliveDuringMapLoad();
	}
...

	{
		...
		Msg( "Map_LoadModelGuts: Mod_LoadFaces begin\n" );
		Mod_LoadFaces();
		Mod_LoadFaceBrushes();
		Msg( "Map_LoadModelGuts: Mod_LoadFaces done\n" );
		Host_KeepAliveDuringMapLoad();
	}
...

	{
		...
		Mod_LoadLeafs();
		Msg( "Map_LoadModelGuts: Mod_LoadLeafs done\n" );
		Host_KeepAliveDuringMapLoad();
	}
...
void CModelLoader::Studio_LoadModel( model_t *pModel, bool bTouchAllData )
{
	...
	if ( ( pModel->nLoadFlags & FMODELLOADER_STATICPROP ) && !( pModel->nLoadFlags & FMODELLOADER_LOADED ) )
	{
