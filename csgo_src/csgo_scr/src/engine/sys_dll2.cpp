...
{ 
	...
	if ( !CommandLine()->CheckParm( "-scaleform" ) )
	{
...
#if !defined( DEDICATED ) && !defined( NO_STEAM ) && IS_WINDOWS_PC
//-----------------------------------------------------------------------------
// Dialog box to prompt user about launching Perfect World or Worldwide
// Steam3Client is fully initialized at this point
//-----------------------------------------------------------------------------
static HWND s_hLauncherSelect_RadioPerfectWorld = NULL;
static HWND s_hLauncherSelect_RadioWorldwide = NULL;
static HBITMAP s_hLauncherSelect_Bitmap = NULL;
static HBRUSH s_hLauncherSelect_BrushNull = NULL;
static INT_PTR CALLBACK DialogProcMessageBoxPrompt(
	_In_ HWND   hwndDlg,
	_In_ UINT   uMsg,
	_In_ WPARAM wParam,
	_In_ LPARAM lParam
)
{
	...
	{
	case WM_INITDIALOG:
		::SetWindowTextA( hwndDlg, "Counter-Strike: Global Offensive" );
		s_hLauncherSelect_RadioPerfectWorld = GetDlgItem( hwndDlg, 1000 /*IDC_RADIO_PERFECTWORLD*/ );
		s_hLauncherSelect_RadioWorldwide = GetDlgItem( hwndDlg, 1001 /*IDC_RADIO_WORLDWIDE*/ );
		s_hLauncherSelect_Bitmap = ( HBITMAP ) ::LoadImageA( ::GetModuleHandle( NULL ), MAKEINTRESOURCE( 1002 /*IDB_BITMAP_BKGND*/ ), IMAGE_BITMAP, 0, 0, LR_DEFAULTCOLOR );
		if ( !s_hLauncherSelect_RadioPerfectWorld || !s_hLauncherSelect_RadioWorldwide || !s_hLauncherSelect_Bitmap )
		{
			EndDialog( hwndDlg, 0 );
			return TRUE;
		}
		if ( !s_hLauncherSelect_BrushNull ) // It is not necessary (but it is not harmful) to delete stock objects by calling DeleteObject.
			s_hLauncherSelect_BrushNull = ( HBRUSH ) ::GetStockObject( HOLLOW_BRUSH );

		// What is selected?
		{
			bool bWorldwide = true;
			char chUserLocalDataFolder[ MAX_PATH ] = {};
			if ( Steam3Client().SteamUser()->GetUserDataFolder( chUserLocalDataFolder, sizeof( chUserLocalDataFolder ) ) && chUserLocalDataFolder[ 0 ] )
			{
				V_strcat_safe( chUserLocalDataFolder, "\\cnlauncher.txt" );
				CUtlBuffer bufPreference;
				if ( g_pFileSystem->ReadFile( chUserLocalDataFolder, NULL, bufPreference ) )
				{
					bWorldwide = ( V_strnicmp( ( const char * ) bufPreference.Base(), "-perfectworld", 13 ) != 0 );
				}
			}
			::SendMessageA( s_hLauncherSelect_RadioPerfectWorld, BM_SETCHECK, bWorldwide ? BST_UNCHECKED : BST_CHECKED, 0 );
			::SendMessageA( s_hLauncherSelect_RadioWorldwide, BM_SETCHECK, bWorldwide ? BST_CHECKED : BST_UNCHECKED, 0 );
		}
		::InvalidateRect( hwndDlg, NULL, TRUE );
		return FALSE; // default focus setting processing
	case WM_ERASEBKGND:
		{
		HDC hdc = ( HDC ) wParam;
		RECT rc;
		...
		HGDIOBJ hOld = SelectObject( hdcMemory, s_hLauncherSelect_Bitmap );
		BITMAP bmp;
		GetObject( s_hLauncherSelect_Bitmap, sizeof( bmp ), &bmp );
		StretchBlt( hdc, 0, 0, rc.right, rc.bottom, hdcMemory, 0, 0, bmp.bmWidth, bmp.bmHeight, SRCCOPY );
		SelectObject( hdcMemory, hOld );
		DeleteDC( hdcMemory );
		}
		return TRUE;
	case WM_DESTROY:
		if ( s_hLauncherSelect_Bitmap )
		{
			DeleteObject( s_hLauncherSelect_Bitmap );
			s_hLauncherSelect_Bitmap = NULL;
		}
		return FALSE;
	case WM_CTLCOLORDLG:
	case WM_CTLCOLORSTATIC:
	case WM_CTLCOLORBTN:
		return ( INT_PTR ) s_hLauncherSelect_BrushNull;
	case WM_COMMAND:
		...
		{
			int nResult = 0;
			if ( BST_CHECKED == ::SendMessageA( s_hLauncherSelect_RadioPerfectWorld, BM_GETCHECK, 0, 0 ) )
				nResult = 2; // Perfect World
			else if ( BST_CHECKED == ::SendMessageA( s_hLauncherSelect_RadioWorldwide, BM_GETCHECK, 0, 0 ) )
				nResult = 1; // Worldwide
			if ( nResult )
			{
				...
				if ( Steam3Client().SteamUser()->GetUserDataFolder( chUserLocalDataFolder, sizeof( chUserLocalDataFolder ) ) && chUserLocalDataFolder[ 0 ] )
				{
					V_strcat_safe( chUserLocalDataFolder, "\\cnlauncher.txt" );
					CUtlBuffer bufPreference;
					bufPreference.PutString( ( nResult == 2 ) ? "-perfectworld" : "-worldwide" );
					g_pFileSystem->WriteFile( chUserLocalDataFolder, NULL, bufPreference );
				}
...
//-----------------------------------------------------------------------------
// Configure game breakpad AppID for the given version suffix string
//-----------------------------------------------------------------------------
#if !defined( NO_STEAM ) && !defined( _GAMECONSOLE )
#if !defined(NO_STEAM) && !defined(DEDICATED) && defined(_WIN32)
/*
This is custom breakpad management code that allows us to set a build id version right before we upload the dump
so that we could have a different version for Javascript Exceptions and another version for regular code crashes.
Here we reach in the guts of breakpad Win32 uploader stack to fix stuff up, so this code may need some tweaks if
Steamworks SDK takes a new drop of breakpad that behaves drastically different (unlikely). To fix up this code simply
trigger a javascript exception and have a breakpoint in the stack walking code below, look at the stack of new
breakpad and see how you can reach to the main handler object to fix up its members.
*/
void *s_pvSteamApiBreakpadReliableUpload = nullptr;
void *s_pvSteamApiBreakpadReliableUploadLookback = nullptr;
struct SteamApiBreakpadInternalWin32_CustomInfoEntry
{	// see breakpad_win32.cpp
wchar_t name[ 0x40 ];
wchar_t value[ 0x40 ];
};
wchar_t *s_wchSteamApiBreakpadReliableUploadRestore = nullptr;
static void ConfigureSteamApiBreakpadReliableCallback( void *context )
{
	if ( s_pvSteamApiBreakpadReliableUpload )
	{
/*
We expect the following call stack ---

 engine.dll!ConfigureSteamApiBreakpadReliableCallback(void * context=0x00000000) Line 777	C++
 crashhandler.dll!CHandlerContext::Breakpad_FilterCallback(void * context=0x45cd7830, _EXCEPTION_POINTERS * exinfo=0x00b7d2b4, MDRawAssertionInfo * assertion=0x00000000) Line 112	C++
 crashhandler.dll!google_breakpad::ExceptionHandler::WriteMinidumpWithException(unsigned long requesting_thread_id=0x00005c98, _EXCEPTION_POINTERS * exinfo=0x00b7d2b4, MDRawAssertionInfo * assertion=0x00000000) Line 899	C++
 crashhandler.dll!google_breakpad::ExceptionHandler::WriteMinidumpForException(_EXCEPTION_POINTERS * exinfo=0x00b7d2b4) Line 813	C++
 crashhandler.dll!CBreakpadFactory::WriteMiniDump(void * handle=0x45cd7830, unsigned int uStructuredExceptionCode=0x00000000, _EXCEPTION_POINTERS * pExceptionPointers=0x00b7d2b4) Line 708	C++
 steamclient.dll!Breakpad_SteamWriteMiniDumpUsingExceptionInfoWithBuildId(unsigned int uStructuredExceptionCode=0x00000000, _EXCEPTION_POINTERS * pExceptionInfo=0x00b7d2b4, unsigned int uBuildID=0x00001c14) Line 162	C++
 engine.dll!CEngineClient::ExecuteExceptionHandler(const char * szStringVal=0x08394200) Line 2694	C++

We can walk the stack back one frame and find the context parameter passed to Breakpad_FilterCallback
 context=0x45cd7830
Which is laid out as follows:
+&(((( CHandlerContext * )context ))->m_CustomInfo).entries	0x45cd7848 {0x45640dc0 {name=0x45640dc0 L"ProductName" value=0x45640e40 L"730" }}	const google_breakpad::CustomInfoEntry * *
+													.entries[1]		{name=0x45640ec0 L"Version" value=0x45640f40 L"13641.0.JC" }	const google_breakpad::CustomInfoEntry *
+&(((( CHandlerContext * )context ))->m_CustomInfo).count	0x45cd784c {0x0000000e}	unsigned int *
*/
		uint32 unDummyVariableOnTheStack = 0;
		uint32 *puiStackTraversalStart = ( reinterpret_cast< uint32 * >( reinterpret_cast< uint32 >( &unDummyVariableOnTheStack )&0xFFFFFFFC ) ) + 1; // this our known good stack
		uint32 *puiStackTraversalEnd = ( reinterpret_cast< uint32 * >( reinterpret_cast< uint32 >( s_pvSteamApiBreakpadReliableUploadLookback ) & 0xFFFFFFFC ) ) - 3; // stack backwards to known good stack in CEngineClient::ExecuteExceptionHandler
		void *pvContextParam = nullptr;
		for ( ; puiStackTraversalStart <= puiStackTraversalEnd; ++ puiStackTraversalStart )
		{
			// We expect the following pattern:
			// [context_ptr] [exinfo (we know this value)] [assertion (known 0x00000000)]
			if ( puiStackTraversalStart[ 2 ] == 0
				&&
				puiStackTraversalStart[1] == reinterpret_cast< uint32 >( s_pvSteamApiBreakpadReliableUpload ) )
			{
				pvContextParam = reinterpret_cast< void * >( puiStackTraversalStart[0] );
				break;
			}
		}
		if ( pvContextParam )
		{
			// Validate that we found a valid context that we know how to work with
			if (
				( *( size_t * ) ( ( ( char* ) pvContextParam ) + 0x24 ) == ( size_t ) ( void * ) ConfigureSteamApiBreakpadReliableCallback ) &&
				( *( size_t * ) ( ( ( char* ) pvContextParam ) + 0x20 ) == ( size_t ) ( void * ) context ) &&
				( *( size_t * ) ( ( ( char* ) pvContextParam ) + 0x1C ) == 0xE )
				)
			{
				SteamApiBreakpadInternalWin32_CustomInfoEntry *pCustomInfoEntry = * ( SteamApiBreakpadInternalWin32_CustomInfoEntry ** ) ( ( ( char* ) pvContextParam ) + 0x18 );
				for ( int j = 0; j < 0xE; ++ j )
				{
					SteamApiBreakpadInternalWin32_CustomInfoEntry *pCheck = pCustomInfoEntry + j;
					if ( !wcsicmp( pCheck->name, L"Version" ) )
					{
						int n = wcsnlen( pCheck->value, 0x40 );
						if ( n > 0 && n < 0x30 && pCheck->value[ n - 1 ] == L'C' )
						{
							pCheck->value[n - 1] = L'S';
							s_wchSteamApiBreakpadReliableUploadRestore = &pCheck->value[n-1];
						}
						break;
					}
				}
...
void ConfigureSteamApiBreakpadWithSuffix( const char *szSuffix )
{
	// AppID of the client will be automatically used
	extern int32 GetHostVersion();
	extern int32 GetClientVersion();
	CFmtStr fmtClientVersion( "%d.%d.%s", GetHostVersion(), GetClientVersion(),
		szSuffix ? szSuffix : ( CommandLine()->FindParm( "-scaleform" ) ? "SF" : "JC" ) );
	Msg( "Using breakpad minidump system %u/%s\n", g_unSteamAppID, fmtClientVersion.Access() );
	SteamAPI_UseBreakpadCrashHandler( fmtClientVersion.Access(), __DATE__, __TIME__, WantsFullMemoryDumps(), NULL,
#if !defined(NO_STEAM) && !defined(DEDICATED) && defined(_WIN32)
		ConfigureSteamApiBreakpadReliableCallback
#else
		NULL
#endif
	);
...
bool CEngineAPI::SetStartupInfo( StartupInfo_t &info ) 
{
	...
	{
		ConfigureSteamApiBreakpadWithSuffix( NULL );
	}
	...
	if ( !Steam3Client().IsInitialized() || !Steam3Client().SteamUser() || !Steam3Client().SteamApps() ||
	...
	//
	// CS:GO by default in China launches in "Perfect World" mode, and allows for users to override
	//
	char const *szIpCountryCode = Steam3Client().SteamUtils() ? Steam3Client().SteamUtils()->GetIPCountry() : NULL;
	if ( ( !V_stricmp( szIpCountryCode, "CN" ) || CommandLine()->HasParm( "-promptperfectworld" ) )
		&& !CommandLine()->HasParm( "-worldwide" ) && !CommandLine()->HasParm( "-ww" ) && !CommandLine()->HasParm( "-noperfectworld" ) && !CommandLine()->HasParm( "-perfectworld" ) )
	{	// We are going to make a decision now about whether we should extend
		// Chinese command line to -perfectworld by default?

		//
		// Check if they own any of the multiplayer licenses?
		//
		bool bUserSelectedToUsePerfectWorld = true;
		int nPublicSteamApp = 624820;
		// int nPublicPwApp = 624821;
		if ( Steam3Client().SteamApps()->BIsSubscribedApp( nPublicSteamApp )
			// || Steam3Client().SteamApps()->BIsSubscribedApp( nPublicPwApp )
			)
		{
#if IS_WINDOWS_PC
			// At this point we know that:
			// + User is playing from mainland China    AND
			// + User hasn't configured a command line preference for which launcher to use
			// Show dialog to pick how user wants to run?
			INT_PTR nDialogPromptResult = ::DialogBox( NULL, MAKEINTRESOURCE( 103 /*IDD_DIALOG_LAUNCH_PROMPT*/ ), NULL, DialogProcMessageBoxPrompt );
			if ( nDialogPromptResult == 1 )
				bUserSelectedToUsePerfectWorld = false;
#else
			bUserSelectedToUsePerfectWorld = false;
#endif
		}
		if ( bUserSelectedToUsePerfectWorld )
		{	// Defaulting to -perfectworld autodetected
			CommandLine()->AppendParm( "-perfectworld", NULL );
			CommandLine()->AppendParm( "-autodetectedperfectworld", NULL );
		}
	}
#endif // !NO_STEAM
	
	//
	// Mount Chinese filesystem extension now
	//
	if ( CommandLine()->FindParm( "-perfectworld" ) )
	{
		g_pFullFileSystem->AddSearchPath( "perfectworld", "COMPAT:GAME", PATH_ADD_TO_HEAD );
		g_pFullFileSystem->AddSearchPath( "audiochinese", "COMPAT:GAME", PATH_ADD_TO_HEAD );
	}
	...
	if ( g_pFileSystem )
	{
		...
		else if ( Steam3Client().SteamUser() && Steam3Client().SteamUser()->GetUserDataFolder( chUserLocalDataFolder, sizeof( chUserLocalDataFolder ) ) && chUserLocalDataFolder[0] )
		{
...
void CEngineAPI::PumpMessages()
{
	...
	{
#ifdef PANORAMA_ENABLE
		if ( g_pIMEManager && g_pInputSystem->IsIMEAllowed() )
		{
			// IME requirement. Pass these messages to IME BEFORE any TranlsateMessage/DispatchMessage.
			...
			{
				g_pIMEManager->HandlePreProcessKeyboardEvent( (PlatWindow_t)msg.hwnd, msg.message, msg.wParam, msg.lParam);
			}
		}
#endif

#if defined( INCLUDE_SCALEFORM )
		if ( g_pScaleformUI && g_pInputSystem->IsIMEAllowed() )
		{
...
#pragma warning( suppress : 4535 ) // warning C4535: calling _set_se_translator() requires /EHa
		...
		try 
		{
			...
			if ( pchSysErrorText && pchSysErrorText[0] == '\n' && pchSysErrorText[1] == 'J' && pchSysErrorText[2] == 'S' && pchSysErrorText[3] == '(' )
			{
				// This is a custom string representing a javascript error
				V_snprintf( m_errorText, nSize, "%s\n", pchSysErrorText + 1 );
			}
			else if ( pchSysErrorText )
			{
				...
				try
				{
					...
					if ( len < nSize )
					{
						...
						for ( iter.SetFirst() ; iter.IsValid() ; iter.Next() )
						{
							...
							if ( pThisCvar->GetFlags() & ( FCVAR_SERVER_CANNOT_QUERY | FCVAR_PROTECTED ) )
								continue;

							if ( !( pThisCvar->GetFlags() & FCVAR_NEVER_AS_STRING ) )
							{
								char var1[ MAX_OSPATH ];
								char var2[ MAX_OSPATH ];

								Q_strncpy( var1, Host_CleanupConVarStringValue( pThisCvar->GetString() ), sizeof( var1 ) );
								Q_strncpy( var2, Host_CleanupConVarStringValue( pThisCvar->GetDefault() ), sizeof( var2 ) );
							...
							{
								if ( pThisCvar->GetFloat() == Q_atof( pThisCvar->GetDefault() ) )
									continue;
							}
							...
							if ( !( pThisCvar->GetFlags() & FCVAR_NEVER_AS_STRING ) )
							{
								add = Q_snprintf( cvarcmd, sizeof(cvarcmd),"%25.25s %25.25s %25.25s\n",
									pThisCvar->GetName(), Host_CleanupConVarStringValue( pThisCvar->GetString() ), pThisCvar->GetDefault() );
							}
							else
							{
								add = Q_snprintf( cvarcmd, sizeof(cvarcmd),"%25.25s %25.25f %25.25f\n",
									pThisCvar->GetName(), pThisCvar->GetFloat(), Q_atof( pThisCvar->GetDefault() ) );
							}
...
public:
	...
	{
#ifdef PANORAMA_ENABLE
		return PanoramaEngineHandler().PanoramaRunFrame(nSlot);
#endif
	}
	...
	{
#ifdef PANORAMA_ENABLE
		return PanoramaEngineHandler().PanoramaRenderFrame(nSlot);
#endif
	}
	...
	{
#ifdef PANORAMA_ENABLE
		return PanoramaEngineHandler().ReleaseGameInputHandler( handle );
#endif
	}
