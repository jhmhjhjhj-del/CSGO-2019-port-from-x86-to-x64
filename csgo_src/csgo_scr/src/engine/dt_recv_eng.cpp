...
class CNetDump
{
	...
	struct Field_t
	{
		...
		CUtlVector< FieldDump_t > m_Fields;
	};
public:
	CNetDump()
	{
		m_Memory.Init( "CNetDump", 128 * 1024 * 1024, 2 * 1024 * 1024, 2 * 1024 * 1024 );
		m_Table.EnsureCapacity( 2000 );
	}
	void AddField(int nPath, const char *pFieldName, const char *pFieldValue)
	{
		int nLen = V_strlen( pFieldValue );
		FieldDump_t fd;
		fd.pFieldValue = (char*)m_Memory.Alloc( nLen );
		fd.nTick = g_ClientGlobalVariables.tickcount;
		V_memcpy( fd.pFieldValue, pFieldValue, nLen + 1 );
		if ( nPath >= m_Table.Count() )
		{
			m_Table.SetCountNonDestructively( nPath + 1 );
			m_Table[ nPath ].m_FieldName = pFieldName;
		}
		else if( m_Table[ nPath ].m_Fields.IsEmpty() )
		{
			m_Table[ nPath ].m_FieldName = pFieldName;
		}
		m_Table[ nPath ].m_Fields.AddToTail( fd );
	}
	...
	void End()
	{
		...
		for ( int i = 0; i < 999; ++i )
		{
			V_snprintf( filename, sizeof( filename ), "net_dump%03d.csv", i );
			if ( !g_pFullFileSystem->FileExists( filename ) )
				break;
		}
		FileHandle_t fh = g_pFullFileSystem->Open( filename, "wt" );
		CUtlVector<int> it;
		g_pFullFileSystem->FPrintf( fh, "Tick" );
		it.SetCount( m_Table.Count() );
		it.FillWithValue( 0 );
		for ( int i = 0; i < m_Table.Count(); ++i )
		{
			CUtlVector<FieldDump_t> &fields = m_Table[ i ].m_Fields;
			if ( fields.IsEmpty() )
				continue;
			g_pFullFileSystem->FPrintf( fh, "\t%s-%d", m_Table[ i ].m_FieldName.Get(), i );
		}

		for(;;)
		{
			int nNextTick = INT_MAX;
			for ( int i = 0; i < m_Table.Count(); ++i )
			{
				if ( it[ i ] < m_Table[ i ].m_Fields.Count() )
				{
					nNextTick = Min( nNextTick, m_Table[ i ].m_Fields[ it[ i ] ].nTick );
				}
			}
			if ( nNextTick == INT_MAX )
				break; // didn't find any tick
			...
			for ( int i = 0; i < m_Table.Count(); ++i )
			{
				...
				if ( it[ i ] < fields.Count() && m_Table[ i ].m_Fields[ it[ i ] ].nTick == nNextTick )
				{
					pFieldValue = m_Table[ i ].m_Fields[ it[ i ]++ ].pFieldValue;
				}
				g_pFullFileSystem->FPrintf( fh, "\t%s", pFieldValue );
			}
		}
...
protected:
	CMemoryStack m_Memory;

	CUtlVector< Field_t > m_Table;

};

CNetDump g_NetDump;

void FnChangeCallback_NetDump( IConVar *var, const char *pOldValue, float flOldValue );

ConVar net_dump_decode( "net_dump_decode", "0", 0, "debug dump of fields of an entity", FnChangeCallback_NetDump );

void FnChangeCallback_NetDump( IConVar *var, const char *pOldValue, float flOldValue )
{
	if ( net_dump_decode.GetInt() <= 0 )
	{
		g_NetDump.End();
	}
}
...
#if NET_DEBUG
		char nameBuffer[ 128 ];
		if ( objectID > 0 && objectID == net_dump_decode.GetInt() )
		{
			const char *pPropName = decodeInfo.m_pProp->GetName();
			if ( !pPropName )
			{
				pPropName = nameBuffer;
				V_snprintf( nameBuffer, sizeof( nameBuffer ), "%p_type%d", decodeInfo.m_pProp, decodeInfo.m_pProp->m_Type );
			}
			decodeInfo.m_Value.m_Type = decodeInfo.m_pProp->m_Type;
			g_NetDump.AddField( path, pPropName, decodeInfo.m_Value.ToString() );
		}
#endif
	}
