...
bool CHLTVClient::CLCMsg_HltvFixupOperatorTick( const CCLCMsg_HltvFixupOperatorTick& msg )
{
	if ( IsFixupOperatorMode() )
	{//DevMsg( "fixup %d-%d=%d {%g,%g,%g}\n", msg.tick(), m_pHLTV->m_nLastTick, msg.tick() - m_pHLTV->m_nLastTick, msg.origin().x(), msg.origin().y(), msg.origin().z() );
		...
		if ( tv_allow_camera_man_override.GetBool() )
		{
			for ( CActiveHltvServerIterator hltv; hltv; hltv.Next() )
				hltv->FixupOperatorTick( msg );
		}
		else
		{
			DevMsg( "HltvFixupOperatorTick(%d) attempt to fixup camera without tv_allow_camera_man_override\n", msg.tick() );
		}
	}
	else
	{
		DevMsg( "HltvFixupOperatorTick(%d) without cameraman_override mode\n", msg.tick() );
	}
	return true;
}
...
bool IsInteractiveCasterDebug()
{
	return false; // no need in debug code in non-dev builds
}
...
bool CHLTVClient::ExecuteStringCommand( const char *pCommandString )
{
	...
	else if ( stricmp( cmd, "cameraman_secondary_override_begin" ) == 0 )
	{
		if ( IsInteractiveCasterDebug() || ( serverGameDLL && m_SteamID.IsValid() ) )
		{
			if ( IsInteractiveCasterDebug() || serverGameDLL->ValidateAndAddActiveCaster( m_SteamID, false ) )
			{
				// for now, also set this client as an active camera man
				// SetActiveCameraMan( true );

				// Fixup operator is an HLTV Client whose local transform overwrites the interactive caster 
				// (player with IsActiveCameraMan() == true ) transform at the same frame number. 
				// Since fixup operator is 3 seconds behind, and potentially has the knowledge of real time,
				// this causally requires us to patch up the interactive caster stream after is has been created,
				// and before we pack it up for http hltv broadcast.
				
				// Establish a back channel to the server
				SetFixupOperatorMode( true );
			}
		}
	}
	else if ( stricmp( cmd, "cameraman_secondary_override_end" ) == 0 )
	{
		// stop overriding
		SetFixupOperatorMode( false );
	}
	else
	{
...
void CHLTVClient::SetFixupOperatorMode( bool bOn )
{
	if ( bOn != m_bFixupOperatorMode )
	{
		// send primary cameraman a message, letting know we're overriding
		m_bFixupOperatorMode = bOn;
		extern void SV_NotifyFixupTargetOfFixupOperatorModeChange( );
		SV_NotifyFixupTargetOfFixupOperatorModeChange( );
	}
}
...
bool CHLTVClient::SendSnapshot( CClientFrame * pFrame )
{
	...
	bf_write	msg( "CHLTVClient::SendSnapshot", buffer.Base(), buffer.Size() );
	...
	m_Server->m_StringTables->WriteUpdateMessageAtTick( NULL, pFrame->tick_count, GetMaxAckTickCount(), msg );
