...
void AddCapsuleOverlay( const Vector &vStart, const Vector &vEnd, const float &flRadius, int r, int g, int b, int a, float flDuration, bool bWireframe, bool bIgnoreZ )
{
	if ( GetBaseLocalClient().IsPaused() )
		return;
	...
	new_overlay->noDepthTest = bIgnoreZ;
	...
	case OVERLAY_CAPSULE:
		{
			...
			if ( pCapsule->noDepthTest )
				pMaterial = RenderUtil_GetDebugMaterial( pCapsule->m_bWireframe ? kDebugMaterialWireframeIgnoreZ : kDebugMaterialVertexColorIgnoreZ );
			else
				pMaterial = RenderUtil_GetDebugMaterial( pCapsule->m_bWireframe ? kDebugMaterialWireframe : kDebugMaterialVertexColor );

			if ( pMaterial )
			{
				RenderCapsule( pCapsule->start, pCapsule->end, pCapsule->flRadius,
					Color( pCapsule->r, pCapsule->g, pCapsule->b, pCapsule->a ), pMaterial );
			}
		}
		break;
...
public:
	...
	{
		CDebugOverlay::AddCapsuleOverlay( vStart, vEnd, flRadius, r, g, b, a, flDuration, false, false );
	}

	void AddCapsuleOverlay( const Vector &vStart, const Vector &vEnd, const float &flRadius, int r, int g, int b, int a, float flDuration, bool bWireframe, bool bIgnoreZ )
	{
		CDebugOverlay::AddCapsuleOverlay( vStart, vEnd, flRadius, r, g, b, a, flDuration, bWireframe, bIgnoreZ );
	}
};
