...
{
	DevMsg( "File '%s' requested from server %s.\n", fileName, m_NetChannel->GetAddress() );

	if ( !cl_allowupload.GetBool() )
	{
		DevMsg( "File uploading disabled.\n" );
...
void CClientState::Disconnect( bool bShowMainMenu )
{
	...
	S_StopAllSounds( true, false /*don't stop main menu music*/ );
...
bool CClientState::SVCMsg_HltvFixupOperatorStatus( const CSVCMsg_HltvFixupOperatorStatus &msg )
{
	VPROF( "HltvReplayStart" );
	g_ClientDLL->OnHltvFixupOperatorStatus( msg );
	return true;
}
