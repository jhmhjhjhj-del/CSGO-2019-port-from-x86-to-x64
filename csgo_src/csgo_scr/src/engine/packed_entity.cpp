...
size_t PackedEntity::GetMemSize() const
{
	size_t nSize = sizeof( *this ) + sizeof( m_Recipients[ 0 ] ) *m_Recipients.Count() + g_pSerializedEntities->GetMemSize( m_SerializedEntity );
	if ( m_pChangeFrameList )
		nSize += m_pChangeFrameList->GetMemSize();
	return nSize;
}
