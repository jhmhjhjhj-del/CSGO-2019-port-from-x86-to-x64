...
void RunDataTableTest()
{
	...
	for( int iIteration=0; iIteration < nIterations; iIteration++ )
	{
		...
		bool bHasUncompressibleProps = false; // we can add a random true here to test this feature, if 

		if( !SendTable_Encode( pSendTable, startEntity, bHasUncompressibleProps, &dtServer, -1, NULL ) )
		{
			Assert(false);
		}
		...
		if ( iIteration == 0 )
		{
			if ( !SendTable_Encode( pSendTable, endEntity, bHasUncompressibleProps, &dtServer, -11111, NULL ) )
			{
				Assert(false);
			}
