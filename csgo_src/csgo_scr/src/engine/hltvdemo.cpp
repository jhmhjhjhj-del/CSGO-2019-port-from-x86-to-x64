...
void CHLTVDemoRecorder::WriteServerInfo()
{
	...
	bf_write	msg( "CHLTVDemoRecorder::WriteServerInfo", buffer.Base(), buffer.Size() );
	...
	hltv->m_StringTables->WriteBaselines( pServer->GetMapName(), msg, m_nStartTick );
#endif
...
void CHLTVDemoRecorder::RecordPlayerAvatar( const CNETMsg_PlayerAvatarData_t* hltvPlayerAvatar )
{
	...
	CUtlBuffer  buffer( 0, NET_MAX_PAYLOAD ); // too large for stack
	bf_write	bfWrite( "CHLTVDemo::NETMsg_PlayerAvatarData", buffer.Base(), buffer.Size() );
...
void CHLTVDemoRecorder::RecordStringTables()
{
	...
	this->hltv->m_NetworkStringTables.WriteStringTables( buf, m_nStartTick );
...
int CHLTVDemoRecorder::WriteSignonData()
{
	...
	// RecordStringTables(); // <- this is unnecessary! we're just writing stringtables out again! The baseline already contains the stringtables for the right tick!

	CUtlBuffer  buffer( 0, NET_MAX_PAYLOAD ); // too large for stack
	bf_write	msg( "CHLTVDemo::WriteSignonData", buffer.Base(), buffer.Size() );
...
void CHLTVDemoRecorder::WriteFrame( CHLTVFrame *pFrame, bf_write *additionaldata )
{
	...
	if ( !m_bIsRecording || pFrame->tick_count <= m_nLastWrittenTick )
		return; // guard against writing the same frames, or frames out of order

	CUtlBuffer  buffer( 0, NET_MAX_PAYLOAD ); // too large for stack
	bf_write	msg( "CHLTVDemo::RecordFrame", buffer.Base(), buffer.Size() );
	...
	hltv->m_StringTables->WriteUpdateMessageAtTick( NULL, m_nLastWrittenTick, MAX( m_nStartTick, m_nDeltaTick ), msg );
#endif
