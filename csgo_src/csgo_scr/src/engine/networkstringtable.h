...
public:
	...
	const void		*GetStringUserDataAtTick( int stringNumber, int *length, int nAtTick ) const; 
	CNetworkStringTableItem *GetStringTableItem( int stringNumber )const;
...
public:
	...
	int				WriteUpdateAtTick( CBaseClient *client, bf_write &buf, int nAtTick, int nTickAck ) const;
	void			ParseUpdate( bf_read &buf, int entries );

	// HLTV change history & rollback
	void			EnableRollback( bool bEnable );
	
	// local backdoor tables
	void			SetMirrorTable( uint nIndex, CNetworkStringTable *table );
	void			UpdateMirrorTable( int tick_ack  );
	void			CopyStringTable(CNetworkStringTable * table);
	// buffer IO
	void			WriteStringTable( bf_write& buf, int nAtTick );
	bool			ReadStringTable( bf_read& buf );

	bool			WriteBaselines( CSVCMsg_CreateStringTable_t &msg, int nAtTick );
...
protected:
	...
	int						m_nEntryBits;
	int						m_nTickCount;
	int						m_nLastChangedTick[2]; // the sequence of ticks when the table changed; it happens infrequently, so [0] is the very last time, [1] is the one before last
	...
	CNetworkStringTable		*m_pMirrorTable[ MIRROR_TABLE_MAX_COUNT ];
...
public:
	...
	int			GetTick()const { return m_nTickCount; }
	...
	// Buffer I/O
	void		WriteStringTables( bf_write& buf, int nAtTick );
	bool		ReadStringTables( bf_read& buf );

	void		WriteUpdateMessageAtTick( CBaseClient *client, int nAtTick, int nTickAck, bf_write &buf );
	void		WriteBaselines( char const *pchMapName, bf_write &buf, int nAtTick ); // by default, write baseline at the latest available tick
...
private:
	...
	int			m_nTickCount;		// current tick, only needed so that new tables created start from this tick
