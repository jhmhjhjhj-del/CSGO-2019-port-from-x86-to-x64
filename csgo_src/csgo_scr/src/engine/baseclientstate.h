...
public: // IServerMessageHandlers
	...
	virtual bool SVCMsg_HltvReplay( const CSVCMsg_HltvReplay &msg ) = 0;
	virtual bool SVCMsg_HltvFixupOperatorStatus( const CSVCMsg_HltvFixupOperatorStatus &msg ) = 0;
	...
	CNetMessageBinder m_SVCMsgHltvFixupOperatorStatus;
...
public:
	...
	char		m_szLevelNameShort[ 80 ]; // removes maps/ and .bsp extension
	char		m_szMapGroupName[ 80 ]; //the name of the map group we are currently playing in
	char		m_szLastLevelNameShort[ 80 ]; // stores the previous value of m_szLevelNameShort when that gets cleared
