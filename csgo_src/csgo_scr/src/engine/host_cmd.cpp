...
#include "gl_model_private.h" // to print the map model name in minidump comment
#include "pr_edict.h"
#include "world.h"
...
void Host_PrintStatus( cmd_source_t commandSource, void ( *print )(const char *fmt, ...), bool bShort )
{
	...
	if ( NET_IsMultiplayer() )
	{
		...
		if ( bWithAddresses )
		{
			...
			uint16 nSDRPort = SteamNetworkingSocketsGameServer()->GetHostedDedicatedServerPort();
			if ( nSDRPort )
			{
				SteamNetworkingPOPID popID = SteamNetworkingSocketsGameServer()->GetHostedDedicatedServerPOPID();
				char szPoPID[ 8 ];
				GetSteamNetworkingLocationPOPStringFromID( popID, szPoPID );
				print( "sdr     : =%s on port %d (%s)\n", Steam3Server().GetGSSteamID().Render(), nSDRPort, szPoPID );
			}
		}
	...
	{
		const char *pMapName = sv.GetMapName();
		if ( !pMapName || !*pMapName )
		{
			if ( host_state.worldmodel && host_state.worldmodel->szPathName )
			{// the map name is in the Game Info log, so this may be duplicate information
				// this is how client gets initialized (this is the name passed to LevelInitPreEntity, before cleanup)
				pMapName = host_state.worldmodel->szPathName;
			}
		}

		print( "map     : %s at: %d x, %d y, %d z\n", pMapName, (int)MainViewOrigin()[0], (int)MainViewOrigin()[1], (int)MainViewOrigin()[2]);
		if ( INetChannel *pNetChan = GetBaseLocalClient().m_NetChannel )
		{
			print( "connected to %s\n", ns_address_render( pNetChan->GetRemoteAddress() ).String() );
		}
	}
#else
	{
...
#endif
			{
				ConMsg( "Server is not active\n" );
			}
#ifndef DEDICATED
			else if ( !GetBaseLocalClient().m_NetChannel )
			{
				ConMsg( "Not connected to server\n" );
			}
			else
			{
				ns_address_render adrrender( GetBaseLocalClient().m_NetChannel->GetRemoteAddress() );
...
CON_COMMAND( version, "Print version info string." )
{
	...
	int nVersion = Plat_GetOSVersion();
	if ( ( nVersion == PLAT_OS_VERSION_UNKNOWN ) || ( nVersion < 0 ) )
	{
		ConMsg( "Failed to determine Windows version (%d)\n", nVersion );
	}
	else
	{
		ConMsg( "Windows version %d.%d\n", nVersion/10, nVersion%10 );
	}
#endif
}
...
{
	bool bShort = !!args.FindArg( "-short" );

	extern bool g_bSSE42;
	if ( g_bSSE42 )
		Msg( "Stringtables use SSE4.2 optimizations\n" );

	SV_PrintStringTables( bShort );
#ifndef DEDICATED
	CL_PrintStringTables( bShort );
#endif
}
