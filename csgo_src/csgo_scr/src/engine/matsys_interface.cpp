...
static CTextureReference g_TabletTexture;
static CTextureReference g_GrassBurnTexture;
static CTextureReference g_ZoneProjectionTexture;
static CTextureReference g_ZoneProjectionSpectatorTexture;
...
static ConVar mat_resolveFullFrameDepth( "mat_resolveFullFrameDepth", "1",
#if DEVELOPMENT_ONLY
	FCVAR_DEVELOPMENTONLY,	// allow internal developers to tinker with rendering full frame depth resolve during playtests
#else
	FCVAR_CHEAT,			// in production the setting is cheat-protected
#endif
	"Enable depth resolve to a texture. 0=disable, 1=enable via resolve tricks if supported in hw, otherwise disable, 2=force extra depth only pass", mat_resolveFullFrameDepth_changed );
...
static void OverrideMaterialSystemConfigFromCommandLine( MaterialSystem_Config_t &config )
{
	...
	{
		// allow users to specify depth resolve on cmd line (allows AMD to reproduce driver crashes)
		int resolveFullFrameDepth = CommandLine()->ParmValue( "-mat_resolveFullFrameDepth", 1 );
		resolveFullFrameDepth = MAX( 1, MIN( 2, resolveFullFrameDepth ) ); // clamp to valid values
...
static ITexture *CreateTabletTexture( void )
{
	int iSize = CommandLine()->ParmValue( "-tabletTextureSize", 1024 );
	return materials->CreateNamedRenderTargetTextureEx2(
		"_rt_Tablet",
...
static ITexture *CreateGrassBurnTexture( void )
{
	int iSize = CommandLine()->ParmValue( "-grassBurnTextureSize", 512 );
	return materials->CreateNamedRenderTargetTextureEx2(
		"_rt_GrassBurn",
		iSize, iSize, RT_SIZE_OFFSCREEN,
		IMAGE_FORMAT_RGB888,
...
static ITexture *CreateZoneProjectionTexture( void )
{
	int iSize = CommandLine()->ParmValue( "-zoneProjectionTextureSize", 512 );
	return materials->CreateNamedRenderTargetTextureEx2(
		"_rt_ZoneProjection",
		iSize, iSize, RT_SIZE_OFFSCREEN,
		IMAGE_FORMAT_RGB888,
...
static ITexture *CreateZoneProjectionSpectatorTexture( void )
{
	int iSize = CommandLine()->ParmValue( "-zoneProjectionTextureSize", 512 );
	return materials->CreateNamedRenderTargetTextureEx2(
		"_rt_ZoneProjectionSpectator",
		iSize, iSize, RT_SIZE_OFFSCREEN,
		IMAGE_FORMAT_RGBA8888,
		MATERIAL_RT_DEPTH_NONE,
		0, 0 );
...
void InitWellKnownRenderTargets( void )
{
	...
	*/
	...
	g_TabletTexture.Init( CreateTabletTexture() );

	g_GrassBurnTexture.Init( CreateGrassBurnTexture() );

	g_ZoneProjectionTexture.Init( CreateZoneProjectionTexture() );
	g_ZoneProjectionSpectatorTexture.Init( CreateZoneProjectionSpectatorTexture() );
...
void ShutdownWellKnownRenderTargets( void )
{
	...
	else
	{
		...
		g_TabletTexture.Shutdown();
		g_GrassBurnTexture.Shutdown();
		g_ZoneProjectionTexture.Shutdown();
		g_ZoneProjectionSpectatorTexture.Shutdown();
	}
#endif
}
