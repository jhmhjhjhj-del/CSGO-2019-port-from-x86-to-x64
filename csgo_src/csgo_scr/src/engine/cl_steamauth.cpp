...
#ifndef _PS3
	SteamAPI_InitSafe(); // offline stub or real Steam
#else
...
void CSteam3Client::GetAuthSessionTicket( void *pTicket, int cbMaxTicket, uint32 *pcbTicket, uint64 unGSSteamID,  bool bSecure )
{
    ...
    if ( m_unGSSteamID == unGSSteamID && m_hAuthTicket != k_HAuthTicketInvalid )
    {
        *pcbTicket = m_steam3CookieLen;
        memcpy( pTicket, m_steam3Cookie, m_steam3CookieLen );
        return;
    }
    ...
    m_steam3CookieLen = *pcbTicket;
    memcpy( m_steam3Cookie, buffer.Base(),  m_steam3CookieLen );
#else
	return;
#endif
}
...
{
	// Offline NO_STEAM still loads steam_api64 stub — without RunCallbacks,
	// GC Tick/PollStubGrantRequests never run → profile/grant bridge cmds
	// sit forever in csgo/cfg/stub_bridge_cmd.txt (no ACK, no inventory).
	VPROF_BUDGET( "CSteam3Client::RunFrame", VPROF_BUDGETGROUP_STEAM );
	SteamAPI_RunCallbacks();
	extern void NET_RunSteamNetworkingCallbacksUser();
	NET_RunSteamNetworkingCallbacksUser();
}
