...
bool CHLTVClientState::SVCMsg_HltvReplay( const CSVCMsg_HltvReplay &msg )
{
	return true;
}

bool CHLTVClientState::SVCMsg_HltvFixupOperatorStatus( const CSVCMsg_HltvFixupOperatorStatus &msg )
{
	return true;
}
...
void CHLTVClientState::ReadPreserveEnt( CEntityReadInfo &u )
{
	...
	{
		Host_Error( "CL_ReadPreserveEnt: entity out of bounds (old=%d, new=%d)", u.m_nOldEntity, u.m_nNewEntity );
	}
...
void CHLTVClientState::SetLocalInfoConvarsForUpstreamConnection( CMsg_CVars &cvars, bool bMaxSlots )
{
	...
	SteamDatagramHostedAddress sdrRouting;
	if ( serverGameDLL->IsValveDS() && SteamNetworkingSocketsGameServer()->GetHostedDedicatedServerAddress( &sdrRouting ) )
	{
		ns_address nsadrsdr;
		nsadrsdr.SetAddrType( NSAT_PROXIED_GAMESERVER );
		nsadrsdr.m_steamID.SetFromSteamID( Steam3Server().GetGSSteamID(), STEAM_P2P_CHANNEL_HLTV );
		NetMsgSetCVarUsingDictionary( cvars.add_cvars(), "hltv_sdr", ns_address_render( nsadrsdr ).String() );
		
		CUtlStringBuilder strBuilder;
		strBuilder.EnsureCapacity( sizeof( sdrRouting ) * 2 + 1 );
		for ( int j = 0; j < sdrRouting.m_cbSize; ++ j )
		{
			strBuilder.AppendFormat( "%02X", uint8( sdrRouting.m_data[j] ) );
		}
		NetMsgSetCVarUsingDictionary( cvars.add_cvars(), "sdr_routing", strBuilder.Access() );
	}
}
