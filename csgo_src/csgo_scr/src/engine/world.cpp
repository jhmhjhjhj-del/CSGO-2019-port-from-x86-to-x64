...
void SV_ClearWorld (void)
{
	...
	Msg( "SV_ClearWorld: enter\n" );
	// Clean up static props from the previous level
#if !defined( DEDICATED )
	if ( !sv.IsDedicated() )
	{
		Msg( "SV_ClearWorld: shadow shutdown\n" );
		g_pShadowMgr->LevelShutdown();
	}
#endif // DEDICATED

	Msg( "SV_ClearWorld: staticprop shutdown\n" );
	...
	Msg( "SV_ClearWorld: spatial init\n" );
	...
	Msg( "SV_ClearWorld: staticprop init\n" );
	StaticPropMgr()->LevelInit();

#if !defined( DEDICATED )
	if ( !sv.IsDedicated() )
	{
		Msg( "SV_ClearWorld: shadow init\n" );
		g_pShadowMgr->LevelInit( host_state.worldbrush->numsurfaces );
	}
#endif // DEDICATED
	Msg( "SV_ClearWorld: done\n" );
}
