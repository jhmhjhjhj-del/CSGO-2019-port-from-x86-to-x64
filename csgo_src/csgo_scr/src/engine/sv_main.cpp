...
void SV_AddToFatPVS( EnginePVSContext_t *pContext, int nClusterIndex )
{
	int		i;
	byte	pvs[ MAX_MAP_LEAFS / 8 ];

	CM_Vis( pvs, sizeof( pvs ), nClusterIndex, DVIS_PVS );
	int nLastInt = pContext->m_nFatBytes & ( ~3 );
	for ( i = 0; i < nLastInt; i += 4 )
	{
		uint *pOut = ( uint * )( pContext->m_pFatPVS + i );
		uint *pIn = ( uint * )( pvs + i );
		*pOut |= *pIn;
	}
	for ( ; i < pContext->m_nFatBytes; i++ )
	{
		pContext->m_pFatPVS[ i ] |= pvs[ i ];
	}
}
...
//-----------------------------------------------------------------------------
void SV_ResetPVS( EnginePVSContext_t *pContext, byte* pvs, int nPVSSize )
{
	pContext->m_pFatPVS = pvs;
	pContext->m_nFatBytes = Bits2Bytes( CM_NumClusters() );

	if ( pContext->m_nFatBytes > nPVSSize )
	{
		Sys_Error( "SV_ResetPVS:  Size %i too big for buffer %i\n", pContext->m_nFatBytes, nPVSSize );
	}

	Q_memset( pContext->m_pFatPVS, 0, pContext->m_nFatBytes );
	pContext->m_ClustersNetworked.RemoveAll();
	pContext->m_AreasNetworked.RemoveAll();
}
...
=============
*/
void SV_AddOriginToPVS( EnginePVSContext_t *pContext, const Vector& vOrigin )
{
	...
	if ( pContext->m_ClustersNetworked.Find( nClusterIndex ) != -1 )
		return;

	// mark as included
	pContext->m_ClustersNetworked.AddToTail( nClusterIndex );
	SV_AddToFatPVS( pContext, nClusterIndex );

	int nArea = CM_LeafArea( nLeafIndex );
	if ( pContext->m_AreasNetworked.Find( nArea ) != -1 )
		return;
	pContext->m_AreasNetworked.AddToTail( nArea );
}
...
uint64 GetTicksPerUS()
{
	static uint64 s_nTicks = GetCPUInformation().m_Speed / 1000000;
	return s_nTicks;
}

class CSendTaskStats
{
public:
	CSendTaskStats()
	{
	...
	void Reset()
	{
		...
		m_nSendTaskUS = 0;
		m_nSendTaskUSSqr = 0;
		for ( int i = 0; i < SENDTASK_PROFILER_COUNT; ++i )
			m_mpPackEntities[i].Reset();
	}
	void Dump()
	{
		if ( m_nSendTaskCount >= 2 )
		{
			Msg( "mean=%s stDev=%s stErr=%s us over %s runs\n",
				V_pretifynum( m_nSendTaskUS / m_nSendTaskCount ),
				V_pretifynum( ( int64 ) sqrt( m_nSendTaskUSSqr / m_nSendTaskCount - Sqr( m_nSendTaskUS / m_nSendTaskCount ) ) ),
				V_pretifynum( ( int64 ) sqrt( ( m_nSendTaskUSSqr / m_nSendTaskCount - Sqr( m_nSendTaskUS / m_nSendTaskCount ) ) / ( m_nSendTaskCount - 1 ) ) ),
				V_pretifynum( m_nSendTaskCount )
			);
		}
		else
		{
	...
	void DumpShort( int nRow, int nMode )
	{
		if ( m_nSendTaskCount >= 2 )
		{
			switch ( nMode )
			{
			case 0: // average duration of SendClientMessage (parallel part) per frame, us
				Msg( "\t%s", V_pretifynum( m_nSendTaskUS / m_nSendTaskCount ) );
				break;
			case 1: // average duration of whatever [nRow] profiler is measuring, us
				Msg( "\t%s", V_pretifynum( m_mpPackEntities[ nRow ].m_numTimeBaseTicks / ( m_nSendTaskCount * GetTicksPerUS() ) ) );
				break;
			case 2: // average count of whatever [nRow] profiler is measuring, us
				Msg( "\t%.1f", m_mpPackEntities[ nRow ].m_numCalls / double( m_nSendTaskCount ) );
				break;
			}
	...
	public:
	uint64 m_nSendTaskCount;
	uint64 m_nSendTaskUS;
	uint64 m_nSendTaskUSSqr;

	CMicroProfiler m_mpPackEntities[ SENDTASK_PROFILER_COUNT ];
};

CSendTaskStats g_SendTaskStats[ 16 ];
extern CMicroProfiler g_mpPackEntity[ SENDTASK_PROFILER_COUNT ];


class CSendTaskProfiler
{
	uint64 m_nStartUS;
	CSendTaskStats *m_pStats;
public:
	CSendTaskProfiler( CSendTaskStats *pStats ):m_pStats( pStats )
	{
		for ( int i = 0; i < SENDTASK_PROFILER_COUNT; ++i)
			g_mpPackEntity[i].Reset();
		m_nStartUS = Plat_USTime();
	}
	~CSendTaskProfiler()
	{
		uint64 nDeltaUS = Plat_USTime() - m_nStartUS;
		m_pStats->m_nSendTaskCount++;
		m_pStats->m_nSendTaskUS += nDeltaUS;
		m_pStats->m_nSendTaskUSSqr += ( nDeltaUS * nDeltaUS );
		for ( int i = 0; i < SENDTASK_PROFILER_COUNT; ++i )
			m_pStats->m_mpPackEntities[i].Add( g_mpPackEntity[ i ] );
	}
};


void DumpSendStats( bool bReset, bool bShort )
{
	if ( bShort )
	{
		Msg( "send stats\n" );
		for ( int nRow = 0; nRow < SENDTASK_PROFILER_COUNT; ++nRow )
		{
			for ( int nMode = 0; nMode < 3; ++nMode )
			{
				...
				for ( int i = 0; i < ARRAYSIZE( g_SendTaskStats ); ++i )
				{
					g_SendTaskStats[ i ].DumpShort( nRow, nMode );
				}
				Msg( "\n" );
			}
		}
	...
	else
	{
		for ( int i = 0; i < ARRAYSIZE( g_SendTaskStats ); ++i )
		{
			if ( g_SendTaskStats[ i ].m_nSendTaskCount > 2 )
			{
				Msg( "[%d]", i );
				g_SendTaskStats[ i ].Dump();
			}
		}
	}
	if ( bReset )
	{
		for ( int i = 0; i < ARRAYSIZE( g_SendTaskStats ); ++i )
			g_SendTaskStats[ i ].Reset();
	}
}

CON_COMMAND( sv_send_stats, "show stats of running parallel send" )
{
	bool bReset = false, bShort = false;
	for ( int i = 0; i < args.ArgC(); ++i )
	{
		if ( !V_stricmp( args.ArgV()[ i ], "-reset" ) )
		{
			bReset = true;
		}
		else if ( !V_stricmp( args.ArgV()[ i ], "-short" ) )
		{
			bShort = true;
		}
	}
...
void OnParallelSendChange( IConVar *var, const char *pOldValue, float flOldValue )
{
	extern void AdjustThreadPoolThreadCount();
	AdjustThreadPoolThreadCount();
}

// The default is 0 to turn parallel send off by default
#define DEFAULT_PARALLEL_SEND "0"

ConVar sv_parallel_send( "sv_parallel_send", DEFAULT_PARALLEL_SEND, FCVAR_RELEASE, "Pack and send snapshots in parallel for smoother server tick rate at the expense of spending more CPU.", OnParallelSendChange );

// parallel_send 12 overhead: 2x win32, 2.2x linux
...
void CGameServer::SendClientMessages ( bool bSendSnapshots )
{
    ...
    if ( receivingClientCount )
    {
		...
		if ( !bHLTVOnly )
		{
			...
			int nParallelSend = sv_parallel_send.GetInt();
			if ( nParallelSend < 0 )
			{
				static int s_nCounter = 0;
				s_nCounter++;
				nParallelSend = s_nCounter % Min<int>( -nParallelSend , ARRAYSIZE( g_SendTaskStats ) );
				if ( !( s_nCounter % 1024 ) )
					DumpSendStats( true, true );
			}
			int nUseProfiler = Max<int>( 0, Min<int>( nParallelSend, ARRAYSIZE( g_SendTaskStats ) ) );
			CSendTaskProfiler profiler( &g_SendTaskStats[ nUseProfiler ] );

			if ( nParallelSend > 0 /*&& pSnapshot->m_nValidEntities*/ && receivingClientCount > 1 )
			{
				VPROF_BUDGET( "SV_ParallelSend", VPROF_BUDGETGROUP_OTHER_NETWORKING );
				MDLCACHE_CRITICAL_SECTION_( g_pMDLCache );
				CParallelSendTask sendJob( nParallelSend, receivingClientCount, pReceivingClients, pSnapshot, nHltvMaxAckCount );
				sendJob.Run();
			}
			else
			{
				// Compute the client packs. The snapshot is NOT changed, only references can be added; this code scope owns one temporary reference, too, so snapshot manager will not be asked to release this snapshot (important for multithreading).
				{
					MDLCACHE_CRITICAL_SECTION_( g_pMDLCache );
					SV_ComputeClientPacks( receivingClientCount, pReceivingClients, pSnapshot );
				}

				// InvalidateSharedEdictChangeInfos() would be appropriate to call here; but stringtables should not have to restore tick, because multiple threads will be packing stringtables at different ticks anyway
...
void CGameServer::StopHltvReplay(  )
{
	for ( int i = 0; i < GetClientCount(); i++ )
	{
		Client( i )->StopHltvReplay( false);
	}
}
...
void SV_BroadcastVoiceData(IClient * cl, const CCLCMsg_VoiceData& msg, bool bSkipHltvClients )
{
	...
	for ( int nClientIndex = 0; nClientIndex < sv.GetClientCount(); nClientIndex++ )
    {
		CBaseClient *pDestClient = static_cast< CBaseClient * >( sv.GetClient( nClientIndex ) );
		...
		if ( bSkipHltvClients && pDestClient->IsHLTV() )
		{
			continue;
		}
		...
		for ( int nSplitScreenUserIndex = 1; nSplitScreenUserIndex < ARRAYSIZE( pDestClient->m_SplitScreenUsers ); ++nSplitScreenUserIndex )
		{
			voiceData.set_audible_mask( voiceData.audible_mask() | ( nSplitScreenUserIndex << 1) );
			CBaseClient *splitUser = pDestClient->m_SplitScreenUsers[ nSplitScreenUserIndex ];
			if ( splitUser )
			{
    ...
    if ( !g_pLocalNetworkBackdoor )
    {
        ...
        for ( int entnum = 0; entnum < sv.num_edicts ; entnum++)
        {
            ...
            if ( !SendTable_Encode(
                pSendTable, 
                handle,
				HasUncompressibleProps( entnum, pSendTable ),
                edict->GetUnknown(), 
                entnum,
                NULL
                ) )
...
static edict_t *g_pSvEdicts_Edicts = nullptr;
static IChangeInfoAccessor *g_pSvEdicts_ChangeInfos = nullptr;

static void SV_AllocateEdicts()
{
	AssertFatal( sv.max_edicts == 0 || sv.max_edicts == MAX_EDICTS );

	if ( !g_pSvEdicts_Edicts )
		g_pSvEdicts_Edicts = ( edict_t * ) MemAlloc_Alloc( sizeof( edict_t ) * MAX_EDICTS );
	V_memset( g_pSvEdicts_Edicts, 0, sizeof( edict_t ) * MAX_EDICTS );

	if ( !g_pSvEdicts_ChangeInfos )
		g_pSvEdicts_ChangeInfos = ( IChangeInfoAccessor * ) MemAlloc_Alloc( sizeof( IChangeInfoAccessor ) * MAX_EDICTS );
	V_memset( g_pSvEdicts_ChangeInfos, 0, sizeof( IChangeInfoAccessor ) * MAX_EDICTS );

	sv.edicts = g_pSvEdicts_Edicts;
    ...
    sv.edictchangeinfo = g_pSvEdicts_ChangeInfos;
}
...
#if !defined( DEDICATED ) && defined( DX_TO_GL_ABSTRACTION )
	// OpenGL platform - SV_FlushMemoryIfMarked() will call modelloader->PurgeUnusedModels()
	// in an attempt to free as much memory as possible. However TOGL layer will defer
	// deleting textures to the next Present call. 
	// Forcing a call to Present by updating the progress bar.
	EngineVGui()->UpdateProgressBar( PROGRESS_OPENGLFLUSHMEMORY );
#endif
...
// 		modelloader->UnloadUnreferencedModels();
	...
	Msg( "Host_NewGame: ReloadMaterials begin\n" );
	if ( materials )
		materials->ReloadMaterials();
	Msg( "Host_NewGame: ReloadMaterials done\n" );
#endif
	...
	Msg( "Host_NewGame: GetModelForName begin %s\n", szModelName );
    host_state.SetWorldModel( modelloader->GetModelForName( szModelName, IModelLoader::FMODELLOADER_SERVER ) );
	Msg( "Host_NewGame: GetModelForName done ok=%d\n", host_state.worldmodel ? 1 : 0 );
    ...
    if ( IsMultiplayer() && !IsGameConsole() )
    {
...
#endif
        Msg( "Host_NewGame: CRC_MapFile begin\n" );
        ...
        Msg( "Host_NewGame: CRC_MapFile done\n" );
	...
	m_StringTables->EnableRollback( false ); // generally, server doesn't need history
    ...
    Msg( "Host_NewGame: SV_CreateNetworkStringTables begin\n" );
    // Create network string tables ( including precache tables )
    SV_CreateNetworkStringTables( mapname );
    Msg( "Host_NewGame: SV_CreateNetworkStringTables done\n" );
    Host_KeepAliveDuringMapLoad();
...
#endif
    Msg( "Host_NewGame: PrecacheModel world begin\n" );
    // Add in world
    PrecacheModel( szModelName, RES_FATALIFMISSING | RES_PRELOAD, host_state.worldmodel );
    Msg( "Host_NewGame: PrecacheModel world done\n" );
    Host_KeepAliveDuringMapLoad();
    ...
    Msg( "Host_NewGame: SV_ClearWorld begin\n" );
    // Clear world interaction links
    // Loads and inserts static props
    SV_ClearWorld();
    Msg( "Host_NewGame: SV_ClearWorld done\n" );
    Host_KeepAliveDuringMapLoad();
...
#error
    ...
    Msg( "Host_NewGame: SpawnServer finish\n" );
    g_pFileSystem->EndMapAccess();
	SV_SetSteamCrashComment();
    return true;
}
...
void SV_NotifyFixupTargetOfFixupOperatorModeChange()
{	
	CHLTVClient *pFixupSource = nullptr;
	// find fixup source
	for ( CHltvServerIterator hltv; hltv; hltv.Next() )
	{
		int nClientCount = hltv->GetClientCount();
		for ( int nClient = 0; nClient < nClientCount; ++nClient )
		{
			if ( CHLTVClient *pClient = dynamic_cast< CHLTVClient * >( hltv->GetClient( nClient ) ) )
			{
				if ( pClient->IsActive() && pClient->IsFixupOperatorMode() )
				{
					pFixupSource = pClient;
					break;
				}
			}
		}
	...
	sv.NotifyFixupTargetOfFixupOperatorModeChange( pFixupSource != nullptr, pFixupSource ? pFixupSource->GetClientName() : "" );
}