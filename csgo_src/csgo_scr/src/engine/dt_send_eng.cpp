...
extern void SendTable_InitWellKnownProps( SendTable * pTable );
...
// parallel_send 12 overhead:  2x (win32,linux)
bool SendTable_Encode(
	const SendTable *pTable,
	SerializedEntityHandle_t handle,
	bool bHasUncompressibleProps,
	const void *pStruct, 
	int objectID,
	...
	)
{
	...
	Assert( pEntity );
	pEntity->Clear();
	pEntity->SetUncompressibleProps( bHasUncompressibleProps );	// this can be a separate function
...
void SendTable_PatchField( SendTable *pTable, SendTableWellKnownPropNamesEnum_t nProp, DVariant *pVar, SerializedEntityHandle_t toEntity, int objectID )
{
	CSerializedEntity *pToEntity = ( CSerializedEntity * )toEntity;
	int nField = SendTable_GetWellKnownProp( pTable, nProp );
	if ( nField < 0 )
		return;
	const SendProp *pProp = pTable->m_pPrecalc->GetProp( nField );
	bf_write toStream( pToEntity->GetFieldData(), ( pToEntity->GetFieldDataBitCount() + 7 ) / 8, pToEntity->GetFieldDataBitCount() );
	int nOffset = pToEntity->GetFieldDataBitOffset( nField );
#ifdef DBGFLAG_ASSERT
	int nOffsetEnd = pToEntity->GetFieldDataBitEndOffset( nField );
#endif
	toStream.SeekToBit( nOffset );

	g_PropTypeFns[ pProp->m_Type ].Encode(
		nullptr, // WARNING: all properties of this kind must not require outer struct pointer for encoding, we don't have it here on the server!
		pVar,
		pProp,
		&toStream,
		objectID
	...
	Assert( toStream.m_iCurBit == nOffsetEnd ); // if the encoded prop is not the same size as it says in the table, we just corrupted the serialized entity
}



void SendTable_PatchUncompressibleProps( const SendTable *pTable, bf_read &fromStream, SerializedEntityHandle_t toEntity )
{
	CSerializedEntity *pToEntity = ( CSerializedEntity * )toEntity;
	if ( pTable->m_pExtension )
	{
		CUtlVectorFixedGrowable< int, 8 > fields;
		pTable->m_pExtension->GetUncompressibleProps( &fields );
		bf_write toStream( pToEntity->GetFieldData(), ( pToEntity->GetFieldDataBitCount() + 7 ) / 8, pToEntity->GetFieldDataBitCount() );
		for ( int nField : fields )
		{
			//SendProp *pProp = pTable->GetProp( nField );
			int nOffset = pToEntity->GetFieldDataBitOffset( nField );
			int nOffsetEnd = pToEntity->GetFieldDataBitEndOffset( nField );
			if ( fromStream.GetNumBitsLeft() < nOffsetEnd - nOffset )
			{
				ExecuteOnce( DevMsg( "SendTable_PatchUncompressibleProps failed\n" ) );
				return;
			}
			toStream.SeekToBit( nOffset );
			toStream.WriteBitsFromBuffer( &fromStream, nOffsetEnd - nOffset );
		}
	}
}


void SendTable_PatchUncompressibleProps( const SendTable *pTable, SerializedEntityHandle_t fromEntity, SerializedEntityHandle_t toEntity )
{
	CSerializedEntity *pToEntity = ( CSerializedEntity * )toEntity;
	Assert( !pToEntity->IsPacked() ); // need to check if packed works correctly, fromEntity isn't guaranteed to have all the needed fields
	if ( pToEntity->HasUncompressibleProps() && pTable->m_pExtension )
	{
		CUtlVectorFixedGrowable< int, 8 > tags;
		pTable->m_pExtension->GetUncompressibleProps( &tags );
		CSerializedEntity *pFromEntity = ( CSerializedEntity * )fromEntity;
		Assert( pFromEntity->GetFieldCount() == pToEntity->GetFieldCount() );
		Assert( !pFromEntity->IsPacked() ); // need to check if packed works correctly, just in case
		bf_read fromStream( pFromEntity->GetFieldData(), ( pFromEntity->GetFieldDataBitCount() + 7 ) / 8, pFromEntity->GetFieldDataBitCount() );
		bf_write toStream( pToEntity->GetFieldData(), ( pToEntity->GetFieldDataBitCount() + 7 ) / 8, pToEntity->GetFieldDataBitCount() );

		for ( int nField : tags )
		{
			uint nToBitOffset = pToEntity->GetFieldDataBitOffset( nField );
			uint nToBitOffsetEnd = pToEntity->GetFieldDataBitEndOffset( nField );
			uint nFromBitOffset = pFromEntity->GetFieldDataBitOffset( nField );
			uint nFromBitOffsetEnd = pFromEntity->GetFieldDataBitEndOffset( nField );
			if ( nToBitOffsetEnd - nToBitOffset == nFromBitOffsetEnd - nFromBitOffset )
			{
				fromStream.Seek( nFromBitOffset );
				toStream.SeekToBit( nToBitOffset );
				toStream.WriteBitsFromBuffer( &fromStream, nToBitOffsetEnd - nToBitOffset );
			}
		}
	}
}
...
SerializedEntityHandle_t SendTable_BuildDeltaProperties( edict_t *pEdict, int nObjectID, bool bHasUncompressibleProps, const SendTable* pTable, SerializedEntityHandle_t oldProps, CalcDeltaResultsList_t &deltaProps, CUtlMemory<CSendProxyRecipients> *pRecipients, bool& bCanReuseOldData )
{
	...
	if( deltaProps.Count() == 0 && !bHasUncompressibleProps )
	{
	...
	pNewProps->SetUncompressibleProps( bHasUncompressibleProps );
...
void SendTable_MergeTagsIntoDelta( const CUtlVectorFixedGrowable< int, 8 > &tags, CalcDeltaResultsList_t &deltaProps )
{
	// 99.9%  of cases both deltaProps is very short (just a few entries), and tags are empty
	if ( !tags.IsEmpty() )
	{
		if ( deltaProps.IsEmpty() )
		{
			deltaProps.SetCount( tags.Count() );
		...
		{
			int nTagIndex = tags.Count() - 1;
			for ( int nDeltaPropIndex = deltaProps.Count() - 1; nDeltaPropIndex >= 0; --nDeltaPropIndex )
			{
				while ( deltaProps[ nDeltaPropIndex ] < tags[ nTagIndex ] )
				{
					deltaProps.InsertAfter( nDeltaPropIndex, tags[ nTagIndex ] );
					if ( nTagIndex == 0 )
						return; // we're done inserting tags
					nTagIndex--;
				}
				if ( deltaProps[ nDeltaPropIndex ] == tags[ nTagIndex ] )
				{
					if ( nTagIndex == 0 )
						return; // we're done inserting tags
					nTagIndex--;
...
void SendTable_AddUncompressibleDeltaProps( const CSerializedEntity *pToEntity, const SendTable *pTable, CalcDeltaResultsList_t &deltaProps )
{
	if ( pToEntity->HasUncompressibleProps() && pTable->m_pExtension )
	{
		CUtlVectorFixedGrowable< int, 8 >tags;
		pTable->m_pExtension->GetUncompressibleProps( &tags );
		SendTable_MergeTagsIntoDelta( tags, deltaProps );
	}
}
	...
	CalcDeltaResultsList_t &deltaProps )
{
	...
	if( pFromEntity == NULL )
	{
		...
		SendTable_AddUncompressibleDeltaProps( pToEntity, pTable, deltaProps );
		return;
	}
...
static bool SendTable_InitTable( SendTable *pTable )
{
	...
	if ( pTable->m_pExtension )
	{
		SendTable_InitWellKnownProps( pTable );
		pTable->m_pExtension->Initialize( pTable, pPrecalc->m_Props.Base(), pPrecalc->m_Props.Count() );
	}
...
static void SendTable_TermTable( SendTable *pTable )
{
	...
	if ( pTable->m_pExtension )
	{
		pTable->m_pExtension->Terminate();
		pTable->m_pExtension = nullptr;
	}
...
// THe following list must be synchronized with SendTableWellKnownPropNamesEnum_t

struct WellKnownPropName_t
{
	const char *pName;
	int nSkipInstances;
};

static WellKnownPropName_t s_WellKnownPropNames[ SPWKN_COUNT ] = {
	{ "m_angRotation", 0},
	{ "m_vecOrigin", 0},
	{ "m_vecOrigin[2]", 0},
	{ "m_vecOrigin", 1},
...
void SendTable_InitWellKnownProps( SendTable * pTable )
{
	if ( !pTable->m_bWellKnownPropsInitialized )
	{
		for ( int nWellKnownNameIndex = 0; nWellKnownNameIndex < SPWKN_COUNT; ++nWellKnownNameIndex )
		{
			pTable->m_nWellKnownProps[ nWellKnownNameIndex ] = -1; // unknown by default
			int nSkipInstances = s_WellKnownPropNames[ nWellKnownNameIndex ].nSkipInstances;
			for ( int nField = 0; nField < pTable->m_pPrecalc->GetNumProps(); ++nField )
			{
				const SendProp *pSendProp = pTable->m_pPrecalc->GetProp( nField );
				const char *pSendPropName = pSendPropName = pSendProp->GetName();

				if ( !V_strcmp( pSendPropName, s_WellKnownPropNames[ nWellKnownNameIndex ].pName  ) )
				{
					if ( nSkipInstances-- == 0 )
					{
						pTable->m_nWellKnownProps[ nWellKnownNameIndex ] = nField;
						break;
					}
				}
			}
		}
		pTable->m_bWellKnownPropsInitialized = true;
	}
}

//ConVar net_patch_enable( "net_patch_enable", "1" );

int SendTable_GetWellKnownProp( SendTable *pTable, SendTableWellKnownPropNamesEnum_t nIndex )
{
	if ( !pTable->m_pPrecalc /*|| !net_patch_enable.GetBool()*/ )
		return -1;
	SendTable_InitWellKnownProps( pTable );
	return pTable->m_nWellKnownProps[ nIndex ];
}
