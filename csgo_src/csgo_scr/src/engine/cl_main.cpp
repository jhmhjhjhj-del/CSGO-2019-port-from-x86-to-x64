...
// Sergiy: network backdoor is not used in CSGO
...
static char cl_snapshotname[MIN(MAX_PATH/2, 64)];
static char cl_snapshot_subdirname[MIN(MAX_PATH/2, 64)];
...
void CL_OnCameramanOverride( IConVar *var, const char *pOldValue, float flOldValue );
ConVar cameraman_override( "cameraman_override", "0", FCVAR_RELEASE, "Set to 1 when you're secondary cameraman and need to override the primary cameraman's observer target and mode", CL_OnCameramanOverride );
...
{
	CL_SetupLocalNetworkBackDoor( false );
}
...
bool IsInteractiveCaster() //!TODO: For security purposes, disable all effects of -interactivecaster command line argument for unapproved clients (?)
{
	static bool s_bInteractiveCaster = CommandLine()->FindParm( "-interactivecaster" ) != 0;
	return s_bInteractiveCaster;
}
...
void CL_FullyConnected( void )
{
	...
	// Note that mdl cache will not immediately delete all studiodata associated with purged models 
	// but defer all studiodata to be deleted at the next call of CMDLCache::UnloadQueuedHardwareData().
	// At this point the modelloader thinks unused models have been purged but it didn't manage to delete
	// the associated materials, textures, ... (by calling materials->UncacheUnusedMaterials)
	// Calling UnloadQueuedHardwareData here to ensure all data associated with purge models are actually
	// removed from the mdl cache. We can then purge all unreferenced materials ...
...
//	COM_TimestampedLog( "    Model Loading time meshes studiohdr load %.4f", g_flLoadStudioHdr );
	...
	if ( IsInteractiveCaster() )
	{
		if ( engineClient->IsHLTV() ) // is this the correct check for whether client is in HLTV mode?
		{
			// cameraman in HLTV mode is always the secondary cameraman. They must be a valid caster, too, and can override the primary cameraman at will issuing cameraman_* commands to the server when cameraman_override convar changes
			if ( cameraman_override.GetBool() )
			{
				// secondary cameraman wants to override primary camera
				engineClient->ServerCmd( "cameraman_secondary_override_begin" ); //!TODO: what are the security ramifications of sending this command to the server (too frequently)?
			}
		}
		else
		{
			engineClient->ServerCmd( "cameraman_request" );
		}
	}
...
void CL_OnCameramanOverride( IConVar *var, const char *pOldValue, float flOldValue )
{
	if ( engineClient->IsHLTV() && IsInteractiveCaster() )
	{
		if ( cameraman_override.GetBool() )
		{
			engineClient->ServerCmd( "cameraman_secondary_override_begin" );
		}
		else
		{
			engineClient->ServerCmd( "cameraman_secondary_override_end" );
		}
	}
}
...
void CL_TakeScreenshot(const char *name)
{
	...
	if ( name != NULL && COM_IsValidPath( name ) )
	{
	...
	else
	{
		...
		if ( Q_strlen( cl_screenshotname.GetString() ) > 0 && COM_IsValidPath( cl_screenshotname.GetString() ) )
		{
...
CON_COMMAND_F( screenshot, "Take a screenshot.", FCVAR_CLIENTCMD_CAN_EXECUTE )
{
	...
	if( args.ArgC() == 2 && COM_IsValidPath( args[1] ) )
	{
...
void CL_StartMovie( const char *filename, int flags, int nWidth, int nHeight, float flFrameRate, int nJpegQuality )
{
...
CON_COMMAND_F( startmovie, "Start recording movie frames.", FCVAR_DONTRECORD )
{
	...
	CL_StartMovie( args[ 1 ], flags, videomode->GetModeWidth(), videomode->GetModeHeight(), flFrameRate, nJpegQuality );
...
void CL_SetSteamCrashComment()
{
	...
	ConVarRef game_type( "game_type" );
	ConVarRef game_mode( "game_mode" );
	...
	Q_snprintf( misc, sizeof( misc ), "skill:%i rate %i update %i cmd %i latency %i msec gt %i gm %i", 
		skill.GetInt(),
		cl_rate->GetInt(),
		(int)cl_updaterate->GetFloat(),
		(int)cl_cmdrate->GetFloat(),
		latency,
		game_type.GetInt(), game_mode.GetInt()
	);
...
void CL_InitLanguageCvar()
{
	...
	if ( IsPC() )
	{
		...
		else
		{
			...
			ISteamUtils *pUtils = Steam3Client().SteamUtils();
			if (pUtils)
			{
				const char *pSteamLanguage = pUtils->GetSteamUILanguage();
				if (pSteamLanguage && pSteamLanguage[0] && V_stricmp_fast(pSteamLanguage, "unknown"))
				{
					V_strncpy(language, pSteamLanguage, sizeof(language));
				}
			}

			ISteamApps *pApps = Steam3Client().SteamApps();
			if (pApps)
			{
				const char *pSteamLanguage = pApps->GetCurrentGameLanguage();
				if (pSteamLanguage && pSteamLanguage[0] && V_stricmp_fast(pSteamLanguage, "unknown"))
				{
					V_strncpy(language, pSteamLanguage, sizeof(language));
				}
