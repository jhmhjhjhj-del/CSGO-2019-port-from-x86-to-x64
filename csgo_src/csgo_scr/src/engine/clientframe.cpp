...
size_t CClientFrame::GetExtraMemSize()const
{
	size_t nSize = transmit_entity.GetNumBits() / 8;
	if ( from_baseline )
		nSize += from_baseline->GetNumBits() / 8;
	if ( transmit_always )
		nSize += transmit_always->GetNumBits() / 8;
	// Snapshot is shared, so must be accounted for elsewhere..
	nSize += m_pSnapshot->GetMemSize();
	return nSize;
}
...
void CClientFrameManager::DumpMem()
{
	uint nClientFrameSize = 0, nClientFrameCount = 0;
	for ( CClientFrame *p = m_Frames; p; p = p->m_pNext )
	{
		nClientFrameSize += p->GetMemSize();
		nClientFrameCount++;
	}
	Msg( " %4u client frames = %12s bytes\n", nClientFrameCount, V_pretifynum( nClientFrameSize ));
}
