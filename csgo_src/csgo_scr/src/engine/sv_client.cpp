...
static ConVar	sv_allowupload ("sv_allowupload", "0", FCVAR_RELEASE, "Allow clients to upload customizations files");
	   ConVar	sv_sendtables ( "sv_sendtables", "0", FCVAR_DEVELOPMENTONLY, "Force full sendtable sending path." );
#if HLTV_REPLAY_ENABLED
	   ConVar	spec_replay_enable( "spec_replay_enable", "0", FCVAR_RELEASE|FCVAR_REPLICATED, "Enable Killer Replay, requires hltv server running (0:off, 1:default, 2:force)" );
#endif
...
static ConVar spec_replay_on_death( "spec_replay_on_death", "0", FCVAR_RELEASE | FCVAR_REPLICATED, "When > 0, sets the mode whereas players see delayed replay, and are segregated into a domain of chat and voice separate from the alive players" );
...
bool CGameClient::CLCMsg_VoiceData( const CCLCMsg_VoiceData& msg )
{
	...
	SV_BroadcastVoiceData( this, msg, spec_replay_on_death.GetInt() && m_nHltvReplayDelay != 0 );
...
bool CGameClient::CLCMsg_HltvReplay( const CCLCMsg_HltvReplay &msg )
{
	...
	else if ( nRequest == REPLAY_EVENT_DEATH || nRequest == REPLAY_EVENT_GENERIC )
	{
	...
	else
	{
		if ( spec_replay_enable.GetInt() == 2 )
			return true;	// Clients cannot cancel replays in this mode
...
/*bool CGameClient::CLCMsg_HltvFixupOperatorReceiver( const CCLCMsg_HltvFixupOperatorReceiver &msg )
{
	sv.m_nFixupReceiverEntityId = m_nFixupReceiverEntityId = msg.receiver_entity_id();
	return true;
} */
...
{
	Assert( !IsHltvReplay() );
	m_EnginePVSContext.m_bOwnPortalOpen = false; // initially, context is not diverged
	m_PackInfo.m_pEnginePVSContext = &m_EnginePVSContext;
	// Compute Vis for each client
	m_PackInfo.m_nPVSSize = (GetCollisionBSPData()->numclusters + 7) / 8;
	serverGameClients->ClientSetupVisibility( &m_EnginePVSContext, (edict_t *)m_pViewEntity,
	...
	int areaCount = m_EnginePVSContext.m_AreasNetworked.Count();
	for ( int j = 0; j < areaCount; j++ )
	{
		...
		m_PackInfo.m_Areas[m_PackInfo.m_AreasNetworked] = m_EnginePVSContext.m_AreasNetworked[ j ];
	...
	if ( IsHltvReplay() )
	{
		...
		else
		{
			...
			if ( msg.GetType() == svc_UserMessage )
			{
				...
				case 52: // CS_UM_ServerRankUpdate
				case 65: // CS_UM_XpUpdate
				case 22: // CS_UM_RadioText
				case 74: // CS_UM_DisconnectToLobby2
...
static CUtlStringToken s_HltvPassThroughRealtimeEventsInPermanentMode[] = {
// Only a subset of events need to travel from realtime into the permanent replay viewers delayed time
	"teamplay_broadcast_audio",
	"player_chat",
	"player_say"
};
...
void CGameClient::HltvQueueGameEvent( IGameEvent *event )
{
...
void CGameClient::FireGameEvent( IGameEvent *event )
{
	if ( IsHltvReplay() )
	{
		...
		{
			//
			// We are in permanent replay mode, only allow realtime passthrough
			// all other events will arrive via delayed feed
			//
			if ( IsInList( eventName, s_HltvPassThroughRealtimeEventsInPermanentMode, ARRAYSIZE( s_HltvPassThroughRealtimeEventsInPermanentMode ) ) )
		...
		else
		{
			...
			else if ( IsInList( eventName, s_HltvUnskippableEvents, ARRAYSIZE( s_HltvUnskippableEvents ) ) )
			{
				...
				StopHltvReplay();
				// We are not in replay now
				// fall through to the real-time event firing case
			}
	...
	if ( !m_HltvQueuedMessages.IsEmpty() )
	{
		// We're not in replay, but we still have replay-queued events waiting to be sent
		// To preserve the event order, put them at the end of the same queue
		// They'll go out with the next SendSnapshot(), in order
		HltvQueueGameEvent( event );
		return;
	}
...
bool CGameClient::ExecuteStringCommand( const char *pCommandString )
{
	...
	if ( m_numClientCommandsInQuota > cmdQuota )
	{
		...
		// Listen-server host: never DOS-kick ourselves — drop the excess command instead.
		// (Offline tools like retake spawn editor can burst stringcmds; a sticky disconnect
		// popup on the host is far worse than ignoring a few cmds.)
#if !defined( DEDICATED )
		if ( !sv.IsDedicated() && m_nClientSlot == GetBaseLocalClient().m_nPlayerSlot )
		{
			ConMsg( "Ignoring stringcmd from listen host: over sv_quota_stringcmdspersecond (%d)\n", cmdQuota );
			return false;
		}
#endif
...
bool CGameClient::SendHltvReplaySnapshot( CClientFrame * pFrame )
{
	...
	CUtlBuffer  buffer( 0, NET_MAX_PAYLOAD ); // too large for stack
	bf_write	msg( "CGameClient::SendHltvReplaySnapshot", buffer.Base(), buffer.Size() );
	...
	while ( pLastFrame && pLastFrame->tick_count <= pFrame->tick_count )
	{
		...
		if ( pDeltaFrame )
		{
			...
			// we skip the voice messages here because we don't want to hear the 10-second-delayed voice, unless we're in segregated domain due to replay on death
	...
	m_pHltvReplayServer->m_StringTables->WriteUpdateMessageAtTick( NULL, pFrame->tick_count, GetMaxAckTickCount(), msg );
...
bool CGameClient::StartHltvReplay( const HltvReplayParams_t &params )
{
	...
	if ( hltv && !IsFakeClient() )
	{
		...
		if ( nOldestHltvTick > 0 ) // we should have some ticks in history to proceed successfully
		{
			...
			if ( spec_replay_enable.GetInt() == 2 )
				nNewReplayStopAt = 0; // replay in this mode never stops
			...
			if ( nNewReplayDelay != m_nHltvReplayDelay )
			{
				...
				msg.set_reason( params.m_nReason );
				...
				//	m_nStringTableAckTick = 0; // do not need to reset the stringtables, but will need to send the delta from future to past
...
EnginePVSContext_t * CGameClient::GetLastPVSContext()
{
	if ( IsActive() && !IsFakeClient() )
		return m_PackInfo.m_pEnginePVSContext; // only return the context if it was hooked up in PackInfo; otherwise it's garbage
	return NULL;
}
...
void CGameClient::StepHltvReplayStatus( int nServerTick )
{
	if ( IsHltvReplay() )
	{
		if ( m_nHltvReplayStopAt && ( m_nHltvLastSendTick >= m_nHltvReplayStopAt ) )
		{
		...
		{
			if ( !m_pCurrentFrame || ( m_nHltvReplayStopAt && ( m_pCurrentFrame->tick_count >= m_nHltvReplayStopAt ) ) )
			{
...
void CGameClient::StopHltvReplay( bool bNotifyClient )
{
	if ( IsHltvReplay() )
	{
		bNotifyClient = true;
...
void CGameClient::FileReceived( const char *fileName, unsigned int transferID, bool bIsReplayDemoFile /* = false */  )
{
	...
	for ( int i=0; i<MAX_CUSTOM_FILES; i++ )
	{
		if ( m_nCustomFiles[i].reqID == transferID )
		{
			...
			// broadcast update to other clients so they start downloading this file
			m_Server->UserInfoChanged( m_nClientSlot );
			return;
		}
	}

	DevMsg( "CGameClient::FileReceived: %s not wanted.\n", fileName );
}
...
{
	if ( m_nFixupReceiverEntityId >= 0 && sv.m_nFixupReceiverEntityId == m_nFixupReceiverEntityId )
		sv.m_nFixupReceiverEntityId = -1; // unsubscribe from fixup operator stream
