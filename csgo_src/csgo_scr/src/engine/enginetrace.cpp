...
private:
	...
	CInterlockedInt m_nOcclusionTestsSuspended; // TODO: get rid of async query, use sv_parallel_send instead
...
CPhysCollide* CEngineTrace::GetCollidableFromDisplacementsInAABB( const Vector& vMins, const Vector& vMaxs )
{
	...
	for ( int nLeaf = 0; nLeaf < iLeafCount; ++nLeaf )
	{
		// Current leaf
		cleaf_t curLeaf = pBSPData->map_leafs[ pLeafList[ nLeaf ] ];
...
bool CEngineTrace::ClipRayToVPhysics( const Ray_t &ray, unsigned int fMask, ICollideable *pEntity, studiohdr_t *pStudioHdr, trace_t *pTrace )
{
	...
	if ( pStudioHdr )
	{
		...
		vcollide_t *pCollide = g_pMDLCache->GetVCollide( pModel->studio, pEntity->GetCollisionScale() );
	...
	else
	{
		...
		if ( !ray.m_IsRay || pModel->type != mod_brush )
		{
			...
			vcollide_t *pCollide = CM_VCollideForModel( nModelIndex, (model_t*)pModel, pEntity->GetCollisionScale() );
...
{
	Assert( m_nOcclusionTestsSuspended > 0 );
...
extern ConVar sv_parallel_send;
...
bool CEngineTrace::IsFullyOccluded( int nOcclusionKey, const AABB_t &aabb0, const AABB_t &aabb1, const Vector &vShadow )
{
	...
	if ( sv_parallel_send.GetBool() || !occlusion_test_async.GetInt() || nOcclusionKey < 0 )
