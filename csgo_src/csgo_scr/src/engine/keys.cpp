...
const char *GetSuggestedBinding( const char *command, KeyValues *defaults )
{
	...
	if ( const char *suggestedKeyString = defaults->GetString( command, NULL ) )
	{
...
//	bool bDown = ( event.m_nType == IE_ButtonPressed ) || ( event.m_nType == IE_ButtonDoubleClicked );
	...
	// The sweet vgui UI for Server Browser born in 1995 is going to run fullscreen in 2018 release of CS:GO Panorama and occlude all input and hover events
	static ConVarRef cv_server_browser_dialog_open( "server_browser_dialog_open" );
	if ( cv_server_browser_dialog_open.GetBool() )
	{
		return false;
	}
	
	// If a keybindings control is waiting for input, let it process input first
	if ( g_ClientDLL->HandleBindWidgetInputCapture( event ) )
	{
		return true;
	}
...
#if defined( PANORAMA_ENABLE ) && defined( PLATFORM_WINDOWS )
	if ( PanoramaEngineHandler().IsDebuggerShown() )
		return false;
#endif
...
void Key_Event( const InputEvent_t &event )
{
	...
	bool bScaleform = ( CommandLine()->CheckParm( "-scaleform" ) != 0 );
#ifdef PANORAMA_ENABLE
	bool bPanorama = !bScaleform;
#endif
	...
	// The hardcoded KEY_BACKQUOTE here ensures that backquote will always close the console,
	// regardless of which key is actually bound to toggleconsole. 
	...
	// Panorama key handling
#ifdef PANORAMA_ENABLE
	if( bPanorama && !IsESC( event ))
	{
		// Special handling for the +voicerecord key requires us to eavesdrop on the whole input system early
		// before it gets filtered in various input layers and systems
		switch ( event.m_nType )
		{
		case IE_ButtonPressed:
		case IE_ButtonReleased:
		...
		case IE_KeyCodeReleased:
			...
			{
				bool bDown = ( event.m_nType != IE_ButtonReleased );
				const char *szKeyBinding = s_KeyContext.m_pKeyInfo[ code ].m_pKeyBinding;
				if ( g_ClientDLL )
					g_ClientDLL->IN_PreKeyEvent( bDown ? 1 : 0, code, szKeyBinding );
			}
		}

		// Do the panorama key filtering
	...
	if ( IsESC( event ) )
	{
		...
		if ( bPanorama )
		{
			if ( bAllowScaleformFilter && FilterKey( event, KEY_UP_PANORAMA, PanoramaHandleInputEvent ) )
				return;
		}
#endif
		if ( bScaleform )
