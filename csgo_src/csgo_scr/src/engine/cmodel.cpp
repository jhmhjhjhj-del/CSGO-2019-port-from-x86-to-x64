...
static void FloodAreaConnections (CCollisionBSPData *pBSPData);
...
{
	BitVec_ClearAll( pBSPData->isPortalOpen.Base(), pBSPData->GetPortalOpenWordCount() );
}
...
//-----------------------------------------------------------------------------
vcollide_t* CM_VCollideForModel( int modelindex, const model_t* pModel, float flScale /* = 1.0f */ )
{
	switch( pModel->type )
	{
	case mod_brush:
	...
	case mod_studio:
		...
		return g_pMDLCache->GetVCollide( pModel->studio, flScale );
	}
		...
		if ( IsAllZeros(separation) )
		{
			...
			else
			{
				...
				if ( t1 <= 1.0f )
				{
					...
					pTrace->contents = CONTENTS_SOLID;
					Vector vStart;
					VectorAdd( ray.m_Start, ray.m_StartOffset, vStart );

					if (pTrace->fraction == 1)
					{
						VectorAdd( vStart, ray.m_Delta, pTrace->endpos);
					}
					else
					{
						VectorMA( vStart, pTrace->fraction, ray.m_Delta, pTrace->endpos );
					}
					return true;
				}
			}
...
*/
	...
	if ( oi.m_pResults )
	{
		// this isn't called in occlusion_test_run
...
class CMicroProfilerGuardTSRec: public CMicroProfilerSample
{
	CMicroProfiler *m_pProfiler;
	int *m_pRecursion;
public:
	CMicroProfilerGuardTSRec( CMicroProfiler *mp, int *pRecursion ) :m_pProfiler( mp ), m_pRecursion( pRecursion ) { ++*m_pRecursion; }
	~CMicroProfilerGuardTSRec()
	{
		if ( !--*m_pRecursion )
...
struct RecursiveOcclusionPassParams_t
{
	int m_nNode;
	float m_p1f;
	float m_p2f;
	...
	void Init( int num, const float p1f, const float p2f, const Vector& p1, const Vector& p2 )
	{
		m_nNode = num;
		m_p1f = p1f;
		m_p2f = p2f;
...
// the recursion is ~17 deep in de_overpass
typedef CUtlVectorFixedGrowable< RecursiveOcclusionPassParams_t, 32 > RecursiveOcclusionPassParamsArray_t;
...
ConVar occlusion_old("occlusion_old", "0", FCVAR_RELEASE);

bool FASTCALL CM_RecursiveOcclusionPass( COcclusionInfo &oi, const Vector& vStart, const Vector& vEnd )
{
	if( occlusion_old.GetBool() )
		return CM_RecursiveOcclusionPass( oi, 0, 0, 1, vStart, vEnd );

	RecursiveOcclusionPassParamsArray_t arr;
	arr.AddToTailGetPtr()->Init( 0, 0.0f, 1.0f, vStart, vEnd );

	CMicroProfiler mpMisc[ 3 ];
	...
	while ( !arr.IsEmpty() )
	{
		...
		RecursiveOcclusionPassParams_t& params = arr.Tail();
		int num = params.m_nNode;
		float p1f = params.m_p1f, p2f = params.m_p2f;
		// find the point distances to the separating plane
		// and the offset for the size of the box

		{
			//CMicroProfilerGuardTS grd( &mpMisc[ 0 ] );
			...
			while ( num >= 0 )
			{
				...
				if ( type < 3 )
				{
					t1 = params.m_p1[ type ] - dist;
					t2 = params.m_p2[ type ] - dist;
					offset = oi.m_extents[ type ];
				}
				else
				{
					t1 = DotProduct( plane->normal, params.m_p1 ) - dist;
					t2 = DotProduct( plane->normal, params.m_p2 ) - dist;
					offset = fabsf( oi.m_extents[ 0 ] * plane->normal[ 0 ] ) +
						fabsf( oi.m_extents[ 1 ] * plane->normal[ 1 ] ) +
						fabsf( oi.m_extents[ 2 ] * plane->normal[ 2 ] );
				}
		...
		if ( num < 0 )
		{
			//CMicroProfilerGuardTS grd( &mpMisc[ 1 ] );
			if ( CM_LeafOcclusionPass( oi, -1 - num, p1f, p2f ) )
			{
				return true;
			}
			arr.RemoveMultipleFromTail( 1 );
		}
		else
		{
			//CMicroProfilerGuardTS grd( &mpMisc[ 2 ] );
			...
			if ( t1 < t2 )
			{
				idist = 1.0 / ( t1 - t2 );
				side = 1;
				frac2 = clamp( ( t1 + offset + DIST_EPSILON )*idist, 0, 1 );
				frac = clamp( ( t1 - offset - DIST_EPSILON )*idist, 0, 1 );
			}
			else if ( t1 > t2 )
			{
				idist = 1.0 / ( t1 - t2 );
			...
			// this is the one to be done after next: go past the node
			float mid2f = p1f + ( p2f - p1f ) * frac2;
			Vector p1 = params.m_p1, p2 = params.m_p2;
			Vector mid2 = VectorLerp( p1, p2, frac2 ), mid = VectorLerp( p1, p2, frac );

			params.Init( node->children[ side ^ 1 ], mid2f, p2f, mid2, p2 );

			// this is the one that will be done next
			arr.AddToTailGetPtr()->Init( node->children[ side ], p1f, midf, p1, mid );
		}
	}
	return false;// didn't find full occlusion yet
}
...
// profile exclusive of CM_RecursiveOcclusionPass: 45ns single-threaded, 52ns multi-threaded, 78ns hyper-threaded (WoW64, turboboost off)
...
#endif
	return CM_RecursiveOcclusionPass( oi, p0, p1 );
...
class COcclusionTestShared
{
protected:
	int64 m_nRecOccluded;
	CInterlockedInt m_nRecFalseOcclusion , m_nRecFalseNonOcclusion ;
	int64 m_nBestCase , m_nWorstCase , m_nTotalTime ;
	int m_nBestIndex , m_nWorstIndex ;
public:
	int m_nRecCount;
	int m_nNumRuns;
	int64 m_nRealizedRuns;
	uint64 m_nStopAtUsTime;
	int m_nMaxProbes;
	OcclusionTestRec_t *m_pRecs;
	bool m_bColdCache;
	int m_nDoubleCheck;
	int64 m_nCpuSpeed;
	int m_nThreads;
	uint32 m_nCacheSizeKb;
	CThreadFastMutex m_Mutex;
	CUtlVectorFixedGrowable< int, 16 > m_ThreadIds;
	float m_flRandoms[ 64 ];
public:
	COcclusionTestShared()
	{
		m_nRecOccluded = 0;
		m_nThreads = 1;
		m_nBestCase = m_nWorstCase = m_nTotalTime = 0;
		m_bColdCache = false;
		m_nDoubleCheck = 0;
		const CPUInformation &cpuInfo = GetCPUInformation();
		m_nCpuSpeed = cpuInfo.m_Speed;
		m_nBestCase = m_nCpuSpeed, m_nWorstCase = 0, m_nTotalTime = 0;
		m_nBestIndex = -1, m_nWorstIndex = -1;
		m_nCacheSizeKb = Max( Max( cpuInfo.m_nL1CacheSizeKb, cpuInfo.m_nL2CacheSizeKb ), cpuInfo.m_nL3CacheSizeKb );
		m_nNumRuns = 1;
		m_nStopAtUsTime = 0;
		m_nRealizedRuns = 0;
		m_nMaxProbes = INT_MAX;
		for ( int i = 0; i < ARRAYSIZE( m_flRandoms ); ++i )
			m_flRandoms[ i ] = RandomFloat( -1, 1 );
	}

	bool Probe( int i )
	{
		return CM_IsFullyOccluded( m_pRecs[ i ].p0, m_pRecs[ i ].vExtents0, m_pRecs[ i ].p1, m_pRecs[ i ].vExtents1, nullptr );
/*
		struct Param_t
		{
			fltx4 d0[ 2 ];
			fltx4 d1[ 2 ];
			fltx4 f4EnterNum[ 2 ];
			fltx4 f4EnterDenum[ 2 ];
			fltx4 f4LeaveNum[ 2 ];
			fltx4 f4LeaveDenum[ 2 ];
		};
		Param_t p;
		for ( int j = 0; j < sizeof( p ) / sizeof( float ); ++j )
			( ( float* ) &p )[ j ] = m_flRandoms[ ( j + i ) % ARRAYSIZE( m_flRandoms ) ];


		return IntersectOcclusionInterval( p.d0, p.d1, p.f4EnterNum, p.f4EnterDenum, p.f4LeaveNum, p.f4LeaveDenum ) == 0;
*/
	}
	...
	void Execute()
	{
		...
		if ( m_bColdCache )
			flushCache.SetCount( m_nCacheSizeKb * 1024 );
		int64 nRecOccluded = 0;
		int64 nWorstCase = 0, nBestCase = m_nCpuSpeed, nTotalTime = 0;
		int nWorstIndex = -1, nBestIndex = -1;
		int64 nRealizedRuns = 0;
		for ( ;; )
		{
			for ( int i = 0; i < m_nRecCount; ++i )
			{
				...
				if ( m_nDoubleCheck )
				{
					if ( bOccluded != DebugCheckOcclusion( m_pRecs[ i ].p0, m_pRecs[ i ].vExtents0, m_pRecs[ i ].p1, m_pRecs[ i ].vExtents1, m_nDoubleCheck ) )
					{
						if ( bOccluded )
						{
							++m_nRecFalseOcclusion;
							Warning( "FALSE OCCLUSION FOUND: #%d\n", i );
						}
						else
							++m_nRecFalseNonOcclusion;
					}
				}
			}
			...
			if ( m_nStopAtUsTime )
			{
				if ( Plat_USTime() >= m_nStopAtUsTime )
					break;
			}
			else
			{
				if ( nRealizedRuns >= m_nNumRuns )
					break;
			}
		}

		int nCurrentThreadId = ThreadGetCurrentId();
		AUTO_LOCK_FM( m_Mutex );
		m_nRealizedRuns += nRealizedRuns;
		if ( m_nWorstCase < nWorstCase )
		{
			m_nWorstCase = nWorstCase;
			m_nWorstIndex = nWorstIndex;
		}
		if ( m_nBestCase > nBestCase )
		{
			m_nBestCase = nBestCase;
			m_nBestIndex = nBestIndex;
		}
		m_nRecOccluded += nRecOccluded;
		m_nTotalTime += nTotalTime;
		m_ThreadIds.AddToTail( nCurrentThreadId );
	}

	void PrintResults()
	{
		int64 nTotalProbes = m_nRecCount * m_nRealizedRuns;
		double flMicrosecondsPerTick = 1e6 / m_nCpuSpeed;
		Msg( "%s/%s (%.1f%%) occluded, occlusion tests run %.3f us ave ([%d]=%.3fus, [%d]=%.1fus) on a %.2fGHz CPU\n",
			V_pretifynum( m_nRecOccluded ), V_pretifynum( nTotalProbes ), ( m_nRecOccluded * 100.0 ) / nTotalProbes,
			( m_nTotalTime * flMicrosecondsPerTick ) / nTotalProbes,
			m_nBestIndex, m_nBestCase * flMicrosecondsPerTick,
			m_nWorstIndex, m_nWorstCase * flMicrosecondsPerTick,
			m_nCpuSpeed * 1e-9
		);
		m_ThreadIds .Sort();
		RemoveDuplicates( m_ThreadIds );
		Msg( "in %d threads, %s runs, %s probes\n", m_ThreadIds.Count(), V_pretifynum( m_nRealizedRuns ), V_pretifynum( nTotalProbes ) );
		if ( m_nDoubleCheck )
		{
			Msg( "%d false non-occlusions\n", (int)m_nRecFalseNonOcclusion );
		}
/*
		for ( int mp = 0; mp < ARRAYSIZE( s_mpMisc ); ++mp )
		{
			if ( s_mpMisc[ mp ].m_numTimeBaseTicks )
				Msg( "[%d]\t%s ticks %.1f%%/probe, %s calls\n", mp, V_pretifynum( s_mpMisc[ mp ].m_numTimeBaseTicks / nTotalProbes ), ( 1000 * s_mpMisc[ mp ].m_numTimeBaseTicks / m_nTotalTime ) * .1, V_pretifynum( s_mpMisc[ mp ].m_numCalls ) );
		}
*/
	}
};
...
class COcclusionTestJob : public CJob
{
	COcclusionTestShared *m_pShared;
public:
	COcclusionTestJob( COcclusionTestShared *pShared)
	{
		m_pShared = pShared;
	}
...
inline bool BitVec_SetOnce( uint32 *pBase, int nBitNum )
{
	uint32 *pInt = pBase + BitVec_Int( nBitNum );
	uint32 nBit = BitVec_Bit( nBitNum );
	if ( *pInt & nBit )
		return false; // the bit is already set
	*pInt |= nBit;
	return true;
}


// this function used to be recursive, but threading networking added a lot of parameters, so recursion became inefficient
void FloodAreas (CCollisionBSPData *pBSPData, uint32 *pIsPortalOpen, uint16 *pFloodIndices )
{
	// if an area has been flooded, the bit will be set
	int nFloodBitWords = ( pBSPData->numareas + 31 ) / 32;
	uint32 *pFlooded = StackAlloc( uint32, nFloodBitWords );
	V_memset( pFlooded, 0, nFloodBitWords * sizeof( uint32 ) );	

	uint16 * pAreaStack = StackAlloc( uint16, pBSPData->numareas );
	// area 0 is not used
	for ( int nRootArea = 1; nRootArea < pBSPData->numareas; nRootArea++ )
	{
		if ( !BitVec_SetOnce( pFlooded, nRootArea ) )
			continue; // already flooded

		int nAreaStackSize = 0;
		int nArea = nRootArea;
		for ( ;; )
		{
			// not yet flooded into this area
			pFloodIndices[ nArea ] = nRootArea;
			AssertDbg( BitVec_IsBitSet( pFlooded, nArea ) );
			darea_t *area = &pBSPData->map_areas[ nArea ];
			dareaportal_t	*p = &pBSPData->map_areaportals[ area->firstareaportal ];
			for ( int i = 0; i < area->numareaportals; i++, p++ )
			{
				if ( BitVec_IsBitSet( pIsPortalOpen, p->m_PortalKey ) && BitVec_SetOnce( pFlooded, p->otherarea ) )
				{
					Assert( nAreaStackSize + 1 < pBSPData->numareas );
					// the portal is open - flood through the portal
					pAreaStack[ nAreaStackSize++ ] = p->otherarea;
				}
			}
			if ( nAreaStackSize <= 0 )
				break;
			nArea = pAreaStack[ --nAreaStackSize ];
		}
	}
}
...
{
 	// flooded.ClearAll();<-implicit  ClearAll() in constructor
	FloodAreas( pBSPData, pBSPData->isPortalOpen.Base(), pBSPData->mapAreaFloodIndex.Base() );
}


void	FloodAreaConnections( EnginePVSContext_t *pEnginePVSContext, CCollisionBSPData *pBSPData )
{
	// flooded.ClearAll();<-implicit  ClearAll() in constructor
	Assert( pEnginePVSContext->m_bOwnPortalOpen );
	FloodAreas( pBSPData, pEnginePVSContext->GetPortalsOpenBits(), pEnginePVSContext->m_pAreaFloodIndices );
}


int SetPortalOpen( uint32 *pIsPortalOpen, int nPortalIndex, bool isOpen )
{
	uint32 *pInt = pIsPortalOpen + BitVec_Int( nPortalIndex );
	uint32 nBit = BitVec_Bit( nPortalIndex );
...
inline bool IsPortalOpen( const uint32 *pIsPortalOpen, int nPortalIndex )
{
	return BitVec_IsBitSet( pIsPortalOpen, nPortalIndex );
}


// global only - portals open(closed) through this function will be open(closed) for every client
void CM_SetAreaPortalState( int portalnum, bool isOpen )
{
	// get the current collision bsp -- there is only one!
	CCollisionBSPData *pBSPData = GetCollisionBSPData();
	...
	if ( SetPortalOpen( pBSPData->isPortalOpen.Base(), portalnum, isOpen ) )
	{
		FloodAreaConnections( pBSPData ); // reflood because we changed a portal state
	}
}



void OwnPortalOpen( EnginePVSContext_t *pEnginePVSContext )
{
	CCollisionBSPData *pBSPData = GetCollisionBSPData();
	int nPortalOpenWordCount = pBSPData->GetPortalOpenWordCount();
	if ( false )
	{
		// this will just redirect to the global state, and will not work in multithreaded case. Useful for debugging.
		pEnginePVSContext->m_PortalStateMemory.SetExternalBuffer( pBSPData->isPortalOpen.Base(), nPortalOpenWordCount );
		pEnginePVSContext->m_pAreaFloodIndices = pBSPData->mapAreaFloodIndex.Base();
	}
	else
	{
		pEnginePVSContext->m_PortalStateMemory.EnsureCapacity( sizeof( uint32 ) * nPortalOpenWordCount + sizeof( uint16 ) * pBSPData->numareas );
		V_memcpy( pEnginePVSContext->GetPortalsOpenBits(), pBSPData->isPortalOpen.Base(), sizeof( uint32 ) * nPortalOpenWordCount );
		pEnginePVSContext->m_pAreaFloodIndices = ( uint16 * )( pEnginePVSContext->GetPortalsOpenBits() + nPortalOpenWordCount );
	}
	// do we need to initialize the flood indices? They'll be overwritten right after this
	pEnginePVSContext->m_bOwnPortalOpen = true;
}

// per-client portal changes
void CM_SetAreaPortalStates( EnginePVSContext_t *pEnginePVSContext, const int *portalnums, const bool *isOpen, int nPortalCount )
{
	if ( nPortalCount == 0 )
		return;
	...
	int nIdx = 0;
	int nPortalChanged = 0;
	if ( !pEnginePVSContext->m_bOwnPortalOpen )
	{
		// see if we need to diverge
		for ( ; ; )
		{
			int nPortal = portalnums[ nIdx ];
			AssertDbg( nPortal < pBSPData->numareaportals );
			if ( isOpen[ nIdx ] != IsPortalOpen( pBSPData->isPortalOpen.Base(), nPortal ) )
			{
				// even if reflood is not needed, we still need to diverge
				OwnPortalOpen( pEnginePVSContext );
				nPortalChanged = 1; // make sure to mark dirty so that flood indices get regenerated
				break;
			}
			if ( ++nIdx >= nPortalCount )
				return; // scanned all portal states, nothing changed
		}
	}

	// continue scanning portals
	for ( ; nIdx < nPortalCount; ++nIdx )
	{
		nPortalChanged |= SetPortalOpen( pEnginePVSContext->GetPortalsOpenBits(), portalnums[ nIdx ], isOpen[ nIdx ] );
	}

	if ( nPortalChanged )
	{
		FloodAreaConnections( pEnginePVSContext, pBSPData );// reflood because we changed a portal state
	}
}

inline const uint16 *GetFloodIndices( EnginePVSContext_t *pContext, CCollisionBSPData *pBSPData )
{
	if ( pContext->m_bOwnPortalOpen )
		return pContext->m_pAreaFloodIndices;
	else
		return pBSPData->mapAreaFloodIndex.Base();
}


bool CM_CheckAreasConnected( EnginePVSContext_t *pContext, int area1, int area2 )
{
	// get the current collision bsp -- there is only one!
	CCollisionBSPData *pBSPData = GetCollisionBSPData();
	...
	const uint16 * pFloodIndices = GetFloodIndices( pContext, pBSPData );
	return ( pFloodIndices[ area1 ] == pFloodIndices[ area2 ] );
}
...
void CM_LeavesConnected( const Vector &vecOrigin, int nCount, const int *pLeaves, bool *pIsConnected )
{
	...
	const uint16 * pFloodIndices = pBSPData->mapAreaFloodIndex.Base();
	for ( int i = 0; i < nCount; ++i )
	{
		int nLeafArea = pBSPData->map_leafs[ pLeaves[i] ].area;
		Assert( nLeafArea < pBSPData->numareas && nLeafArea >= 0 );
		pIsConnected[i] = ( pFloodIndices[nArea] == pFloodIndices[ nLeafArea ] );
	}
}
...
//-----------------------------------------------------------------------------
int CM_WriteAreaBits( EnginePVSContext_t *pEnginePVSContext, byte *buffer, int buflen, int area )
{
	if ( buflen < 32 )
	{
	...
	CCollisionBSPData *pBSPData = GetCollisionBSPData();
	Assert( pBSPData );
	int bytes = ( pBSPData->numareas + 7 ) >> 3;
	Assert( buflen >= bytes );
	...
	if ( map_noareas.GetInt() )
	{
		...
		V_memset( buffer, fill, buflen );
	}
	else
	{
		V_memset( buffer, 0x00, buflen );

		const uint16 * pFloodIndices = GetFloodIndices( pEnginePVSContext, pBSPData );
		int floodnum = pFloodIndices[ area ];
		for ( int i = 0 ; i < pBSPData->numareas; ++i )
		{
			if ( pFloodIndices[ i ] == floodnum || !area )
			{
				buffer[ i >> 3 ] |= ( 1 << ( i & 7 ) );
			}
		}
...
bool CM_GetAreaPortalPlane( const Vector &vViewOrigin, int portalKey, VPlane *pPlane )
{
	...
	darea_t *pArea = &pBSPData->map_areas[iArea];
