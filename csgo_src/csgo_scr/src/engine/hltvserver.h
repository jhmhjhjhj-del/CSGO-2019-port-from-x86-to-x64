...
public:
	...
	float GetAdditionalTvDelay()const;
	float GetAdditionalTvDelay1()const;
protected:
...
private:
	...
	struct SHLTVDeltaFrame_t
	{
		...
		//a bit list that contains all of the entities that should just be copied forward [m_nTotalEntities]
...
public:
	...
	CThreadFastMutex				m_FrameMutex; // locks frame cache and m_pOldestDeltaFrame list
