...
void CDemoRecorder::WriteSplitScreenPlayers()
{
	...
	bf_write	msg( "DemoFileWriteSplitScreenPlayers", buffer.Base(), buffer.Size() );
...
void CDemoRecorder::WriteBSPDecals()
{
	...
	CUtlBuffer  buffer( 0, NET_MAX_PAYLOAD ); // too large for stack
	bf_write	msg( "DemoFileWriteBSPDecals", buffer.Base(), buffer.Size() );
...
void CDemoRecorder::RecordStringTables()
{
	...
	networkStringTableContainerClient->WriteStringTables( buf, m_nStartTick );
...
void CDemoRecorder::WriteDemoCvars()
{
	...
	for ( iter.SetFirst() ; iter.IsValid() ; iter.Next() )
	{
		...
		const ConVar *varAsCvar = ( const ConVar * )var;

		if ( !varAsCvar->IsFlagSet( FCVAR_DEMO ) )
			continue;

		char cvarcmd[MAX_OSPATH];

		V_sprintf_safe( cvarcmd,"%s \"%s\"", varAsCvar->GetName(), Host_CleanupConVarStringValue( varAsCvar->GetString() ) );
...
void CDemoRecorder::StartupDemoFile( void )
{
	...
	FOR_EACH_MAP_FAST( GetBaseLocalClient().m_mapPlayerAvatarData, iData )
	{
		...
		bf_write	bfWrite( "CDemoRecorder::NETMsg_PlayerAvatarData", buffer.Base(), buffer.Size() );
...
void CDemoPlayer::SkipToImportantTick( const DemoImportantTick_t *pTick )
{
	...
	{
		nTargetTick = pTick->nTick - nTicksBeforeEvent;
...
CDemoPlayer::CDemoPlayer()
{
	...
	m_bScanMode = false;
	m_szScanMode[0] = 0;
	m_hStatsFile = FILESYSTEM_INVALID_HANDLE;
}
...
bool CDemoPlayer::StartPlayback( const char *filename, bool bAsTimeDemo, CDemoPlaybackParameters_t const *pPlaybackParameters, int nStartingTick )
{
	...
	// Reset server-authoritative convars for playback
...
void CDemoPlayer::SetStatsFileName( const char *pFileName )
{
	if ( pFileName )
	{
		V_strcpy_safe( m_statsFileName, pFileName );
	}
	else
	{
		V_strcpy_safe( m_statsFileName, "UNKNOWN" );
	}
}

bool CDemoPlayer::OpenStatsFile()
{
	CloseStatsFile();

	if ( V_isempty( m_statsFileName ) )
	{
		return false;
	}

	m_hStatsFile = g_pFileSystem->Open( m_statsFileName, "wt" );
	if ( m_hStatsFile == FILESYSTEM_INVALID_HANDLE )
	{
		return false;
	}

	ConMsg( "Opened demo frame stats file '%s'\n", m_statsFileName );

	g_pFileSystem->FPrintf( m_hStatsFile, "demo tick,frame start time,frame start delta,frame swap time,frame swap delta,FPS variability\n\n" );

	return true;
}

void CDemoPlayer::CloseStatsFile()
{
	if ( m_hStatsFile != FILESYSTEM_INVALID_HANDLE )
	{
		ConMsg( "Closed demo frame stats file '%s'\n", m_statsFileName );

		g_pFileSystem->Close( m_hStatsFile );
		m_hStatsFile = FILESYSTEM_INVALID_HANDLE;
	}
}
...
// Input  : flCurTime - 
//-----------------------------------------------------------------------------
void CDemoPlayer::MarkFrame( double flFrameStartTime, double flFrameStartDelta, double flFrameSwapTime, double flFrameSwapDelta, float flFPSVariability )
{
	m_flTotalFPSVariability += flFPSVariability;

	if ( m_hStatsFile != FILESYSTEM_INVALID_HANDLE )
	{
		g_pFileSystem->FPrintf( m_hStatsFile, "%d,%f,%f,%f,%f,%f\n",
								GetPlaybackTick(),
								flFrameStartTime,
								flFrameStartDelta,
								flFrameSwapTime,
...
void CDemoPlayer::WriteTimeDemoResults( void )
{
	...
	g_pFileSystem->FPrintf( fileHandle, "%s,", m_statsFileName );
...
void CDemoPlayer::ScanForImportantTicks()
{
	...
	while ( !demofinished )
	{
			...
			case dem_stringtables:
				{
					...
					bf_read buf2( "dem_stringtables", data, DEMO_RECORD_BUFFER_SIZE );
					m_DemoFile.ReadStringTables( &buf2 );
					buf2.Seek( 0 );
								
					if ( !demoScanStringTables.ReadStringTables( buf2 ) )
					{
		...
		int length = m_DemoFile.ReadRawData( static_cast< char* >( buffer.Base() ), buffer.Size() );

		if ( length > 0 )
		{
			bf_read message;
			message.StartReading( buffer.Base(),  length );
			...
			while ( true )
			{
				...
				unsigned char cmd2 = message.ReadVarInt32();
				INetMessageBinder *pMsgBind = pNetChannel->FindMessageBinder( cmd2, 0 );
				if ( pMsgBind )
				{
...
{
	g_pClientDemoPlayer->SetStatsFileName( pStatsFileName );
...
void CL_TimeDemo_f( const CCommand &args )
{
	if ( args.ArgC() < 2 || args.ArgC() > 3 )
	{
		ConMsg ("timedemo <demoname> <optional stats.txt> : gets demo speeds, writing perf results to the optional stats.txt\n");
		return;
	}
