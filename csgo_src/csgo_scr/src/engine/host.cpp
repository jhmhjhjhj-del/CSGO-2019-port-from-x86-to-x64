...
#if !defined( DEDICATED )
static bool HostOfflineTraceEnabled()
{
	static int s_nEnabled = -1;
	if ( s_nEnabled < 0 )
	{
		s_nEnabled = ( CommandLine()->FindParm( "-offline_trace" )
			|| CommandLine()->FindParm( "-offline_ui_trace" ) ) ? 1 : 0;
	}
	return s_nEnabled != 0;
}

static void HostOfflineTrace( PRINTF_FORMAT_STRING const char *pszFmt, ... )
{
	if ( !HostOfflineTraceEnabled() )
		return;
	...
	char szPath[512];
	if ( g_pFileSystem->GetSearchPath( "DEFAULT_WRITE_PATH", false, szPath, sizeof( szPath ) ) )
	{
		V_strcat_safe( szPath, "offline_ui_trace.log" );
	}
	else
	{
		Q_strncpy( szPath, "csgo/offline_ui_trace.log", sizeof( szPath ) );
	}

	FileHandle_t fh = g_pFileSystem->Open( szPath, "ab", NULL );
	if ( fh == FILESYSTEM_INVALID_HANDLE )
	{
		fh = g_pFileSystem->Open( "csgo/offline_ui_trace.log", "ab", "MOD" );
	}
	if ( fh == FILESYSTEM_INVALID_HANDLE )
		return;
	...
	V_vsnprintf( szLine, sizeof( szLine ), pszFmt, args );
	va_end( args );
	V_strcat( szLine, "\r\n", sizeof( szLine ) );
	g_pFileSystem->Write( szLine, V_strlen( szLine ), fh );
...
void Host_ReadConfiguration( const int iController, const bool readDefault )
{
	...
	HostOfflineTrace( "Host_ReadConfiguration begin controller=%d", iController );
#endif
	if ( sv.IsDedicated() )
		return;
	...
	Cbuf_Execute();
#if defined( NO_STEAM ) && !defined( DEDICATED )
	HostOfflineTrace( "Host_ReadConfiguration after Cbuf_Execute" );
#endif
...
#if !defined(DEDICATED)
	g_pClientDemoPlayer->MarkFrame( g_HostTimes.framestarttime, g_HostTimes.framestarttimeduration, g_HostTimes.swaptime, g_HostTimes.frametime, g_HostTimes.m_flFPSVariability );
#endif
}
...
void _Host_RunFrame_Client( bool framefinished )
{
	...
	static int s_nClientTrace = 0;
	const bool bTrace = ( ++s_nClientTrace <= 3 );

	// Get any current state update from server, etc.
	CL_ReadPackets( framefinished );
	if ( bTrace ) HostOfflineTrace( "_Host_RunFrame_Client after CL_ReadPackets" );

	GetBaseLocalClient().CheckUpdatingSteamResources();
	if ( bTrace ) HostOfflineTrace( "_Host_RunFrame_Client after CheckUpdatingSteamResources" );
	GetBaseLocalClient().CheckFileCRCsWithServer();
	if ( bTrace ) HostOfflineTrace( "_Host_RunFrame_Client after CheckFileCRCsWithServer" );

	// Resend connection request if needed.
	GetBaseLocalClient().RunFrame();
	if ( bTrace ) HostOfflineTrace( "_Host_RunFrame_Client after RunFrame" );
	...
	if ( bTrace ) HostOfflineTrace( "_Host_RunFrame_Client after Steam3Client RunFrame" );
...
void _Host_RunFrame_Render()
{
...

	{
		...
		static int s_nRenderTrace = 0;
		if ( ++s_nRenderTrace <= 3 )
			HostOfflineTrace( "_Host_RunFrame_Render UpdateScreen frame=%d", s_nRenderTrace );
		Host_UpdateScreen();
	}
	{
...
{
	static int s_nHostInnerFrameTrace = 0;
	const int nTraceFrame = ++s_nHostInnerFrameTrace;
	const bool bTrace = ( nTraceFrame <= 3 );
	if ( bTrace ) HostOfflineTrace( "_Host_RunFrame begin frame=%d", nTraceFrame );

	g_HostTimes.MarkFrameStartTime();

	// Spread static-prop VPhysics across frames (x64 has no soft OOM throttle like x86).
	if ( StaticPropMgr() )
		StaticPropMgr()->PumpDeferredVPhysicsCreates();
	...
	int numticks;
	{
		...
		_Host_SetGlobalTime();
		if ( bTrace ) HostOfflineTrace( "_Host_RunFrame after SetGlobalTime frame=%d", nTraceFrame );
...

	{
		...
		if ( bTrace ) HostOfflineTrace( "_Host_RunFrame after Cbuf_Execute frame=%d", nTraceFrame );
		...
		else
		{
			...
			if ( bTrace ) HostOfflineTrace( "_Host_RunFrame before NET_Init frame=%d", nTraceFrame );
			NET_Init( NET_IsDedicated() );
			if ( bTrace ) HostOfflineTrace( "_Host_RunFrame after NET_Init frame=%d", nTraceFrame );
		}

		g_HostTimes.EndFrameSegment( FRAME_SEGMENT_CMD_EXECUTE );

		if ( bTrace ) HostOfflineTrace( "_Host_RunFrame before tick loop frame=%d numticks=%d", nTraceFrame, numticks );

		// Msg( "Running %i ticks (%f remainder) for frametime %f total %f tick %f delta %f\n", numticks, remainder, host_frametime, host_time );
#if !defined( _CERT )
		// Once/sec offline clock/speed probes — OFF by default (Msg+condebug tanks listen FPS in fights).
		static ConVar host_offline_diag( "host_offline_diag", "0", FCVAR_RELEASE,
			"1=print SPEEDDIAG/CLOCKDIAG once per second while sv active" );
		if ( host_offline_diag.GetBool() && sv.IsActive() && numticks > 0 )
		{
			static double s_flSpeedDiagWall = 0.0;
			static int s_nSpeedDiagTicks = 0;
			static int s_nSpeedDiagFrames = 0;
			static DWORD s_dwSpeedDiagTickCount = 0;
			static int s_nSpeedDiagHostTick = 0;
			s_nSpeedDiagTicks += numticks;
			s_nSpeedDiagFrames++;
			const double now = Sys_FloatTime();
			const DWORD nowTC = GetTickCount();
			if ( s_flSpeedDiagWall <= 0.0 )
			{
				s_flSpeedDiagWall = now;
				s_dwSpeedDiagTickCount = nowTC;
				s_nSpeedDiagHostTick = host_tickcount;
			}
			else if ( ( now - s_flSpeedDiagWall ) >= 1.0 )
			{
				const double wallPlat = now - s_flSpeedDiagWall;
				const double wallTC = (double)( nowTC - s_dwSpeedDiagTickCount ) / 1000.0;
				const float ipt = host_state.interval_per_tick;
				const float tpsPlat = (float)( s_nSpeedDiagTicks / wallPlat );
				const float tpsTC = ( wallTC > 0.001 ) ? (float)( s_nSpeedDiagTicks / wallTC ) : 0.0f;
				const int hostDelta = host_tickcount - s_nSpeedDiagHostTick;
				const float clockRatio = ( wallTC > 0.001f ) ? (float)( wallPlat / wallTC ) : 0.0f;
				Msg( "SPEEDDIAG ticks/s=%.1f (expect ~%.0f) frames=%d ft=%.5f ipt=%.5f host_ts=%.2f sv_ts=%.2f\n",
					tpsPlat, ipt > 0.0f ? ( 1.0f / ipt ) : 64.0f,
					s_nSpeedDiagFrames, host_frametime, ipt,
					host_timescale.GetFloat(), sv.GetTimescale() );
				Msg( "CLOCKDIAG plat_s=%.3f gettick_s=%.3f ratio=%.4f (1.0=ok) host_ticks=%d tps_vs_gettick=%.1f\n",
					wallPlat, wallTC, clockRatio, hostDelta, tpsTC );
				s_flSpeedDiagWall = now;
				s_dwSpeedDiagTickCount = nowTC;
				s_nSpeedDiagHostTick = host_tickcount;
				s_nSpeedDiagTicks = 0;
				s_nSpeedDiagFrames = 0;
			}
		}
#endif
...
// threaded path only supported in listen server
...
#endif
		{
			...
			for ( int tick = 0; tick < numticks; tick++ )
			{ 
				if ( bTrace && tick == 0 ) HostOfflineTrace( "_Host_RunFrame tick0 begin frame=%d", nTraceFrame );
				...
				NET_RunFrame( now );
				if ( bTrace && tick == 0 ) HostOfflineTrace( "_Host_RunFrame tick0 after NET_RunFrame frame=%d", nTraceFrame );
				...
				CL_CheckClientState();
				if ( bTrace && tick == 0 ) HostOfflineTrace( "_Host_RunFrame tick0 after CL_CheckClientState frame=%d", nTraceFrame );
#endif
				//-------------------
				// input processing
				//-------------------
				...
				if ( bTrace && tick == 0 ) HostOfflineTrace( "_Host_RunFrame tick0 after input frame=%d", nTraceFrame );
				...
				_Host_RunFrame_Server( bFinalTick );
				if ( bTrace && tick == 0 ) HostOfflineTrace( "_Host_RunFrame tick0 after server frame=%d", nTraceFrame );
				...
				if ( bTrace && tick == 0 ) HostOfflineTrace( "_Host_RunFrame tick0 after client frame=%d", nTraceFrame );

				toolframework->Think( bFinalTick );
#endif

				if ( bTrace && tick == 0 ) HostOfflineTrace( "_Host_RunFrame tick0 end frame=%d", nTraceFrame );
...
#endif
			if ( bTrace ) HostOfflineTrace( "_Host_RunFrame after tick loop frame=%d numticks=%d shouldrender=%d", nTraceFrame, numticks, shouldrender ? 1 : 0 );
		}
#ifndef DEDICATED
		else
		{
		...
		if ( shouldrender )
		{
			...
			if ( bTrace ) HostOfflineTrace( "_Host_RunFrame before render frame=%d", nTraceFrame );
			...
			if ( bTrace ) HostOfflineTrace( "_Host_RunFrame after render frame=%d", nTraceFrame );
		...
		if ( !sv.IsDedicated() )
		{
			if ( bTrace ) HostOfflineTrace( "_Host_RunFrame before ClientDLL_Update frame=%d", nTraceFrame );
			...
			if ( bTrace ) HostOfflineTrace( "_Host_RunFrame after ClientDLL_Update frame=%d", nTraceFrame );
		}
#endif
		if ( pGameJob )
		{
...
/*
==============================
Host_KeepAliveDuringMapLoad

Pump Win32 messages during long synchronous map load so the HWND stays
responsive. Nested V_RenderVGuiOnly is suppressed while sv.IsLoading (avoids
Panorama thrash); without a pump Windows AppHang-kills the process mid-BSP.
==============================
*/
void Host_KeepAliveDuringMapLoad()
{
#if defined( WIN32 ) && !defined( _X360 ) && !defined( DEDICATED )
	MSG msg;
...
void Host_RunFrame( float time )
{
	...
	static int s_nHostRunFrameTrace = 0;
	if ( ++s_nHostRunFrameTrace <= 5 || ( s_nHostRunFrameTrace % 600 ) == 0 )
	{
		HostOfflineTrace( "Host_RunFrame tick=%d", s_nHostRunFrameTrace );
	}
...
{
#if defined( NO_STEAM ) && !defined( DEDICATED )
	HostOfflineTrace( "Host_PostInit begin" );
#endif
	if ( serverGameDLL )
	{
	...
	{
#if defined( NO_STEAM )
		HostOfflineTrace( "Host_PostInit before g_ClientDLL->PostInit" );
#endif
		g_ClientDLL->PostInit();
#if defined( NO_STEAM )
		HostOfflineTrace( "Host_PostInit after g_ClientDLL->PostInit" );
#endif
	}
...
#if defined( NO_STEAM )
		HostOfflineTrace( "Host_PostInit before EngineVGui->PostInit" );
#endif
		EngineVGui()->PostInit();
#if defined( NO_STEAM )
		HostOfflineTrace( "Host_PostInit after EngineVGui->PostInit" );
#endif
	}
...
#if defined( NO_STEAM ) && !defined( DEDICATED )
	HostOfflineTrace( "Host_PostInit end" );
#endif
#endif
}
...
{
	if ( CommandLine()->FindParm("-insecure") || CommandLine()->FindParm("-tools") || CommandLine()->FindParm( "-edit" ) )
		g_bAllowSecureServers = false;
...
public:
	...
	{
		bool bResult = g_ClientDLL->GetScaleformSlotInitController()->OnFileLoadedByScaleform( pszFilename, pvBuffer, numBytesLoaded );
		if ( !bResult )
...
#ifndef DEDICATED
	if ( bDedicated )
	{	// KLUDGE: NET_SetDedicated should be the right call, but it breaks GOTV relays, so made a separate state
		// to track the fact that this is a dedicated server and should not do user license checking
		extern void NET_SetHostInitializedAsDedicatedServer();
		NET_SetHostInitializedAsDedicatedServer();
	}
#endif
#endif
...
#if defined( NO_STEAM ) && !defined( DEDICATED )
		HostOfflineTrace( "Host_Init after ClientDLL_Init" );
#endif

		TRACEINIT( SCR_Init(), SCR_Shutdown() );
#if defined( NO_STEAM ) && !defined( DEDICATED )
		HostOfflineTrace( "Host_Init after SCR_Init" );
#endif

		TRACEINIT( R_Init(), R_Shutdown() ); 
#if defined( NO_STEAM ) && !defined( DEDICATED )
		HostOfflineTrace( "Host_Init after R_Init" );
#endif

		TRACEINIT( Decal_Init(), Decal_Shutdown() );
#if defined( NO_STEAM ) && !defined( DEDICATED )
		HostOfflineTrace( "Host_Init after Decal_Init" );
#endif

		// hookup interfaces
		EngineVGui()->Connect();
#if defined( NO_STEAM ) && !defined( DEDICATED )
		HostOfflineTrace( "Host_Init after EngineVGui Connect" );
#endif
	}
	else
#endif
...
#if defined( NO_STEAM )
	HostOfflineTrace( "Host_Init before Host_ReadConfiguration" );
#endif
	Host_ReadConfiguration( -1, false );
#if defined( NO_STEAM )
	HostOfflineTrace( "Host_Init after Host_ReadConfiguration" );
#endif
	TRACEINIT( S_Init(), S_Shutdown() );
#if defined( NO_STEAM )
	HostOfflineTrace( "Host_Init after S_Init" );
#endif
...
#if defined( NO_STEAM )
	HostOfflineTrace( "Host_Init before matchframework voice register mf=%d voice=%d", g_pMatchFramework ? 1 : 0, pIEngineVoice ? 1 : 0 );
#endif
	if ( g_pMatchFramework )
	{
...
#if defined( NO_STEAM )
	HostOfflineTrace( "Host_Init after matchframework voice register" );
#endif
...
#if defined( NO_STEAM )
		HostOfflineTrace( "Host_Init after CreateMilesAudioEngine" );
#endif
	}
#endif
...
#if defined( NO_STEAM ) && !defined( DEDICATED )
	HostOfflineTrace( "Host_Init before Host_PostInit" );
#endif
	Host_PostInit();
#if defined( NO_STEAM ) && !defined( DEDICATED )
	HostOfflineTrace( "Host_Init after Host_PostInit" );
#endif
	EndLoadingUpdates();
...
{
	if ( !sv.IsDedicated() )
		return; // if it's not a dedicated server (especially if it's a listen server), the user may be unsophisticated, so playing with thread pool may cause unexpected perf issues

	extern ConVar occlusion_test_async, sv_parallel_send;
	int nTargetThreads = sv_parallel_send.GetInt();
	if ( nTargetThreads )
	{
		if ( nTargetThreads < 0 )
			nTargetThreads = GetCPUInformation().m_nLogicalProcessors;
		else
			nTargetThreads = Min<int>( GetCPUInformation().m_nLogicalProcessors, nTargetThreads );
	}
	else
	{
		nTargetThreads = Max( 0, Min( 4, occlusion_test_async.GetInt() ) );
	}
	...
	if ( nTargetThreads > g_pThreadPool->NumThreads() )
	{
		...
		if ( nTargetThreads != 0 )
		{
			...
			ThreadPoolStartParams_t startParams;
			GetThreadPoolStartParams( startParams );
			startParams.nThreads = nTargetThreads;
...
bool Host_NewGame( char *mapName, char *mapGroupName, bool loadGame, bool bBackgroundLevel, bool bSplitScreenConnect, const char *pszOldMap, const char *pszLandmark )
{
	...
	Msg( "Host_NewGame: map=%s group=%s loadGame=%d\n",
		mapName ? mapName : "?", mapGroupName ? mapGroupName : "?", loadGame ? 1 : 0 );
	...
	{
		Msg( "Host_NewGame: SV_ActivateServer FAILED\n" );
		return false;
	}
	Msg( "Host_NewGame: SV_ActivateServer OK\n" );
	...
	if ( !sv.IsDedicated() )
	{
		...
		Msg( "Host_NewGame: connect queued: %s\n", str );
	}
	else
	{
