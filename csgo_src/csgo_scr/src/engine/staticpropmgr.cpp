...
ConVar disableStaticPropLoading( "r_disable_static_prop_loading", "0", FCVAR_CHEAT, "If non-zero when a map loads, static props won't be loaded" );
...
private:
	...
	matrix3x4_t				m_CollisionToWorld; // we need a separate matrix because currently scaled collision models have their scale baked in
	float					m_flRadius;
...
public:
	...
	virtual void PumpDeferredVPhysicsCreates();

	// Internal methods
	const Vector &ViewOrigin() const { return m_vecLastViewOrigin; }
	int CreateVPhysicsBatch( int nMax );

	// Computes the opacity for a single static prop
	void DrawStaticProps( IClientRenderable **pProps, const RenderableInstance_t *pInstances, int count, bool bShadowDepth, bool bSSAODepth, bool drawVCollideWireframe );
	...
	void DrawStaticProps_Fast( IClientRenderable **pProps, const RenderableInstance_t *pInstances, int count, bool bShadowDepth, bool bSSAODepth );
...
private:
	...
	// Deferred static-prop VPhysics (spread across Host frames)
	bool							m_bDeferredPhysActive;
	int								m_nDeferredPhysCursor;
	IPhysicsEnvironment*			m_pDeferredPhysEnv;
	IVPhysicsKeyHandler*			m_pDeferredPhysDefaults;
	void*							m_pDeferredPhysGameData;
};
...
bool CStaticProp::Init( int index, StaticPropLump_t &lump, model_t *pModel )
{
	...
	if ( !m_pModel )
	{
		Log_Warning( LOG_StaticPropManager, "CStaticProp::Init: missing model for static prop index %d (PropType %d)\n",
			index, lump.m_PropType );
	...
	if ( !sv.IsDedicated() && g_pClientAlphaPropertyMgr )
	{
		CleanUpAlphaProperty();
		...
		if ( m_pClientAlphaProperty )
		{
			...
			if ( pStudioHdr && ( pStudioHdr->flags & STUDIOHDR_FLAGS_NO_FORCED_FADE ) )
			{
	...
	AngleMatrix( lump.m_Angles, lump.m_Origin, m_CollisionToWorld );
	m_ModelToWorld = m_CollisionToWorld;
	m_ModelToWorld.ScaleUpper3x3Matrix( m_flScale );
	...
	{
		modelinfo->GetIlluminationPoint( m_pModel, this, m_Origin, m_Angles, m_flScale, &m_LightingOrigin );
	}
...
float CStaticProp::GetCollisionScale() const
{
	return m_flScale;
}
...
int	CStaticProp::DrawModelSlow( int flags, const RenderableInstance_t &instance )
{
	...
	if ( m_pModel && (flags & STUDIO_WIREFRAME_VCOLLIDE) )
	{
		if ( m_nSolidType == SOLID_VPHYSICS )
		{
			...
			vcollide_t * pCollide = CM_VCollideForModel( -1, m_pModel, m_flScale ); 
			if ( pCollide && pCollide->solidCount == 1 )
			{
				static color32 debugColor = {0,255,255,0};
				DebugDrawPhysCollide( pCollide->solids[0], NULL, m_CollisionToWorld, debugColor, false );
			}
		}
...
void CStaticProp::InsertPropIntoKDTree()
{
	...
	if ( m_nSolidType == SOLID_NONE || !m_pModel )
		return;

	static int s_nKdTreeLog = 0;
	const bool bLogKd = ( s_nKdTreeLog < 30 ) || ( s_nKdTreeLog >= 38 && s_nKdTreeLog <= 50 );
	if ( bLogKd )
	{
		Msg( "InsertPropIntoKDTree[%d]: solid=%d model=%s\n",
			s_nKdTreeLog, (int)m_nSolidType,
	...
	propToWorld.ScaleUpper3x3Matrix( m_flScale );
	TransformAABB( propToWorld, m_pModel->mins, m_pModel->maxs, mins, maxs ); 

	// Offline x64: CM_VCollideForModel + CollideGetAABB for KD-tree AABB refinement
	// AVs on some static prop .phy (first instance of a model in the prop loop). Studio
	// bounds above are sufficient for spatial partition; full vphysics runs in CreateVPhysics.
#if 0
	...
	if ( m_nSolidType == SOLID_VPHYSICS )
	{
		if ( bLogKd )
		{
			Msg( "InsertPropIntoKDTree[%d]: CM_VCollideForModel begin\n", s_nKdTreeLog );
		}
		vcollide_t *pCollide = CM_VCollideForModel( -1, m_pModel, m_flScale );
		if ( bLogKd )
		{
			Msg( "InsertPropIntoKDTree[%d]: vcollide=%p solidCount=%d physcollision=%p\n",
				s_nKdTreeLog, pCollide, pCollide ? pCollide->solidCount : -1, physcollision );
		}
		if ( pCollide && pCollide->solidCount > 0 && pCollide->solids && pCollide->solids[0] && physcollision )
		{
			physcollision->CollideGetAABB( &mins, &maxs, pCollide->solids[0], m_Origin, m_Angles );
			if ( bLogKd )
			{
				Msg( "InsertPropIntoKDTree[%d]: CollideGetAABB done\n", s_nKdTreeLog );
			}
		}
		else
		{
			...
			char szModel[MAX_PATH];
			Q_strncpy( szModel, m_pModel ? modelloader->GetName( m_pModel ) : "unknown model", sizeof( szModel ) );
			DevWarning( 1, "SOLID_VPHYSICS static prop with no vphysics model! (%s)\n", szModel );
#endif
			m_nSolidType = SOLID_NONE;
			if ( bLogKd )
			{
				Msg( "InsertPropIntoKDTree[%d]: no vcollide, solid->NONE\n", s_nKdTreeLog );
			}
			++s_nKdTreeLog;
			return;
		}
	...
	if ( bLogKd )
	{
		Msg( "InsertPropIntoKDTree[%d]: SpatialPartition CreateHandle begin\n", s_nKdTreeLog );
	}
	...
	if ( bLogKd )
	{
		Msg( "InsertPropIntoKDTree[%d]: CreateHandle done partition=%u\n", s_nKdTreeLog, (unsigned)m_Partition );
		++s_nKdTreeLog;
	}
...
void CStaticProp::CreateVPhysics( IPhysicsEnvironment *pPhysEnv, IVPhysicsKeyHandler *pDefaults, void *pGameData )
{
	...
	{		
		if ( m_nSolidType == SOLID_VPHYSICS )
		{
			// This works because VCollideForModel only uses modelindex for mod_brush
			// and props are always mod_Studio.
			pVCollide = CM_VCollideForModel( -1, m_pModel, m_flScale );
		}
...
CStaticPropMgr::CStaticPropMgr()
{
	...
	m_bDeferredPhysActive = false;
	m_nDeferredPhysCursor = 0;
	m_pDeferredPhysEnv = NULL;
	m_pDeferredPhysDefaults = NULL;
	m_pDeferredPhysGameData = NULL;
}
...
{
	const int nBufEnd = buf.TellPut();
	int count = buf.GetInt();
	const int nDictBytes = count * (int)sizeof( StaticPropDictLump_t );

	Msg( "UnserializeModelDict: count=%d tell=%d bufEnd=%d\n", count, buf.TellGet(), nBufEnd );

	if ( count < 0 || count > 8192 || buf.TellGet() + nDictBytes > nBufEnd )
	{
		Warning( "UnserializeModelDict: bogus model count %d (buf tell=%d end=%d)\n", count, buf.TellGet(), nBufEnd );
		return;
	}
	...
	for ( int i=0; i < count; i++ )
	{
		...
		lump.m_Name[ STATIC_PROP_NAME_LENGTH - 1 ] = '\0';

		if ( i < 3 || ( i % 50 ) == 0 )
		{
			Msg( "UnserializeModelDict: load model %d/%d %s\n", i, count, lump.m_Name );
		}

		m_StaticPropDict[i] = modelloader->GetModelForName( lump.m_Name, IModelLoader::FMODELLOADER_STATICPROP );
		if ( !m_StaticPropDict[i] )
		{
			Warning( "UnserializeModelDict: failed to load model %d/%d %s\n", i, count, lump.m_Name );
		}
		else if ( m_StaticPropDict[i]->type == mod_studio && g_pMDLCache )
		{
			// Static prop MDL load defers vcollide; preload .phy here so prop-loop
			// does not AV on the first SOLID_VPHYSICS instance of each model.
			MDLCACHE_CRITICAL_SECTION_( g_pMDLCache );
			if ( i < 5 || ( i % 25 ) == 0 )
			{
				Msg( "UnserializeModelDict: preload vcollide %d/%d %s\n", i, count, lump.m_Name );
			}
			vcollide_t *pVC = g_pMDLCache->GetVCollide( m_StaticPropDict[i]->studio );
			if ( !pVC && ( i < 5 || ( i % 25 ) == 0 ) )
			{
				Msg( "UnserializeModelDict: no vcollide %d/%d %s\n", i, count, lump.m_Name );
			}
		}
	}
	Msg( "UnserializeModelDict: done\n" );
...
void CStaticPropMgr::UnserializeLeafList( CUtlBuffer& buf )
{
	...
	const int nLeafBytes = nCount * (int)sizeof( StaticPropLeafLump_t );

	Msg( "UnserializeLeafList: count=%d tell=%d bufEnd=%d\n", nCount, buf.TellGet(), nBufEnd );

	m_StaticPropLeaves.Purge();
	if ( nCount > 0 )
	{
		if ( nCount < 0 || buf.TellGet() + nLeafBytes > nBufEnd )
		{
			Warning( "UnserializeLeafList: bogus leaf count %d\n", nCount );
			return;
		}

		m_StaticPropLeaves.AddMultipleToTail( nCount );
		buf.Get( m_StaticPropLeaves.Base(), nLeafBytes );
	}
	Msg( "UnserializeLeafList: done\n" );
}
...
void CStaticPropMgr::UnserializeModels( CUtlBuffer& buf )
{
	...
	Msg( "UnserializeModels: lumpVersion=%d count=%d tell=%d bufEnd=%d\n",
		nLumpVersion, count, buf.TellGet(), nBufEnd );

	if ( count < 0 || count > 65536 )
	{
		Warning( "UnserializeModels: bogus prop count %d\n", count );
		return;
	}
	...
	for ( int i = 0; i < count; ++i )
	{
		...
		if ( lump.m_PropType < 0 || lump.m_PropType >= m_StaticPropDict.Count() )
		{
			Log_Warning( LOG_StaticPropManager, "UnserializeModels: bad PropType %d (count %d), skipping prop %d\n",
				lump.m_PropType, m_StaticPropDict.Count(), i );
			m_StaticProps.Remove( j );
			continue;
		}

		model_t *pDictModel = m_StaticPropDict[lump.m_PropType];
		const char *pszModelName = pDictModel ? modelloader->GetName( pDictModel ) : "NULL";
		extern ConVar developer;
		const bool bUnserLog = ( developer.GetInt() >= 2 );
		if ( bUnserLog && ( i < 60 || ( i % 200 ) == 0 ) )
		{
			Msg( "UnserializeModels: prop %d/%d type=%d solid=%d scale=%.3f model=%s\n",
				i, count, lump.m_PropType, (int)lump.m_Solid, lump.m_flScale, pszModelName );
		}

		m_StaticProps[j].Init( j, lump, pDictModel );
		...
		if ( bUnserLog && ( i < 60 || ( i % 200 ) == 0 ) )
		{
			Msg( "UnserializeModels: prop %d kdtree done\n", i );
		}
	}
	{
		extern ConVar developer;
		if ( developer.GetInt() >= 2 )
			Msg( "UnserializeModels: done props=%d\n", m_StaticProps.Count() );
	}
}
...
void CStaticPropMgr::OutputLevelStats( void )
{
	...
	for( i = 0; i < m_StaticProps.Count(); i++ )
	{
		...
		for( bodyPart = 0; bodyPart < pStudioHdr->numbodyparts; bodyPart++ )
		{
			...
			{
				mstudiomodel_t *pModel2 = pBodyPart->pModel( model );
				totalVerts += pModel2->numvertices;
			}
		}
	}
...
void CStaticPropMgr::UnserializeStaticProps()
{
	...
	if (!size)
	{
		Msg( "UnserializeStaticProps: no static prop lump\n" );
		return;
	}

	Msg( "UnserializeStaticProps: begin size=%d version=%d\n", size, Mod_GameLumpVersion( GAMELUMP_STATIC_PROPS ) );
	COM_TimestampedLog( "UnserializeStaticProps - start");

	MEM_ALLOC_CREDIT();
	CUtlBuffer buf( 0, size );
	Msg( "UnserializeStaticProps: Mod_LoadGameLump begin\n" );
	const bool bLumpLoaded = Mod_LoadGameLump( GAMELUMP_STATIC_PROPS, buf.PeekPut(), size );
	Msg( "UnserializeStaticProps: Mod_LoadGameLump ok=%d\n", bLumpLoaded ? 1 : 0 );
	if ( bLumpLoaded )
	{
		buf.SeekPut( CUtlBuffer::SEEK_HEAD, size );
		Msg( "UnserializeStaticProps: modeldict begin\n" );
		COM_TimestampedLog( "UnserializeModelDict" );
		UnserializeModelDict( buf );
		Msg( "UnserializeStaticProps: leaflist begin\n" );
		COM_TimestampedLog( "UnserializeLeafList" );
		UnserializeLeafList( buf );
		Msg( "UnserializeStaticProps: models begin\n" );
		COM_TimestampedLog( "UnserializeModels" );
		UnserializeModels( buf );
	}

	Msg( "UnserializeStaticProps: end props=%d\n", m_StaticProps.Count() );
...
//-----------------------------------------------------------------------------
static ConVar r_staticprop_physics_per_frame( "r_staticprop_physics_per_frame", "48", FCVAR_RELEASE,
	"Max static-prop VPhysics creates per Host frame (0=all sync with batch yields)." );
static ConVar r_staticprop_physics_batch_yield( "r_staticprop_physics_batch_yield", "16", FCVAR_RELEASE,
	"When syncing all props, ThreadSleep(0) every N creates so other work can breathe." );

int CStaticPropMgr::CreateVPhysicsBatch( int nMax )
{
	if ( !m_bDeferredPhysActive || !m_pDeferredPhysEnv )
		return 0;

	const int nCount = m_StaticProps.Count();
	int nDone = 0;
	extern ConVar developer;
	const bool bPhysLog = ( developer.GetInt() >= 2 );

	while ( m_nDeferredPhysCursor < nCount && ( nMax <= 0 || nDone < nMax ) )
	{
		const int i = m_nDeferredPhysCursor++;
		if ( bPhysLog && ( i < 5 || i == 50 || ( i % 200 ) == 0 ) )
		{
			model_t *pModel = (model_t *)m_StaticProps[i].GetModel();
			Msg( "CreateVPhysicsRepresentations: prop %d/%d model=%s scale=%.3f solid=%d begin\n",
				i, nCount,
				pModel ? modelloader->GetName( pModel ) : "NULL",
				m_StaticProps[i].GetCollisionScale(),
				(int)m_StaticProps[i].GetSolid() );
		}
		m_StaticProps[i].CreateVPhysics( m_pDeferredPhysEnv, m_pDeferredPhysDefaults, m_pDeferredPhysGameData );
		++nDone;

		const int nYieldEvery = r_staticprop_physics_batch_yield.GetInt();
		if ( nYieldEvery > 0 && ( nDone % nYieldEvery ) == 0 )
			ThreadSleep( 0 );
	}

	if ( m_nDeferredPhysCursor >= nCount )
	{
		if ( bPhysLog || developer.GetInt() >= 1 )
			Msg( "CreateVPhysicsRepresentations: done count=%d\n", nCount );
		m_bDeferredPhysActive = false;
		m_pDeferredPhysEnv = NULL;
		m_pDeferredPhysDefaults = NULL;
		m_pDeferredPhysGameData = NULL;
...
void CStaticPropMgr::PumpDeferredVPhysicsCreates()
{
	if ( !m_bDeferredPhysActive )
		return;
	const int nPerFrame = r_staticprop_physics_per_frame.GetInt();
	if ( nPerFrame <= 0 )
		return;
	CreateVPhysicsBatch( nPerFrame );
}
...
void CStaticPropMgr::CreateVPhysicsRepresentations( IPhysicsEnvironment	*pPhysEnv, IVPhysicsKeyHandler *pDefaults, void *pGameData )
{
	...
	const bool bPhysLog = ( developer.GetInt() >= 2 );
	if ( bPhysLog )
		Msg( "CreateVPhysicsRepresentations: begin count=%d per_frame=%d\n", nCount, r_staticprop_physics_per_frame.GetInt() );

	m_bDeferredPhysActive = true;
	m_nDeferredPhysCursor = 0;
	m_pDeferredPhysEnv = pPhysEnv;
	m_pDeferredPhysDefaults = pDefaults;
	m_pDeferredPhysGameData = pGameData;

	const int nPerFrame = r_staticprop_physics_per_frame.GetInt();
	if ( nPerFrame <= 0 )
	{
		// Sync path: still yield every batch so x64 doesn't freeze the process for seconds.
		CreateVPhysicsBatch( 0 );
		return;
	}

	// First frame worth now; Host pumps the rest across subsequent frames.
	CreateVPhysicsBatch( nPerFrame );
}
...
void CStaticPropMgr::DrawStaticProps_Fast( IClientRenderable **pProps, const RenderableInstance_t *pInstances, int count, bool bShadowDepth, bool bSSAODepth )
{
	...
	if ( bSSAODepth )
		sInfo.flags |= STUDIO_SSAODEPTHTEXTURE;
...
void CStaticPropMgr::DrawStaticProps( IClientRenderable **pProps, const RenderableInstance_t *pInstances, int count, bool bShadowDepth, bool bSSAODepth, bool drawVCollideWireframe )
{
	...
	else
	{
		...
		{
			DrawStaticProps_Fast( pProps, pInstances, count, bShadowDepth, bSSAODepth );
		}
	}
}
