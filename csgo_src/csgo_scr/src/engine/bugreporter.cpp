...
// 	bufDescription.Put( (char *)pStart, nBytesToPut );
	...
	else
	{
		...
		{			
			char bufTmpSayCmd[256];
			Q_snprintf( bufTmpSayCmd, sizeof(bufTmpSayCmd), "say \"Bug Submitted [%s]: %s\"\n", assignedto, title );
			Cbuf_AddText( Cbuf_GetCurrentPlayer(), bufTmpSayCmd, kCommandSrcCode, TIME_TO_TICKS(1.5) );
		}
...
bool CBugUIPanel::AutoFillToken( char const *token, bool partial )
{
	...
	// HACK: Map Number is actually 'Component' dropdown for CSGO, try to match and populate defaults for that as well
	c = m_pMapNumber->GetItemCount();
	for ( i = 0; i < c; i++ )
	{
		char szComponent[ MAX_PATH ];
		m_pMapNumber->GetItemText( i, szComponent, sizeof( szComponent ) );
		if ( Compare( szComponent, token, partial ) )
		{
			Msg( "  COMPONENT: %s\n", szComponent );
			m_pMapNumber->ActivateItem( i );
			return true;
		}
	}
#endif
