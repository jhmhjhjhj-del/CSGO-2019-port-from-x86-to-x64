...
void CClientState::SetSound( int tableIndex )
{
	...
	static ConVarRef snd_stream("snd_disable_legacy_audio_cache_hidden");

	bool bLoadNow = ( data && !snd_stream.GetBool() && ( data->flags & RES_PRELOAD ) ) || IsGameConsole();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CClientState::DumpPrecacheStats( const char * pTableName )
{
	...
	if ( !Q_strcmp(MODEL_PRECACHE_TABLENAME, pTableName ) )
	{
		items = model_precache;
	}
	else if ( !Q_strcmp(GENERIC_PRECACHE_TABLENAME, pTableName ) )
	{
		items = generic_precache;
	}
	else if ( !Q_strcmp(SOUND_PRECACHE_TABLENAME, pTableName ) )
	{
		items = sound_precache;
	}
	else if ( !Q_strcmp(DECAL_PRECACHE_TABLENAME, pTableName ) )
	{
		items = decal_precache;
	}

	INetworkStringTable *table = GetStringTable( pTableName );
			...
			case PreserveEnt:
				{
					...
					else
					{
						...
						//             overlooking.
						if ( oldEntity >= MAX_EDICTS || oldEntity < 0 || u.m_nNewEntity >= MAX_EDICTS )
						{
							Host_Error( "CL_ReadPreserveEnt: entity out of bounds (old=%d, new=%d)", oldEntity, u.m_nNewEntity );
						}
