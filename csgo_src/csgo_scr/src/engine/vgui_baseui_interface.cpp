...
static bool sbWasIMEEnabled = false;
...
// 		}
		...
		( CommandLine()->FindParm( "-toconsole" ) || CommandLine()->FindParm( "-console" ) || CommandLine()->FindParm( "-rpt" ) || CommandLine()->FindParm( "-allowdebug" ) ) )
	{
		...
		sbWasIMEEnabled = g_pInputSystem->IsIMEAllowed();
...
void CEngineVGui::HideConsole()
{
	...
	if ( staticGameConsole )
	{
		...
		{
			g_pInputSystem->SetIMEAllowed( sbWasIMEEnabled );
		}
...
void CEngineVGui::StartCustomProgress()
{
	...
	// we've starting loading a level/connecting to a server
	staticGameUIFuncs->ShowCustomProgress();
...
void CEngineVGui::UpdateProgressBar( float progress, const char *pDesc, bool showDialog )
{
	...
	// Offline x64 / Panorama: full nested V_RenderVGuiOnly every ~50ms during
	// sv.IsLoading() AppHang'd the main thread. Paint throttled (~5 Hz) so loading
	// shell actually appears; Host_KeepAliveDuringMapLoad every tick.
	static double s_flLastLoadPaint = -1.0;
	if ( sv.IsLoading() )
	{
		Host_KeepAliveDuringMapLoad();
		const double flNow = Plat_FloatTime();
		if ( s_flLastLoadPaint < 0.0 || ( flNow - s_flLastLoadPaint ) >= 0.2 )
		{
			s_flLastLoadPaint = flNow;
			extern void V_RenderVGuiOnly();
			V_RenderVGuiOnly();
		}
		return;
	}
	s_flLastLoadPaint = -1.0;
...
#if defined( INCLUDE_SCALEFORM )
				if ( g_pScaleformUI) g_pScaleformUI->RunFrame( g_ClientGlobalVariables.frametime / timeScale );
#endif
			}
			else
			{
	...
	{
#if defined( INCLUDE_SCALEFORM )
		if ( g_pScaleformUI) g_pScaleformUI->RunFrame( 0 );
#endif
...
void CEngineVGui::UpdateSecondaryProgressBarWithFile( float progress, const char *pPath, int nBytesTotal )
{
	...
	// make sure progress is a valid number to display
	if ( progress <= 0.0f )
		progress = 0.0f;
	...
	char const *szLocToken = ( nBytesTotal <= 0 ) ? "#SFUI_DownLoading_Pct" : "#SFUI_DownLoading_";
	wchar_t const *wchLocToken = g_pVGuiLocalize->Find( szLocToken );
	if ( !wchLocToken )
		wchLocToken = L"Downloading %s1 ....  %s2";
	
	wchar_t szWideBuff[ 256 ];
	g_pVGuiLocalize->ConstructString( szWideBuff, sizeof( szWideBuff ), wchLocToken, 3, wszFile, wszPercent, wszMegs );
...
//-----------------------------------------------------------------------------
void CEngineVGui::NotifyOfServerConnect(const char *szGameString, int nIP, int connectionPort, int queryPort)
{
	if (!staticGameUIFuncs)
		return;

	staticGameUIFuncs->OnConnectToServer2( szGameString, nIP, connectionPort, queryPort);
}
