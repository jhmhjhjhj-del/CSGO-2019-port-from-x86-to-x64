	...
	if( (dwHeader & 0xFFE00000) ==  0xFFE00000 )
	{
		...
		{	
			...
			{	
				...
				uint32 dwOffset2 = m_dwSyncOffset+m_dwComputedFrameSize;
				try
				{
					CMPAHeader m_SubsequentFrame( m_pMPAFile, dwOffset2, true );	
				...
				catch( CMPAException& Exc )
				{
					...
					{
						if( dwOffset2 + m_pMPAFile->m_dwBegin > m_pMPAFile->m_dwEnd )
