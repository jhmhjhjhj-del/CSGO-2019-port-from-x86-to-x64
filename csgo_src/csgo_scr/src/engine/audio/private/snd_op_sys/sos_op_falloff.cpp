...
SOS_REGISTER_INPUT_FLOAT( CSosOperatorFalloffTail, m_flInputVolumeMin, SO_SINGLE, "input_volume_min" )
...
void CSosOperatorFalloffTail::SetDefaults( void *pVoidMem ) const
{
	...
	SOS_INIT_INPUT_VAR( m_flInputVolumeMin, SO_SINGLE, 0.0 )
...
//	else if ( flDistance > flDistantMin && flDistance <= flTailMin )
	...
	if ( flResult < pStructMem->m_flInputVolumeMin[0] )
	{
		flResult = pStructMem->m_flInputVolumeMin[0];
	}
