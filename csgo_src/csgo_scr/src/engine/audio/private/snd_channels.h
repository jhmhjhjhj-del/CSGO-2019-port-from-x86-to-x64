...
struct hrtf_info_t
{
    ...
    float       stereo_blend; // 1.0 = emit stereo data without spatialization, 0.0 = emit binaural data with spatialization.
	bool		follow_entity;   // If true, we update the position of the entity every frame, otherwise we use the position of the sound.
	bool		is_positional_voice;
	bool		bilinear_filtering;  // If true, we use more expensive bilinear filtering for this sound.
	bool		lock_position;   // If true, the vec will not be modified after the sound starts.

	bool		is_radio; //if it's a radio sound it's played relative to a listener of (0,0,0) rather than the listener's world position.
	bool		use_hrtf;
	bool		use_occlusion;
...
const int PHONON_FRAME_SIZE = 512;
...
struct channel_t
{
	...
	union
	{
		...
		struct
		{
			...
			bool		m_bAlertSound : 1;					// This sound should play even if the game is out of focus.
