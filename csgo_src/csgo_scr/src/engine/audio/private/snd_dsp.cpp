...
rva_t * RVA_Alloc ( int *D, int *a, int *b, int m, flt_t *pflt, int fparallel, float fmoddly, float fmodrate, float ftaps )
{
	...
	if ( fmoddly > 0.0 )
	{
		...
		for (i = 0; i < m; i++)
		{
			float flD = (float)prva->pdlys[i]->D;

			modtime = flD / (float)(SOUND_DMA_SPEED);	// seconds per delay
...
inline void DSP_ProcessQuadToStereo(dsp_t *pdsp, portable_samplepair_t *pbfront, portable_samplepair_t *pbrear, int sampleCount, bool bcrossfading )
{
	...
	if ( !bcrossfading ) 
	{
		...
		else
		{
			...
			while ( count-- )
			{
				int fl = PSET_GetNext( pdsp->ppset[0], (pbf->left + pbr->left) >> 1);
				int fr = PSET_GetNext( pdsp->ppset[1], (pbf->right + pbr->right) >> 1);
				fl = CLIP_DSP( fl );
				fr = CLIP_DSP( fr );
...
inline void DSP_ProcessQuadToQuad(dsp_t *pdsp, portable_samplepair_t *pbfront, portable_samplepair_t *pbrear, int sampleCount, bool bcrossfading )
{
	...
	if ( !bcrossfading ) 
	{
		...
		else
		{
			while ( count-- )
			{
				int fl = PSET_GetNext( pdsp->ppset[0], pbf->left );
				int fr = PSET_GetNext( pdsp->ppset[1], pbf->right );
				int rl = PSET_GetNext( pdsp->ppset[2], pbr->left );
				int rr = PSET_GetNext( pdsp->ppset[3], pbr->right );
	...
	if ( !bcrossfading ) 
	{
		...
		else
		{
			while ( count-- )
			{
				...
				int rl = PSET_GetNext( pdsp->ppset[2], pbr->left );
				int rr = PSET_GetNext( pdsp->ppset[3], pbr->right );
				int fc = PSET_GetNext( pdsp->ppset[4], pbc->left );
...
void DSP_Process( int idsp, portable_samplepair_t *pbfront, portable_samplepair_t *pbrear, portable_samplepair_t *pbcenter, int sampleCount )
{
	...
	// We now process DSP even if not connected, because we want to use DSP on the titlescreen.
