...
ConVar snd_hrtf_stereo_blend("snd_hrtf_stereo_blend", "1", FCVAR_CHEAT);
ConVar snd_hrtf_voice_delay("snd_hrtf_voice_delay", "0.1", FCVAR_ARCHIVE|FCVAR_RELEASE);

//defined in snd_dma.cpp
extern IPLhandle g_phononEngineRenderer;
...
namespace
{
	...
	static bool s_bPhononInitFailed = false;
	...
	struct PhononJob
	{
		...
		short stereo_delay_line[PHONON_FRAME_SIZE * 2 * 2];

		float fSubmitTime, fCompleteTime;

		IPLDirectSoundPath path;
	...
	void ProcessPhononJob(PhononJob& job)
	{
		...
		const IPLAudioFormat& monoFormat = g_phononInputFormat;
		const IPLAudioFormat& stereoFormat = g_phononOutputFormat;

		if ( job.bOcclude && !job.directSoundEffect )
		{
			IPLerror error = iplCreateDirectSoundEffect( g_phononEngineRenderer, (job.bHRTF || job.nChannels == 1) ? monoFormat : stereoFormat, job.bHRTF ? monoFormat : stereoFormat, &job.directSoundEffect );
			if ( error )
			{
				job.bOcclude = false;
				DevMsg( "Failed to create direct sound effect: %d\n", (int)error );
				snd_occlusion.SetValue(false);
				return;
			}
		}

		if ( !job.bOcclude && !job.bHRTF )
		{
			//job isn't doing anything this frame.
			if ( job.nChannels == 1 )
			{
				const short *in = (const short*)job.inputBuf;
				short* out = (short*)job.outputBuf;

				for ( int i = 0; i != PHONON_FRAME_SIZE; ++i )
				{
					*out++ = *in;
					*out++ = *in++;
				}
			...
			{
				memcpy( job.outputBuf, job.inputBuf, sizeof( job.inputBuf ) );
			}

			return;
		}
		else if ( job.bOcclude && !job.bHRTF )
		{
			//special case occlusion without hrtf for now.
			//TODO: more elegantly wrap this into the general case.
			const float flSigned16ToUnitScale = 1.0f / 32768.0f;

			const short *in = (const short*)job.inputBuf;
			float ALIGN16 phononInputBuf[PHONON_FRAME_SIZE*2];
			float ALIGN16 phononOutputBuf[PHONON_FRAME_SIZE*2];
			ConvertShortToFloat( phononInputBuf, in, PHONON_FRAME_SIZE*job.nChannels, flSigned16ToUnitScale );

			IPLAudioBuffer input_buffer;
			input_buffer.format = job.nChannels == 1 ? monoFormat : stereoFormat;
			...
			output_buffer.format = stereoFormat;
			...
			options.applyDistanceAttenuation = IPL_FALSE;
			options.applyAirAbsorption = IPL_FALSE;
			options.directOcclusionMode = IPL_DIRECTOCCLUSION_TRANSMISSIONBYFREQUENCY;
			iplApplyDirectSoundEffect(job.directSoundEffect, input_buffer, job.path, options, output_buffer);

			if(job.path.occlusionFactor < 1.0f)
			{
				job.stereo_blend *= job.path.transmissionFactor[0] * job.path.transmissionFactor[1] * job.path.transmissionFactor[2];
			}

			{
				TM_ZONE(TELEMETRY_LEVEL0, TMZF_NONE, "ConvertSIMD");
				float flScaleFactorFloat = 32768.0f;
		...
		float ALIGN16 phononDirectSoundInputBuf[PHONON_FRAME_SIZE];

		float ALIGN16 phononInputBuf[PHONON_FRAME_SIZE];

		float* inputBuf = phononInputBuf;
		if ( job.bOcclude )
		{
			inputBuf = phononDirectSoundInputBuf;
		}
...
//		else
		{
			...
			if ( job.nChannels == 1 )
			{
				...
				ConvertShortToFloat( inputBuf, in, PHONON_FRAME_SIZE, flSigned16ToUnitScale );
			}
			else
			{
				TM_ZONE( TELEMETRY_LEVEL0, TMZF_NONE, "SumStereoShortToFloat" );
				SumStereoShortToFloat( inputBuf, in, PHONON_FRAME_SIZE, flSigned16ToUnitScale * flStereoScale );
			}
		}
		...
		if ( job.bOcclude )
		{
			...
			input_buffer.numSamples = PHONON_FRAME_SIZE;
			input_buffer.interleavedBuffer = phononDirectSoundInputBuf;
			input_buffer.deinterleavedBuffer = nullptr;

			IPLAudioBuffer output_buffer;
			output_buffer.format = g_phononInputFormat;
			output_buffer.numSamples = PHONON_FRAME_SIZE;
			output_buffer.interleavedBuffer = phononInputBuf;
			output_buffer.deinterleavedBuffer = nullptr;
...

		{
			...
			memcpy(job.stereo_delay_line, &job.stereo_delay_line[PHONON_FRAME_SIZE * 2], PHONON_FRAME_SIZE * 2 * sizeof(short));

			if (job.nChannels == 1)
			{
				short* inputBuf2 = (short*) job.inputBuf;
				for (int i = 0; i < PHONON_FRAME_SIZE; ++i)
				{
					job.stereo_delay_line[2 * (PHONON_FRAME_SIZE + i) + 0] = inputBuf2[i];
					job.stereo_delay_line[2 * (PHONON_FRAME_SIZE + i) + 1] = inputBuf2[i];
				}
			}
			else if (job.nChannels == 2)
			{
				memcpy(&job.stereo_delay_line[PHONON_FRAME_SIZE * 2], job.inputBuf, PHONON_FRAME_SIZE * 2 * sizeof(short));
			}

			float left_delay = 0.0f, right_delay = 0.0f;
			iplApplyBinauralEffectWithParameters(job.phononEffect, input_buffer, iplvec, job.interpolation, output_buffer, &left_delay, &right_delay);

			if (snd_hrtf_stereo_blend.GetBool())
			{
				float stereo_blend = job.stereo_blend;
				int sampling_rate = 44100;

				int left_delay_samples = ((int)floorf(left_delay * sampling_rate)) + (PHONON_FRAME_SIZE / 4);
				int right_delay_samples = ((int)floorf(right_delay * sampling_rate)) + (PHONON_FRAME_SIZE / 4);
				int min_delay_samples = MIN(left_delay_samples, right_delay_samples);
				int delta_left_delay_samples = left_delay_samples - min_delay_samples;
				int delta_right_delay_samples = right_delay_samples - min_delay_samples;

				for (int i = 0, index = 0; i < PHONON_FRAME_SIZE; ++i)
				{
					for (int j = 0; j < 2; ++j, ++index)
					{
						int channel_delay_samples = (j == 0) ? delta_left_delay_samples : delta_right_delay_samples;
						int delay_samples = min_delay_samples + ((int)floorf(channel_delay_samples));
						int delayed_sample_index = PHONON_FRAME_SIZE + i - delay_samples;

						phononOutputBuf[index] = (1.0f - stereo_blend) * phononOutputBuf[index] + stereo_blend * (job.stereo_delay_line[2 * delayed_sample_index + j] / 32768.0f);
					}
				}
			}
		}
	...
	{
		if ( g_phononContext || s_bPhononInitFailed )
		{
			return;
		}
		...
		IPLRenderingSettings dspParams;
		...
		IPLerror phononError = iplCreateBinauralRenderer(iplContext, dspParams, hrtfParams, &g_phononContext);
		if (phononError != IPL_STATUS_SUCCESS)
		{
			s_bPhononInitFailed = true;
			DevMsg( "Failed to init phonon: %d (HRTF disabled)\n", (int)phononError );
			snd_hwcompat.SetValue( true );
			return;
		}
	...
	CThreadEvent g_eventPhononThreadJobComplete;
	...
	void PhononThread()
	{
		...
		while (!g_bPhononThreadExit)
		{
			...
			while (g_phononJobQueue.PopItem(&job))
			{
				...
				g_eventPhononThreadJobComplete.Set();
			}
...
ConVar snd_hrtf_async("snd_hrtf_async", "0", FCVAR_NONE, "");
ConVar snd_hrtf_lerp_method("snd_hrtf_lerp_method", "1", FCVAR_NONE, "");
...
public:
	...
	void SetDelayVoice()
	{
		m_flDelayUntil = Plat_FloatTime() + snd_hrtf_voice_delay.GetFloat();
	}
	...
	virtual bool IsReadyToMix()
	{
		...
		if ( m_flDelayUntil > 0.0f ) {
			if ( Plat_FloatTime() < m_flDelayUntil )
			{
				return false;
			}
...
CWaveDataHRTF::CWaveDataHRTF(IWaveData* pData, hrtf_info_t* dir) : m_pData(pData), m_pDir(dir), m_bJobPending(false), m_nDataStreamReadPos(0), m_bEOF(false), m_bAsync(snd_hrtf_async.GetBool()), m_bHasStartedMixing(false), m_bBenchmarking(snd_hrtf_benchmark.GetBool()), m_flDelayUntil(-1.0f)
{
	if ( m_bBenchmarking )
	{
		g_mutexBenchSkips.Lock();
...
void CWaveDataHRTF::TrySubmitJob()
{
	...
	if (nLoaded < PHONON_FRAME_SIZE)
	{
		...
		if( Source().IsLooped() )
		{
			// If the source is looped then try to read more data to fill out the frame.
			const int nAdditionalWanted = PHONON_FRAME_SIZE - nLoaded;
			const int nAdditionalLoaded = m_pData->ReadSourceData((void**)&buf, m_nDataStreamReadPos, nAdditionalWanted, copyBuf);

			if ( nAdditionalLoaded == nAdditionalWanted )
			{
				memcpy(&m_pJob->inputBuf[nBytesLoaded], buf, nAdditionalLoaded * sizeof(short) * m_nChannels);
				bEOF = false;
			}
	...
	m_pJob->stereo_blend = m_pDir->stereo_blend;
	m_pJob->bComplete = false;

	if ( snd_occlusion.GetBool() && m_pDir->use_occlusion )
	{
		m_pJob->bOcclude = true;
		m_pJob->path = m_pDir->path;
	}
	else
	{
		m_pJob->bOcclude = false;
	}

	m_pJob->bHRTF = IsUsingHRTF() && m_pDir->use_hrtf;
...
IWaveData *CreateWaveDataHRTFForVoice(IWaveData* pData, hrtf_info_t* dir)
{
	if (dir == nullptr)
		return pData;
...
bool RunHRTFEffect( const short* input, short* output, int nsamples, Vector dir, float lerp, float stereo_blend )
{
	static PhononJob* job = nullptr;
	if ( job == nullptr )
	{
		SetupPhononContext();
		...
		job->nChannels = 2;
		job->nDataWidth = 2;
		job->interpolation = IPL_HRTFINTERPOLATION_BILINEAR;
		job->lerp = lerp;
		job->stereo_blend = stereo_blend;
		job->bOcclude = false;
		job->bHRTF = true;
		job->phononEffect = 0;
		job->directSoundEffect = 0;
	}

	job->vec = dir;
	job->bComplete = false;

	memcpy( job->inputBuf, input, sizeof( job->inputBuf ) );

	g_phononMutex.Lock();
	ProcessPhononJob(*job);
	g_phononMutex.Unlock();

	memcpy( output, job->outputBuf, sizeof( job->outputBuf ) );
