...
ConVar snd_stream("snd_disable_legacy_audio_cache_hidden", "1", FCVAR_HIDDEN, "Disables using an audio cache and relies on streaming audio data instead." );
...
int CAudioSourceStreamWave::GetOutputData( void **pData, int64 samplePosition, int sampleCount, char copyBuf[AUDIOSOURCE_COPYBUF_SIZE] )
{
	...
	if ( GetType() == WAVE_FORMAT_PCM ) 
	{
		...
		if ( m_hWaveFileAccess != FILESYSTEM_INVALID_HANDLE )
		{
			// Don't fetch more samples than fit in the buffer
			if ( sampleCount * SampleSize() > AUDIOSOURCE_COPYBUF_SIZE )
				sampleCount = AUDIOSOURCE_COPYBUF_SIZE / SampleSize();
...
CAudioSource *CreateWave( CSfxTable *pSfx, bool bStreaming )
{
	...
	if ( IsPC() || !IsGameConsole() )
	{
		...
		if( pInfo && pInfo->IsLoading() )
		{
			pSfx->m_bIsLoading = true;
			return nullptr;
		}

		pSfx->m_bIsLoading = false;
...
float AudioSource_GetSoundDuration( char const *pName )
{
	...
	CSfxTable *pSfx = S_FindName( pName, nullptr );

	if(pSfx != nullptr && pSfx->pSource == nullptr)
	{
		//If we don't already have the sound data, try to load it now so we can determine the duration.
		pSfx = S_PrecacheSound(pName);
	}

	if ( pSfx )
	{
		return AudioSource_GetSoundDuration( pSfx );
	}
...
namespace
{
IThreadPool *g_AudioLoadingThreadPool;

class CRebuildAudioCachedInfoJob : public CJob
{
public:
	CRebuildAudioCachedInfoJob( CAudioSource* as, CAudioSourceCachedInfo* info ) : m_as( as ), m_info( info )
	{
		m_info->SetLoading( true );
		SetFlags( JF_QUEUE );
	}

	JobStatus_t DoExecute()
	{
		m_as->GetCacheData( m_info );
		delete m_as;

		m_info->SetLoading( false );
		return JOB_OK;
	}
private:
	CAudioSource* m_as;
	CAudioSourceCachedInfo* m_info;
};

static void StartAudioLoadingThreadPool()
{
	if ( g_AudioLoadingThreadPool == nullptr )
	{
		ThreadPoolStartParams_t params;
		params.nThreads = 1;
		params.nThreadsMax = 1;
		g_AudioLoadingThreadPool = CreateNewThreadPool();
		g_AudioLoadingThreadPool->Start(params);
	}
}
...
void ShutdownAudioLoadingThread()
{
	if ( g_AudioLoadingThreadPool != nullptr )
	{
		g_AudioLoadingThreadPool->Stop();
		DestroyThreadPool( g_AudioLoadingThreadPool );
		g_AudioLoadingThreadPool = nullptr;
	}
}
...
void CAudioSourceCachedInfo::Rebuild( char const *filename )
{
	...
	if ( as )
	{
		if ( snd_stream.GetBool() )
		{
			StartAudioLoadingThreadPool();
			CRebuildAudioCachedInfoJob* job = new CRebuildAudioCachedInfoJob( as, this );
			g_AudioLoadingThreadPool->AddJob( job );
			job->Release();
		}
		else
		{
...
CAudioSourceCache::CacheType_t *CAudioSourceCache::AllocAudioCache( char const *cachename, bool bNeverCheckDisk )
{
	...
	cache->SetNoSave( snd_stream.GetBool() );
	return cache;
}
...
CAudioSourceCachedInfo *CAudioSourceCache::GetInfo( int audiosourcetype, bool soundisprecached, CSfxTable *sfx )
{
	...
	if ( info && info->Format() == 0 && info->HasErrorLoading() == false )
	{
		if ( g_pFullFileSystem->FileExists( fn, "BSP" ) || ( snd_stream.GetBool() && strstr(fn, ".wav") ))
		{			
...
CAudioSourceCache::CacheType_t *CAudioSourceCache::BuildCacheFromList( char const *cachename, CUtlRBTree< FileNameHandle_t, int >& soundlist, bool showprogress /*= false*/, float flProgressStart /*= 0.0f*/, float flProgressEnd /*= 1.0f*/ )
{
	...
	newCache = AllocAudioCache( cachename, snd_stream.GetBool() );
	bool bInit = newCache->Init();
	Assert( newCache );
	if ( bInit && !snd_stream.GetBool() )
	{
