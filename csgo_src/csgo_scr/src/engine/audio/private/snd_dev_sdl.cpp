...
{
	if ( atoi( pOldString ) != snd_surround.GetInt() )
	{
...
void CAudioDeviceSDLAudio::OpenWaveOut( void )
{
	...
	int nChannelsDesired = snd_surround.GetInt();
	if ( nChannelsDesired == 0 )
	{
		//This really means 'headphones' which we want 2 channels for
		nChannelsDesired = 2;
		m_bHeadphone = true;
	}
	else if ( nChannelsDesired <= 0 || nChannelsDesired == 5 )
	{
		nChannelsDesired = 6;
	}
	...
	if ( snd_surround.GetInt() == 1 && m_numChannels == 2 )
	{
		//'headphones' setting.
		snd_surround.SetValue( 1 );
	}
	else if( m_numChannels == 6 )
	{
		snd_surround.SetValue( 5 );
	}
	else
	{
		snd_surround.SetValue( m_numChannels );
	}
	...
	if ( first_time )
	{
		...
		snd_surround.InstallChangeCallback( &OnSDLSpeakerChannelsChanged );
	}
}
