...
ConVar voice_scale("voice_scale", "1.0", FCVAR_ARCHIVE | FCVAR_RELEASE, "Overall volume of voice over IP 0.0-1.0", true, 0.0f, true, 1.0f );
ConVar voice_caster_scale( "voice_caster_scale", "1", FCVAR_ARCHIVE, "Caster Volume 0.0-1.0", true, 0.0f, true, 1.0f );
...
struct SVoiceVolumeInfo
{
	XUID player_id;
	float volume;
};

static CUtlVector<SVoiceVolumeInfo>* g_pVecVoiceVolumes;

float Voice_GetPlayerVoiceVolume( XUID player_id )
{
	if ( g_pVecVoiceVolumes == nullptr )
	{
		return 1.0f;
	}

	for ( int i = 0; i < g_pVecVoiceVolumes->Count(); ++i )
	{
		if ( (*g_pVecVoiceVolumes)[i].player_id == player_id )
		{
			return (*g_pVecVoiceVolumes)[i].volume;
		}
	}
...
void Voice_SetPlayerVoiceVolume( XUID player_id, float fVol )
{
	if ( g_pVecVoiceVolumes == nullptr )
	{
		g_pVecVoiceVolumes = new CUtlVector< SVoiceVolumeInfo >;
	}
	...
	for(int i = 0; i < g_pVecVoiceVolumes->Count(); ++i)
	{
		if((*g_pVecVoiceVolumes)[i].player_id == player_id)
		{
			(*g_pVecVoiceVolumes)[i].volume = fVol;
			return;
		}
	}

	struct SVoiceVolumeInfo info = { player_id, fVol };

	g_pVecVoiceVolumes->AddToTail( info );
}
...
public:
	...
	void Apply( short *pBuffer, int buffer_size, bool bCaster, float fUserTarget );
...
void CGainManager::Apply( short *pSamples, int nSamples, bool bCaster, float fUserTarget )
{
	if ( nSamples == 0 )
		return;
	...
	{
		m_fTargetGain = fUserTarget * ( voice_gain_target.GetFloat() * Clamp( voice_scale.GetFloat(), 0.0f, 2.0f ) ) / (float)iPeak;
	}
...
void Voice_AddIncomingData(
	...
	{
		player_info_t pi;
		if( engineClient->GetPlayerInfo( pChannel->m_iEntity, &pi ) && pi.xuid )
		{
			fUserVol = Voice_GetPlayerVoiceVolume( pi.xuid );
		}
	}

	pChannel->m_GainManager.Apply( decompressedBuffer, nDecompressedSamplesTotal, pChannel->m_bCaster, fUserVol );
...
CON_COMMAND( voice_player_volume, "" )
{
	//Don't allow showing of player names in overwatch
	if( engineClient->GetDemoPlaybackParameters() )
		return;

	if ( args.ArgC() == 1 )
	{
		Msg( "Player#     Player Name    Volume\n" );
		Msg( "-------     -----------    ------\n" );

		const int nMaxClients = engineClient->GetMaxClients();
		for ( int i = 1; i <= nMaxClients; ++i )
		{
			...
			Msg( " % 2d   % 17s      %0.2f\n", i, pi.name, Voice_GetPlayerVoiceVolume( pi.xuid ) );
		}
		Msg( "-------     -----------    ------\n" );
		Msg( "Use voice_player_volume player# volume -- to set the player's volume to the given amount\n" );
		return;
	}

	if ( args.ArgC() != 3 )
	{
		Msg( "Usage: voice_player_volume player#  -- where player# is a number you can find with \"voice_show_mute\"\n" );
		return;
	}

	int iUserId = Q_atoi(args.Arg(1));
	float fVol = Q_atof(args.Arg(2));
	player_info_t pi;
	if( !engineClient->GetPlayerInfo(iUserId, &pi) || !pi.xuid )
	{
		Msg("Player# is invalid. Use voice_player_volume to find a list of players\n");
		return;
	}
	...
	Voice_SetPlayerVoiceVolume( pi.xuid, fVol );
	Msg( "Set volume for %s to %0.2f\n", pi.name, fVol );
