...
bool CAudioDirectSound::Init( void )
{
	...
	if ( SNDDMA_InitDirect() == SIS_SUCCESS)
	{
		...
		if ( g_pBIK != NULL )
		{
			...
			if ( snd_surround_speakers.IsValid() && snd_surround_speakers.GetInt() >= 5 )
			{
...
{
	DWORD speaker_config = snd_surround_speakers.GetInt();
	if ( speaker_config < 0 )
	{
		...
		snd_surround_speakers.SetValue((int)speaker_config);
	}
...
void OnSndSurroundCvarChanged( IConVar *pVar, const char *pOldString, float flOldValue )
{
	...
	snd_surround_speakers.SetValue( (int)newSpeakerConfig );
