...
#include "vstdlib/asyncfunction.h"
...
struct SoundTryLockGuard {
	SoundTryLockGuard() : m_locked(false)
	{
	}
	~SoundTryLockGuard() {
		if(m_locked)
...
bool CSfxTable::IsPrecachedSound()
{
	...
	if ( sv.IsActive() )
	{
		...
		m_bIsPrecached = sv.LookupSoundIndex( pName ) != 0 ? true : false;
	}
	else
	{
		...
		// Client uses -1
		// WE SHOULD FIX THIS!!!
		m_bIsPrecached = ( GetBaseLocalClient().LookupSoundIndex( pName ) != -1 ) ? true : false;
	}

	m_bIsPrecachedValid = true;
...
ConVar snd_surround( "snd_surround_speakers", "-1", FCVAR_RELEASE|FCVAR_ARCHIVE );
#if USE_AUDIO_DEVICE_V1
...
ConVar snd_musicvolume( "snd_musicvolume_fixed", "1.0", FCVAR_HIDDEN, "Overall music volume" );
ConVar snd_dzmusic_volume("snd_dzmusic_volume", "0.2", FCVAR_ARCHIVE | FCVAR_RELEASE, "Relative volume of the Danger Zone victory music.");
...
ConVar snd_mvp_volume("snd_mvp_volume", "1.0", FCVAR_ARCHIVE | FCVAR_RELEASE, "Relative volume of the MVP music.");

ConVar snd_mixahead( "snd_mixahead", "0.025", FCVAR_ARCHIVE );
...
ConVar snd_mix_async( "snd_mix_async", "1", FCVAR_ARCHIVE | FCVAR_RELEASE, "Sets sound to get mixed asynchronously on a different thread" );
ConVar snd_mix_async_onetime_reset( "snd_mix_async_onetime_reset", "0", FCVAR_ARCHIVE | FCVAR_RELEASE | FCVAR_HIDDEN );

ConVar snd_music_volume_onetime_reset( "snd_music_volume_onetime_reset_2", "0", FCVAR_ARCHIVE | FCVAR_RELEASE | FCVAR_HIDDEN );
#ifdef _DEBUG
...
ConVar snd_mute_losefocus("snd_mute_losefocus", "1", FCVAR_ARCHIVE|FCVAR_NOT_CONNECTED);
static ConVar sound_device_override( "sound_device_override", "", FCVAR_RELEASE|FCVAR_ARCHIVE|FCVAR_NOT_CONNECTED, "ID of the sound device to use" );
...
void S_Startup( void )
{
	...
	if ( !g_AudioDevice )
	{
		...
		int nSpeakerConfig = snd_surround.GetInt();
		if ( nSpeakerConfig < 0 ) 
		{
			nSpeakerConfig = 0;
		}
		...
		if(nSpeakerConfig == 4)
		{
			//Just treat quad as 5.1 when seating up the device. We won't play anything out of the front channel.
			nSpeakerConfig = 5;
		}
...
void S_Init( void )
{
	...
	if ( !g_phononEngineScene )
	{
		...
		settings.numRays = 1024;
		settings.numDiffuseSamples = 16;
		settings.numBounces = 1;
		settings.irDuration = 1.0f;
		settings.ambisonicsOrder = 0;
		settings.maxConvolutionSources = 16;

		IPLerror result = iplCreateScene( g_phononEngineContext, nullptr, settings, 1, &g_phononEngineScene );
		if ( result != IPL_STATUS_SUCCESS )
		{
			DevMsg("Error initializing occlusion\n");
		}
		else
		{
			IPLMaterial mat;
			mat.scattering = 0.0f;
			mat.highFreqAbsorption = 1.0f;
			mat.highFreqTransmission = 0.0f;
			mat.midFreqAbsorption = 1.0f;
			mat.midFreqTransmission = 0.0f;
			mat.lowFreqAbsorption = 1.0f;
			mat.lowFreqTransmission = 0.0f;

			iplSetSceneMaterial( g_phononEngineScene, 0, mat );

			iplFinalizeScene( g_phononEngineScene, nullptr );

			result = iplCreateEnvironment( g_phononEngineContext, nullptr, settings, g_phononEngineScene, nullptr, &g_phononEngineEnvironment );
			if ( result != IPL_STATUS_SUCCESS )
			{
				DevMsg("Error initializing occlusion: environment failed\n");
			}
			else
			{
				IPLRenderingSettings rendering_settings;
				rendering_settings.convolutionType = IPL_CONVOLUTIONTYPE_PHONON;
				rendering_settings.frameSize = PHONON_FRAME_SIZE;
				rendering_settings.samplingRate = 44100;

				IPLAudioFormat output_format;
				output_format.channelLayoutType = IPL_CHANNELLAYOUTTYPE_SPEAKERS;
				output_format.channelLayout = IPL_CHANNELLAYOUT_MONO;
				output_format.numSpeakers = 1;
				output_format.speakerDirections = nullptr;
				output_format.channelOrder = IPL_CHANNELORDER_INTERLEAVED;

				result = iplCreateEnvironmentalRenderer( g_phononEngineContext, g_phononEngineEnvironment, rendering_settings, output_format, nullptr, nullptr, &g_phononEngineRenderer );
				if ( result != IPL_STATUS_SUCCESS )
				{
					DevMsg("Error initializing occlusion: renderer failed\n");
				}
			}
		}
	}
...
void ShutdownAudioLoadingThread();

CTSQueue< StartSoundParams_t >	g_QueuedSounds;
static CUtlVector< StartSoundParams_t* > g_pendingSounds;
...
void S_Shutdown(void)
{
	...
	g_QueuedSounds.RemoveAll();
	g_pendingSounds.RemoveAll();
...
CAudioSource *S_LoadSound( CSfxTable *pSfx, channel_t *ch, SoundError &soundError )
{
	...
	if ( ch && !ch->pMixer )
	{
		ch->pMixer = pSfx->pSource->CreateMixer(ch->initialStreamPosition, ch->skipInitialSamples, ch->flags.m_bUpdateDelayForChoreo, soundError, (ch->wavtype == CHAR_HRTF || ch->wavtype == CHAR_HRTF_BLEND) ? &ch->hrtf : nullptr);
		if ( !ch->pMixer )
		{
			return NULL;
		}
...
void S_SpatializeChannel( int nSlot, int arVolume[CCHANVOLUMES/2], int master_vol, const Vector *psourceDir, float gain, float mono )
{
 ...
 // add in distance effect
	scale = gain * rscale / 2;
	arVolume[IFRONT_RIGHT] = (int) (master_vol * scale);

	scale = gain * lscale / 2;
	arVolume[IFRONT_LEFT] = (int) (master_vol * scale);

	arVolume[IFRONT_RIGHT] = clamp( arVolume[IFRONT_RIGHT], 0, 255 );
	arVolume[IFRONT_LEFT] = clamp( arVolume[IFRONT_LEFT], 0, 255 );
...
DEVELOPMENT_ONLY_CONVAR( devonly_snd_music_lerp_default_duration, 1.0 );
DEVELOPMENT_ONLY_CONVAR( devonly_snd_music_lerp_voip_duration, 0.17 );
...
static float SND_GetMusicVolumeGainMultiplierInOverlay()
{
	...
	static double s_flTargetUpdateDuration = 0.0f;
	static float s_flLastKnownTakeoverVolume = 0.0f;
	...
	if ( flTimeNow - s_flLastUpdateTime > 0.1 )
	{
		...
		static ConVarRef cl_voip_lobby_audio_volume( "cl_voip_lobby_audio_volume" );
		float flCurrentTakeoverVolume = 0.0f;
		bool bPartyVoiceChat = ( cl_voip_lobby_audio_volume.IsValid() && cl_voip_lobby_audio_volume.GetBool() );
		bool bGameOverlayActive = Steam3Client().IsGameOverlayActive();
		
		// honor main menu embedded stream volume
		if ( cl_embedded_stream_video_playing.IsValid() )
			flCurrentTakeoverVolume = cl_embedded_stream_video_playing.GetFloat();

		// but if it is negligible then in-game overlay or lobby voip will duck music somewhat
		if ( flCurrentTakeoverVolume < 0.09f )
			flCurrentTakeoverVolume = ( bGameOverlayActive || bPartyVoiceChat ) ? 0.001f : 0.0f;

		if ( s_flLastKnownTakeoverVolume != flCurrentTakeoverVolume )
		{
			s_flMusicVolumeOverlayMultiplierPrevious = ( ( s_flTargetUpdateDuration <= 0 ) || ( flTimeNow > s_flLastUpdateTime + s_flTargetUpdateDuration ) )
				? s_flMusicVolumeOverlayMultiplierTarget
				: ( s_flMusicVolumeOverlayMultiplierPrevious + ( flTimeNow - s_flLastUpdateTime ) * ( s_flMusicVolumeOverlayMultiplierTarget - s_flMusicVolumeOverlayMultiplierPrevious ) / s_flTargetUpdateDuration );
			s_flLastUpdateTime = flTimeNow;
			s_flLastKnownTakeoverVolume = flCurrentTakeoverVolume;
			s_flMusicVolumeOverlayMultiplierTarget = ( flCurrentTakeoverVolume > 0 ) ? ( ( flCurrentTakeoverVolume >= 0.01f ) ? 0.0f : snd_musicvolume_multiplier_inoverlay.GetFloat() ) : 1.0f;

			// How fast do we fade?
			s_flTargetUpdateDuration = devonly_snd_music_lerp_default_duration.GetFloat();
			if ( bPartyVoiceChat && ( s_flMusicVolumeOverlayMultiplierTarget < s_flMusicVolumeOverlayMultiplierPrevious ) )
				s_flTargetUpdateDuration = devonly_snd_music_lerp_voip_duration.GetFloat();	// duck music faster for voice
		}
	}
	return ( ( s_flTargetUpdateDuration <= 0 ) || ( flTimeNow > s_flLastUpdateTime + s_flTargetUpdateDuration ) )
		? s_flMusicVolumeOverlayMultiplierTarget
		: ( s_flMusicVolumeOverlayMultiplierPrevious + ( flTimeNow - s_flLastUpdateTime ) * ( s_flMusicVolumeOverlayMultiplierTarget - s_flMusicVolumeOverlayMultiplierPrevious ) / s_flTargetUpdateDuration );
}
float SND_GetGain( int nSlot, gain_t *gs, const channel_t *ch, const Vector &vecListenerOrigin, bool fplayersound, bool fmusicsound, bool flooping, vec_t dist, bool bAttenuated, bool bOkayToTrace )
{
	...
	if ( ch->flags.m_bCompatibilityAttenuation )
	{
		...
		vec_t flSsound_nominal_clip_dist = 1000.0;
		float flGoldsrcDistMult = flAttenuation / flSsound_nominal_clip_dist;
...
float SND_GetGainObscured( int nSlot, gain_t *gs, const channel_t *ch, const Vector &vecListenerOrigin, bool fplayersound, bool flooping, bool bAttenuated, bool bOkayToTrace, Vector *pOrigin )
{
	...
	if (tr.DidHit() && tr.fraction < 0.99)
	{
		...
		for (count = 0, i = 0; i < 4; i++)
		{
			...
			Ray_t rayCorner;
			rayCorner.Init( MainViewOrigin( nSlot ), endpoints[i] );
			g_pEngineTraceClient->TraceRay( rayCorner, MASK_BLOCK_AUDIO, &filter, &tr );
...
static ConVar sv_voice_proximity_minvolume( "sv_voice_proximity_minvolume", "0", FCVAR_REPLICATED | FCVAR_RELEASE );

static ConVar voice_positional_seconds_after_death( "voice_positional_seconds_after_death", "6.0", FCVAR_RELEASE );
...
void SND_Spatialize(channel_t *ch)
{
	...
	if (ch->wavtype == CHAR_HRTF || ch->wavtype == CHAR_HRTF_BLEND)
	{
		Vector origin;
		...
		if (pEnt != nullptr)
		{
			...
			static ConVarRef sv_voice_proximity_positional( "sv_voice_proximity_positional" );

			if ( ch->hrtf.is_positional_voice && g_ClientDLL->PlayerOnSameTeamAsLocalPlayer( pEnt ) == false && !sv_voice_proximity_positional.GetBool() )
			{
				// positional voice shouldn't play positionally for opposing players.
				ch->hrtf.stereo_blend = 1.0f;
				ch->hrtf.use_hrtf = 0.0f;
			}

			else if ( ch->hrtf.is_positional_voice && voice_positional_seconds_after_death.GetFloat() >= 0.0f )
			{
				//If we're using positional voice, but the player is dead, if we have voice_positional_seconds_after_death,
				//we want to continue playing voice positionally for that amount of time before quickly lerping to
				//playing it non-positionally.
				float timeSinceDeath = 0.0f;
				bool isDead = g_ClientDLL->PlayerDeathInfo(pEnt, &timeSinceDeath);

				ch->hrtf.stereo_blend = 0.0f;
				ch->hrtf.use_hrtf = 1.0f;

				if ( isDead && timeSinceDeath > voice_positional_seconds_after_death.GetFloat() )
				{
					float t = timeSinceDeath - voice_positional_seconds_after_death.GetFloat();
					...
					ch->hrtf.stereo_blend = t;
					ch->hrtf.use_hrtf = 1.0f - t;

					origin = origin*(1.0f - t) + listener_origin[0]*t;
				}
			}
		}
		else
		...
		if (ch->hrtf.lock_position == false)
		{
			QAngle listener_angles;

			Vector listener_forward_vec = listener_forward[0];
			...
			Vector listener_origin_modified;

			if ( ch->hrtf.is_radio )
			{
				listener_forward_vec.Zero();
				listener_forward_vec.z = 1.0f;

				listener_origin_modified.Zero();

				//only calculate the position once, locked in for the rest of playing the sound.
				ch->hrtf.lock_position = true;
			}
			else
			{
				listener_origin_modified = listener_origin[0] - snd_hrtf_distance_behind.GetFloat()*listener_forward_vec;
				const float dist_to_sound = MIN((listener_origin_modified - origin).Length(), (listener_origin[0] - origin).Length());
				if ( dist_to_sound < snd_hrtf_distance_behind.GetFloat() )
				{
					listener_origin_modified = listener_origin[0] - dist_to_sound*listener_forward_vec;
				}
			}

			// sound_pos is really sound_pos_listener_relative
			Vector sound_pos = listener_origin_modified - origin;
			VectorAngles(listener_forward_vec, listener_angles);
		...
		if (ch->wavtype == CHAR_HRTF_BLEND && fDistance < fMinDistance)
		{
			ch->hrtf.stereo_blend = 1.0f;
		}
		else if (ch->wavtype == CHAR_HRTF_BLEND && fMinDistance <= fDistance && fDistance <= fMaxDistance)
		{
			ch->hrtf.stereo_blend = 1.0f - ((fDistance - fMinDistance) / (fMaxDistance - fMinDistance));
		}
	}
	...
	if ( fdopplerwav )
	{
		...
		FOR_EACH_VALID_SPLITSCREEN_PLAYER( nSlot )
		{
			...
			{
				float flDist = (nearPoint - listener_origin[nSlot]).Length();
				if ( flDist < bestDist )
				{
					dopplerSlot = nSlot;
					bestDist = flDist;
		...
		ch->sfx->pSource->GetType() == CAudioSource::AUDIO_SOURCE_VOICE )
	{
		...
		static ConVarRef sv_voice_proximity("sv_voice_proximity");

		if(sv_voice_proximity.GetFloat() > 0.0f)
		{ 

			float voice_dist = FLT_MAX;
			...
			FOR_EACH_VALID_SPLITSCREEN_PLAYER(hh)
			{
				...
				// normalize source_vec and get distance from listener to source
				float checkDist = VectorNormalize(source_vec[hh]);
				if(checkDist < voice_dist)
				{
					voice_dist = checkDist;
				}
			}

			float volume_adjustment = 0.0f;

			if ( voice_dist < sv_voice_proximity.GetFloat() )
			{
				float r = 1.0f - ( voice_dist / sv_voice_proximity.GetFloat() );
				volume_adjustment = r + (1.0f - r) * sv_voice_proximity_minvolume.GetFloat();
			}
			else
			{
				volume_adjustment = sv_voice_proximity_minvolume.GetFloat();
			}

			gain *= volume_adjustment;
		}
...
{	
	...
	if ( TestSoundChar( pSndName, CHAR_MUSIC ) )
		target_chan->wavtype = CHAR_MUSIC;

	static ConVarRef panorama_sound_pos("@panorama_sound_pos");

	if ( ( IsUsingHRTF() || snd_occlusion.GetBool() ) && (TestSoundChar(pSndName, CHAR_HRTF) || TestSoundChar(pSndName, CHAR_HRTF_BLEND) || TestSoundChar(pSndName, CHAR_HRTF_FORCE)) && (!IsSoundSourceViewEntity(target_chan->soundsource) || TestSoundChar(pSndName, CHAR_HRTF_FORCE) || (TestSoundChar(pSndName, CHAR_RADIO) && panorama_sound_pos.GetBool()) || target_chan->hrtf.lock_position))
	{
		target_chan->wavtype = TestSoundChar(pSndName, CHAR_HRTF_BLEND) ? CHAR_HRTF_BLEND : CHAR_HRTF;
		target_chan->hrtf.lerp = 1.0; //snd_hrtf_ratio.GetFloat();
		target_chan->hrtf.use_hrtf = IsUsingHRTF();
		target_chan->hrtf.is_radio = TestSoundChar( pSndName, CHAR_RADIO );
	}

	else if ( TestSoundChar( pSndName, CHAR_RADIO ) )
...
static int S_StartSound_Immediate( StartSoundParams_t& params )
{
	...
	static int g_nSoundDelay = 0;
	static int g_nSoundPlay = 0;

	SoundTryLockGuard guardSound;

	if ( guardSound.TryLock() == false )
	{
		//If the sound system happens to be locked, don't hang waiting for the mutex. Instead queue the sound to play later.
		StartSoundParams_t* paramsCopy = new StartSoundParams_t;
		params.Copy(*paramsCopy);
		paramsCopy->m_nQueuedGUID = nGUID;
		g_pendingSounds.AddToTail(paramsCopy);
	...
	if(params.pSfx != nullptr && params.pSfx->pSource != nullptr && params.pSfx->pSource->IsLooped() && params.soundsource >= 1 && params.soundsource < entitylist->GetMaxEntities())
	{
		//Check to see if this is a looped sound and the entity playing it is currently non-existent or dormant. In that case
		//we don't start it.
		IClientEntity* pEnt = entitylist->GetClientEntity(params.soundsource);
		if(pEnt != nullptr && pEnt->IsDormant())
		{
			return 0;
		}
	}
...
// 		}
	...
	ch->hrtf.is_positional_voice = false;
	ch->hrtf.bilinear_filtering = params.m_bHRTFBilinear;
	ch->hrtf.lock_position = params.m_bHRTFLock;
	ch->hrtf.is_radio = false;
	ch->hrtf.use_occlusion = false;
	ch->hrtf.use_hrtf = false;
	ch->hrtf.ignore_occlusion = params.m_bIgnoreOcclusion;


	if (ch->hrtf.lock_position)
	{
...
// 			pStackList->SetScriptHash( params.m_nSoundScriptHash );
	...
	ch->flags.m_bAlertSound = params.m_bAlertSound;
	...
	else
	{
		...
		if ( !pSource && !params.pSfx->m_bIsLateLoad )
		{
			...
			case SE_CANT_CREATE_MIXER:
				pText = "File is missing from disk or is invalid.";
				break;
			}
			if ( pText != NULL )
			{
	...
	{
		//If we are mixing sounds asynchronously we should spatialize the sound now in case
		//there is a hitch immediately after the sound is played, which would cause us to
		//play the sound without correct spatialization for the duration of the hitch.
		SND_Spatialize(ch);
	}
...
static void PumpPendingSounds()
{
	CUtlVector< StartSoundParams_t* > v;
	for ( int i = 0; i < g_pendingSounds.Count(); ++i )
	{
		v.AddToTail( g_pendingSounds[i] );
	}
...
int S_StartSound( StartSoundParams_t& params )
{
	...
	if ( params.pSfx != NULL )
	{
		params.pSfx->GetFileName( nameBuf, sizeof(nameBuf) );

		if ( snd_stream.GetBool() )
		{
			SoundError soundError;
			S_LoadSound( params.pSfx, nullptr, soundError );

			if ( params.pSfx->m_bIsLoading )
			{
				StartSoundParams_t* paramsCopy = new StartSoundParams_t;
				params.Copy( *paramsCopy );
				paramsCopy->m_nQueuedGUID = guid;
				g_pendingSounds.AddToTail( paramsCopy );
...
// 	}
	...
	pStartParams.m_bIgnoreOcclusion = pScriptParams.m_bIgnoreOcclusion;
	...
	if ( pStartParams.fromserver && snd_show_events.GetBool() )
	{
		char name[256];
		DevMsg( "SoundEvent: %s: %s (from server)\n", pStartParams.m_pSoundEntryName, pStartParams.pSfx->getname( name, sizeof(name) ) );
	}
...
bool S_IsSoundStillPlaying( int guid )
{
	...
	for (int i = 0; i != g_pendingSounds.Count(); ++i)
	{
		if(g_pendingSounds[i]->m_nQueuedGUID == guid)
		{
			return true;
		}
	}
...
bool S_SetOriginByGuid( int guid, const Vector& origin )
{
	SoundTryLockGuard guardSound;

	if ( guardSound.TryLock() )
	{
		if(channel_t *pChannel = S_FindChannelByGuid(guid))
		{
			pChannel->origin = origin;
			pChannel->hrtf.follow_entity = false;
			pChannel->flags.bUpdatePositions = false;

			pChannel->hrtf.lerp = 1.0f;
...
void S_GetActiveSounds( CUtlVector< SndInfo_t >& sndlist )
{
	...
	for ( int i = 0; i < list.Count(); i++ )
	{
		...
		info.m_bIsMusic			= ch->sfx != nullptr && ch->sfx->m_bIsMusic;

		sndlist.AddToTail( info );
	}
}

void S_StopAllSounds( bool bClear, bool bStopMainMenuMusic )
{
	THREAD_LOCK_SOUND();
	int		i;
	...
	for ( i = 0; i < list.Count(); i++ )
	{
		...
		if ( bStopMainMenuMusic == false && pChannel->sfx && ( V_stristr( pName, "mainmenu_press_play" ) || V_stristr( pName, "game_ready" ) ) )
		{
			// Don't stop the sound of the 'play' button having been pressed to start into a game, so it doesn't
			// cut out during the loading screen.
			continue;
		}

		if( bStopMainMenuMusic == false && pChannel->sfx && pChannel->sfx->m_bIsMusic && V_stristr(pName, "mainmenu") )
		{
			//don't stop main menu music.
			continue;
		}
	...
	g_pendingSounds.PurgeAndDeleteElements();
...
//	memset( &soundfade, 0, sizeof( soundfade ) );
	...
	Assert( (bStopMainMenuMusic && g_ActiveChannels.GetActiveCount() == 0) || (bStopMainMenuMusic == false && g_ActiveChannels.GetActiveCount() <= 1) );
}
...
void S_Update( const CAudioState *pAudioState )
{
	...
	//Iterate over the playing channels and remove any sounds that are looped and playing on an entity that has
	//become dormant.
	{
		CChannelList list;
		g_ActiveChannels.GetActiveChannels(list);
		...
		{
			channel_t* ch = list.GetChannel(i);
			if(ch != nullptr && ch->sfx != nullptr && ch->sfx->pSource != nullptr && ch->sfx->pSource->IsLooped() && ch->soundsource >= 1 && ch->soundsource < entitylist->GetMaxEntities())
			{
				IClientEntity* pEnt = entitylist->GetClientEntity(ch->soundsource);
				if(pEnt != nullptr && pEnt->IsDormant())
				{
					S_StopChannelUnlocked(ch);
				}
			}
		}
	}
	...
	{
		channel_t	*ch = stopList.GetChannel(i);
	...
	if ( snd_show.GetInt() || snd_show_print.GetInt())
	{
		...
		for ( int idxChan = 0; idxChan < list.Count(); idxChan++ )
		{
			channel_t *ch = list.GetChannel(idxChan);
			if ( !ch->sfx || !ch->sfx->pSource )
				continue;
			...
			if (snd_show.GetInt())
			{
				if (ch->wavtype == CHAR_HRTF || ch->wavtype == CHAR_HRTF_BLEND)
				{
					...
					Con_NXPrintf(&np, "%s %02i hrtf(%.2f %.2f %.2f) yaw(%.2f) pitch(%.2f) xfade(%.2f) vol(%03d) ent(%03d) pos(%6d %6d %6d) timeleft(%f) looped(%d) %50s",
						nSoundEntryName,
						total + 1,
						ch->hrtf.vec.x,
						ch->hrtf.vec.y,
...
CON_COMMAND_F( snd_dumpclientsounds, "Dump sounds to console", FCVAR_CHEAT )
{
	S_DumpClientSounds();
// 	con_nprint_t np;
// 	np.time_to_live = 2.0f;
...
//	time.
	...
	{
		//sound system has been shut down by main thread.
		return;
	}
...
ConVar snd_mix_async_frequency("snd_mix_async_frequency", "120.0");

float S_MixAsync()
{
	float flMixTime = (1.0f / snd_mix_async_frequency.GetFloat());

	S_Update_Guts(flMixTime + snd_mixahead.GetFloat());
	return flMixTime;
}

static AsyncFunctionHandle_t g_nMixFunction = INVALID_ASYNC_FUNCTION_HANDLE;

void S_ShutdownMixThread()
{
	if ( g_nMixFunction != INVALID_ASYNC_FUNCTION_HANDLE )
	{
		g_pScheduledFunctions->UnregisterScheduledFunction(g_nMixFunction);
		g_nMixFunction = INVALID_ASYNC_FUNCTION_HANDLE;
	}
}
...
{
	if ( IsUsingHRTF() || snd_occlusion.GetBool() )
	{
		StartPhononThread();
	}

	if ( !snd_mix_async_onetime_reset.GetBool() )
	{
		snd_mix_async.SetValue( "1" );
		snd_mixahead.SetValue( "0.025" );
		snd_mix_async_onetime_reset.SetValue( "1" );
	}

	if ( !snd_music_volume_onetime_reset.GetBool() )
	{
		snd_roundend_volume.SetValue("0");
		snd_roundstart_volume.SetValue("0");
		snd_menumusic_volume.SetValue("0.3");
		snd_tensecondwarning_volume.SetValue("0");
		snd_mapobjective_volume.SetValue("0");
		snd_deathcamera_volume.SetValue("0.3");
		snd_music_volume_onetime_reset.SetValue("1");
	}
	...
	else
	{
		if (g_nMixFunction == INVALID_ASYNC_FUNCTION_HANDLE)
		{
			g_nMixFunction = g_pScheduledFunctions->RegisterScheduledFunction(UtlMakeDelegate(S_MixAsync), Plat_FloatTime());
		}
	}
}
...
//A function which makes the main thread hitch for n seconds, used to
//debug what happens to the sound system in the event of hitches.
static void S_DebugSleep(const CCommand& args)
{
	float delay = 1.0f;
...
static ConCommand snd_debug_sleep("snd_debug_sleep", S_DebugSleep);
...
CON_COMMAND_F( snd_print_channel_by_index, "Prints the content of a channel from its index. snd_print_channel_by_index <index>.", FCVAR_CHEAT )
{
	if ( args.ArgC() != 2 )
	{
...
CON_COMMAND_F( snd_print_channel_by_guid, "Prints the content of a channel from its guid. snd_print_channel_by_guid <guid>.", FCVAR_CHEAT )
{
	if ( args.ArgC() != 2 )
	{
...
CON_COMMAND_F( snd_print_channels, "Prints all the active channel.", FCVAR_CHEAT)
{
	AUTO_LOCK( g_SndMutex );
...
CON_COMMAND_F( snd_set_master_volume, "Sets the master volume for a channel. snd_set_master_volume <guid> <mastervolume>.", FCVAR_CHEAT )
{
	if ( args.ArgC() != 3 )
	{
