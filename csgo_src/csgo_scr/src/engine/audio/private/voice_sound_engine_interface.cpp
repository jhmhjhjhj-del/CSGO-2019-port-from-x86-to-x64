...
static const int nUpsampleVoiceFactor = 2;

static ConVar sv_voice_proximity_use_falloff( "sv_voice_proximity_use_falloff", "0", FCVAR_REPLICATED | FCVAR_RELEASE );
static ConVar voice_positional( "voice_positional", "0", FCVAR_RELEASE | FCVAR_ARCHIVE );
...
public:
	...
	virtual int					GetType( void )
	{
		...
		return ( sv_voice_proximity_positional.GetBool() && sv_voice_proximity_use_falloff.GetBool() ) ? AUDIO_SOURCE_WAV : AUDIO_SOURCE_VOICE;
	}
...
public:
	...
	{
		static ConVarRef sv_voice_proximity_positional("sv_voice_proximity_positional");
		const char *pName = ( sv_voice_proximity_positional.GetBool() || voice_positional.GetBool() ) ? "~?VoiceSfx" : "?VoiceSfx";
	...
	: CAudioSourceWave( pSfx )
{
	...
	tmp.nSamplesPerSec = Voice_SamplesPerSec()*nUpsampleVoiceFactor;
...
extern IWaveData *CreateWaveDataHRTFForVoice(IWaveData* pData, hrtf_info_t* dir);
...
CAudioMixer *CAudioSourceVoice::CreateMixer( int initialStreamPosition, int skipInitialSamples, bool bUpdateDelayForChoreo, SoundError &soundError, struct hrtf_info_t* pHRTFVec )
{
	...
	static ConVarRef sv_voice_proximity_positional( "sv_voice_proximity_positional" );
	if ( !sv_voice_proximity_positional.GetBool() && !voice_positional.GetBool() )
	{
		pHRTFVec = nullptr;
	}
	else if ( pHRTFVec != nullptr )
	{
		pHRTFVec->is_positional_voice = true;
	}

	CAudioMixer *pMixer = CreateWaveMixer( CreateWaveDataHRTFForVoice( pVoice, pHRTFVec), WAVE_FORMAT_PCM, pHRTFVec ? 2 : 1, BYTES_PER_SAMPLE*8, initialStreamPosition, skipInitialSamples, bUpdateDelayForChoreo );
	if(!pMixer)
	{
		delete pVoice;
		soundError = SE_CANT_CREATE_MIXER;
...
{
	char tmpBuf[AUDIOSOURCE_COPYBUF_SIZE];

	if ( sampleCount < 0 || ( sampleCount > ( AUDIOSOURCE_COPYBUF_SIZE / BYTES_PER_SAMPLE ) ) )
	{
		sampleCount = AUDIOSOURCE_COPYBUF_SIZE / BYTES_PER_SAMPLE;
	}
	int nSamplesRequested = sampleCount / nUpsampleVoiceFactor;

	int nSamplesGotten = Voice_GetOutputData(
		m_iChannel,
		nUpsampleVoiceFactor == 1 ? copyBuf : tmpBuf,
		AUDIOSOURCE_COPYBUF_SIZE,
		(int)samplePosition,
		nSamplesRequested );
	
	COMPILE_TIME_ASSERT( sizeof( short ) == BYTES_PER_SAMPLE );
	if ( nUpsampleVoiceFactor != 1 )
	{
		short* src = (short*)tmpBuf;
		short* dst = (short*)copyBuf;
		int n;
		for ( n = 0; n < nSamplesGotten-1; ++n )
		{
			dst[n*2] = src[n];
			...
			lerp += src[n+1];
			lerp /=2;
			dst[n*2+1] = (short)lerp;
		}

		dst[n*2] = src[n];
		dst[n*2+1] = src[n];

		nSamplesGotten *= nUpsampleVoiceFactor;
	}
	...
	if( nSamplesGotten < sampleCount )
	{
		if ( copyBuf != NULL )
		{
			memset( copyBuf + ( nSamplesGotten * BYTES_PER_SAMPLE ), 0, ( sampleCount - nSamplesGotten ) * BYTES_PER_SAMPLE );
		}
...
{
	return Voice_SamplesPerSec()*nUpsampleVoiceFactor;
}
	...
	int nViewEntityIndex )
{
	...
	if ( sv_voice_proximity_positional.GetBool() || voice_positional.GetBool() )
	{
		params.speakerentity = iEntity;
		params.m_bHRTFFollowEntity = true;
	}
