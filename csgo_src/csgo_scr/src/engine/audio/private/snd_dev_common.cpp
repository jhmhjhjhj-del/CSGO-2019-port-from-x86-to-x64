...
static void InterpSpeakerVol( float flRadialWeight, snd_pan_mode_t nPanMode, float flExponent, float flYawSource, float flSpeakerMin, float flSpeakerMax, float &flMinValue, float &flMaxValue, bool bDraw = true)
{
	...
	if ( flSpeakerMax - flSpeakerMin == 0.0f )
	{
		//this can happen if min/max are equal and the yaw is exactly equal to both.
		//Results in flFactor = 0.0f/0.0f so avoid it by special casing it here.
		Assert( flYawSource - flSpeakerMin == 0.0f );
		flFactor = 0.0f;
	}
	else
	{
		//the common case.
		flFactor = ( flYawSource - flSpeakerMin ) / (flSpeakerMax - flSpeakerMin );
	}
...
void IAudioDevice2::TransferSamples( uint32 )
{
	...
	if ( SND_IsRecording() )
	{
		...
		{
			short dataBuf[ MIX_BUFFER_SIZE*2 ];
			ConvertFloat32Int16_Clamp_Interleave2( dataBuf, mixBuffers[0].m_flData, mixBuffers[1].m_flData, MIX_BUFFER_SIZE );
			WaveAppendTmpFile( cl_movieinfo.moviename, dataBuf, 16, MIX_BUFFER_SIZE*2 );
		}
	}

	AudioStreamMix( ChannelCount(), mixBuffers );
