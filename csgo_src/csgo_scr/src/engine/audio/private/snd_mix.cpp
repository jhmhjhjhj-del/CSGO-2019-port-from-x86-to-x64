...
double MIX_GetMaxRate( double rate, int sampleCount )
{
	...
	// don't overflow signed fixedint for rate either.
	// The conversion to/from int is a floor() operation, making sure that we don't overflow
	// even when adding a fractional amount close to 1 to this value.
	//
	// This limits our pitch shifting to a maximum of just under 8.0
	const double fix_rate_max = FIX_TODOUBLE( FIX( FIX_INTPART( fixedint( 0x7fffffff ) ) ) );
	rate_max = fpmin( rate_max, fix_rate_max );
...
bool MIX_ScaleChannelVolume( paintbuffer_t *ppaint, channel_t *pChannel, float volume[CCHANVOLUMES], int mixchans )
{
	...
	extern IVEngineClient *engineClient;
	if ( snd_mute_losefocus.GetBool() && engineClient && !engineClient->IsActiveApp() && !pChannel->flags.m_bAlertSound )
	{
		for (i = 0; i < CCHANVOLUMES; i++)
		{
			volume[i] = 0.0f;
...
static ConVar snd_mainmusic_hrtf("snd_mainmusic_hrtf", "0.0", FCVAR_NONE, "Makes main menu music sound like a radio");
static ConVar snd_mainmusic_hrtf_transition_speed("snd_mainmusic_hrtf_transition_speed", "0.03", FCVAR_NONE, "Makes main menu music sound like a radio");

void SW_Mix16Stereo_MusicHRTF(portable_samplepair_t * RESTRICT pOutput, float * RESTRICT pVolume, short * RESTRICT pData, int nInputOffset, fixedint nRateScaleFix, int nOutCount, float lerp)
{
	static float prev_lerp = -1.0f;

	if( g_AudioDevice != nullptr && g_AudioDevice->MixChannelCount() != 2 )
	{
		//not stereo output, so can't use HRTF.
		lerp = 0.0f;
	}
	else if( prev_lerp >= 0.0f && lerp != prev_lerp )
	{
		float fTransitionSpeed = snd_mainmusic_hrtf_transition_speed.GetFloat();
		if( lerp > prev_lerp )
		{
			prev_lerp += fTransitionSpeed;
			if(prev_lerp > lerp) {
				prev_lerp = lerp;
			}
		}
		else if ( lerp < prev_lerp )
		{
			prev_lerp -= fTransitionSpeed;
			if(prev_lerp < lerp) {
				prev_lerp = lerp;
	...
	if ( lerp <= 0.0f || nOutCount != PHONON_FRAME_SIZE )
	{
	...
	short hrtf_out[1024];
	RunHRTFEffect(pData, hrtf_out, PHONON_FRAME_SIZE, Vector(0.1f, 0.0f, -1.0f), 1.0f, 0.0f);

	int nSampleIndex = 0;
	fixedint nSampleFrac = nInputOffset;

	float fLeftLowQuality = 0.0f;
	float fRightLowQuality = 0.0f;
	int nSampleWindow = 1;

	float invr = 1.0f - lerp;

	for(int i = 0; i < nOutCount; i++)
	{
		float fLeftHighQuality = (pVolume[0] * (int)(pData[nSampleIndex])) / 256.0f;
		float fRightHighQuality = (pVolume[1] * (int)(pData[nSampleIndex + 1])) / 256.0f;

		if((i%nSampleWindow) == 0)
		{
			int nLeft = pVolume[0] * (int)(hrtf_out[nSampleIndex]);
			int nRight = pVolume[1] * (int)(hrtf_out[nSampleIndex + 1]);

			fLeftLowQuality = nLeft / 32.0f;
			fRightLowQuality = nRight / 32.0f;
		}

		pOutput[i].left += fLeftLowQuality*lerp + fLeftHighQuality*invr;
		pOutput[i].right += fRightLowQuality*lerp + fRightHighQuality*invr;
...
void Mix16StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput, float *volume, short *pData, int inputOffset, fixedint rateScaleFix, int outCount)
{
	...
	if ( ( nWavType == CHAR_HRTF || nWavType == CHAR_HRTF_BLEND || nWavType == CHAR_HRTF_FORCE ) && pChannel->hrtf.use_hrtf == false )
	{
		nWavType = 0;
	}
	...
	{
	case CHAR_HRTF:
	case CHAR_HRTF_BLEND:
	case CHAR_HRTF_FORCE:
	...
	case CHAR_MUSIC:
		SW_Mix16Stereo_MusicHRTF( pOutput, volume, pData, inputOffset, rateScaleFix, outCount, snd_mainmusic_hrtf.GetFloat() );
		break;
	default:
	case CHAR_SPATIALSTEREO:
		if ( bUseHighQualityPitch )
