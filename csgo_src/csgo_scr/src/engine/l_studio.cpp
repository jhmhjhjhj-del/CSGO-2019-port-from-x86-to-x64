...
#endif
static ConCommand r_flushlod( "r_flushlod", FlushLOD_f, "Flush and reload LODs.", FCVAR_CHEAT );
...
extern ConVar r_staticlight_streams;
...
void CModelRender::DebugDrawLightingOrigin( const DrawModelState_t& state, const ModelRenderInfo_t &pInfo )
{
	...
	const model_t *pModel = pInfo.pRenderable->GetModel();
	if ( pModel )
	{
		Vector vecTextPos = (state.m_pStudioHdr->hull_max + state.m_pStudioHdr->hull_min) * 0.5f;
		VectorTransform( vecTextPos, *state.m_pModelToWorld, pt0 );
		CDebugOverlay::AddTextOverlay( pt0, 0, 0, 255, 255, 255, 255, CFmtStr( "%s", pModel->szPathName ) );
	}
...
bool CModelRender::UpdateStaticPropColorData( IHandleEntity *pProp, ModelInstanceHandle_t handle )
{
	...
	if ( !bDebugColor && ( inst.m_nFlags & MODEL_INSTANCE_HAS_DISKCOMPILED_COLOR ) )
	{
		...
		// if we get here we might have a problem - only valid return false paths from LoadStaticPropColorData are:
		//	  1. map has no baked lighting || r_proplightingfromdisk is set to false
		//	  2. colormeshhandle no longer valid/not in cache
		// If 2, we have a problem - between the FindOrCreateStaticPropColorData at the top of this fn, and the LoadStaticPropColorData call
		// the colormeshdata has been evicted or destroyed. This is bad because we're about to write into that data below. 
		// This seems unlikely but we're noticing a number of minidump crashes where we have invalid colormeshinfo (the meshbuilder dies on invalid mesh ptrs),
		// so speculatively protecting against that below (**).
	}
	...
	{
		ModelInstance_t& instance = m_ModelInstances[ handle ];
		if ( !CacheGet( instance.m_ColorMeshHandle ) )
		{
			return false;
		}
	}
	...
	// (**) make doubly sure we have valid mesh data to write into (see note above inside LoadStaticPropColorData block)
	if ( pColorMeshData->m_nMeshes <= 0 )
	{
		return false;
	}
...
void CModelRender::RestoreAllStaticPropColorData( void )
{
	...
	// RestoreAllStaticPropColorData() only called from RestoreMaterialSystemObjects in matsys_interface.cpp
	// That call is followed by a call to StaticPropMgr()->RestoreStaticProps, which in turn will recompute static lighting for all props (ultimately calling UpdateStaticPropColorData)
	// This loop here was also somewhat dangerous as all models, not just static props, were having their static lighting data updated/computed - this included all the character models, and other dyn objects
	// I've added the check for static props here in case this path is resurrected.
/*
	// rebake
	FOR_EACH_LL( m_ModelInstances, i )
	{
		if ( m_ModelInstances[ i ].m_pModel && ( m_ModelInstances[ i ].m_pModel->flags & MODELFLAG_STUDIOHDR_IS_STATIC_PROP ) )
		{
