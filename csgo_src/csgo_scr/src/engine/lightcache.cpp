...
static int g_CacheEntriesBuiltThisFrame = 0;
static int g_MaxCacheEntriesBuiltPerFrame = 0;
static bool g_HighwaterCacheEntries = false;
...
static float LightIntensityAndDirectionInBox( dworldlight_t* pLight, 
											  ...
											  Vector *pDirection, bool bStaticProp = false ) 
{
	...
	if ( !r_oldlightselection.GetBool() )
	{
		...
		case emit_skylight:
			{
				int flags = fFlags | LIGHT_NO_RADIUS_CHECK;

				if ( !bStaticProp )
					flags |= LIGHT_OCCLUDE_VS_PROPS;

				// init intensity with value from center
				float flMaxIntensity = LightIntensityAndDirectionAtPoint(pLight, pZBuf, mid, flags, NULL, pDirection);

				// test for skylight contribution at voxel corners in addition to voxel center. 
				// just use the center on low csm detail mode (or possibly TODO - when beyond the furthest cascade)
				if ( materials->GetCurrentConfigForVideoCard().GetCSMQualityMode() > CSMQUALITY_VERY_LOW )
				{
					Vector vecCorners[8];
					...
					for ( int i = 0; i < 8; i++ )
						flMaxIntensity = MAX( flMaxIntensity, LightIntensityAndDirectionAtPoint( pLight, pZBuf, vecCorners[i], flags, NULL, pDirection ) );
				}
...
//-----------------------------------------------------------------------------
// Does the CSM light exist in lighting state?
//-----------------------------------------------------------------------------
static bool DoesLightingStateHaveCSMLight( dworldlight_t** pLights, int nWorldLights )
{
	for (int i = 0; i < nWorldLights; i++)
	{
		if ( pLights[ i ]->type == emit_skylight )
		{
			return true;
		}
	}
...
//
// Note:
// When we compile VS and PS data, we have the possibility that one of the lights we add here
// will be completely discarded. PS and VS lighting model assumes one dlight is the CSM light, so if 
// none of lightingState lights is the CSM light (type == emit_skylight), one will get discarded
// and replaced with an empty/null light (see CanonicalizeMaterialLightingStateForCSM)
// We check for that situation here and ensure we add the light that would have been discarded to the ambient cube
// just as we do when there are no remaining slots and we swap out the darkest light below.
// CanonicalizeMaterialLightingStateForCSM should now implicitly never discard a light 
// since we've ensured we only have (nWorldLights - 1) non-CSM lights in the lightingState here.
//-----------------------------------------------------------------------------
												 ...
												 bool bStaticProp = false)
{
	...
	else
	{
		...
		ratio = LightIntensityAndDirectionInBox( pWorldLight, pZBuf, bucketOrigin, mins, maxs, dynamic ? LIGHT_NO_OCCLUSION_CHECK : 0, &direction, bStaticProp );
	}
	...
	if (pWorldLight->type == emit_surface || illum >= r_worldlightmin.GetFloat()) // FIXME: tune this value
	{
		...
		if ( lightingState.numlights < nWorldLights )
		{
			...
			if ((lightingState.numlights < nWorldLights) ||
				DoesLightingStateHaveCSMLight(lightingState.locallight, nWorldLights))
				return pVis;

			// All slots used and no CSM light - we need to discard one light here (and have it added to the ambient cube), or risk it being completely discarded and ignored in CanonicalizeMaterialLightingStateForCSM
			--lightingState.numlights;
		}
...
static ITexture *FindEnvCubemapForPoint( const Vector& origin )
{
	...
	{
		int smallestIndex = 0;
		Vector blah0 = origin - pBrushData->m_pCubemapSamples[0].origin;
		float smallestDist = DotProduct( blah0, blah0 );
		int i;
		for( i = 1; i < pBrushData->m_nCubemapSamples; i++ )
		{
			Vector blah1 = origin - pBrushData->m_pCubemapSamples[i].origin;
			float dist = DotProduct( blah1, blah1 );
...
{
	// early on in the map allow building light cache quickly
	if ( r_framecount < 60 )
		return true;

	// allow some cache misses per frame
	if ( g_CacheEntriesBuiltThisFrame < lightcache_maxmiss.GetInt() )
		return true;

	// don't refill light cache for fast objects, just use nearest entry
...
{
	int bestDist = 3; // only allow finding lightcache entries with xyz within 3
							   ...
							   unsigned int flags, bool bDebugModel )
{
	...
	// per-frame initialization -- todo: should probably have a frame-start call instead, but this works to detect edge
	if ( r_framecount != g_FrameIndex )
	{
		g_FrameIndex = r_framecount;
		g_CacheEntriesBuiltThisFrame = 0;
		
		if ( g_HighwaterCacheEntries )
		{
			g_HighwaterCacheEntries = false;
			//DevMsg( "Light cache new highwater mark %d\n", g_MaxCacheEntriesBuiltPerFrame );
		}
	}
	...
	*/
	...
	else if ( !AllowFullCacheMiss(flags) )
	{
		...
		// If there are no valid lightcache entries close to our target,
		// we have to generate the cache data -- even if this exceeds the
		// normal budget for lightcache entries built per frame.
		if ( pCache )
		{
			AssertMsg( pCache->bucket != 0xFFFF, "using invalid light cache entry" );
	...
	if ( !pCache )
	{
		...
		g_CacheEntriesBuiltThisFrame++;
		AssertMsg( g_CacheEntriesBuiltThisFrame != MAX_CACHE_ENTRY, "lightcache per-frame working set exceeds cache size" );

		if ( g_MaxCacheEntriesBuiltPerFrame < g_CacheEntriesBuiltThisFrame )
		{
			g_MaxCacheEntriesBuiltPerFrame = g_CacheEntriesBuiltThisFrame;
			g_HighwaterCacheEntries = true;
		}
