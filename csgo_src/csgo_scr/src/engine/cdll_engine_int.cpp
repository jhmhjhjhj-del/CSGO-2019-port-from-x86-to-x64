...
// Windows system headers to support RaiseException/GetExceptionInformation/EXCEPTION_CONTINUE_EXECUTION in CEngineClient::ExecuteExceptionHandler
#include "winlite.h"
#endif
...
public:
	...
	virtual int DemoFindNextImportantTick( const char* szEventName, int nStartTick );
	virtual int DemoFindPrevImportantTick( const char* szEventName, int nStartTick );
...
// For non-split screen games this will always be zero
	...
	virtual void SetGameTimescale( float flTimescale );
	...
	// Exception info upload from outside of engine.dll
	virtual void ExecuteExceptionHandler( const char *szStringVal ) OVERRIDE;

	//Interface to control player voice volume.
	virtual float GetPlayerVoiceVolume(XUID player_id) OVERRIDE;
	virtual void SetPlayerVoiceVolume(XUID player_id, float fVol) OVERRIDE;
};
...
{
	float flScale = sv.GetTimescale();

	flScale *= host_timescale.GetFloat();

	extern float CL_GetHltvReplayTimeScale();
	flScale *= CL_GetHltvReplayTimeScale();

	if ( demoplayer->IsPlayingBack() )
		flScale *= demoplayer->GetPlaybackTimeScale();
...
int CEngineClient::DemoFindNextImportantTick( const char* szEventName, int nStartTick )
{
	if ( nStartTick == -1 )
		nStartTick = demoplayer->GetPlaybackTick();

	const DemoImportantGameEvent_t* pGameEvent = demoplayer->GetImportantGameEvent( szEventName );
	if ( !pGameEvent )
		return -1;

	nStartTick += pGameEvent->flSeekForwardOffset / host_state.interval_per_tick;
	int nImportantTickIndex = demoplayer->FindNextImportantTick( nStartTick, szEventName );
	if(nImportantTickIndex < 0)
		return -1;
...
int CEngineClient::DemoFindPrevImportantTick( const char* szEventName, int nStartTick )
{
	if ( nStartTick == -1 )
		nStartTick = demoplayer->GetPlaybackTick();
	...
	nStartTick -= pGameEvent->flSeekBackwardOffset / host_state.interval_per_tick;
	int nImportantTickIndex = demoplayer->FindPreviousImportantTick( nStartTick, szEventName );
	if ( nImportantTickIndex < 0 )
		return -1;
...
float CEngineClient::GetGameTimescale()
{
	return sv.GetTimescale();
}

void CEngineClient::SetGameTimescale( float flTimescale )
{
...
// Exception info upload from outside of engine.dll
#if DEVELOPMENT_ONLY
// in trunk builds upload dumps every 5 seconds
#define dev_js_exception_handler_throttle_seconds_default 5
#else
// in production builds upload dumps no more frequently than once every 10 minutes
#define dev_js_exception_handler_throttle_seconds_default 600
#endif
DEVELOPMENT_ONLY_CONVAR( dev_js_exception_handler_throttle_seconds, dev_js_exception_handler_throttle_seconds_default );

static uint32 Helper_GetExceptionHash( const char *szStringVal )
{
	const char *pch = strchr( szStringVal, ')' );
	if ( !pch || pch == szStringVal ) return 0;
	return CRC32_ProcessSingleBuffer( szStringVal, pch - szStringVal );
}
static CUtlRBTree< uint32, int, CDefLess< uint32 > > s_rbExecutedExceptionHandlers;

void CEngineClient::ExecuteExceptionHandler( const char *szStringVal )
{
#if !defined(NO_STEAM) && !defined(DEDICATED) && defined(_WIN32)
	// see: Sys_Error_Internal implementation conditions for the minidump in sys_dll.cpp
	if ( Plat_IsInDebugSession() || CommandLine()->FindParm( "-nominidumps" ) || CommandLine()->FindParm( "-nobreakpad" ) )
		return;

	uint32 unHash = Helper_GetExceptionHash( szStringVal );
	bool bNewException = ( s_rbExecutedExceptionHandlers.InsertIfNotFound( unHash ) != s_rbExecutedExceptionHandlers.InvalidIndex() );

	static double s_dblLastExceptionTime = 0;
	double dblExceptionTime = Plat_FloatTime();
	if ( !bNewException && s_dblLastExceptionTime &&
		( dblExceptionTime - s_dblLastExceptionTime < dev_js_exception_handler_throttle_seconds.GetFloat() ) )
		return;	// Already reported such exception, throttle

	// Mark the time of last exception reported
	s_dblLastExceptionTime = dblExceptionTime;

	uint32 msStart = Plat_MSTime();
	DevWarning( "Generating exception information...\n" );

	extern void Status_Update();
	extern void BuildMinidumpComment( char const *pchSysErrorText );
	extern bool g_bUpdateMinidumpComment;
	Status_Update();
	BuildMinidumpComment( szStringVal );
	bool bSave = g_bUpdateMinidumpComment;
	g_bUpdateMinidumpComment = false;

	// Temporarily override the build ID for breakpad upload
	extern void *s_pvSteamApiBreakpadReliableUpload;
	extern void *s_pvSteamApiBreakpadReliableUploadLookback;
	void *pvSavedSteamApiBreakpadReliableUpload = s_pvSteamApiBreakpadReliableUpload;
	void *pvSavedSteamApiBreakpadReliableUploadLookback = s_pvSteamApiBreakpadReliableUploadLookback;
	s_pvSteamApiBreakpadReliableUploadLookback = &szStringVal;

	// see: Sys_Error_Internal implementation raising exception for the minidump in sys_dll.cpp
	__try
	{
		RaiseException
		(
			0,							// dwExceptionCode
			0, // !EXCEPTION_NONCONTINUABLE, i.e. can continue	// dwExceptionFlags
			0,							// nNumberOfArguments,
			NULL						// const ULONG_PTR* lpArguments
		);

		// Assume that we always continue
	}
	// Write the minidump from inside the filter (GetExceptionInformation() is only valid in the filter)
	__except ( SteamAPI_WriteMiniDump( 0, s_pvSteamApiBreakpadReliableUpload = GetExceptionInformation(), build_number() ), EXCEPTION_CONTINUE_EXECUTION )
	{
		// We always continue because the above filter evaluates to EXCEPTION_CONTINUE_EXECUTION
	}

	extern wchar_t *s_wchSteamApiBreakpadReliableUploadRestore;
	if ( s_wchSteamApiBreakpadReliableUploadRestore && ( *s_wchSteamApiBreakpadReliableUploadRestore == L'S' ) )
	{
		*s_wchSteamApiBreakpadReliableUploadRestore = L'C';
		s_wchSteamApiBreakpadReliableUploadRestore = nullptr;
	}

	s_pvSteamApiBreakpadReliableUploadLookback = pvSavedSteamApiBreakpadReliableUploadLookback;
	s_pvSteamApiBreakpadReliableUpload = pvSavedSteamApiBreakpadReliableUpload;
	g_bUpdateMinidumpComment = bSave;

	uint32 msEnd = Plat_MSTime();
	DevWarning( "... processed exception information in %u ms\n", msEnd - msStart );
#endif
}

float Voice_GetPlayerVoiceVolume( XUID player_id );
void Voice_SetPlayerVoiceVolume (XUID player_id, float fVol );


float CEngineClient::GetPlayerVoiceVolume(XUID player_id)
{
	return Voice_GetPlayerVoiceVolume( player_id );
}

void CEngineClient::SetPlayerVoiceVolume(XUID player_id, float fVol)
{
	Voice_SetPlayerVoiceVolume( player_id, fVol );
}
...
bool ClientDLL_Load()
{
	...
	// Work out which client dll to load
	// see: sys_dll2.cpp : 891 for which client version is used for crash dump reporting
	const char *pszClientDll = "client_panorama" DLL_EXT_STRING;
	if ( CommandLine()->FindParm( "-scaleform" ) )
	{
		if ( CommandLine()->FindParm( "-panorama" ) )
		{
			Sys_Error( "Please only specify one of '-scaleform' or '-panorama' in launch options" );
		}

		pszClientDll = "client" DLL_EXT_STRING;
	}

	if ( !Host_AllowLoadModule( pszClientDll, "GAMEBIN", false ) )
	{
	...
	g_ClientDLLModule = g_pFileSystem->LoadModule( pszClientDll, "GAMEBIN", false );
...
void ClientDLL_Init( void )
{
	...
	if ( g_ClientDLL )
	{
		...
		ConMsg( "ClientDLL_Init after g_ClientDLL->Init\n" );
...
void ClientDLL_Update( void )
{
	...
	if ( g_ClientDLL )
	{
		g_ClientDLL->HudUpdate( GetLocalClient().IsConnected() );
	}
}
