...
extern void NET_CloseOSSocket( int hSocket, int sock = -1 );

// Called when we get a bad packet from a client.  Returns true if we are OK to spew about it, or false if
// we've been doing a lot of that and should throttle the spew to avoid risking being DoSed
static bool BShouldSpewBadPacket()
{
	static int s_nBadPacketBatchCount;
	static double s_flTimeLastBackPacketBatchStart;

	// Limit to 5 reports every 5 seconds

	double now = Plat_FloatTime();
	if ( now > s_flTimeLastBackPacketBatchStart + 5.0 )
	{
		s_flTimeLastBackPacketBatchStart = now;
		s_nBadPacketBatchCount = 0;
	}
	else if ( s_nBadPacketBatchCount >= 5 )
	{
		return false;
	}
	++s_nBadPacketBatchCount;
	return true;
}

static bool BShouldSpewNetChanOverflow()
{
	static int s_nBadPacketBatchCount;
	static double s_flTimeLastBackPacketBatchStart;
...
{
	if ( ++m_FileDeniedCounter > 20u )
	{	// Don't even send a denied message to avoid traffic
		return;
	}
...
bool CNetChan::SendFile(const char *filename, unsigned int transferID, bool bIsReplayDemoFile )
{
	...
	// Reset the counter of denied files
	m_FileDeniedCounter = 0;
...
bool CNetChan::EnqueueVeryLargeAsyncTransfer( INetMessage &msg )
{
	...
	bf_write	bufWrite( "EnqueueVeryLargeAsyncTransfer", buffer.Base(), buffer.Size() );
	...
	V_memcpy( data->buffer, buffer.Base(), bufWrite.GetNumBytesWritten() );
...
void CNetChan::Shutdown( int nReason, const char *pszReason )
{
	// send disconnect
	...
	// Send a disconnect message.  But if we have a steam net connection, we don't
	// need to do this, because the close mechanism includes a termination code and
	// reason.
	if ( pszReason && m_hSteamNetConnection == k_HSteamNetConnection_Invalid )
	{
		CNETMsg_Disconnect_t disconnect;
		disconnect.set_text( pszReason );
		disconnect.WriteToBuffer( m_StreamUnreliable );
		Transmit();	// push message out
	}

	// If we have a steam net connection, makes sure and specifically close it.
	if ( m_hSteamNetConnection != k_HSteamNetConnection_Invalid )
	{
		NET_CloseSteamNetConnection( m_hSteamNetConnection, nReason, pszReason );
		Assert( m_hSteamNetConnection == k_HSteamNetConnection_Invalid );
	}

	#if defined( USE_STEAM_SOCKETS )
		if ( remote_address.IsType<netadr_t>() )
		{
			uint64 steamIDRemote = g_pSteamSocketMgr->GetSteamIDForRemote( remote_address.AsType<netadr_t>() );
			NET_TerminateSteamConnection( m_Socket, steamIDRemote );
		}
	#endif
	...
	if ( m_MessageHandler )
	{
		m_MessageHandler->ConnectionClosing( pszReason );
...
CNetChan::~CNetChan()
{
	// If we still have an open connection associated with this netchan, it's weird
	// that we haven't already cleaned it up
	Shutdown( k_EConnectionEnd_UnusualGeneric, "CNetChan destructed" );
}
...
void CNetChan::SetSteamNetConnection( HSteamNetConnection hNetConnection )
{
	m_hSteamNetConnection = hNetConnection;
}
...
inline uint64 GetDebugSteamIDForAddress( ns_address &remote_address )
{
	switch ( remote_address.GetAddressType() )
	{
		case NSAT_NETADR:
			return g_pSteamSocketMgr->GetSteamIDForRemote( remote_address );

		case NSAT_PROXIED_CLIENT:
		case NSAT_PROXIED_GAMESERVER:
			return remote_address.m_steamID.GetSteamID().ConvertToUint64();
	}
	return 0;
}
...
int CNetChan::SendDatagram(bf_write *datagram)
{
	...
	// Send the datagram
	int	bytesSent = NET_SendNetChanPacket( this, send.GetData(), send.GetNumBytesWritten(), bSendVoice ? &m_StreamVoice : 0, bCompress );
		...
		( !net_showudp_remoteonly.GetBool() || !( remote_address.IsLocalhost() || remote_address.IsLoopback() ) ) )
	{
		...
		char desc[ 128 ];
		uint64 steamID = GetDebugSteamIDForAddress( remote_address );
...
{
	// No encryption key if we're using Steam Networking sockets,
	// since that API already does encryption.
	if ( m_hSteamNetConnection != k_HSteamNetConnection_Invalid )
		return nullptr;
...
bool CNetChan::_ProcessMessages( bf_read &buf, bool wasReliable  )
{
	...
	while ( true )
	{
		...
		if ( pMsgBind )
		{
			int startbit2 = buf.GetNumBitsRead();
				
			INetMessage	* netmsg = pMsgBind->CreateFromBuffer( buf );
			if ( !netmsg )
			{
				if ( BShouldSpewBadPacket() )
				{
			...
			int numBitsForThisMessage = buf.GetNumBitsRead() - startbit2;
			if ( !pMsgBind->BIncomingMessageForProcessing( net_time, ( ( numBitsForThisMessage + 7 ) / 8 ) ) )
			{
				if ( BShouldSpewNetChanOverflow() )
				{
					Msg( "Msg from %s: %s rejected by msg bind (%d bytes)\n", GetAddress(), netmsg->GetName(), ( numBitsForThisMessage + 7 ) / 8 );
					Assert( 0 );
				}
				return false;
			}

			UpdateMessageStats( netmsg->GetGroup(), numBitsForThisMessage );
...
bool CNetChan::SendReliableViaStream( dataFragments_t *data)
{
	...
	// FIXME - dead code, this always fails
	return false;
}
...
{
	// !SPEED! Whoa we are searching the whole queue here!
	// Maybe we should track the number of packets queued
	// using a threadsafe counter, so we don't have to do this???
	return g_pQueuedPackedSender->HasQueuedPackets( this );
}
...
bool CNetChan::IsValidFileForTransfer( const char *pszFilename )
{
	...
	if ( V_stristr( pszFilename, "maps/" ) &&
		...
		// <Feb 2018> -- CS:GO doesn't really need uploading files from clients to game servers
		// and there have been exploits writing files by getting the name truncated at filesystem
		// layer, so we are not allowing the forbidden extensions to appear anywhere in the filename
		!V_stristr( pszFilename, ".cfg" ) &&
		!V_stristr( pszFilename, ".lst" ) &&
		!V_stristr( pszFilename, ".lmp" ) &&
		!V_stristr( pszFilename, ".exe" ) &&
		!V_stristr( pszFilename, ".vbs" ) &&
		!V_stristr( pszFilename, ".com" ) &&
		!V_stristr( pszFilename, ".bat" ) &&
		!V_stristr( pszFilename, ".dll" ) &&
		!V_stristr( pszFilename, ".ini" ) &&
		!V_stristr( pszFilename, ".log" ) &&
		!V_stristr( pszFilename, ".lua" ) &&
		!V_stristr( pszFilename, ".nut" ) &&
		!V_stristr( pszFilename, ".vdf" ) &&
		!V_stristr( pszFilename, ".smx" ) &&
		!V_stristr( pszFilename, ".gcf" ) &&
		!V_stristr( pszFilename, ".sys" )
			)
		return false;
	...
	// <Feb 2018> -- this list of extensions is redundant since we are checking full substrings
	// above in CS:GO, but keeping it here for ease of merging across branches if needed
