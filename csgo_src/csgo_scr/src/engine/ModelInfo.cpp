...
static void R_StudioGetLightingCenter( IClientRenderable *pRenderable, studiohdr_t* pStudioHdr, const Vector& origin,
								const QAngle &angles, float scale, Vector* pLightingOrigin )
{
	Assert( pLightingOrigin );
	matrix3x4_t matrix;
	AngleMatrix( angles, origin, matrix );
	matrix.ScaleUpper3x3Matrix( scale );
...
public:
	...
	vcollide_t *GetVCollide( const model_t *model, float scale ) const;
	vcollide_t *GetVCollide( int modelIndex ) const;
	vcollide_t *GetVCollide( int modelIndex, float scale ) const;
	...
	const QAngle& angles, float scale, Vector* pLightingOrigin )
{
	...
	if (pStudioHdr)
	{
		R_StudioGetLightingCenter( pRenderable, pStudioHdr, origin, angles, scale, pLightingOrigin );
	}
	else
	{
...
vcollide_t *CModelInfo::GetVCollide( const model_t *pModel, float scale ) const
{
	if ( !pModel )
		return NULL;
	...
	if ( pModel->type == mod_studio )
	{
		...
		vcollide_t *col = g_pMDLCache->GetVCollide( pModel->studio, scale );
...
{
	return GetVCollide( pModel, 1.0f );
}


vcollide_t *CModelInfo::GetVCollide( int modelIndex, float scale ) const
{
	...
	if ( modelIndex < MAX_MODELS )
	{
		...
		if ( pModel )
		{
			...
			{
			case mod_brush:
				Assert( scale == 1.0f ); // scaling not implemented for brushes
		...
		else
		{
			...
			Assert( scale == 1.0f ); // uh oh, hope we never hit this.  CM system doesn't have a way to give us scaled vcollides
			return CM_GetVCollide( modelIndex - 1 );
		}
	}
	return NULL;
...
{
	return GetVCollide( modelIndex, 1.0f );
}
