...
#define MAX_SNOWFLAKES 1024
#define MIN_SNOWFLAKES 128
bool g_bInitSnow = false;
int g_nNumFlakesDead = 0;

struct snowflake_t // pretty snowflake for the holidays
{
	float m_flX;
	float m_flY;
	float m_flSeed;
	...
	{
		m_bActive = true;
		m_flX = RandomFloat(); // start at a random position
		m_flY = RandomFloat();
		m_flSeed = RandomFloat() * 3.14159f; // seed controls sine wave horizontal motion
		m_flSpeed = RandomFloat( 0.05f, 0.08f ); // fall speed
		m_flSize = RandomFloat( 0.005f, 0.02f ); // size scale
		// snowflake texture is a 2x2 atlas of different flake styles, pick random
		m_flU = 0.0f;
		if ( RandomInt( 0, 1 ) == 1 )
			m_flU = 0.5f;
		m_flV = 0.0f;
	...
	{
		// fall down
		m_flY += host_frametime * RemapVal( m_flY, 0.0f, 1.0f, m_flSpeed, m_flSpeed * 0.7f );
		// waft side to side
		m_flX += 0.0001f * sin( realtime + m_flSeed );
		if ( m_flY > 1.0f )
		{
			// went off the bottom
			m_flY = -m_flSize;
			m_flSeed = RandomFloat() * 3.14159f;
			m_flSpeed = RandomFloat( 0.04f, 0.06f ); // slightly slower
			m_flSize = RandomFloat( 0.004f, 0.015f ); // slightly smaller
			
			if ( m_bActive && g_nNumFlakesDead < (MAX_SNOWFLAKES - MIN_SNOWFLAKES) ) // keep min snowflakes alive
			{
				m_bActive = RandomInt( 0, 3 ) != 0;
				if ( !m_bActive )
					g_nNumFlakesDead++;
			}
		}
...
snowflake_t g_Snow[MAX_SNOWFLAKES];

void DrawHolidaySnow( void )
{
	static ConVarRef sv_holiday_mode( "sv_holiday_mode" );
	if ( sv_holiday_mode.GetInt() != 2 ) // christmas
		return;

	// find the snowflake material
	IMaterial *pMatSnow = materials->FindMaterial( "dev/snowflake", TEXTURE_GROUP_OTHER, true );
	if ( !pMatSnow )
		return;

	// init the snowflake array
	if ( g_bInitSnow == false )
	{
		for ( int n = 0; n < ARRAYSIZE( g_Snow ); n++ )
		{
			g_Snow[n].Init();
		}
		g_bInitSnow = true;
	...
	// we need screen dimensions to make sure flake quads are square
	int screenWidth, screenHeight;
	g_pMaterialSystem->GetBackBufferDimensions( screenWidth, screenHeight );
	if ( screenWidth != 0 )
	{
		float flAspect = (float)screenHeight / (float)screenWidth;

		// scale num flakes to screen size
		int nNumFlakes = clamp( screenHeight, MIN_SNOWFLAKES, MAX_SNOWFLAKES );

		for ( int n = 0; n < nNumFlakes; n++ )
		{
			if ( !g_Snow[n].m_bActive )
				continue;

			g_Snow[n].Fall(); // update each flake
			pRenderContext->DrawScreenSpaceRectangle( pMatSnow, g_Snow[n].m_flX, g_Snow[n].m_flY, g_Snow[n].m_flSize * flAspect, g_Snow[n].m_flSize, g_Snow[n].m_flU, g_Snow[n].m_flV, g_Snow[n].m_flU + 0.5f, g_Snow[n].m_flV + 0.5f );
		}
	}
}
...
void V_RenderVGuiOnly_NoSwap()
{
	...
	// Can't include panoramaenginehandler.h in this file on win64 build for some reason
	// Just look at command line instead
	...
	if ( !CommandLine()->HasParm( "-scaleform" ) && !cbTextMode )
	{
		static int s_nVguiOnlyPan = 0;
		const int nPan = ++s_nVguiOnlyPan;
		static ConVarRef refChain( "panorama_render_chain" );
		const int nChain = refChain.IsValid() ? refChain.GetInt() : 0;
		const bool bBreadcrumb = ( nChain > 0 && nPan <= 40 );
		if ( bBreadcrumb || ( nChain >= 2 && ( nPan % 120 ) == 0 ) )
			ConMsg( "PanRenderChain CALLSITE V_RenderVGuiOnly_NoSwap -> RunFrame+RenderFrame FrontEnd(lobby) #%d\n", nPan );
		if ( bBreadcrumb )
			ConMsg( "PanCrashBC #%d before PanoramaRunFrame\n", nPan );
		pRenderContext->PanoramaRunFrame( k_EPanoramaSlotFrontEnd );
		if ( bBreadcrumb )
			ConMsg( "PanCrashBC #%d after PanoramaRunFrame, before PanoramaRenderFrame\n", nPan );
		pRenderContext->PanoramaRenderFrame( k_EPanoramaSlotFrontEnd );
		if ( bBreadcrumb )
			ConMsg( "PanCrashBC #%d after PanoramaRenderFrame\n", nPan );

		// DrawHolidaySnow(); // let it snow let it snow let it snow -- Jan2019: commenting it out otherwise playing back a demo/overwatch will make it snow on the main menu
	}
#endif
...
void V_RenderVGuiOnly( void )
{
	...
	if ( !CommandLine()->CheckParm( "-scaleform" ) )
	{
		static int s_nVguiBegin = 0;
		const int nVB = ++s_nVguiBegin;
		static ConVarRef refChainVB( "panorama_render_chain" );
		if ( ( refChainVB.IsValid() ? refChainVB.GetInt() : 0 ) >= 2 && ( nVB <= 16 || ( nVB % 120 ) == 0 ) )
			ConMsg( "PanRenderChain CALLSITE V_RenderVGuiOnly BeginFrame #%d\n", nVB );
		CMatRenderContextPtr pRenderContext;
		pRenderContext.GetFrom( materials );
		pRenderContext->PanoramaRenderFrame( k_EPanoramaSlotBeginFrame );
		pRenderContext.SafeRelease();
	}
#endif
	...
	if ( !CommandLine()->CheckParm( "-scaleform" ) )
	{
		static int s_nVguiEnd = 0;
		const int nVE = ++s_nVguiEnd;
		static ConVarRef refChainVE( "panorama_render_chain" );
		if ( ( refChainVE.IsValid() ? refChainVE.GetInt() : 0 ) >= 2 && ( nVE <= 16 || ( nVE % 120 ) == 0 ) )
			ConMsg( "PanRenderChain CALLSITE V_RenderVGuiOnly EndFrame #%d\n", nVE );
		CMatRenderContextPtr pRenderContext;
		pRenderContext.GetFrom( materials );
		pRenderContext->PanoramaRenderFrame( k_EPanoramaSlotEndFrame );
		pRenderContext.SafeRelease();
	}
#endif
