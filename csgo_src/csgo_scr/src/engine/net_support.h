...
public:
	...
	virtual int SendPacketToNsAddress( int sock, const ns_address &to, const void *data, int length ) OVERRIDE;

    virtual ISteamNetworkingUtils *GetSteamNetworkingUtils() OVERRIDE;
	virtual ISteamNetworkingSockets *GetSteamNetworkingSockets() OVERRIDE;
	virtual ISteamNetworkingSockets *GetSteamNetworkingSocketsGameServer() OVERRIDE;

	virtual CRC64_t BuildServerInstanceCRCToken() OVERRIDE;

	virtual CSteamID GetSteamIdForRemoteNetworkAddress( ns_address &netAdrRemote ) OVERRIDE;
