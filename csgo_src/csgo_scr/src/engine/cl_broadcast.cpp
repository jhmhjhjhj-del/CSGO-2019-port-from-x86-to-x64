...
ConVar tv_playcast_delay_resync( "tv_playcast_delay_resync", "0", FCVAR_RELEASE, "To alleviate intermittent network connectivity problems, this is the number of seconds to wait before actually re-syncing the stream after failure" );
...
{
	if ( Plat_FloatTime() - m_dResyncTimerStart > m_DemoStream.GetBroadcastKeyframeInterval() + tv_playcast_delay_resync.GetFloat() )
	{
		m_dResyncTimerStart = Plat_FloatTime();
		...
		m_nStreamState = STREAM_SYNC;
		m_DemoStream.Resync( );
		// TODO: this loading screen is resulting in UI showstopper where user is unable to do anything
		// something regressed probably in Panorama where the loading screen doesn't shut itself down
		// after the client finished re-sync'ing.
		// Repro is relatively easy: modify hltvbroadcastrelay.js to drop a delta packet and immediately
		// following full packet to cause the client stall during playback. As of Feb 2019 the client would
		// deadlock the UI on this loading screen, however the stream will correctly re-sync and keep playing
		// behind the screen.
		// SCR_BeginLoadingPlaque();
	}
}
