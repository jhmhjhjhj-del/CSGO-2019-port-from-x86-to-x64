...
// Send a packet to the given netchan
int			NET_SendNetChanPacket( INetChannel *chan, const void *data, int length, bf_write *pVoicePayload = NULL, bool bUseCompression = false );

// Send a raw packet to a netchan right now.  No queueing, no parsing, no splitting.
// Might be called from the send queue thread
int			NET_SendNetChanPacketRaw( INetChannel *chan, const void *pvData, int length, int iGameDataLength );

// Send a packet to the specified address, not associated with a netchan
int			NET_SendPacketToNsAddress( int sock, const ns_address &to, const void *data, int length, uint32 unMillisecondsDelay = 0 );
...
/// Close steam networking sockets connection
...
/// Called when we cancel or fail to get a server reservation.  Closes the temporary
/// steam net connection to the server.
...
public:
	...
	virtual int sendto( int s, const char * buf, int len, int flags, const netadr_t &to ) = 0;
