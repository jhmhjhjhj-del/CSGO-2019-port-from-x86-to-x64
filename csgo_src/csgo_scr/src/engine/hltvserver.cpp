...
bool IsHltvActive( int nInstance )
{
	return g_pHltvServer[ nInstance ] && g_pHltvServer[ nInstance ]->IsActive();
}
...
ConVar tv_broadcast_url1( "tv_broadcast_url1", "http://localhost:8080", FCVAR_RELEASE, "URL of the broadcast relay1" );

CUtlVector< netadr_t > g_array_tv_challenge_steam3_iprange;
void FnOnChange_tv_challenge_steam3_iprange( IConVar *var, const char *pOldValue, float flOldValue )
{
	ConVarRef cv( var );

	CUtlVector< netadr_t > arrSplitAddresses;
	CUtlVector< char * > arrTokens;
	V_SplitString( cv.GetString(), ",", arrTokens );
	FOR_EACH_VEC( arrTokens, i )
	{
		int numBits = 32;
		if ( char *szSlash = strchr( arrTokens[ i ], '/' ) )
		{
			*szSlash = 0;
			++szSlash;
			numBits = V_atoi( szSlash );
			numBits = clamp( numBits, 1, 32 );
		}
		netadr_t adrToken( arrTokens[ i ] );
		adrToken.SetPort( numBits );
		arrSplitAddresses.AddToTail( adrToken );
	}
	arrTokens.PurgeAndDeleteElements();

	g_array_tv_challenge_steam3_iprange.Swap( arrSplitAddresses );
}
ConVar tv_challenge_steam_iprange( "tv_challenge_steam_iprange", "", FCVAR_RELEASE, "Comma-separated IP ranges which require Steam3 challenge protocol even for GOTV connections", FnOnChange_tv_challenge_steam3_iprange );
...
{
	uint nSize = sizeof( *this ) + sizeof( *m_pCopyEntities ) * m_nTotalEntities;
	
	if ( m_pClientFrame )
	{
		nSize += m_pClientFrame->GetMemSize();
	}
	...
	//delete our entity linked list
	for ( SHLTVDeltaEntity_t *p = m_pEntities; p; p = p->m_pNext )
	{
		nSize += p->GetMemSize();
	}

	// note: m_pRelativeFrame and m_pSourceFrame are not owned by this struct, so has to be counted separately, by other delta frames; this delta frame only counts the "current" snapshot object
	// also, m_pNewerDeltaFrame is just the pointer to the next element in the linked list, so it's not included in the memory calculation
...
size_t CHLTVServer::SHLTVDeltaEntity_t::GetMemSize() const
{
	size_t nMemSize = sizeof( *this );
	// player bit vec is a fixed-size bitvec
	nMemSize += m_nNumRecipients * sizeof( *m_pNewRecipients );
	if ( ( m_SerializedEntity != knNoPackedData ) && ( m_SerializedEntity != SERIALIZED_ENTITY_HANDLE_INVALID ) )
		g_pSerializedEntities->GetMemSize( m_SerializedEntity );
	return nMemSize;
}
...
uint CHLTVFrame::GetMemSize()const
{
	...
	nSize += CClientFrame::GetExtraMemSize();
	return nSize;
}
...
void CHLTVServer::StartMaster(CGameClient *client) //<<	setting up master client
{
	m_nExternalTotalViewers = 0;
	m_nExternalLinkedViewers = 0;
...
{
	m_Broadcast.StartRecording( m_nInstanceIndex ? tv_broadcast_url1.GetString() : tv_broadcast_url.GetString() );
}
...
void CHLTVServer::BroadcastEvent(IGameEvent *event)
{
	...
	//
	// Special implementation allowing hltv_* game events passthrough to clients watching permanent killer replay
	//
	...
	{
		//
		// Also broadcast this TV event explicitly to replay viewers feeding off this HLTV instance
		//
		if ( IServer* pBaseServer = m_MasterClient ? m_MasterClient->GetServer() : NULL )
		{
			for ( int i = 0, cClients = pBaseServer->GetClientCount(); i < cClients; ++i )
			{
				IClient *cl = pBaseServer->GetClient( i );
				if ( !cl ) continue;
				if ( !cl->IsActive() ) continue;
				if ( !cl->GetHltvReplayDelay() ) continue;
				if ( ( ( CGameClient * ) cl )->m_pHltvReplayServer != this ) continue;

				//
				// Kludge here: we don't want to overburden the game client code to know about these events
				// that we are jamming down their netchannel originating for HLTV director, so we explicitly
				// bypass the virtualization of SendNetMsg implementation in game client and call its base
				// method directly to just stuff it onto the wire and deliver.
				//
				if ( !( ( CGameClient * ) cl )->CBaseClient::SendNetMsg( eventMsg, /* bForceReliable = */ true ) )
				{
					DevMsg( "BroadcastMessage: Reliable broadcast message overflow for hltv replay client %s\n", cl->GetClientName() );
				}
			}
		}
	}
...
{
	double flTimerStart = Plat_FloatTime();
	...
	if( !m_CurrentFrame || ( !m_DemoRecorder.IsRecording() && !m_Broadcast.IsRecording() ) )
	{
	...
	AUTO_LOCK_FM( m_FrameMutex ); // the mutex is not needed now, as this function will never be called while another thread has access to the frames
	...
	while( m_pOldestDeltaFrame )
	{
		...
		m_Broadcast.WriteFrame( pExpandedFrame );
		nFramesFlushed++;
	...
	m_DemoRecorder.StopRecording( pGameInfo );

	double flTimeElapsed = Plat_FloatTime() - flTimerStart;
	Msg( "StopRecording: %u frames flushed in %.2f seconds\n", nFramesFlushed, flTimeElapsed );
}
...
void CHLTVServer::InstallStringTables( void )
{
	...
	m_StringTables = m_Server->m_StringTables;
...
void CHLTVServer::UninstallStringTables( void )
{
	if ( !m_Server )
		return;
	// TODO: implement refcounting and disabling rollback when not necessary anymore
}
...
extern int g_nFixupTargetEdict;
void CHLTVServer::FixupOperatorTick( const CCLCMsg_HltvFixupOperatorTick& msg )
{
	extern ConVar tv_allow_camera_man_override;
	if ( !tv_allow_camera_man_override.GetBool() )
		return;
	Assert( m_nInstanceIndex <= 1 );
	int nMsgTick = msg.tick();
	if ( g_nFixupTargetEdict < 0 )
	{
		DevMsg( "Fixup operator tick %d without primary cameraman\n", nMsgTick );
		return;
	}

	// TODO: interpolate missing frames
	CHLTVFrame *pFrame = nullptr;
	if ( m_nLastPatchedTick >= 0 && m_nLastPatchedTick < nMsgTick )
		pFrame = ( CHLTVFrame* )GetClientFrame( m_nLastPatchedTick + 1, false );
	if ( !pFrame )
		pFrame = ( CHLTVFrame* )GetClientFrame( msg.tick(), true );

	for ( ; pFrame ; pFrame = ( CHLTVFrame* )pFrame->m_pNext )
	{
		if ( pFrame->tick_count <= m_nLastPatchedTick )
			continue; // we already patched this frame

		if ( pFrame->tick_count > nMsgTick )
			break; // don't need to fix past it

		if( PackedEntity *pPackedEntity = framesnapshotmanager->GetPackedEntity( *pFrame->GetSnapshot(), g_nFixupTargetEdict ) )
		{
			if ( msg.has_eye_angles() )
			{
				DVariant angle;
				angle.m_Type = DPT_Float; // not used
				angle.m_Vector[ 0 ] = msg.eye_angles().x();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_EYE_ANGLE_0, &angle, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
				angle.m_Vector[ 0 ] = msg.eye_angles().y();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_EYE_ANGLE_1, &angle, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
			}

			if ( msg.has_origin() )
			{
				DVariant origin;
				origin.m_Type = DPT_VectorXY; // not used
				origin.m_Vector[ 0 ] = msg.origin().x();
				origin.m_Vector[ 1 ] = msg.origin().y();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VEC_ORIGIN, &origin, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VEC_ORIGIN_SECOND, &origin, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
				origin.m_Type = DPT_Float;
				origin.m_Float = msg.origin().z();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VEC_ORIGIN_2, &origin, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VEC_ORIGIN_2_SECOND, &origin, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
				pFrame->m_bFixupApplied = true;
			}

			if ( msg.has_observer_mode() )
			{
				DVariant observer_mode;
				observer_mode.m_Type = DPT_Int; // not used
				observer_mode.m_Int = msg.observer_mode();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_OBSERVER_MODE, &observer_mode, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
			}

			if ( msg.has_cameraman_scoreboard() )
			{
				DVariant cameraman_scoreboard;
				cameraman_scoreboard.m_Type = DPT_Int;
				cameraman_scoreboard.m_Int = msg.cameraman_scoreboard();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_CAMERAMAN_SCOREBOARD, &cameraman_scoreboard, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
			}

			if ( msg.has_observer_target() )
			{
				DVariant observer_target;
				observer_target.m_Type = DPT_Int;
				observer_target.m_Int = msg.observer_target();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_OBSERVER_TARGET, &observer_target, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
			}

			if ( msg.has_view_offset() )
			{
				DVariant view_offset;
				view_offset.m_Type = DPT_Vector;
				view_offset.m_Vector[ 0 ] = msg.view_offset().x();
				view_offset.m_Vector[ 1 ] = msg.view_offset().y();
				view_offset.m_Vector[ 2 ] = msg.view_offset().z();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VIEW_OFFSET_ENTITYBASE, &view_offset, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );

				view_offset.m_Type = DPT_Float;
				view_offset.m_Float = msg.view_offset().x();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VIEW_OFFSET_0, &view_offset, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
				view_offset.m_Float = msg.view_offset().y();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VIEW_OFFSET_1, &view_offset, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
				view_offset.m_Float = msg.view_offset().z();
				SendTable_PatchField( pPackedEntity->m_pServerClass->m_pTable, SPWKN_VIEW_OFFSET_2, &view_offset, pPackedEntity->GetPackedData(), g_nFixupTargetEdict );
			}

			//bf_read props( "fixup_delta_props", msg.props_data().data(), msg.props_data().size() );
			//SendTable_PatchUncompressibleProps( pPackedEntity->m_pServerClass->m_pTable, props, pPackedEntity->GetPackedData() );
			m_nLastPatchedTick = pFrame->tick_count;
		}
	}
	m_Broadcast.FixupOperatorTick( m_nLastPatchedTick ); // fixed up to this frame
}
...
void CHLTVServer::EntityPVSCheck( CClientFrame *pFrame )
{
	...
	// setup engine PVS
	EnginePVSContext_t pvsContext;
	SV_ResetPVS( &pvsContext, PVS, nPVSSize );
	...
	SV_AddOriginToPVS( &pvsContext, m_vPVSOrigin );
...
float GetTvDelay( int nInstance )
{
	static ConVarRef tv_delay( "tv_delay" );
	static ConVarRef tv_delay1( "tv_delay1" );

	return nInstance ? tv_delay1.GetFloat() : tv_delay.GetFloat();
}
...
CClientFrame *CHLTVServer::AddNewFrame( CClientFrame *clientFrame )
{
	...
	float flTvDelayToKeep  = GetTvDelay( m_nInstanceIndex );
...
CClientFrame *CHLTVServer::GetDeltaFrame( int nTick )
{
	...
	AUTO_LOCK_FM( m_FrameMutex ); // we need to lock frame cache because we're potentially modifying it from multiple sendPacket threads
...
void CHLTVServer::UpdateTick( void )
{
	...
	int nExpandToTick = nNewTick;

	if ( IsMasterProxy() )
	{
		bool bHltvServer1IsActive = m_nInstanceIndex == 1 || IsHltvActive( 1 );
		int nAdditionalTvDelay1 = bHltvServer1IsActive ? TIME_TO_TICKS( GetAdditionalTvDelay1() ) : 0;
		// get tick from director, he decides delay etc
		int nDirectorTick = m_Director->GetDirectorTick();
		
		nExpandToTick = Max( m_nFirstTick, nDirectorTick + Max(0,-nAdditionalTvDelay1) ); // all hltv instances must expand frames to the same frame as the one with the least delay, so that Fixup message can be applied to all of them at once, without building any complicated queues
		
		// add delay on instance 1, but don't allow delay less than 0th
		// By the latest config (2017/06/16), delay1 < delay0, so the "additional" delay on instance 1 is negative,
		// Instance 1 is delayed less than instance 0. Instance 0 is the "main" instance that the public consumes content from.
		// Instance 1 is only used for the fixup operator and not for broadcast
		int nSendToClientTick = m_nInstanceIndex ? nDirectorTick - nAdditionalTvDelay1 : nDirectorTick;

		nNewTick = Max( m_nFirstTick, nSendToClientTick );
#if HLTV_REPLAY_ENABLED
		if ( spec_replay_enable.GetBool() )
		{
	...
	ExpandDeltaFramesToTick( nExpandToTick );
	...
	if ( IsMasterProxy() )
	{
		...
		if ( m_DemoRecorder.IsRecording() || m_Broadcast.IsRecording() )
		{
			...
			CHLTVFrame *pBroadcastFrame = ( CHLTVFrame * )GetClientFrame( nNewTick, false );

			// find a delayed frame to broadcast or demo-record
			if ( m_DemoRecorder.IsRecording() )
			{
				m_DemoRecorder.WriteFrame( pBroadcastFrame, &m_DemoEventWriteBuffer );
			}
			if ( m_Broadcast.IsRecording() )
			{
				m_Broadcast.WriteFrame( pBroadcastFrame, &m_DemoEventWriteBuffer );
			}
...
float CHLTVServer::GetAdditionalTvDelay()const
{
	if ( m_nInstanceIndex )
		return GetAdditionalTvDelay1();
	else
		return 0;
}

float CHLTVServer::GetAdditionalTvDelay1()const
{
	return GetTvDelay( 1 ) - GetTvDelay( 0 );
}
...
{
	m_StringTables = NULL; // stringtables do not belong to this server, they belong to the parent server, so we need not clean them up. Just disconnect from the parent server's stringtables.
...
{
	// we can't have replay in flight when we're shutting down hltv server they may be feeding off of
	for ( int iClient = 0; iClient < sv.GetClientCount(); ++iClient )
	{
		//CBaseClient *pClient = dynamic_cast< CBaseClient * >( sv.GetClient( iClient ) );
		// if( pClient->m_pHltvSlaveServer == this ) ..
		sv.GetClient( iClient )->StopHltvReplay(false); // we don't need to notify clients about speculatively stopping replay due to reasons unrelated to the client actions
	}
		...
		case dem_usercmd:
			{
				...
				ACTIVE_SPLITSCREEN_PLAYER_GUARD( nPlayerSlot );
#endif
				char userCmdBuffer[256];
				int  length = sizeof( userCmdBuffer );
				m_DemoFile.ReadUserCmd( userCmdBuffer, length );
...
extern bool IsInteractiveCasterDebug();

int	CHLTVServer::GetChallengeType ( const ns_address &adr )
{
	if ( IsInteractiveCasterDebug() ) // always false in production
	{
		// when -interactivecaster observer connects to HLTV port from the same machine (a common test case during development)
		// the server may think it's a relay and not send a challenge; this will cause the server to not have the real steam account id
		// for the interactive caster client; this will cause htlv server to ignore the requests to override the primary cameraman from that client
		// because it's not authorized because its steam id is not in the authorized tournament caster list.
		// By issuing this challenge, we're attempting to ascertain that the steam id will be real and verifiable for that secondary caster.
	...
	// Non-valve DS can opt into Steam3 authentication
	if ( adr.GetAddressType() == NSAT_NETADR )
	{
		FOR_EACH_VEC( g_array_tv_challenge_steam3_iprange, idx )
		{
			uint32 const unIPMasked = adr.GetIP() & ( (~0u) << (32 - g_array_tv_challenge_steam3_iprange[idx].GetPort() ) );
			uint32 const unCheckSteam3 = g_array_tv_challenge_steam3_iprange[ idx ].GetIPHostByteOrder();
			if ( unIPMasked == unCheckSteam3 )
			...
			if ( *pszRelaySdrAddr )
			{
				...
				if ( nsadrsdr.SetFromString( pszRelaySdrAddr ) && ( nsadrsdr.GetAddressType() == NSAT_PROXIED_GAMESERVER ) )
				{
					...
					if ( serverGameDLL->IsValveDS() && serverGameDLL->OnEngineClientProxiedRedirect(
						adr.m_steamID.GetSteamID().ConvertToUint64(), pszRelaySdrAddr, pBestProxy->GetUserSetting( "sdr_routing" ) ) )
					{
						//
						// Build a P2P HLTV channel SDR address for client redirect
						//
						fmtAdditionalInfo.AppendFormat( " @ %s", pszRelayAddr );
						pszRelayAddr = pszRelaySdrAddr;
					}
				}
			}
		}
...
CON_COMMAND( tv_status, "Show GOTV server status." ) 
{
	...
	for ( CActiveHltvServerSelector hltv( args ); hltv; hltv.Next() )
	{
		...
		{
			ConMsg( "Master \"%s\", delay %.0f, rate %.1f\n", hltv->GetName(), hltv->GetDirector()->GetDelay() + hltv->GetAdditionalTvDelay(), hltv->GetSnapshotRate() );
		}
...
CON_COMMAND( tv_mem, "hltv memory statistics" )
{
	...
	framesnapshotmanager->DumpMem();

	for ( int i = 0; i < args.ArgC(); ++i )
	{
		if ( !V_stricmp( args[ i ], "ent" ) && i + 1 < args.ArgC() )
		{
			framesnapshotmanager->DumpEntMem( V_atoi(args[++i] ) );
		}
	}
}
...
void CHLTVServer::DumpMem()
{
	...
	uint nFrameCacheSize = m_FrameCache.Count() * sizeof( m_FrameCache[0] );
	for ( int i = 0; i < m_FrameCache.Count(); ++i )
	{
		nFrameCacheSize += m_FrameCache[ i ].pFrame->GetMemSize() ;
	}
	Msg( " %4d frame cache -> %12s bytes\n", m_FrameCache.Count(), V_pretifynum(nFrameCacheSize) );

	CClientFrameManager::DumpMem();
