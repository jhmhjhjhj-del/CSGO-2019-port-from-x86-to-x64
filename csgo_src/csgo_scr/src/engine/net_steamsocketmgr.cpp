...
public:
	...
	virtual int sendto( int s, const char * buf, int len, int flags, const netadr_t &to ) OVERRIDE;
...
private:
	...
	CSteamSocket *FindSocketForAddress( const netadr_t &adr );
...
CSteamSocket *CSteamSocketMgr::FindSocketForAddress( const netadr_t &adr )
{

	int idx = m_mapAdrToSteamSocket.Find( adr );
...
int CSteamSocketMgr::sendto( int s, const char * buf, int len, int flags, const netadr_t &to )
{
	if ( IsValid() )
	{
		...
		else if ( to.GetPort() == STEAM_CNX_PORT )
		{
	...
	sockaddr sadr;
	to.ToSockadr( &sadr );
	int ret = ::sendto( s, buf, len, flags, &sadr, sizeof(sadr) );

	// Check for hiding certain errors
	if ( ret < 0 )
	{
		...
		if ( ( err == WSAEADDRNOTAVAIL) && ( to.GetType() == NA_BROADCAST ) )
			return 0;

		ConDMsg("sendto failed %s : %s\n", NET_ErrorString(err), CUtlNetAdrRender( to ).String() );
		ret = len;
	}
...
{
	if ( !remote.IsType<netadr_t>() )
	{
		Assert( false );
		return 0ull;
	}

	const CSteamSocket *pSocket = FindSocketForAddress( remote.m_adr );
	if ( pSocket )
	{
