...
public:	// INetChannel interface
	...
	void		Reset( void );
	void		Clear( void );
	void		Shutdown( int nReason, const char *pszReason );
...
public:
	...
	void	SetSteamNetConnection( HSteamNetConnection hNetConnection );
	virtual HSteamNetConnection GetSteamNetConnection() const OVERRIDE { return m_hSteamNetConnection; }
...
// don't use any vars below this (only in net_ws.cpp)
	...
	unsigned int	m_FileDeniedCounter;	// increasing every time file transmission is denied
