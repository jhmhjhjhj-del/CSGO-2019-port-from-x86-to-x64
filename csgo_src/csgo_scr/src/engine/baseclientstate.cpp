...
ConVar cl_teammate_color_1( "cl_teammate_color_1", "248 246 45" );
ConVar cl_teammate_color_2( "cl_teammate_color_2", "161 25 240" );
ConVar cl_teammate_color_3( "cl_teammate_color_3", "0 181 98" );
...
void CL_ClanIdChanged( IConVar *pConVar, const char *pOldString, float flOldValue )
{
	...
	if ( pFriends )
	{
		...
		for ( int k = 0; k < iGroupCount; ++ k )
		{
			...
			{
				CSteamID clanID2( newId, Steam3Client().SteamUtils()->GetConnectedUniverse(), k_EAccountTypeClan );
				// valid clan, accept the change
				const char *szClanTag = pFriends->GetClanTag( clanID2 );
				char chLimitedTag[ MAX_CLAN_TAG_LENGTH ];
				CopyStringTruncatingMalformedUTF8Tail( chLimitedTag, szClanTag, MAX_CLAN_TAG_LENGTH );

				const char *szClanName = pFriends->GetClanName( clanID2 );
...
void CServerMsg_CheckReservation::Release()
{
	if ( m_pParent )
		...
		switch ( m_eState )
		{
			default:
			case AOS_RUNNING:
				AssertMsg1( false, "CServerMsg_CheckReservation::Release - unexpected state %d", (int)m_eState );
				break;
			...
			case AOS_ABORTED:
			case AOS_FAILED:
				NET_CheckReservationFailedOrAborted( m_socket, m_eState );
				break;
		}		
	#endif
...
void CServerMsg_CheckReservation::SendMsg( const ns_address &serverAdr, int socket, uint32 token )
{
	...
	msg.WriteLongLong( 0 );
		...
		{	// when we begin reservation with a remote server we allow disconnecting from current server
			if ( !NET_InitSteamDatagramProxiedGameserverConnection( socket, serverAdr, true ) )
				return;
		}
	#endif

	NET_SendPacketToNsAddress( socket, serverAdr, msg.GetData(), msg.GetNumBytesWritten() );
}
...
void CServerMsg_CheckReservation::ResponseReceived( const ns_address &from, bf_read &msg, int32 hostVersion, uint32 token )
{
	...
	int numTotalClientsInReservation = msg.ReadByte();
	if ( numPlayersAwaiting == 0 )
	{
		DevMsg( "Server confirmed all players reservation%u/%d\n", uiReservationStage, numTotalClientsInReservation );
		CServerMsg::ResponseReceived( ( uint8( numTotalClientsInReservation ) << 8 ) | uint8( numPlayersAwaiting ) );
	}
	else
	{
		DevMsg( "Server reservation%u is awaiting %d/%d\n", uiReservationStage, numPlayersAwaiting, numTotalClientsInReservation );
		...
		{
			m_result = ( ( uint8( numTotalClientsInReservation ) << 8 ) | uint8( numPlayersAwaiting ) );
...
void CServerMsg_Ping::SendMsg( const ns_address &serverAdr, int socket, uint32 token )
{
	...
	msg.WriteLong( token );
		...
		if ( serverAdr.GetAddressType() == NSAT_PROXIED_GAMESERVER )
		{
			if ( !NET_InitSteamDatagramProxiedGameserverConnection( socket, serverAdr ) )
				return;
		}
	#endif
...
#if !defined ( DEDICATED ) && defined ( PANORAMA_ENABLE )
	if ( IAvatarImageMgr *pIAvatarImageMgr = g_ClientDLL ? g_ClientDLL->GetAvatarImageMgr() : NULL ) // dedicated servers on Windows don't have clientdll
	{
		pIAvatarImageMgr->Cleanup();
	}
#endif 
...
void CBaseClientState::ConnectionStart(INetChannel *chan)
{
	...
	m_SVCMsgHltvReplay.Bind< CSVCMsg_HltvReplay_t >( chan, UtlMakeDelegate( this, &CBaseClientState::SVCMsg_HltvReplay ) );
	m_SVCMsgHltvFixupOperatorStatus.Bind< CSVCMsg_HltvFixupOperatorStatus_t >( chan, UtlMakeDelegate( this, &CBaseClientState::SVCMsg_HltvFixupOperatorStatus ) );
}
...
void CBaseClientState::ConnectionStop( )
{
	...
	m_SVCMsgHltvFixupOperatorStatus.Unbind();
}
...
void CBaseClientState::SendConnectPacket ( const ns_address &netAdrRemote, int challengeNr, int authProtocol, uint64 unGSSteamID, bool bGSSecure )
{
	...
	NET_SendPacketToNsAddress( m_Socket, netAdrRemote, msg.GetData(), msg.GetNumBytesWritten() );
...
void CBaseClientState::FullConnect( const ns_address &adr, int nEncryptionKey )
{
	...
	m_NetChannel->SetChallengeNr( m_nChallengeNr );
...
void CBaseClientState::Disconnect( bool bShowMainMenu )
{
	...
	if ( m_NetChannel )
	{
		m_NetChannel->Shutdown( k_EConnectionEnd_UsualGeneric, "Disconnect" );
		m_NetChannel = NULL;
	}
		...
		for ( int i = 0; i < m_Remote.Count(); ++i )
		{
			...
			switch ( remote.m_adrRemote.GetAddressType() )
			{
				...
				case NSAT_PROXIED_GAMESERVER:
						...
						if ( !NET_InitSteamDatagramProxiedGameserverConnection( m_Socket, remote.m_adrRemote ) )
							continue;
...
bool CBaseClientState::ProcessConnectionlessPacket( netpacket_t *packet )
{
	...
	case S2C_CHALLENGE:		
		...
		if ( m_nSignonState != SIGNONSTATE_CHALLENGE )
			break;	// must be sending challenges, otherwise anybody can send us this packet and cause a "disconnect"
...
bool AvatarImageNETMsgBitsProvider( uint64 xuid, unsigned char **ppRGBA, uint32 *pWidth, uint32 *pHeight )
{
	CSteamID steamID( xuid );
	if ( !steamID.IsValid() || !steamID.BIndividualAccount() || !steamID.GetAccountID() )
		return false;
	...
	const CNETMsg_PlayerAvatarData& msg = *data.Element( idxData );
	const char *pRGB = msg.rgb().data();

	// Unpack into RGBA. This image is always expected to be of dimensions 64 x 64. See
	// CBaseClientState::AllocOwnPlayerAvatarData below
	unsigned char *pRGBA = (unsigned char*)malloc( 64 * 64 * 4 );
	*ppRGBA = pRGBA;
...
bool CBaseClientState::NETMsg_PlayerAvatarData( const CNETMsg_PlayerAvatarData& msg )
{
	...
	if ( IAvatarImageMgr *pIAvatarImageMgr = g_ClientDLL ? g_ClientDLL->GetAvatarImageMgr() : NULL ) // dedicated servers on Windows don't have clientdll
	{
		pIAvatarImageMgr->SetImageBitsProvider( uint64( pClientDataCopy->accountid() ), AvatarImageNETMsgBitsProvider );
	}
#endif
...
#if 0
	// hack debugging of mapgroups on listen server
	Q_strncpy( m_szMapGroupName, "mg_active", sizeof( m_szMapGroupName ) );
#endif
...
#if defined( INCLUDE_SCALEFORM )
			if ( g_pScaleformUI ) g_pScaleformUI->SlotRelease( SF_SS_SLOT( msg.slot() ) );
#endif
		}
		break;
	}
...
void CBaseClientState::HandleReserveServerChallengeResponse( int nChallengeNr )
{
	...
	for ( int i = 0; i < m_netadrReserveServer.Count(); ++i )
	{
		NET_SendPacketToNsAddress( m_Socket, m_netadrReserveServer.Get( i ).m_adrRemote, msg.GetData(), msg.GetNumBytesWritten() );
	}
}
