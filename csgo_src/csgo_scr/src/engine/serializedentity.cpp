...
, m_nFieldDataBits( 0 )
, m_pFieldData( NULL )
, m_bHasUncompressibleProps( false )
{
}
...
void CSerializedEntity::Copy( const CSerializedEntity &other )
{
	if( this != &other )
	{
		...
		m_bHasUncompressibleProps = other.m_bHasUncompressibleProps;
	}
}
...
void CSerializedEntity::Swap( CSerializedEntity& other )
{
	...
	V_swap( m_bHasUncompressibleProps, other.m_bHasUncompressibleProps );
...
size_t CSerializedEntity::GetMemSize() const
{
	size_t nSize = sizeof( *this );

	nSize += m_nFieldCount * sizeof( *m_pFields );
	nSize += m_nFieldCount * sizeof( *m_pFieldDataOffsets );
	nSize += Bits2Bytes( m_nFieldDataBits );
...
public:
	...
	virtual size_t GetMemSize( SerializedEntityHandle_t handle ) OVERRIDE
	{
		if ( handle == SERIALIZED_ENTITY_HANDLE_INVALID )
			return 0;

		return reinterpret_cast< CSerializedEntity * >( handle )->GetMemSize();
	}
};
