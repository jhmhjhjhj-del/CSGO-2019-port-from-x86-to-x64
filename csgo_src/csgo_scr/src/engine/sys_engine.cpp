...
#ifdef NO_STEAM
static ConVar fps_max_menu( "fps_max_menu", "0", FCVAR_RELEASE, "Frame rate limiter, main menu" );
#else
...
void CEngine::Frame( void )
{
	...
	// Don't yield if we're currently loading, cause that sucks for load times.
	...
	if ( IsPC() && !game->IsActiveApp() && !sv.IsDedicated() 
		...
		&& !scr_drawloading
#endif
		&& !( cl_embedded_stream_video_playing.IsValid() && ( cl_embedded_stream_video_playing.GetFloat() > 0 ) )
