...
static void SetDispTraceSurfaceProps( trace_t *pTrace, CDispCollTree *pDisp )
{
	...
	if ( pTrace->IsDispSurfaceProp5() )
	{
		pTrace->surface.surfaceProps = pDisp->GetSurfaceProps( 4 );
	}
	else if ( pTrace->IsDispSurfaceProp4() )
	{
...
static CUtlVector< int > s_diagDisplacementsWithInvalidFaces;
...
void CM_CreateDispPhysCollide( dphysdisp_t *pDispLump, int dispLumpSize )
{
	...
	s_diagDisplacementsWithInvalidFaces.RemoveAll();
	for ( int i = 0; i < g_DispCollTreeCount; i++ )
	{
		...
		if ( !pDispTree->BCollisionFacesAreAllValid() )
		{
			s_diagDisplacementsWithInvalidFaces.AddToTail( i );
			DevWarning( "DISP_VPHYSICS entire displacement vdisp_%04d will have no collision, dimensions (%.2f %.2f %.2f) from (%.2f %.2f %.2f) to (%.2f %.2f %.2f)\n", i,
				abs( g_pDispBounds[ i ].maxs.x - g_pDispBounds[ i ].mins.x ), abs( g_pDispBounds[ i ].maxs.y - g_pDispBounds[ i ].mins.y ), abs( g_pDispBounds[ i ].maxs.z - g_pDispBounds[ i ].mins.z ),
				XYZ( g_pDispBounds[ i ].mins ), XYZ( g_pDispBounds[ i ].maxs )
				);
...
#ifndef DEDICATED
static void debug_drawdisp_boundbox_impl( int iDisp )
{
	if ( iDisp >= 0 && iDisp < g_DispCollTreeCount )
	{
		Vector vmins = g_pDispBounds[ iDisp ].mins;
		Vector vmaxs = g_pDispBounds[ iDisp ].maxs;
		Vector vorg = ( vmaxs + vmins ) / 2;
		CDebugOverlay::AddBoxOverlay( vorg, vmins - vorg, vmaxs - vorg, QAngle( 0, 0, 0 ), 255, 0, 0, 127, 10 );
		DevMsg( "Added debug draw box for displacement %d\n", iDisp );
	}
}
CON_COMMAND_F( debug_drawdisp_boundbox, "Create debug boxes for invalid displacements", FCVAR_CHEAT )
{
	if ( args.ArgC() > 1 )
	{
		if ( !V_stricmp( "invalid", args.Arg( 1 ) ) )
		{
			FOR_EACH_VEC( s_diagDisplacementsWithInvalidFaces, i )
			{
				debug_drawdisp_boundbox_impl( s_diagDisplacementsWithInvalidFaces[ i ] );
			}
			DevMsg( "Added debug draw boxes for %d total displacement(s)\n", s_diagDisplacementsWithInvalidFaces.Count() );
		}
		else
		{
			debug_drawdisp_boundbox_impl( V_atoi( args.Arg( 1 ) ) );
		}
	}
	else
	{
		DevMsg( "Usage:\n debug_drawdisp_boundbox invalid - to display invalid displacements\n debug_drawdisp_boundbox idx - to display specific displacement\n" );
	}
}
CON_COMMAND_F( debug_drawbox, "Create debug box", FCVAR_CHEAT )
{
	if ( args.ArgC() != 12 )
	{
		DevMsg( "Usage: debug_drawbox x1 y1 z1 x2 y2 z2 r g b a t\n" );
		return;
	}

	Vector v1( V_atof( args.Arg( 1 ) ), V_atof( args.Arg( 2 ) ), V_atof( args.Arg( 3 ) ) );
	Vector v2( V_atof( args.Arg( 4 ) ), V_atof( args.Arg( 5 ) ), V_atof( args.Arg( 6 ) ) );
	Vector rgb( V_atof( args.Arg( 7 ) ), V_atof( args.Arg( 8 ) ), V_atof( args.Arg( 9 ) ) );
	int nAlpha = V_atoi( args.Arg( 10 ) );
	int nTime = V_atoi( args.Arg( 11 ) );

	Vector vorg = ( v1 + v2 ) / 2;
	CDebugOverlay::AddBoxOverlay( vorg, v1 - vorg, v2 - vorg, QAngle( 0, 0, 0 ), rgb.x, rgb.y, rgb.z, nAlpha, nTime );
}
#endif
...
void CM_PreStab( TraceInfo_t *pTraceInfo, const unsigned short *pDispList, int dispListCount, Vector &vStabDir, int collisionMask, int &contents )
{
	...
	for( int i = 0; i < dispListCount; i++ )
	{
		int dispIndexOther = pDispList[i];
		pDispTree = &g_pDispCollTrees[dispIndexOther];
