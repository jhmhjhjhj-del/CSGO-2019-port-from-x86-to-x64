...
void CollisionBSPData_Destroy( CCollisionBSPData *pBSPData )
{
	...
	if ( pBSPData->isPortalOpen.Base() )
	{
		pBSPData->isPortalOpen.Detach();
	}
...
void CollisionBSPData_LoadTextures( CCollisionBSPData *pBSPData )
{
	...
	int stringDataCount = lhStringData.LumpSize() / sizeof( const char );	// Yes, it's div by 1. This is the correct pattern to copy for other db-like chunks, though.

	if ( stringDataCount > 0 && pStringData[ stringDataCount - 1 ] != '\0' )
		Sys_Error( "Unable to load corrupt BSP %s", lh.GetLoadName() );
	...
	int stringTableCount = lhStringData.LumpSize() / sizeof( int );
	...
	static const char* s_WarningShaders[] = {
		"VertexLitGeneric",
		// add more?
	};
	...
	{
		Assert( in->nameStringTableID >= 0 && in->nameStringTableID < stringTableCount );
		if ( in->nameStringTableID < 0 || in->nameStringTableID >= stringTableCount )
			Sys_Error( "Unable to load corrupted map %s.", lh.GetLoadName() );

		int stringByteStart = pStringTable[ in->nameStringTableID ];
		
		Assert( stringByteStart >= 0 && stringByteStart < stringDataCount );
		if ( stringByteStart < 0 || stringByteStart >= stringDataCount )
			Sys_Error( "Unable to load corrupted map %s", lh.GetLoadName() );

		const char *pInName = &pStringData[ stringByteStart ];
		...
		{
			for ( int k = 0; k < ARRAYSIZE( s_WarningShaders ); k++ )
			{
				if ( !V_stricmp( s_WarningShaders[k], material->GetShaderName() ) )
				{
					Warning( "WARNING: BSP loaded %s material: %s\n", material->GetShaderName(), pBSPData->map_surfaces[i].name );
					Assert( false );
				}
			}
...
void CollisionBSPData_LoadAreas( CCollisionBSPData *pBSPData )
{
	...
	nAreaCount = lh.LumpSize() / sizeof(*in);
	if (nAreaCount > MAX_MAP_AREAS)
	{
		Sys_Error( "Map has too many areas");
	}

	int nSize = nAreaCount * ( sizeof(darea_t) + sizeof( uint16 ) ); // allocate space for map_areas and mapAreaFloodIndex
	pBSPData->map_areas.Attach( nAreaCount, (darea_t*)Hunk_AllocName( nSize, va( "%s [%s]", lh.GetLoadName(), "Areas" ) ) );
	pBSPData->mapAreaFloodIndex.Attach( nAreaCount, ( uint16 * )( pBSPData->map_areas.Base() + nAreaCount ) );
	pBSPData->numareas = nAreaCount;

	for ( i = 0; i < nAreaCount; i++ )
	{
		pBSPData->map_areas[i] = in[i];
	}
}
...
void CollisionBSPData_LoadAreaPortals( CCollisionBSPData *pBSPData )
{
	...
	pBSPData->isPortalOpen.Attach( count, (uint32*)Hunk_AllocName( pBSPData->GetPortalOpenWordCount() * sizeof( uint32 ), va( "%s [%s]", lh.GetLoadName(), "AreaPortals" ), false ) );
...
void CollisionBSPData_LoadPhysics( CCollisionBSPData *pBSPData )
{
	...
	if ( pBSPData->map_cmodels[0].vcollisionData.solidCount == 0 )
	{
		Sys_Error( "No physics for world in map %s", pBSPData->mapPathName );
	}
}
...
void CollisionBSPData_LoadDispInfo( CCollisionBSPData *pBSPData, texinfo_t *pTexinfo, int texinfoCount )
{
	...
	for ( i = 0; i < coreDispCount; ++i )
	{
		...
		for ( int j = 0; j < pFaces->numedges; j++ )
		{
		...
		// Lookup surface props.
		// We want to do this before we create the collision tree since we build the collision tree
		// slightly differently if $surfacepropalpha is supplied, applying different surface properties
		// to triangles with high alpha.
		bool bSurfacePropsFound[CDispCollTree::kMaxSurfacePropTypes] = { 0 };
		short nSurfaceProp[CDispCollTree::kMaxSurfacePropTypes];
		bool bTexInfoFlagsFound = false;
		int nTexInfoFlags = 0;
		float fSurfaceAlphaFilter = 0.5f;
		...
		if ( pTex->texdata >= 0 )
		{
			...
			{
				COMPILE_TIME_ASSERT( CDispCollTree::kMaxSurfacePropTypes == 5 );

				static const char* kSurfacePropNames[CDispCollTree::kMaxSurfacePropTypes] = {
					"$surfaceprop",
					"$surfaceprop2",
					"$surfaceprop3",
					"$surfaceprop4",
				...
				IMaterialVar *pVar;
				bool bVarFound;
				pVar = pMaterial->FindVar( kSurfacePropNames[0], &bVarFound, false );
				if ( bVarFound )
				{
					...
					for ( int j = 0; j < 4; ++j )
					{
						bSurfacePropsFound[j] = true;
						nSurfaceProp[j] = nPropIndex;
					}
				}

				for ( int j = 1; j < CDispCollTree::kMaxSurfacePropTypes; ++j )
				{
					pVar = pMaterial->FindVar( kSurfacePropNames[j], &bVarFound, false );
					if ( bVarFound )
					{
						const char *pProps = pVar->GetStringValue();
						bSurfacePropsFound[j] = true;
						nSurfaceProp[j] = physprops->GetSurfaceIndex( pProps );
					}
				}

				pVar = pMaterial->FindVar( "$surfacepropalphafilter", &bVarFound, false );
				if ( bVarFound )
				{
					fSurfaceAlphaFilter = pVar->GetFloatValue();
				}
			}

			bTexInfoFlagsFound = true;
			nTexInfoFlags = pBSPData->map_surfaces[pTex->texdata].flags;
		}

		// If $surfacepropalpha is specified, we want to apply that surface property to triangles with high alpha value
		bool bHasSurfaceAlpha = bSurfacePropsFound[4];

		// new collision
		FDispCollTreeParams dispTreeParams;
		dispTreeParams.bSurfacePropAlphaSpecified = bHasSurfaceAlpha;
		dispTreeParams.fSurfacePropAlphaFilter = fSurfaceAlphaFilter * 3.0f; // because we have three verts per tri

		pDispTree->Create( &coreDisp, dispTreeParams );
		...
		for ( int j = 0; j < CDispCollTree::kMaxSurfacePropTypes; ++j )
		{
			if ( bSurfacePropsFound[j] )
				pDispTree->SetSurfaceProps( j, nSurfaceProp[j] );
		}

		if ( bTexInfoFlagsFound )
			pDispTree->SetTexinfoFlags( nTexInfoFlags );
	}
