...
static ConVar con_timestamp( "con_timestamp", "0", FCVAR_RELEASE, "Prefix console.log entries with timestamps" );
...
void ConsoleLogFileCallback(IConVar *var, const char *pOldValue, float flOldValue )
{
	...
	// Assume failure. A player may get this path from a URL that was passed to them by another
	// player, so we have to guard against maliciousness in this code. We now severely restrict
	// where the log file can be written.
	con_debuglog = false;
	...
	// Need an extension
	const char* pExtension = V_GetFileExtension( logFile );
	if ( !pExtension )
		return;

	// And it needs to be log.
	if ( V_stricmp( "log", pExtension ) != 0 )
		return;

	// Fine, this is an acceptable place to create a file, I guess?
	con_debuglog = true;
}
...
private:
	...
	CThreadFastMutex	m_NotifyTextMutex;
...
{
	static bool s_bAllowed = !CommandLine()->CheckParm( "-noconsole" );
	return s_bAllowed;
}
...
void Con_ColorPrint( const Color& clr, char const *msg )
{
	...
	struct _AutoFreeMsgCopy {
		_AutoFreeMsgCopy() : m_p( NULL ) {}
		~_AutoFreeMsgCopy() { free( const_cast< char * >( m_p ) ); }
		const char *m_p;
	} autodelete_msgCopy;
	static bool s_bPerfectWorld = !!CommandLine()->FindParm( "-perfectworld" );
	if ( s_bPerfectWorld && g_ClientDLL )
	{
		const char *pszDup = g_ClientDLL->AllocateCensoredString( msg );
		if ( pszDup != msg )
		{
			autodelete_msgCopy.m_p = pszDup;
			msg = pszDup;
		}
	}
...
void CConPanel::AddToNotify( const Color& clr, char const *msg )
{
	...
	AUTO_LOCK_FM( m_NotifyTextMutex );
...
void CConPanel::DrawNotify( void )
{
	...
	CUtlVector<CNotifyText> arrRenderNotifyTextCopy;
	if ( m_NotifyText.Count() )
	{
		AUTO_LOCK_FM( m_NotifyTextMutex );
		arrRenderNotifyTextCopy.AddMultipleToTail( m_NotifyText.Count(), m_NotifyText.Base() );
	}
	int c = arrRenderNotifyTextCopy.Count();
	for ( int i = 0; i < c; i++ )
	{
		CNotifyText *notify = &arrRenderNotifyTextCopy[ i ];
...
int CConPanel::ProcessNotifyLines( int &left, int &top, int &right, int &bottom, bool bDraw )
{
	...
	for ( int i = 0; i < MAX_DBG_NOTIFY; i++ )
	{
		if ( realtime < da_notify[i].expire || da_notify[i].expire == -1 )
		{
			...
			if ( yBaseline + nFontTall > videomode->GetModeHeight() - nYMargin )
				return count;
