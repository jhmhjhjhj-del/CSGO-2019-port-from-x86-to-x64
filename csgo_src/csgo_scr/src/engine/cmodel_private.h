...
inline T &CRangeValidatedArray<T>::operator[]( int i )
{
	if ( i < 0 || i >= m_nCount )
	{
		Assert( !"Bad map, possibly malicious." );
...
public:
	...
	int									numareas;
	CRangeValidatedArray<darea_t>		map_areas;
	CRangeValidatedArray<uint16>		mapAreaFloodIndex;
	int									numareaportals;
	...
	int									numportalopen;
	int GetPortalOpenWordCount()const	{ return ( numportalopen + 31 ) / 32; }
	CRangeValidatedArray< uint32 >		isPortalOpen;
	int									allcontents;
