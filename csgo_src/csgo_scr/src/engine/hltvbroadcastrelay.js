...
var stats = { post_field: 0, get_field: 0, get_start: 0, get_frag_meta: 0, sync: 0, not_found: 0, new_acct: 0, err: zeroArray(8), requests: 0, started: Date.now(), version: 7 };

"use strict";
var port = 8080;

function prettyNum(n) {
	return n.toLocaleString({ useGrouping:true,maximumFractionDigits: 1 });
}
...
sizeAccum.prototype.toAvg = function () {
	if (this.count) {
		return prettyNum(this.gz / this.count) + " <span style=\"font-size:0.7em;color:#D3D3D3\">" + (100 * this.gz / this.ungz).toFixed(1) + "%</span>";
	}
	else
		return '';
}

sizeAccum.prototype.toSum = function () {
	if (this.count)
		return prettyNum(this.gz);
	else
		return '';
}

function datasize( obj, field, accum ) {
    var f = obj[field];
    if (f == null)
        return '';
    if (typeof f === 'string' || Buffer.isBuffer(f)) {
    	var ungzlen = obj[field + "_ungzlen"];
    	if (ungzlen) {
    		if (accum) {
    			accum.gz += f.length;
    			accum.count += 1;
    		...
    		return prettyNum(f.length) + " <span style=\"font-size:0.7em;color:#D3D3D3\">" + (100 * f.length / ungzlen).toFixed(1) + "%</span>"
		}
    	return prettyNum(f.length);
    }
    else if (f == null)
    	return '';
...
function listSingleAccount( accId, acc )
{
    ...
    var last_frag = 0, accumFull = new sizeAccum(), accumDelta = new sizeAccum();
    for (var fragment in acc) {
		...
		if (f) {
		    ...
		    if (acc[0] != null && acc[0].signup_fragment == fragment)
		        html += '<b>signup</b>';
		    html += '</td><td>' + datasize(f, 'full', accumFull) + '</td>';
		    html += '<td>' + datasize(f, 'delta', accumDelta) + '</td>';
			...
			html += '<td colspan="5"><center>------------=null=------------</center></td>';
    }
    html += '<tr><td colspan="3">Average</td><td>' + accumFull.toAvg() + '</td><td>' + accumDelta.toAvg() + '</td></tr>';
    html += '<tr><td colspan="3">Sum</td><td>' + accumFull.toSum() + '</td><td>' + accumDelta.toSum() + '</td></tr>';
    html += "</table>";
	return html;
}
...
function postField( request, param, response, acc, fragment, field )
{
	...
	// BEGIN DEBUGGING CODE HERE _
	/*
	var nSimulatedFailureFull = 24;
	var nSimulatedFailureDelta = 23;
	if ((fragment == nSimulatedFailureDelta && field == "delta")
		|| (fragment == nSimulatedFailureFull && field == "full")) {
		console.log("DEBUG FAILURE [" + fragment + "]." + field);
		return;
	}
	*/
	// END DEBUGGING CODE ABOVE ^^
	...
	request.on('end', function () {
		...
		zlib.gzip(totalBuffer, function (error, compressedBlob) {
			...
			if (error) {
				...
				acc[fragment][field] = totalBuffer;
			} else {
				...
				acc[fragment][field] = compressedBlob;
				acc[fragment][field + "_ungzlen"] = totalBuffer.length;
			}
		});
	});
}
...
function serveBlob( request, response, fragmentRec, field ) {
    ...
    if (blob == null) {
        ...
        if (Buffer.isBuffer(blob)) {
			...
			if(ungzipped_length)
			{
				//console.log("Serving " + blob.length + " bytes, gzipped from " + ungzipped_length + " : " + request.url);
				headers['Content-Encoding'] = 'gzip';
			}
			else
			{
				//console.log("Serving " + blob.length + " bytes, uncompressed: " + request.url);
			}
