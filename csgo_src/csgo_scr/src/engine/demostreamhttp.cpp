...
ConVar tv_playcast_retry_timeout( "tv_playcast_retry_timeout", "12", FCVAR_RELEASE, "In case of intermittent network problems, how long should playcast retry fragment retrieval before resorting to resync" );


static double s_flDemoFailTimeEnd = 0;
bool DemoShouldFailRequest()
{
	if ( s_flDemoFailTimeEnd > 0 && s_flDemoFailTimeEnd > Plat_FloatTime() )
	{
		return true;
	}
	else
	{
		s_flDemoFailTimeEnd = 0;
		return false;
	}
}
	...
	m_flBroadcastKeyframeInterval( 3 )
{
	...
	m_dSyncTimeoutEnd = -1;
	m_dLastSuccessfulFragmentRetrieval = Plat_FloatTime();
	m_nLastSuccessfullyRetrievedFragmentNumber = 0;
	m_dSyncRetryTimeEnd = 0;
}
...
bool CDemoStreamHttp::PrepareForStreaming( const char * pUrl )
{
	...
	DevMsg( "%.2f. Broadcast: Synchronizing stream\n", Plat_FloatTime() );
	m_nState = STATE_SYNC;
	return true;
}
...
void CDemoStreamHttp::SendSync( int nResync )
{
	...
	if ( m_bSyncFromGc )
	{
		...
		DevMsg( "%.2f. Requesting sync from GC, start fragment %d match id %llu instance %d\n", Plat_FloatTime(), m_SyncParams.m_nStartFragment, params.m_nMatchId, params.m_nInstanceId );
...
#endif
	{
		...
		DevMsg( "%.2f. Requesting sync from relay %s\n", Plat_FloatTime(), request );
...
void CDemoStreamHttp::OnStart( HTTPRequestHandle hRequest )
{
	...
	if ( !m_pStreamSignup )
	{
		DevMsg( "%.2f. Broadcast failed to start: cannot retrieve startup packet data\n", Plat_FloatTime() );
		StopStreaming();
		return;
	}

	DevMsg( "%.2f. Received signup fragment %d\n", Plat_FloatTime(), m_SyncResponse.nSignupFragment );
	if ( m_pClient )
	{
...
{
	m_dLastSuccessfulFragmentRetrieval = Plat_FloatTime();
	m_nLastSuccessfullyRetrievedFragmentNumber = MAX( m_nLastSuccessfullyRetrievedFragmentNumber, nFragment );
	...
	{
		DevMsg( "%.2f. Broadcast playback failed to retrieve %s frame of fragment %d\n", Plat_FloatTime(), AsString( nType ), nFragment ); // TODO: implement fault-tolerant recovery; we can request the next fragment's full frame
		StopStreaming();
	}
}

void CDemoStreamHttp::OnFragmentRequestFailure( const HTTPRequestCompleted_t * pResponse, int nFragment, FragmentTypeEnum_t nType )
{
	Fragment_t &fragment = Fragment( nFragment );
	fragment.ClearStreaming( nType );
	...
	if ( pResponse )
	{
		double dNow = Plat_FloatTime();
		if ( ( m_dLastSuccessfulFragmentRetrieval + tv_playcast_retry_timeout.GetFloat() >= dNow )
			&& ( nFragment >= m_nLastSuccessfullyRetrievedFragmentNumber - 4 ) )
		{
			Msg( "%.2f. Playcast cannot retrieve fragment %d, retrying\n", dNow, nFragment );
			RequestFragment( nFragment, nType );
		}
		else
		{
			Msg( "%.2f. Playcast failed to retrieve fragment %d (succ=%d, %.1f sec)\n", dNow, nFragment, m_nLastSuccessfullyRetrievedFragmentNumber, dNow - m_dLastSuccessfulFragmentRetrieval );
		}
	}
	else
	{
		DevMsg( "Playcast Canceling Fragment %d\n", nFragment );
	}
}
...
bool CDemoStreamHttp::OnEngineGotvSyncPacket( const CEngineGotvSyncPacket *pPkt )
{
	...
	if ( !pPkt->has_tick() )
	{
		...
		DevMsg( "%.2f. Waiting %.2f seconds\n", Plat_FloatTime(), flDelay );
...
bool CDemoStreamHttp::OnSync( const char *pBuffer, int nBufferSize, int nResync )
{
	...
	{
		DevMsg( "%.2f. Broadcast sync: malformed response: %s\n", Plat_FloatTime(), pBuffer );
		StopStreaming();
		return false;
	}
}
...
bool CDemoStreamHttp::OnSync( int nResync )
{
	...
	{
		DevMsg( "%.2f. Broadcast resync %d: the stream seems to have stopped (rcvage %.1f, rtdelay %.1f)\n", Plat_FloatTime(), nResync, m_SyncResponse.flReceiveAge, m_SyncResponse.flRealTimeDelay );
		StopStreaming();
		return false;
	}
	...
	{
		DevMsg( "%.2f. Broadcast m_SyncResponse: unexpected response. fragment %d must be at/after start fragment %d\n", Plat_FloatTime(), m_SyncResponse.nFragment, m_SyncResponse.nSignupFragment );
		StopStreaming();
		return false;
	}
	else
	{
		DevMsg( "%.2f. Broadcast: Buffering stream tick %d fragment %d signup fragment %d\n", Plat_FloatTime(), m_SyncResponse.nStartTick, m_SyncResponse.nSignupFragment, m_SyncResponse.nSignupFragment );
		m_nState = STATE_START;
		m_dSyncTimeoutEnd = -1;
		...
		if ( nResync )
		{
			if ( !m_pClient )
			{
				DevMsg( "%.2f. Broadcast resync failed: Client not connected to Stream\n", Plat_FloatTime() );
				StopStreaming();
				return false;
			}
			...
			else
			{
				...
				DevMsg( "%.2f. Resync %d response requires full stream restart because signup fragment changed from %d to %d\n", Plat_FloatTime(), nResync, m_nStreamSignupFragment, m_SyncResponse.nSignupFragment );
...
void CDemoStreamHttp::RequestFragment( int nFragment, FragmentTypeEnum_t nType )
{
	...
	{
		CFmtStr url(nType == FRAGMENT_FULL ? "/%d/full" : "/%d/delta", nFragment);
		if (demo_debug.GetBool())
			DevMsg("%.2f. Broadcast playback requesting %s\n", Plat_FloatTime(), url.Access());
		fragment.SetStreaming( nType );
		SendGet( url, new CFragmentRequest( nFragment, nType ) );
	}
}
...
{
	m_pParent->m_dSyncRetryTimeEnd = 0; // no timeout

	uint32 nBodySize;
	if( !s_pSteamHTTP->GetHTTPResponseBodySize( pResponse->m_hRequest, &nBodySize ) || nBodySize >= 1024 )
	{
		DevMsg( "%.2f. Broadcast sync: response buffer overflow (%d bytes)\n", Plat_FloatTime(), nBodySize );
		return;
	};
	...
	{
		DevMsg( "%.2f. Broadcast sync: cannot read response body\n", Plat_FloatTime() );
		return;
	}
...
{
	DevMsg("%.2f. (%d) stream resync failed. ", Plat_FloatTime(), m_nResync);
	...
	else if ( m_pParent->m_dSyncRetryTimeEnd > 0 )
	{
		if ( Plat_FloatTime() > m_pParent->m_dSyncRetryTimeEnd )
		{
			bRetry = false; // stop retrying
			m_pParent->m_dSyncRetryTimeEnd = 0;
			DevMsg( "Not retrying sync\n" );
		}
		else
		{
			DevMsg( "Retrying sync\n" );
		}
	}
	else
	{
		// start coutdown of retries
		m_pParent->m_dSyncRetryTimeEnd = Plat_FloatTime() + tv_playcast_retry_timeout.GetFloat();
		DevMsg( "Starting sync retry timeout %.2f\n", tv_playcast_retry_timeout.GetFloat() );
	}

	if ( !bRetry && ( !m_nResync || m_nResync > 5 ) )
	{
		CPendingRequest::OnFailure( pResponse );
	}
	else
	{
		DevMsg("%.2f. Retrying sync\n", Plat_FloatTime());
...
void CDemoStreamHttp::CPendingRequest::OnFailure( const HTTPRequestCompleted_t * pResponse )
{
	if ( !pResponse )
	{
		DevMsg( "%.2f. Broadcast IO error. Please try again later.\n", Plat_FloatTime());
	}
	else
	{
		char reason[ 1024 ];
		if ( !s_pSteamHTTP->GetHTTPResponseHeaderValue( pResponse->m_hRequest, "X-Reason", ( uint8*)reason, sizeof( reason ) ) )
			reason[ 0 ] = '\0';
		DevMsg( "%.2f. Broadcast Streaming error %d %s\n", Plat_FloatTime(), pResponse->m_eStatusCode, reason );
	}
...
CDemoStreamHttp::Buffer_t * CDemoStreamHttp::MakeBuffer( HTTPRequestHandle hRequest )
{
	...
	{
		DevMsg( "%.2f. Could not get http response body (size %d bytes)\n", Plat_FloatTime(), nBodySize );
		delete[] pMemory;
		return NULL;
	}
...
void CDemoStreamHttp::SendGet( const char *pPath, CPendingRequest *pRequest )
{
	...
	else
	{
		...
		DevMsg( "%.2f. Broadcast streaming: unexpected failure getting %s\n", Plat_FloatTime(), pPath );
...
void CDemoStreamHttp::CPendingRequest::Run( void *pvParam, bool bIOFailure, SteamAPICall_t hSteamAPICall )
{
	...
	else
	{
		...
		if ( nStatus != k_EHTTPStatusCode200OK || DemoShouldFailRequest() ) // we should always get a 200
		{
			OnFailure( ( HTTPRequestCompleted_t * )pvParam );
		}
		else
...
{
	if ( demo_debug.GetBool() )
		DevMsg( "%.2f. Failed to retrieve %s fragment %d\n", Plat_FloatTime(), s_pFragmentTypeName[ m_nType ], m_nFragment );
	m_pParent->OnFragmentRequestFailure( pResponse, m_nFragment, m_nType );
