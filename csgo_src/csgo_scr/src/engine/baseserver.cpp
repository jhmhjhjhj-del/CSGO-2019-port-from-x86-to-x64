...
static bool g_bServerNotifyVarChangeCallbackInstalled = false;
								...
								const byte *pbEncryptionKey, int nEncryptionKeyIndex )
{
	...
	for ( int slot = 0 ; slot < m_Clients.Count() ; slot++ )
	{
		...
		{
			// Check if he's still early in the connection state machine, then he probably
			// this is just a redundant request, because he didn't receive an earlier S2C_CONNECTION
			// reply we sent.
			if ( client->GetSignonState() == SIGNONSTATE_CONNECTED )
			{
				const unsigned char *pCurrentEncryptionKey = client->m_NetChannel->GetChannelEncryptionKey();
				if (
					( pbEncryptionKey && pCurrentEncryptionKey && memcmp( pbEncryptionKey, pCurrentEncryptionKey, NET_CRYPT_KEY_LENGTH ) == 0 )
					|| !pbEncryptionKey // NOTE: Not setting a particular encryption key now.  Assume they didn't request it before!
				) {

					// We already did all of the security checks.  Just re-send
					// his reply.  If he changed any info in this connect request
					// compared to the earlier one we accepted, well we're ignoring
					// that change.
					NET_OutOfBandPrintf ( m_Socket, adr, "%c.%08X.0000.0000.0000.0000.", S2C_CONNECTION, nEncryptionKeyIndex );
					return client;
				}
				Warning( "%s is reconnecting, with a different encryption key!\n", sAdr.String() );
			}

			// Otherwise, hm, weird.  We'll tear down any existing client
			ConMsg ( "%s:reconnect\n", sAdr.String() );
	...
	{
		RejectConnection( adr, "#Valve_Reject_Banned_From_Server" );
		return NULL;
	}
...
bool CBaseServer::ProcessConnectionlessPacket(netpacket_t * packet)
{
	...
	switch ( c )
	{
		...
		case C2S_CONNECT :	{	char cdkey[STEAM_KEYSIZE];
								...
								bool bNetEncryptPrivateKey = false;

								// SteamNetworkingSockets do encryption, no need to do it again at this layer
								if ( packet->from.GetAddressType() == NSAT_PROXIED_CLIENT )
								{
									if ( nEncryptionKeyIndex != 0 )
									{
										DevMsg( "Rejecting connection request from %s, relayed clients should not use Source engine certificate, since SDR connection is already encrypted\n", ns_address_render( packet->from ).String() );
										RejectConnection( packet->from, "Don't use certificate for SDR connection\n" );
									}
								}
								else
								{
									bNetEncryptPrivateKey = NET_CryptGetNetworkCertificate( k_ENetworkCertificate_PrivateKey, &pbNetEncryptPrivateKey, &cbNetEncryptPrivateKey );
								}
		...
		case A2S_PING: 
			{
				if ( msg.ReadLong() == GetHostVersion() )
				{
					...
					char	buffer[64];
					bf_write msgPingResponse(buffer,sizeof(buffer));

					msgPingResponse.WriteLong( CONNECTIONLESS_HEADER );
					msgPingResponse.WriteByte( S2A_PING_RESPONSE );
					msgPingResponse.WriteLong( GetHostVersion() );
					msgPingResponse.WriteLong( token );

					NET_SendPacketToNsAddress( m_Socket, packet->from, msgPingResponse.GetData(), msgPingResponse.GetNumBytesWritten() );
				}
				...
				case 1:
					{
						  ...
						  NET_SendPacketToNsAddress( m_Socket, packet->from, ( unsigned char * ) buf.Base(), buf.TellPut() );
					}
					break;
				}
			}
...
{
	VPROF_BUDGET( "FillServerInfo", VPROF_BUDGETGROUP_OTHER_NETWORKING );
...
void CBaseServer::ReplyChallenge( const ns_address &adr, bf_read &inmsg )
{
	...
	// Check if we are running with a special flag and not advertise to Steam
	bool bHasSpecialFlag = g_pLaunchOptions && g_pLaunchOptions->GetFirstSubKey() && g_pLaunchOptions->GetFirstSubKey()->GetNextKey() && g_pLaunchOptions->GetFirstSubKey()->GetNextKey()->GetNextKey();
	if ( bHasSpecialFlag && !V_strcmp( g_pLaunchOptions->GetFirstSubKey()->GetNextKey()->GetNextKey()->GetString(), "server_is_unavailable" ) )
	{
	...
	if ( StringHasPrefix( context, "connect" ) )
	{
		...
		bool bServerRequiresEncryptedChannels = ( ( this == &sv ) &&
			( adr.GetAddressType() != NSAT_PROXIED_CLIENT ) && // SteamNetworkingSockets do encryption, no need to do it again at this layer
	...
	NET_SendPacketToNsAddress( m_Socket, adr, msg.GetData(), msg.GetNumBytesWritten() );
...
void CBaseServer::ReplyReservationCheckRequest( const ns_address &adr, bf_read &msgIn )
{
	...
	uint8 uiTotalClientsInReservation = 0; // total number of clients in reservation
	if ( uiReservationStage && reservationMatch && uiClientSteamID && sv_mmqueue_reservation.GetString()[0] == 'Q' )
	{
		...
		if ( bThisClientShouldJoin )
		{
			...
			uiTotalClientsInReservation = m_arrReservationPlayers.Count();
			...
			if ( bThisClientJustConfirmed && serverGameDLL )
			{
				...
				bool bReady = serverGameDLL->ReportGCQueuedMatchStart( uiMinReservationLevel, arrConfirmedAccounts.Base(), arrConfirmedAccounts.Count() );
				
				bool bSpoofForcefulConnect = ( uiActualAwaitingClients && bReady && ( uiReservationStage == 2 ) );
				if ( bSpoofForcefulConnect )
				{
					// We are spoofing auto-accept, so force all other accounts as accepted on our side
					for ( int k = 0; k < m_arrReservationPlayers.Count(); ++k )
					{
						QueueMatchPlayer_t &qmp = m_arrReservationPlayers[ k ];
						if ( qmp.m_uiAccountID )
							qmp.m_uiReservationStage = uiReservationStage;
					}
				}

				if ( !uiActualAwaitingClients || bSpoofForcefulConnect )
				{
					...
					DevMsg( "Reservation time extended +%d sec (%d, %d)\n", sv_mmqueue_reservation_extended_timeout.GetInt(), uiActualAwaitingClients, bReady );
				}

				if ( bSpoofForcefulConnect )
				{
					uiActualAwaitingClients = 0;
				}
			}
		}
	}
	...
	//
	{
		...
		msg.WriteByte( uiTotalClientsInReservation );
	...
	if ( !uiAwaitingClients )
	{
		...
		{
			char	buffer2[64];
			bf_write msgReserveCheckResponse(buffer2,sizeof(buffer2));

			msgReserveCheckResponse.WriteLong( CONNECTIONLESS_HEADER );
			msgReserveCheckResponse.WriteByte( S2A_RESERVE_CHECK_RESPONSE );
			msgReserveCheckResponse.WriteLong( GetHostVersion() );
			msgReserveCheckResponse.WriteLong( m_arrReservationPlayers[jj].m_uiToken );
			msgReserveCheckResponse.WriteLong( uiReservationStage );
			msgReserveCheckResponse.WriteByte( uiAwaitingClients );
			msgReserveCheckResponse.WriteByte( uiTotalClientsInReservation );

			NET_SendPacketToNsAddress( m_Socket, m_arrReservationPlayers[jj].m_adr, msgReserveCheckResponse.GetData(), msgReserveCheckResponse.GetNumBytesWritten() );
		}
	}
}
...
void CBaseServer::NotifyFixupTargetOfFixupOperatorModeChange( bool bOn, const char *pName )
{
	if ( g_nFixupTargetEdict >= 0 )
	{
		// do we have any hltv client fixing up the primary cameraman?
		int nClients = GetClientCount();
		IClient *pFixupTargetClient = nullptr;

		for ( int i = 0; i < nClients; ++i )
		{
			IClient *pClient = GetClient( i );
			if ( pClient && pClient->IsActive() && pClient->GetPlayerSlot() + 1 == g_nFixupTargetEdict )
			{
				pFixupTargetClient = pClient;
			}
		}
		if ( pFixupTargetClient )
		{
			CSVCMsg_HltvFixupOperatorStatus_t msgStatus;
			msgStatus.set_mode( bOn );
			msgStatus.set_override_operator_name( pName );
			pFixupTargetClient->SendNetMsg( msgStatus, true );
		}
	}
}
...
void CBaseServer::SendReservationStatus( EReservationStatus_t kEReservationStatus )
{
	...
	NET_SendPacketToNsAddress( m_Socket, m_ReservationStatus.m_Remote, msg.GetData(), msg.GetNumBytesWritten() );
...
bool CBaseServer::CheckChallengeType( CBaseClient * client, int nNewUserID, const ns_address &adr, int nAuthProtocol, const char *pchLogonCookie, int cbCookie )
{
	...
	// If Steam protocol is enforced for the connection, but the client attempts to downgrade then deny!
	if ( ( GetChallengeType( adr ) == PROTOCOL_STEAM ) && ( nAuthProtocol != PROTOCOL_STEAM ) )
	{
		RejectConnection( adr, "Bad protocol challenge.\n" );
		return false;
	}
...
void CBaseServer::Init( bool bIsDedicated )
{
	...
	g_bSSE42 = !CommandLine()->FindParm( "-nosse4" ) && GetCPUInformation().m_bSSE42;
	...
	if ( !g_pKVrulesConvars )
	{
		...
		if ( !g_pKVrulesConvars->LoadFromFile( g_pFullFileSystem, "gamerulescvars.txt", "MOD" ) && bIsDedicated )
		{
			Warning( "Failed to load gamerulescvars.txt, game rules cvars might not be reported to management tools.\n" );
		}
	}
	
	if ( !g_bServerNotifyVarChangeCallbackInstalled )
	{
		g_pCVar->InstallGlobalChangeCallback( ServerNotifyVarChangeCallback );
		g_bServerNotifyVarChangeCallbackInstalled = true;
	}
...
void CBaseServer::ForwardPacketsFromMasterServerUpdater()
{
	...
	while ( 1 )
	{
		...
		NET_SendPacketToNsAddress( m_Socket, adr, packetData, len );
	}
}
		...
		if ( client->IsConnected() )
		{
			if ( adr.CompareAdr ( client->m_NetChannel->GetRemoteAddress() ) )
			{
				...
				for ( int k = host_state.max_splitscreen_players; k -- > 1; )
				{
					...
					// Perform a silent netchannel shutdown, don't send disconnect msg.  But if we have a real
					// connection, we need to provide something.
					if ( pSsUser->m_NetChannel->GetSteamNetConnection() == k_HSteamNetConnection_Invalid )
						pSsUser->m_NetChannel->Shutdown( k_EConnectionEnd_UnusualGeneric, NULL );
					else
						pSsUser->m_NetChannel->Shutdown( k_EConnectionEnd_UnusualGeneric, "Recycle CBaseClient (splitscreen)" );
				...
				if ( client->m_NetChannel->GetSteamNetConnection() == k_HSteamNetConnection_Invalid )
					client->m_NetChannel->Shutdown( k_EConnectionEnd_UnusualGeneric, NULL );
				else
					client->m_NetChannel->Shutdown( k_EConnectionEnd_UnusualGeneric, "Recycle CBaseClient" );
