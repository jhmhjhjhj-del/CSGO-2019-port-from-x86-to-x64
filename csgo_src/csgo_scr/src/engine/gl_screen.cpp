...
void SCR_BeginLoadingPlaque( const char *levelName /*= NULL*/ )
{
	...
	if ( !scr_drawloading )
	{
		...
		//Stop sounds except main menu music
		S_StopAllSounds( true, false );

		//We no longer want to prevent sound on the loading screen, so
		//we can play main menu music.
//		S_PreventSound( true ); //this will stop audio from reaching the mixer until SCR_EndLoadingPlaque is called.
		...
		{
			Assert( GetGameUI() );
			GetGameUI()->SetBackgroundMusicDesired( false, true );
		}
...
void SCR_UpdateScreen( void )
{
	...
	{
		static int s_nScrBegin = 0;
		const int nB = ++s_nScrBegin;
		static ConVarRef refChainBegin( "panorama_render_chain" );
		if ( ( refChainBegin.IsValid() ? refChainBegin.GetInt() : 0 ) >= 2 && ( nB <= 16 || ( nB % 120 ) == 0 ) )
			ConMsg( "PanRenderChain CALLSITE SCR_UpdateScreen BeginFrame #%d (reset only)\n", nB );
	...
	if ( !CommandLine()->CheckParm( "-scaleform" ) )
	{
		static int s_nScrEnd = 0;
		const int nE = ++s_nScrEnd;
		static ConVarRef refChainEnd( "panorama_render_chain" );
		if ( ( refChainEnd.IsValid() ? refChainEnd.GetInt() : 0 ) >= 2 && ( nE <= 16 || ( nE % 120 ) == 0 ) )
			ConMsg( "PanRenderChain CALLSITE SCR_UpdateScreen EndFrame #%d (reset only)\n", nE );
		pRenderContext.GetFrom( materials );
