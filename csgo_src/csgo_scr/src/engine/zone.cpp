...
//
// [iestyn:2018.11.22] the hunk allocator is being deprecated (see zone.h)
//
...
CHunkDeprecated CHunkDeprecated::s_Hunk;

void Memory_Init( void )
{
	// $TODO: [iestyn] when Memory_Init is called Hunk_Size() will always return zero, so this logic seems broken... is host_parms.memsize obsolete? Need to do some code archaeology.
	//
	int nTargetCacheSize = Max( 0x100000, (int)host_parms.memsize - Hunk_Size() );
	g_pDataCache->SetSize( nTargetCacheSize );
}

void Memory_Shutdown( void )
{
	CHunkDeprecated::s_Hunk.Purge();
	g_pDataCache->SetSize( 0 ); // This disconnects the engine data cache
}
...
#else
	// Let's have 128 MB hunk on 32-bit platforms too so that we could support Danger Zone
	int nMaxBytes = 128*MB;
#endif
