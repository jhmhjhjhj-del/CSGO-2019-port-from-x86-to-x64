...
void CPluginGameUIDialog::OnCommand( const char *cmd )
{
	if ( !Q_stricmp( cmd, "close" ) )
	{
		if ( Q_strlen(m_szEntryCommand) > 0 )
		{
			...
			Cbuf_AddText( Cbuf_GetCurrentPlayer(), userCMD, kCommandSrcNetServer );
		}
		Close();
...
// XXX(JohnS): 6/19/18 - Disabled by default
//             On top of there being a security issue I fixed in 'plugin messages', then pop up a random dialog with
//             completely server controlled text onto the client's main menu.  This seems like a huge mis-feature, and
//             they seem to be somewhat rare in the wild.
ConVar cl_showpluginmessages ( "cl_showpluginmessages2", "0", FCVAR_ARCHIVE, "Allow plugins to display messages to you" );
