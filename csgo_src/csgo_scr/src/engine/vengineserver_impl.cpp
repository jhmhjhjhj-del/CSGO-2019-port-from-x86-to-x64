...
void InvalidateSharedEdictChangeInfos()
{
	if ( g_SharedEdictChangeInfo.m_iSerialNumber == 0xFFFF )
	{
		...
		g_SharedEdictChangeInfo.m_iSerialNumber = 1; // skip the special "null" serial number
...
public:
	...
	*/
	...
	virtual void SendUserMessage( IRecipientFilter& filter, int message, const ::google::protobuf::Message &msg ) OVERRIDE
	{
	...
	virtual void StopClientsHltvReplay( ) OVERRIDE
	{	// _some_ user messages are special and real-time and can't be skipped or queued.
		// stop replay for those messages
		sv.StopHltvReplay(  );
	}
	virtual void SetFixupTarget( int edict ) OVERRIDE
	{
		g_nFixupTargetEdict = edict;// from now on, this entity packs will be marked as "has uncompressible data", and their uncompressible props won't be compressed and will be left over for patching with CCLCMsg_HltvFixupOperatorTick message
	}
	...
	virtual void AddOriginToPVS( EnginePVSContext_t *pContext, const Vector& origin )
	{
		::SV_AddOriginToPVS( pContext, origin );
	}
	
	virtual void ResetPVS( EnginePVSContext_t *pContext, byte* pvs, int pvssize )
	{
		::SV_ResetPVS( pContext, pvs, pvssize );
	}
	
	virtual void		SetAreaPortalState( int portalNumber, bool isOpen )
	{
		CM_SetAreaPortalState( portalNumber, isOpen);
	}

	virtual void		SetAreaPortalStates( EnginePVSContext_t *pEnginePVSContext, const int *portalNumbers, const bool *isOpen, int nPortals )
	{
		CM_SetAreaPortalStates( pEnginePVSContext, portalNumbers, isOpen, nPortals );
	}
	...
	virtual void SetFakeClientConVarValue( edict_t *pEntity, const char *pConVarName, const char *pValue )
	{
		...
		{
			client->SetUserCVar( pConVarName, pValue );
...
private:
	...
	virtual int	CheckAreasConnected( EnginePVSContext_t *pContext, int area1, int area2 );
	virtual int GetArea( const Vector& origin );
	virtual void GetAreaBits( EnginePVSContext_t *pEnginePVSContext, int area, unsigned char *bits, int buflen );
...
//-----------------------------------------------------------------------------
static CMemoryPoolMT s_PVSInfoAllocator( 128, 128 * 64, CUtlMemoryPool::GROW_SLOW, "pvsinfopool", 128 );
...
void CVEngineServer::PlaybackTempEntity( IRecipientFilter& filter, float delay, const void *pSender, const SendTable *pST, int classID  )
{
	...
	// write all properties, if init or reliable message delta against zero values; assuming temp ents never have uncompressible props
	if( !SendTable_Encode( pST, handle, false, pSender, classID, NULL ) )
	{
...
int	CVEngineServer::CheckAreasConnected( EnginePVSContext_t *pContext, int area1, int area2 )
{
	if ( area1 == area2 )
		return true; // obvious early-out so that we can assume different areas later
	
	if ( !pContext )
	{
		for ( int i = 0; i < sv.GetClientCount(); ++i )
		{
			if ( IClient *pClient = sv.GetClient( i ) )
			{
				pContext = pClient->GetLastPVSContext();
				if ( pContext )
					break; // found one
			}
		}
	...
	if ( !pContext )
	{
		// could not find a valid client, or a valid client PVS state; nothing is connected
		return false;
	}
	return CM_CheckAreasConnected( pContext, area1, area2);
}
...
void CVEngineServer::GetAreaBits( EnginePVSContext_t *pEnginePVSContext, int area, unsigned char *bits, int buflen )
{
	CM_WriteAreaBits( pEnginePVSContext, bits, buflen, area );
}
...
// Note: this function must be locked (not called for the same pEdict from two threads)
...
void CVEngineServer::CleanUpEntityClusterList( PVSInfo_t *pPVSInfo )
{
	...
	{
		pPVSInfo->m_nClusterCount = 0;
		ThreadMemoryBarrier(); // make sure no access to the array in multithreaded environment
		pPVSInfo->m_pClusters = 0;
		s_PVSInfoAllocator.Free( pPVSInfo->m_pClusters );
