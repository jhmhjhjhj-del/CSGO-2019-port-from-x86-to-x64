...
steam_no_good:
	...
	extern ConVar tv_enable1;
	NET_SteamDatagramServerListen( NS_SERVER, STEAM_P2P_CHANNEL_DEFAULT );
	if ( tv_enable.GetBool() )
		NET_SteamDatagramServerListen( NS_HLTV, STEAM_P2P_CHANNEL_HLTV );
	//if ( tv_enable1.GetBool() )
	//	NET_SteamDatagramServerListen( NS_HLTV1, STEAM_P2P_CHANNEL_HLTV1 );
...
void CSteam3Server::Shutdown()
{
	if ( !BIsActive() )
		return;
	Msg( "CSteam3Server::Shutdown" );

	// Let network system know we are nuking these interfaces,
	// so it can go ahead and shutdown the appropriate Steam
	// networking sockets interfaces
	extern void NET_SteamGameServer_Shutdown();
	NET_SteamGameServer_Shutdown();

#if !defined( NO_STEAM )
	Msg( "SteamGameServer_Shutdown" );
...
#ifndef DEDICATED
	if ( !NET_IsDedicated()
		&& ( pValidateAuthTicketResponse->m_eAuthSessionResponse == k_EAuthSessionResponseNoLicenseOrExpired ) && pValidateAuthTicketResponse->m_SteamID.IsValid()
		&& Steam3Client().IsInitialized() && Steam3Client().SteamUser()
		&& ( Steam3Client().SteamUser()->GetSteamID() == pValidateAuthTicketResponse->m_SteamID ) )
	{
#if DEVELOPMENT_ONLY
		DevMsg( "Debug spew: allowing local user to play without a license or expired license!\n" );
#endif
		return;
	}
#endif
...
void CSteam3Server::OnValidateAuthTicketResponseHelper( CBaseClient *cl, EAuthSessionResponse eAuthSessionResponse )
{
	...
	case k_EAuthSessionResponseNoLicenseOrExpired:
		cl->Disconnect( "This Steam account does not own this game. Please login to the correct Steam account" );
		break;
...
#if defined( NO_STEAM )
	else if ( client && !client->IsFakeClient() && !bClientIsTV )
	{
		// Activate() returns early under NO_STEAM — no GS, no CreateUnauthenticatedUser.
		// Stamp offline stub SteamID so server GetSteamID / stub_items paths work (USP, paints).
#ifndef DEDICATED
		if ( Steam3Client().SteamUser() )
			steamID = Steam3Client().SteamUser()->GetSteamID();
#endif
		if ( !steamID.IsValid() )
			steamID.Set( 13371337u, k_EUniversePublic, k_EAccountTypeIndividual );
	}
#endif
...
#endif
		extern void NET_RunSteamNetworkingCallbacksGameServer();
		NET_RunSteamNetworkingCallbacksGameServer();
	}
}
