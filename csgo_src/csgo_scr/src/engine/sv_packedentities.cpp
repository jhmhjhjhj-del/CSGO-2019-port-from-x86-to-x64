...
ConVar tv_allow_camera_man_override( "tv_allow_camera_man_override", "0", FCVAR_RELEASE, "Allows cameraman_override to have effect. When this is set, the primary interactive caster will have all the relevant fields present in all network packets, in every snapshot. This allows the secondary cameraman (-interactivecaster that connects to a tv port) to override those fields some seconds later regardless of whether they changed originally or not." );
...
static void ValidateIncrementalChanges( ServerClass* pServerClass, const CalcDeltaResultsList_t& incremental, edict_t* edict, int edictIdx, bool bHasUncompressibleProps, SerializedEntityHandle_t tPrevProps )
{
	...
	SendTable_Encode( pSendTable, comparePackedProps, bHasUncompressibleProps, edict->GetUnknown(), edictIdx, &recip );
...
// the edict that is fixup target. This is a clumsy way to identify the entity that needs patching (main fixup operator), 
// but it should be easy to replace when/if we have an architecture for it
int g_nFixupTargetEdict = -1; // for debugging, always transmit entity 8

bool IsCameramanOverrideActive()
{
	return g_nFixupTargetEdict >= 0 && tv_allow_camera_man_override.GetBool();
}

bool HasUncompressibleProps( int edictIdx, const SendTable *pSendTable )
{
	return ( edictIdx == g_nFixupTargetEdict ) && tv_allow_camera_man_override.GetBool() && pSendTable->m_pExtension;
}
	...
	CFrameSnapshot *pSnapshot )
{
	...
	bool bHasUncompressibleProps = HasUncompressibleProps( edictIdx, pSendTable );
	//if the entity has no state changes, we can just reuse the previously sent packet; but not if it has perpetual delta props that will later be patched in hltv stream
	if( !edict->HasStateChanged() )
	{
		...
		if ( bUsedPrev && !sv_debugmanualmode.GetInt() && !bHasUncompressibleProps )
		{
	...
	if( bCanFastPath )
	{
		...
		newPackedProps = SendTable_BuildDeltaProperties( edict, edictIdx, bHasUncompressibleProps, pSendTable, pPrevFrame->GetPackedData(), deltaProps, &recip, bCanReuseOldData );
			...
			if( bCanReuseOldData && pPrevFrame->CompareRecipients( recip ) )
			{
				if( framesnapshotmanager->UsePreviouslySentPacket( pSnapshot, edictIdx, iSerialNum ) )
				{
					...
					if( sv_validate_edict_change_infos.GetBool() )
						ValidateIncrementalChanges( pServerClass, deltaProps, edict, edictIdx, bHasUncompressibleProps, pPrevFrame->GetPackedData() );	
	...
	{	
		...
		if( !SendTable_Encode( pSendTable, newPackedProps, bHasUncompressibleProps, edict->GetUnknown(), edictIdx, &recip ) )
		{							 
...
// parallel_send 12 overhead:  2.5-3x win32, 2.2x linux
void SendTask_SetupFrame( CGameClient * pClient, CFrameSnapshot * snapshot )
{
	if ( pClient->IsHltvReplay() )
	{
		pClient->SetupHltvFrame( snapshot->m_nTickCount ); // threading note: this blocks on walking and expanding delta frames
		// skip all the transmit checks if we already have packs prepared by HLTV that we'll be sending anyway
	}
	else
	{
		CCheckTransmitInfo *pInfo = &pClient->m_PackInfo;
		pClient->SetupPackInfo( snapshot );
		if ( pClient->m_nDeltaTick < 0 )
		{
			serverGameEnts->PrepareForFullUpdate( pClient->edict );
		}

		// CheckTransmit is ~1.7x overhead in 5-6 threads
		serverGameEnts->CheckTransmit( pInfo, snapshot->m_pValidEntities, snapshot->m_nValidEntities ); // may call BuildEntityClusterList, and CServerNetworkProperty::IsInPVS at the same time
		pClient->SetupPrevPackInfo();
	}
}
...
void SV_ComputeClientPacks( 
	...
	CFrameSnapshot *snapshot )
{
	VPROF_BUDGET( "SV_ComputeClientPacks", VPROF_BUDGETGROUP_OTHER_NETWORKING );
	MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
	// Do some setup for each client
	for ( int iClient = 0; iClient < clientCount; ++iClient )
	{
		SendTask_SetupFrame( clients[ iClient ], snapshot );
	}

	Assert( !g_pLocalNetworkBackdoor ); // network backdoor is not supported in CS:GO
