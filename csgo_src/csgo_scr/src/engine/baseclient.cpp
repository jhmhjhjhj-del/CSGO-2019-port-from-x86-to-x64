...
CBaseClient::CBaseClient()
{
	...
	m_nSignonStateConfirmationCount = 0;
...
char const *CBaseClient::GetUserSetting( char const *pConVarName ) const
{
	if ( !m_ConVars || !pConVarName || !pConVarName[0] )
	{
		return "";
	}

	const char * value = m_ConVars->GetString( pConVarName, "" );
	...
	if ( value[0]==0 )
	{
		...
		{
			return m_pAttachedTo->GetUserSetting( pConVarName );
		}

// 		// check if this var even existed
// 		if ( m_ConVars->GetDataType( pConVarName ) == KeyValues::TYPE_NONE )
// 		{ 
// 			DevMsg( "GetUserSetting: con var '%s' unknown.\n", pConVarName );
// 		}
	}
...
void CBaseClient::SetUserCVar( const char *pConVarName, const char *value)
{
	if ( !pConVarName || !value )
		return;

	m_ConVars->SetString( pConVarName, value );
}
...
void CBaseClient::Clear()
{
	...
	if ( m_NetChannel )
	{
		m_NetChannel->Shutdown( k_EConnectionEnd_UnusualGeneric, "Disconnect by server.\n");
		m_NetChannel = NULL;
	}
	...
	m_bFixupOperatorMode = false;
}

ConVar sv_signon_dos_disconnect( "sv_signon_dos_disconnect", "20", 0, "Number of extra signon state confirmations required to disconnect a misbehaving client." );
...
bool CBaseClient::ProcessSignonStateMsg(int state, int spawncount)
{
	...
	// Check for multiple-confirmation of same state and boot users who are misbehaving.
	// These messages are on the reliable channel so we shouldn't end up with multiples
	// This avoids a DoS attack where clients were repeatedly confirming their current
	// signon state in order to crash the server.
	m_nSignonStateConfirmationCount++;
	if ( m_nSignonStateConfirmationCount > 1 )
	{
		int numExtraConfirmations = m_nSignonStateConfirmationCount - 1;
		if ( sv_signon_dos_disconnect.GetBool() && numExtraConfirmations >= sv_signon_dos_disconnect.GetInt() )
		{
			Disconnect( "Connection error" );
		}
		
		// Ignore multiple confirmations
		return true;
	}
	...
	switch( GetSignonState() )
	{
		...
		case SIGNONSTATE_NEW		:	// client got server info, send prespawn datam_Client->SendServerInfo()
										...
										Assert( GetSignonState() == SIGNONSTATE_PRESPAWN );
										break;

		case SIGNONSTATE_PRESPAWN	:	SpawnPlayer();
										Assert( GetSignonState() == SIGNONSTATE_SPAWN );
										break;
		...
		case SIGNONSTATE_SPAWN		:	{
										...
										Assert( GetSignonState() == SIGNONSTATE_FULL );
										break;
...
void CBaseClient::PerformDisconnection( const char *pReason )
{
	...
	if ( m_NetChannel )
	{
		...
		// !KLUDGE!
		// Try to guess if the reason is "usual" or "unusual"
		static const char *arpszUsualReasons[] = {
		...
		int nReason = k_EConnectionEnd_UnusualGeneric;
		for ( const char *pszUsualReason: arpszUsualReasons )
		{
			if ( V_strnicmp( pReason, pszUsualReason, V_strlen(pszUsualReason) ) == 0 )
			{
		...
		m_NetChannel->Shutdown( nReason, pReason );
		m_NetChannel = NULL;
	}
...
{
	VPROF_BUDGET( "SendServerInfo", VPROF_BUDGETGROUP_OTHER_NETWORKING );
	...
	Assert( m_nSignonTick == sv.GetTick() );
	// inside SV_SendClientUpdates() we step stringtable a tick ahead
	// it's only called on final tick (so, usually every tick, but not always)
	// hence this assert
	Assert( m_nSignonTick == m_Server->m_StringTables->GetTick() || m_nSignonTick + 1 == m_Server->m_StringTables->GetTick() );
	...
	m_Server->m_StringTables->WriteBaselines( m_Server->GetMapName(), msg, m_nSignonTick );
#endif
...
void CBaseClient::ConnectionStart(INetChannel *chan)
{
	...
	m_NetMessages[ NETMSG_VoiceData ].Bind< CCLCMsg_VoiceData_t >( chan, UtlMakeDelegate( this, &CBaseClient::CLCMsg_VoiceData ), CNetMessageRatelimitPolicy( MINIMUM_TICK_INTERVAL, 3, 512 ) );
	m_NetMessages[ NETMSG_BaselineAck ].Bind< CCLCMsg_BaselineAck_t >( chan, UtlMakeDelegate( this, &CBaseClient::CLCMsg_BaselineAck ) );
	m_NetMessages[ NETMSG_ListenEvents ].Bind< CCLCMsg_ListenEvents_t >( chan, UtlMakeDelegate( this, &CBaseClient::CLCMsg_ListenEvents ), CNetMessageRatelimitPolicy( MINIMUM_TICK_INTERVAL, 10, 1024 ) );
	...
	m_NetMessages[ NETMSG_CmdKeyValues ].Bind< CCLCMsg_CmdKeyValues_t >( chan, UtlMakeDelegate( this, &CBaseClient::CLCMsg_CmdKeyValues ), CNetMessageRatelimitPolicy( MINIMUM_TICK_INTERVAL, 3, 512 ) );
	m_NetMessages[ NETMSG_HltvReplay ].Bind< CCLCMsg_HltvReplay_t >( chan, UtlMakeDelegate( this, &CBaseClient::CLCMsg_HltvReplay ), CNetMessageRatelimitPolicy( MINIMUM_TICK_INTERVAL, 3, 512 ) );
	
	m_NetMessages[ NETMSG_UserMessage ].Bind< CSVCMsg_UserMessage_t >( chan, UtlMakeDelegate( this, &CBaseClient::SVCMsg_UserMessage ) );
	m_NetMessages[ NETMSG_HltvFixupOperatorTick ].Bind< CCLCMsg_HltvFixupOperatorTick_t >( chan, UtlMakeDelegate( this, &CBaseClient::CLCMsg_HltvFixupOperatorTick ) );
}
...
void CBaseClient::ConnectionStop( )
{
	...
	m_NetMessages[ NETMSG_HltvFixupOperatorTick ].Unbind();
}
...
void CBaseClient::ApplyConVars( const CMsg_CVars& list, bool bCreateIfNotExisting )
{
	...
	for ( int i = 0; i < convars_size; ++i )
	{
		...
		if ( const char *subStr = strchr( name, '/' ) ) // our FCVAR_USERINFO convars are stored in a key value, so having slashes can allow clients ...
			continue;  // ... to create subkeys and bypass validation checks below. We no longer accept user info cvars with kv delimiters (see: KeyValues::FindKey)
...
bool CBaseClient::CLCMsg_ListenEvents( const CCLCMsg_ListenEvents& msg )
{
	...
	for( int i = 0, num = MIN( msg.event_mask_size(), EventArray.GetNumDWords() ); i < num; i++ )
	{
...
bool CBaseClient::SendSnapshot( CClientFrame *pFrame )
{
	...
	if ( !g_pLocalNetworkBackdoor )
	{
		...
		m_Server->m_StringTables->WriteUpdateMessageAtTick( this, pFrame->tick_count, GetMaxAckTickCount(), msg );
	}
#endif
...
{
	VPROF_BUDGET( "FillSignOnFullServerInfo", VPROF_BUDGETGROUP_OTHER_NETWORKING );
	//
