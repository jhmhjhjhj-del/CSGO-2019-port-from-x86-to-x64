...
public:
	...
	{
		Warning("Can't call SetOriginByGuid from server\n");
		return false;
	}
	...
	virtual IAudioOutputStream	*CreateOutputStream( uint nSampleRate, uint nChannels, uint nBits ) { Assert( !"Client-only service" ); return NULL; }
	virtual void				DestroyOutputStream( IAudioOutputStream *pOutputStream ) { Assert( !"Client-only service" ); }
