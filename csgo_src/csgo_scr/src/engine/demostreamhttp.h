...
protected:
	...
	void OnFragmentRequestFailure( const HTTPRequestCompleted_t * pResponse, int nFragment, FragmentTypeEnum_t nType );
...
protected:
	...
	double m_dLastSuccessfulFragmentRetrieval; // last real time a fragment was successfully retrieved; used to count down timeout when we should aggressively retry getting new fragments in case network falls out for a few seconds
	int m_nLastSuccessfullyRetrievedFragmentNumber; // last successfully retrieved fragment number
	double m_dSyncRetryTimeEnd; // next real time when we should stop retrying /sync request and fail; 0 means there's no countdown; used to count down timeout of retrying /sync in case network falls out for a few seconds
