...
static ConVar snd_show_events("snd_show_events", "0", FCVAR_DEVELOPMENTONLY, "Logs all sound events that are emitted");
...
public:
	...
	virtual int EmitSound( IRecipientFilter& filter, int iEntIndex, int iChannel, const char *pSoundEntry, HSOUNDSCRIPTHASH nSoundEntryHash, const char *pSample, 
		...
		const Vector *pOrigin, const Vector *pDirection, CUtlVector< Vector >* pUtlVecOrigins, bool bUpdatePositions, float soundtime = 0.0f, int speakerentity = -1, const CSoundParameters* params=nullptr  );
	...
	virtual bool	SetOriginByGuid(int guid, const Vector& origin);
...
int CEngineSoundClient::EmitSoundInternal( IRecipientFilter& filter, int iEntIndex, int iChannel, const char *pSoundEntry, HSOUNDSCRIPTHASH nSoundEntryHash, const char *pSample, 
	float flVolume, soundlevel_t iSoundLevel, int nSeed, int iFlags, int iPitch, 
	const Vector *pOrigin, const Vector *pDirection, CUtlVector< Vector >* pUtlVecOrigins, bool bUpdatePositions, float soundtime /*= 0.0f*/, int speakerentity /*= -1*/, const CSoundParameters* pAdditionalParams /*=nullptr*/  )
{
	...
	{
		//game may already be shutting down and entitylist cleared. Don't play anything.
		return 0;
	}
	...
	if( (pAdditionalParams == nullptr || pAdditionalParams->m_bUISound == false) && filter.IsLocalUIFilter() == false )
	{
	...
	} else if(pAdditionalParams != nullptr) {
		delay = pAdditionalParams->delay_msec/1000.0f;
	}
	...
	if ( pAdditionalParams != nullptr )
	{
		params.m_bAlertSound = pAdditionalParams->m_bAlertSound;
	}
	...
	const Vector *pOrigin, const Vector *pDirection, CUtlVector< Vector >* pUtlVecOrigins, bool bUpdatePositions, float soundtime /*= 0.0f*/, int speakerentity /*= -1*/, const CSoundParameters* pAdditionalParams /*=nullptr*/   )
{
	...
	return EmitSound( filter, iEntIndex, iChannel, pSoundEntry, nSoundEntryHash, pSample, flVolume, ATTN_TO_SNDLVL( flAttenuation ), nSeed, iFlags, 
		iPitch, pOrigin, pDirection, pUtlVecOrigins, bUpdatePositions, soundtime, speakerentity, pAdditionalParams );
	...
	const Vector *pOrigin, const Vector *pDirection, CUtlVector< Vector >* pUtlVecOrigins, bool bUpdatePositions, float soundtime /*= 0.0f*/, int speakerentity /*= -1*/, const CSoundParameters* pAdditionalParams /*=nullptr*/   )
{
	...
	else
	{
		return EmitSoundInternal( filter, iEntIndex, iChannel, pSoundEntry, nSoundEntryHash, pSample, flVolume, iSoundLevel, nSeed,
			iFlags, iPitch, pOrigin, pDirection, pUtlVecOrigins, bUpdatePositions, soundtime, speakerentity, pAdditionalParams );
	}
}
...
bool CEngineSoundClient::SetOriginByGuid(int guid, const Vector& origin)
{
	return S_SetOriginByGuid(guid, origin);
}
...
{
	return AudioStreamCreate( nSampleRate, nChannels, nBits );
}

void CEngineSoundClient::DestroyOutputStream( IAudioOutputStream *pOutputStream )
{
	AudioStreamDestroy( pOutputStream );
}
