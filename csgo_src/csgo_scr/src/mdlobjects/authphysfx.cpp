...
int CAuthPhysFx::FindNodeIndex( const char *pName )const
{
...
CLockedResource< PhysFeModelDesc_t > CAuthPhysFx::Compile( CResourceStream *pStream, const CVClothProxyMeshOptions *pOptions )const
{
	...
	// when we move over edge length, we start getting unpleasant vibrations. 
	// Multiple extra iterations may smooth it out, but convergence is about sqrt(N). We might want to expand the threshold by sqrt(N), but on the other hand we're already having issues with convergence if we're resorting to extra iterations.
	float flEdgeLength = GetAverageEdgeLength() * m_flTeleportDistanceMultiplier;
	builder.m_flTeleportDeltaSq = Sqr( flEdgeLength ); 
	...
	builder.m_nExtraIterations = m_nExtraIterations;
	if ( m_FollowSimCtrl.IsEmpty() )
	{
		builder.m_flFollowSimCtrl = 0.0f;
	}
	else
	{
		if ( m_flFollowSimCtrl > 0.0f )
		{
			int nFollowedNode = FindNodeIndex( m_FollowSimCtrl );
			if ( nFollowedNode >= 0 )
			{
				builder.m_nFollowSimCtrl = nFollowedNode;
				builder.m_flFollowSimCtrl = Min( Max( 0.0f, m_flFollowSimCtrl ), 1.0f );
			}
			else
			{
				Warning( "Cannot compile Follow ctrl: bone %s not found\n", m_FollowSimCtrl.Get() );
			}
		}
	}
