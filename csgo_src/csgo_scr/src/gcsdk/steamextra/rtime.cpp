...
// CRTimeDuration
//-----------------------------------------------------------------------------
bool CRTimeDuration::ParseDurationHoursMinutesSeconds( const char* szDuration )
{
	MakeZero();
	int numColons = 0;
	int amount = 0;
	char c;
	while( (c = *szDuration++) != '\0' )
	{
		if ( c >= '0' && c <= '9' )
		{
			amount *= 10;
		...
		else if ( c == ':' )
		{
			...
			if ( numColons == 3 )
				return false;
			m_nDiffs[kDiffMinutes] = m_nDiffs[kDiffSeconds];
			m_nDiffs[kDiffSeconds] = amount;
			amount = 0;
		}
		else
		{
	...
	m_nDiffs[kDiffHours] = m_nDiffs[kDiffMinutes];
	m_nDiffs[kDiffMinutes] = m_nDiffs[kDiffSeconds];
	m_nDiffs[kDiffSeconds] = amount;
	return true;
}

bool CRTimeDuration::ParseDurationISO8601( const char* szDuration )
{
	MakeZero();
	if ( !szDuration )
		return false;
	...
	while( (c = *szDuration++) != '\0' )
	{
		...
		if ( foundTimeMarker )
		{
			switch ( toupper( c ) )
			{
			case 'H': m_nDiffs[kDiffHours] += amount; break;
			case 'M': m_nDiffs[kDiffMinutes] += amount; break;
			case 'S': m_nDiffs[kDiffSeconds] += amount; break;
		...
		else
		{
			switch ( toupper( c ) )
			{
			case 'Y': m_nDiffs[kDiffYears] += amount; break;
			case 'M': m_nDiffs[kDiffMonths] += amount; break;
			case 'W': m_nDiffs[kDiffDays] += amount * 7; break;
			case 'D': m_nDiffs[kDiffDays] += amount; break;
			default: return false;
			}
		}
	}
...
bool CRTimeDuration::ParseDuration( const char* szDuration )
{
	MakeZero();
	if ( !szDuration )
		return false;
	if ( strchr( szDuration, ':' ) )
		return ParseDurationHoursMinutesSeconds( szDuration );
	return ParseDurationISO8601( szDuration );
}

void CRTimeDuration::TimeAdd( int amount, ETimeUnit unit )
{
	switch(unit) {
	case k_ETimeUnitSecond: m_nDiffs[kDiffSeconds] += amount; break;
	case k_ETimeUnitMinute: m_nDiffs[kDiffMinutes] += amount; break;
	case k_ETimeUnitHour: m_nDiffs[kDiffHours] += amount; break;
	case k_ETimeUnitDay: m_nDiffs[kDiffDays] += amount; break;
	case k_ETimeUnitWeek: m_nDiffs[kDiffDays] += amount * 7; break;
	case k_ETimeUnitMonth: m_nDiffs[kDiffMonths] += amount; break;
	case k_ETimeUnitYear: m_nDiffs[kDiffYears] += amount; break;
	default: break;
	}
}

time_t CRTimeDuration::ApplyToTm( struct tm *pTm ) const
{
	pTm->tm_year += m_nDiffs[kDiffYears];
	pTm->tm_mon += m_nDiffs[kDiffMonths];
	pTm->tm_mday += m_nDiffs[kDiffDays];
	pTm->tm_hour += m_nDiffs[kDiffHours];
	pTm->tm_min += m_nDiffs[kDiffMinutes];
	pTm->tm_sec += m_nDiffs[kDiffSeconds];
	return mktime( pTm );
}

int CRTimeDuration::Divide( RTime32 start, RTime32 target ) const
{
	bool zero = true;
	...
	for ( ;; )
	{
		RTime32 testTarget = CRTime::RTime32Add( start, *this * max );
		if ( testTarget < k_RTime32MinValid )
			return 0;
	...
	{
		int mid = ( min + max ) / 2;
		RTime32 testTarget = CRTime::RTime32Add( start, *this * mid );
		if ( testTarget < k_RTime32MinValid )
			return 0;
		if ( testTarget > target )
			max = mid;
	...
	return flipSign ? ( -min ) : min;
}

const char* CRTimeDuration::Render() const
{
	...
	*buf++ = 'P';
	...
	for ( int i = 0; i < kNumDiff; ++i )
	{
		if ( m_nDiffs[i] == 0 )
			continue;
		bAny = true;
		if ( i >= kDiffHours && !bShort )
		{
			bShort = true;
			*buf++ = 'T';
		}
		buf += sprintf( buf, "%d", m_nDiffs[i] );
		static const char diffChars[] = "YMDHMS";
		*buf++ = diffChars[i];
	}
	if ( !bAny )
	{
...
RTime32 CRTime::RTime32Add( const RTime32 rtime32, const CRTimeDuration& duration )
{
	...
	if ( !ptmCur )
		return k_RTime32Nil;
	ptmCur->tm_isdst = -1;
	return duration.ApplyToTm( ptmCur );
}
