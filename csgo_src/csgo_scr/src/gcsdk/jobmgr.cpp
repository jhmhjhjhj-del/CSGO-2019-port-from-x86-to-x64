...
{
	m_WorkThreadPool.SetWorkThreadAutoConstruct( cThreads, NULL, bPreCreateThreads );
}
...
bool JobTypeSortFuncByMsg( JobType_t const * const &lhs, JobType_t const * const &rhs )
{
	...
	{
		return ( lhs->m_nValidContexts < rhs->m_nValidContexts );
	}
...
bool CJobMgr::BLaunchJobFromNetworkMsg( void *pParent, const JobMsgInfo_t &jobMsgInfo, IMsgNetPacket *pNetPacket, EGCMsgContext nCreateContext )
{
	...
	{
		JobType_t jobSearch = { pNetPacket->GetTargetJobName(), (MsgType_t)0, (uint32)nCreateContext, NULL, NULL, NULL };
		...
		if ( GMapJobTypesByName().IsValidIndex( iJobType ) )
		{
			...
			if ( ( pJobType->m_nValidContexts & (uint32)nCreateContext ) == 0 )
				return false;
			...
			// Start the job
			job->StartJobFromNetworkMsg( pNetPacket, jobMsgInfo.m_JobIDSource, (uint32)nCreateContext );
			return true;
		}
	}
	else
	{
		JobType_t jobSearch = { NULL, jobMsgInfo.m_eMsg, (uint32)nCreateContext, NULL, NULL, NULL };
