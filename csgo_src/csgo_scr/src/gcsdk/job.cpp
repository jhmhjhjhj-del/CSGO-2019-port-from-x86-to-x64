...
CJob::~CJob()
{
	...
	// DoNotYield depth tracked on CJobMgr in CS:GO headers
}
...
//-----------------------------------------------------------------------------
// Purpose: CS:GO-matching CLock / DoNotYield (March 2019 headers)
//-----------------------------------------------------------------------------
CLock::CLock()
: m_pJob( NULL )
, m_pJobToNotifyOnLockRelease( NULL )
...
{
	return m_pchConstStr ? m_pchConstStr : "None";
}
...
void CLock::Dump( const char *pszPrefix, int nPrintMax, bool bPrintWaiting ) const
{
	...
	if ( m_pJob != NULL )
	{
		EmitInfo( SPEW_JOB, SPEW_ALWAYS, LOG_ALWAYS, "%s%s: Lock owner: %s, Type: %d, %d Waiting\n",
			pszPrefix, GetName(), m_pJob->GetName(), (int32)m_nsLockType, m_nWaitingCount );
	}
	else
	{
