...
void CToolFileMenuButton::OnShowMenu( vgui::Menu *menu )
{
	...
	if ( g_p4 )
	{
		bool bP4Connected = g_p4->IsConnectedToServer( false );
		...
		if ( bP4Connected && m_pFileMenuCallback->GetPerforceFileName( pPerforceFile, sizeof(pPerforceFile) ) )
		{
			...
			bool bOpenedForEdit = g_p4->GetFileState( pPerforceFile ) != P4FILE_UNOPENED;
			bool bFileInPerforce = g_p4->IsFileInPerforce( pPerforceFile );
...
void CToolFileMenuButton::OnPerforceP4Win( )
{
	...
	if ( m_pFileMenuCallback->GetPerforceFileName( pPerforceFile, sizeof(pPerforceFile) ) )
	{
		if ( g_p4->IsFileInPerforce( pPerforceFile ) )
		{
			g_p4->OpenFileInP4Win( pPerforceFile );
		}
	}
}
...
void CToolFileMenuButton::OnPerforceListOpenFiles( )
{
	...
	g_p4->GetOpenedFileListInPath( "GAME", openedFiles );
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		if ( pOpenType )
		{
			pOpenedFiles->AddOperation( pOpenType, g_p4->String( openedFiles[i].m_sLocalFile ) );
		}
	}
