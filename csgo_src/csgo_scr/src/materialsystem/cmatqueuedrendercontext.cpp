...
#pragma error
	...
	{
		// Get more scratch space so that vcollide_wireframe visualization doesn't crash due to out of space on large maps.
		vertSize = 32 * 1024 * 1024;
	}
...
void CMatQueuedRenderContext::DrawScreenSpaceRectangle( IMaterial *pMaterial, float flDestX, float flDestY, float flW, float flH, float flU0, float flV0, float flU1, float flV1, void *pClientRenderable )
{
	IMaterial *pRealTimeVersionMaterial = ((IMaterialInternal *)pMaterial)->GetRealTimeVersion();
	pRealTimeVersionMaterial->CallBindProxy( pClientRenderable, &m_CallQueueExternal );
	m_queue.QueueCall( m_pHardwareContext, &IMatRenderContext::DrawScreenSpaceRectangle, pMaterial, flDestX, flDestY, flW, flH, flU0, flV0, flU1, flV1, pClientRenderable );
}
...
//------------------------------------------------------------------------------
void CMatQueuedRenderContext::SetClientRenderStateBufferState( ClientRenderStateBuffer_t eBuffer, void const *pvBufferBase, int numBufferBytes )
{
	CMatRenderData< unsigned char > renderData( this );
	if ( pvBufferBase && ( numBufferBytes > 0 ) && !IsRenderData( pvBufferBase ) )
	{
		renderData.Lock( numBufferBytes );
		memcpy( renderData.Base(), pvBufferBase, numBufferBytes );
		pvBufferBase = renderData.Base();
	}

	m_queue.QueueCall( m_pHardwareContext, &IMatRenderContextInternal::SetClientRenderStateBufferState, eBuffer, pvBufferBase, numBufferBytes );
}
