...
enum InternalTextureFlags
{
	...
	TEXTUREFLAGSINTERNAL_EXCLUDE_FORCE_HW	= 0x00008000, // skip this texture in CTextureManager::ForceAllTexturesIntoHardware
};
...
public:
	...
	virtual void ExcludeTextureFromForceIntoHardware( bool bExclude );
	virtual bool IsExcludedFromForceIntoHardware() const { return ( (m_nInternalFlags & TEXTUREFLAGSINTERNAL_EXCLUDE_FORCE_HW) != 0 ); }
...
public:
	...
	virtual void ExcludeTextureFromForceIntoHardware( bool bExclude ) { NULL; }
	virtual bool IsExcludedFromForceIntoHardware() const { return false; }
...
void ITextureInternal::Destroy( ITextureInternal *pTex )
{
	...
	if ( iIndex != g_pTextureRefList->InvalidIndex () )
	{
		if ( g_pTextureRefList->Element( iIndex ) != 0 )
			Warning( "Destroying a texture that is in the queue!\n" );
	}
...
int GetCreationFlags( int iTextureFlags, int iInternalTextureFlags, ImageFormat fmt )
{
	...
	else
	{
		...
		static ConVarRef mat_using_d3d9ex( "mat_using_d3d9ex" );
		if ( !mat_using_d3d9ex.GetBool() )
#endif
		{
...
int CTexture::ComputeActualSize( bool bIgnorePicmip, IVTFTexture *pVTFTexture )
{
	...
	{
		// limiting large textures on OpenGL to 1024, override with -allow2048 on cl
		static int s_nMaxDimensionLimit = 0;
...
{
	if ( ( nAdditionalCreationFlags & TEXTUREFLAGS_ASYNC_DOWNLOAD ) || ( m_nFlags & TEXTUREFLAGS_ASYNC_DOWNLOAD ) )
	{
...
void CTexture::ExcludeTextureFromForceIntoHardware( bool bExclude )
{
	if ( bExclude )
	{
		m_nInternalFlags |= TEXTUREFLAGSINTERNAL_EXCLUDE_FORCE_HW;
	}
	else
	{
		m_nInternalFlags &= ~TEXTUREFLAGSINTERNAL_EXCLUDE_FORCE_HW;
...
bool CTexture::FinishAsyncDownload( AsyncTextureContext_t *pContext, void *pData, int nNumReadBytes, bool bAbort, float flMaxTimeMs )
{
	...
	bool bAsyncAborted = false;
	// For non-exclude async downloads, never abort
	if ( m_nFlags & TEXTUREFLAGS_ASYNC_DOWNLOAD )
	{
		bAsyncAborted = bAbort;
		bAbort = false;
	}
	...
	if ( !bAbort && g_pShaderAPI->CanDownloadTextures() )
	{
			  ...
			  ( pContext->m_nDesiredTempDimensionLimit == m_nDesiredTempDimensionLimit ) ) )
		{
			...
			MaterialLock_t hLock = MaterialSystem()->Lock();
			// if we had an async load error, then intentionally fall through to normal Download() which will use the error texture
			// this is in place of checking if file exists before scheduling async download, avoiding a FileExists spike on the main thread
			if ( ( m_nFlags & TEXTUREFLAGS_ASYNC_DOWNLOAD ) && !bAsyncAborted )
			{
	...
	m_pP4Module( NULL )
{
	...
	if ( m_pP4Module )
	{
		...
		if ( factory )
		{
			g_p4 = ( IP4 * )factory( P4_INTERFACE_VERSION, NULL );

			if ( g_p4 )
			{
				extern CreateInterfaceFn g_fnMatSystemConnectCreateInterface;
				g_p4->Connect( g_fnMatSystemConnectCreateInterface );
				g_p4->Init();
			}
		}
	}
	...
	else if ( !stricmp( szCmd, "save" ) )
	{
		...
		for ( int k = 0; k < s_OverrideMap.GetNumStrings(); ++ k )
		{
			...
			if ( g_pFullFileSystem->FileExists( szTextureContentPath ) )
			{
				...
				for ( int l = 0; l < 1024; ++ l ) bufTxtFileBuffer.PutChar( 0 );
