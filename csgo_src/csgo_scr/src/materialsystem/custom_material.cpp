...
CCustomMaterial::CCustomMaterial( KeyValues *pKeyValues )
	...
	, m_nUniqueCustomMaterialId( UINT16_MAX )
{
...
void CCustomMaterial::Finalize()
{
	...
	m_nUniqueCustomMaterialId = MaterialSystem()->GetCustomMaterialManager()->GetUnusedCustomMaterialNameId();

	V_snprintf( szUniqueMaterialName, sizeof( szUniqueMaterialName ), "cs_custom_material_%i", m_nUniqueCustomMaterialId );
...
{
	// free up custom material id
	MaterialSystem()->GetCustomMaterialManager()->MarkCustomMaterialNameIdUnused( m_nUniqueCustomMaterialId );
...
CCustomMaterialManager::CCustomMaterialManager()
	: m_nNextCustomMaterialNameUnusedId( 0 )
{
#ifndef DEDICATED
...
bool CCustomMaterialManager::Init()
{
	...
	m_CustomMaterialNameUnusedIdFreeList.RemoveAll();
	m_nNextCustomMaterialNameUnusedId = 0;
#endif
	return true;
}
...
bool CCustomMaterialManager::Process()
{
	...
	static ConVar mat_custommaterial_finalize_per_frame( "mat_custommaterial_finalize_per_frame", "2", FCVAR_RELEASE,
		"Max custom materials finalized per frame (0=unlimited)." );
	int nFinalizeBudget = mat_custommaterial_finalize_per_frame.GetInt();
	if ( nFinalizeBudget <= 0 )
		nFinalizeBudget = m_pCustomMaterials.Count() + 1;
...
uint16 CCustomMaterialManager::GetUnusedCustomMaterialNameId()
{
	uint16 id = m_CustomMaterialNameUnusedIdFreeList.Count() ? m_CustomMaterialNameUnusedIdFreeList.RemoveAtTail() : ( ++m_nNextCustomMaterialNameUnusedId );
	AssertMsg( id != UINT16_MAX, "CCustomMaterialManager - unique material identifier overflow" );
	return id;
}

void CCustomMaterialManager::MarkCustomMaterialNameIdUnused( uint16 nId )
{
	if ( nId != UINT16_MAX )
	{
		// Recycle id
		m_CustomMaterialNameUnusedIdFreeList.Insert( nId );
	}
}
