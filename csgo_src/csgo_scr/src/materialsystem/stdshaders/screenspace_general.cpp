...
static bool NeedsVS30( char const *pShaderName )
{
	size_t iLength = Q_strlen( pShaderName );
	if ( ( iLength > 4 ) &&
		( Q_stricmp( pShaderName + iLength - 5, "_ps30" ) == 0 ) )
	{
		return true;
	}
...
BEGIN_VS_SHADER_FLAGS( screenspace_general_dx9, "Help for screenspace_general", SHADER_NOT_EDITABLE )
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM( C5_X,SHADER_PARAM_TYPE_FLOAT,"0","")
		SHADER_PARAM( C5_Y,SHADER_PARAM_TYPE_FLOAT,"0","")
		SHADER_PARAM( C5_Z,SHADER_PARAM_TYPE_FLOAT,"0","")
		SHADER_PARAM( C5_W,SHADER_PARAM_TYPE_FLOAT,"0","")
		...
		SHADER_PARAM( VERTEXNORMAL, SHADER_PARAM_TYPE_INTEGER, "0", "specify vertex normal (compressed) in material format" )
		...
		SHADER_PARAM( VIEWPROJMAT, SHADER_PARAM_TYPE_MATRIX, "0", "" )
		SHADER_PARAM( INVVIEWPROJMAT, SHADER_PARAM_TYPE_MATRIX, "0", "" )
		SHADER_PARAM( BLENDOPMIN, SHADER_PARAM_TYPE_INTEGER, "0", "" )
	END_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		...
		bool bNeedsVS30 = false;
		if ( !bCustomVertexShader )
		{
			bNeedsVS30 = NeedsVS30( params[ PIXSHADER ]->GetStringValue() );
		}
		...
		SHADOW_STATE
		{
			...
			if ( params[VERTEXNORMAL]->GetIntValue() )
			{
				fmt |= (VERTEX_NORMAL | VERTEX_FORMAT_COMPRESSED);
			}
			...
			else if ( IS_FLAG_SET( MATERIAL_VAR_MULTIPLY ) || params[MULTIPLYCOLOR]->GetIntValue() )
			{
			...
			if ( params[BLENDOPMIN]->GetIntValue() )
			{
				pShaderShadow->BlendOp( SHADER_BLEND_OP_MIN );
...
//			if (params[ALPHATESTED]->GetFloatValue())
		...
		DYNAMIC_STATE
		{
			...
			float c0[]={
				...
				params[C5_X]->GetFloatValue(),
				params[C5_Y]->GetFloatValue(),
				params[C5_Z]->GetFloatValue(),
				params[C5_W]->GetFloatValue()
			};
			...
			if ( params[VIEWPROJMAT]->IsDefined() )
				pShaderAPI->SetPixelShaderConstant( 11, params[VIEWPROJMAT]->GetMatrixValue().Base(), 4 );

			if ( params[INVVIEWPROJMAT]->IsDefined() )
				pShaderAPI->SetPixelShaderConstant( 15, params[INVVIEWPROJMAT]->GetMatrixValue().Base(), 4 );
