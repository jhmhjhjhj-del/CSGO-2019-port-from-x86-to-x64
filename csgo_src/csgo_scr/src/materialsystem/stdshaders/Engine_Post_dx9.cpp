...
BEGIN_VS_SHADER_FLAGS( Engine_Post_dx9, "Engine post-processing effects (software anti-aliasing, bloom, color-correction", SHADER_NOT_EDITABLE )
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM( PANORAMA,					SHADER_PARAM_TYPE_BOOL,		"0",				"panorama" )
	END_SHADER_PARAMS
	...
	SHADER_INIT_PARAMS()
	{
		...
		if ( !params[ PANORAMA ]->IsDefined() )
		{
			params[ PANORAMA ]->SetIntValue( 0 );
		}
	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			if ( params[ PANORAMA ]->GetIntValue() != 0 )
			{
				// Alpha writes added to support FXAA and subsequent compositing of 3d panels in panorama.
