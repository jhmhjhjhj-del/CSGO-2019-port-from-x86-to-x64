...
{	
	...
	InitIntParam( info.m_nLayeredDamageBlend, params, 0 );
}
	                         ...
	                         VertexLitGeneric_DX9_Vars_t &info, VertexCompressionType_t vertexCompression, CBasePerMaterialContextData **pContextDataPtr )
{
	...
	if( pShader->IsSnapshotting() )
	{
		...
		else
		{
			...
			SET_STATIC_PIXEL_SHADER_COMBO( PHONGWARPTEXTURE, phongInfo.m_bHasPhongWarp && phongInfo.m_bHasPhong );
			SET_STATIC_PIXEL_SHADER_COMBO( WRINKLEMAP, phongInfo.m_bHasBaseTextureWrinkle );
			SET_STATIC_PIXEL_SHADER_COMBO( DETAILTEXTURE, bWorldNormal ? 0 : phongInfo.m_bHasDetailTexture );
			ClampDetailBlendModeAndWarn( nDetailBlendMode, 0, 7 );
			SET_STATIC_PIXEL_SHADER_COMBO( DETAIL_BLEND_MODE, nDetailBlendMode );
			ClampDecalBlendModeAndWarn( nDecalBlendMode, 0, 1 );
			SET_STATIC_PIXEL_SHADER_COMBO( DECAL_BLEND_MODE, bWorldNormal ? 2 : ( phongInfo.m_bHasDecalTexture ? nDecalBlendMode : 2 ) );
		...
		// enable alpha writes all the time, true for now to enable correct compositing of RT contents (i.e. Panorama)
	...
	else // not snapshotting -- begin dynamic state
	{
		...
		if ( ( !pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )
		{
			...
			float fDoNotWriteBaseAlphaToDestAlpha = ( bHasBaseAlphaEnvmapMask || bFullyOpaque ) ? 1 : 0;
			...
			float vShaderControls[4] = { fHasBaseAlphaPhongMask, fDoNotWriteBaseAlphaToDestAlpha, bNoTint ? -1.0f : ( 1.0f - fBlendTintByBaseAlpha ), fInvertPhongMask };
			...
			vRimParams[2] = info.m_nLayeredDamageBlend == -1 ? 0 : params[info.m_nLayeredDamageBlend]->GetIntValue();
