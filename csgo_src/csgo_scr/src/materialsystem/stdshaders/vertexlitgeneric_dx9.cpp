...
BEGIN_VS_SHADER( VertexLitGeneric, "Help for VertexLitGeneric" )
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM( VERTEXCOLORPOWER, SHADER_PARAM_TYPE_FLOAT, "0.0", "" );

		SHADER_PARAM( GRADIENTMODULATION, SHADER_PARAM_TYPE_BOOL, "", "Turn on gradient color modulation" )
		SHADER_PARAM( GRADIENTCOLORSTOP0, SHADER_PARAM_TYPE_VEC4, "[1 1 1 0]", "First gradient stop and position" )
		SHADER_PARAM( GRADIENTCOLORSTOP1, SHADER_PARAM_TYPE_VEC4, "[1 1 1 0.5]", "Second gradient stop and position" )
		SHADER_PARAM( GRADIENTCOLORSTOP2, SHADER_PARAM_TYPE_VEC4, "[1 1 1 1]", "Third gradient stop and position" )

		SHADER_PARAM( LAYEREDDAMAGEBLEND, SHADER_PARAM_TYPE_BOOL, "0", "When enabled, blend mode 4 turns into a special blend mode for layering damage, with the detail texture being the lower strata with no spec or env, and the base texture being the upper strata which casts a drop shadow.  The blend is a thresholded value based on the detail alpha, allowing for what looks like chipped off paint or cracks." )
	...
	void SetupVars( VertexLitGeneric_DX9_Vars_t& info )
	{
		...
		info.m_nVertexColorPower = VERTEXCOLORPOWER;

		info.m_nGradientModulation = GRADIENTMODULATION;
		info.m_nGradientColorStop0 = GRADIENTCOLORSTOP0;
		info.m_nGradientColorStop1 = GRADIENTCOLORSTOP1;
		info.m_nGradientColorStop2 = GRADIENTCOLORSTOP2;

		info.m_nLayeredDamageBlend = LAYEREDDAMAGEBLEND;
	}
