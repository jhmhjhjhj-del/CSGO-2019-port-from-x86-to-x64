...
BEGIN_VS_SHADER_FLAGS( Spritecard, "Help for Spritecard", SHADER_NOT_EDITABLE )
	...
	SHADER_DRAW
	{
		...
		// FORCE DEPTH BLEND DISABLED // TEMP HOTFIX 12/7/2018
		{
			bDepthBlend = false;
		}
		...
		SHADOW_STATE
		{
			...
			// enable writes to dest alpha
			// required if performing any kind of RT compositing, i.e. 3D scene as UI element in panorama
			// use separate alpha blending (and max op) to ensure opaque pixels not overwritten by spritecard alpha
			// should be safe to enable since we should no longer be interpreting dest alpha as depth (use fast depth resolve instead)
			pShaderShadow->EnableAlphaWrites( true );
			pShaderShadow->EnableBlendingSeparateAlpha( true );
			pShaderShadow->BlendOpSeparateAlpha( SHADER_BLEND_OP_MAX );
