...
BEGIN_VS_SHADER( UnlitTwoTexture_DX9, "Help for UnlitTwoTexture_DX9" )
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM( BEACHFOAM, SHADER_PARAM_TYPE_BOOL, "0", "" )
	...
	SHADER_INIT_PARAMS()
	{
		...
		if ( !params[BEACHFOAM]->IsDefined() )
		{
			params[BEACHFOAM]->SetIntValue( 0 );
		}
		// Cloak Pass
	...
	SHADER_DRAW
	{
		...
		if ( bDrawStandardPass )
		{
			...
			SHADOW_STATE
			{
				...
				bool bBeachFoam = (params[BEACHFOAM]->IsDefined() && params[BEACHFOAM]->GetIntValue() > 0);
				if ( bBeachFoam )
					nCustomMode = 2;
				...
				if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				{
					...
					SET_STATIC_PIXEL_SHADER_COMBO( CUSTOM_MODE, nCustomMode );
			...
			DYNAMIC_STATE
			{
				...
				else if ( params[BEACHFOAM]->IsDefined() && params[BEACHFOAM]->GetIntValue() > 0 )
				{
					float fvConst4[4] = { (float)pShaderAPI->CurrentTime(), 0, 0, 0 };
					pShaderAPI->SetPixelShaderConstant( 4, fvConst4, 1 );
				}
