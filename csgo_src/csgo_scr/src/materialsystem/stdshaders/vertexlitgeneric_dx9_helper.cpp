...
{	
	...
	InitFloatParam( info.m_nVertexColorPower, params, 0.0 );
	...
	InitIntParam( info.m_nGradientModulation, params, 0 );

	if ( ( info.m_nGradientColorStop0 != -1 ) && !params[info.m_nGradientColorStop0]->IsDefined() )
	{
		params[info.m_nGradientColorStop0]->SetVecValue( 1.0f, 1.0f, 1.0f, 0.0f );
	}
	if ( ( info.m_nGradientColorStop1 != -1 ) && !params[info.m_nGradientColorStop1]->IsDefined() )
	{
		params[info.m_nGradientColorStop1]->SetVecValue( 1.0f, 1.0f, 1.0f, 0.5f );
	}
	if ( ( info.m_nGradientColorStop2 != -1 ) && !params[info.m_nGradientColorStop2]->IsDefined() )
	{
		params[info.m_nGradientColorStop2]->SetVecValue( 1.0f, 1.0f, 1.0f, 1.0f );
	}
}
...

{
	...
	static ConVarRef r_staticlight_streams_indirect_only( "r_staticlight_streams_indirect_only", true );
		...
		if  ( pShader->IsSnapshotting() )
		{
			...
			if ( bHasFlashlight )
			{
				...
				else
				{
					...
					// enable alpha writes all the time (if not additive) to enable correct compositing of RT contents (i.e. Panorama)
					pShaderShadow->EnableAlphaWrites( ( nBlendType == BT_BLENDADD || nBlendType == BT_ADD ) ? false : true );
...
bool bDistanceAlphaFromDetail = false;
		...
		if ( pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) ) )
		{
			...
			else if ( info.m_nBaseAlphaEnvMapMaskMinMaxExp != -1 )
			{
				...
				float flVertexColorPower = params[info.m_nVertexColorPower]->GetFloatValue();
				if ( flVertexColorPower > 0.0f )
				{
					flConsts[0] = flVertexColorPower;
				}
			...
			if ( bSupportsSM3 )
			{
				if ( !bHasBump && IsBoolSet( info.m_nGradientModulation, params ) )
				{
					pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstantGammaToLinear( 43, info.m_nGradientColorStop0 );
					pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstantGammaToLinear( 44, info.m_nGradientColorStop1 );
					pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstantGammaToLinear( 45, info.m_nGradientColorStop2 );
					float vGradientStops[4];
					vGradientStops[0] = 1;
					vGradientStops[1] = params[info.m_nGradientColorStop0]->GetVecValue()[3];
					vGradientStops[2] = params[info.m_nGradientColorStop1]->GetVecValue()[3];
					vGradientStops[3] = params[info.m_nGradientColorStop2]->GetVecValue()[3];
					pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant( 46, vGradientStops );
				}
				else
				{
					float vGradientStops[4] = { 0, 0, 0, 0 };
					pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant( 46, vGradientStops );
				}
			}			
