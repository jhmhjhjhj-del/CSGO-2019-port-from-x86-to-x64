	...
	CBasePerMaterialContextData **pContextDataPtr )
{
	...
	bool bNormalMap			= ( info.m_nNormalMap != -1 ) && params[info.m_nNormalMap]->IsTexture() && ( ( nDecalStyle == 3 ) || ( nDecalStyle == 4 ) || ( nDecalStyle == 5 ) );
	...
	if ( ( pShader->IsSnapshotting() && pShaderShadow ) || (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )
	{
		...
		if ( pShader->IsSnapshotting() )
		{
			...
			if ( g_pHardwareConfig->SupportsPixelShaders_3_0() )
			{
				...
				SET_STATIC_VERTEX_SHADER_COMBO( NORMALMAP, bNormalMap );
			...
			if ( bAnisoDirTexture )
			{
				pShaderShadow->EnableTexture( SHADER_SAMPLER10, true );		// Anisotropy Direction map
			}
			if ( bNormalMap )
			{
				pShaderShadow->EnableTexture( SHADER_SAMPLER8, true );		// Normal map
			}
		...
		if ( pShaderAPI && ( (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) ) )
		{
			...
			if( bNormalMap && ( nDecalStyle == 3 || nDecalStyle == 4 || nDecalStyle == 5 ) )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER8, TEXTURE_BINDFLAGS_NONE, info.m_nNormalMap, -1 );
			}
			if( bEnvmapTexture )
			{
			...
			if ( bAnisoDirTexture )
			{
				pContextData->m_SemiStaticCmdsOut.BindTexture( pShader, SHADER_SAMPLER10, TEXTURE_BINDFLAGS_NONE, info.m_nAnisoDirTexture, -1 );
			}
