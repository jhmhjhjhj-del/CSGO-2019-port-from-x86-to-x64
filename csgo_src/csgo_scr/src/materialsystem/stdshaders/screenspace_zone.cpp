...
#include "screenspacezone_ps30.inc"

// NOTE: This has to be the last file included!
#include "tier0/memdbgon.h"

DEFINE_FALLBACK_SHADER( screenspace_zone, screenspace_zone_dx9 )
BEGIN_VS_SHADER_FLAGS( screenspace_zone_dx9, "Help for screenspace_zone", SHADER_NOT_EDITABLE )
	BEGIN_SHADER_PARAMS
	...
	SHADER_DRAW
	{
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_PIXEL_SHADER( screenspacezone_ps30 );
			SET_STATIC_PIXEL_SHADER( screenspacezone_ps30 );
		...
		DYNAMIC_STATE
		{
			...
			{
				pShaderAPI->SetPixelShaderConstant( 5, pvecWorldMinMaxBuffer[0].Base(), 1 );
			}
			else
			{
				Assert( false );
				float flConst1[4] = { 0, 0, 0, 0 };
				pShaderAPI->SetPixelShaderConstant( 5, flConst1, 1 );
			}
			...
			DECLARE_DYNAMIC_PIXEL_SHADER( screenspacezone_ps30 );
			SET_DYNAMIC_PIXEL_SHADER( screenspacezone_ps30 );
