...
private:
	...
	void DrawResolutionColor( IShader *pShader, IMaterialVar **ppParams, VertexCompressionType_t vertexCompression, int nMode );

	int GetModulationSnapshotCount( IMaterialVar **params );

private:

	int GetTotalMaterialTexels( IShader *pShader, IMaterialVar **ppParams, bool bLargestOnly = false );
...
bool CShaderSystem::ComputeVertexFormatFromSnapshot( IMaterialVar **params, ShaderRenderState_t* pRenderState )
{
	...
	for ( int mod = 1; mod < nModulationSnapshotCount; ++mod )
	{
		...
		StateSnapshot_t* pSnapshotsTest = (StateSnapshot_t*)malloc( 
								  ...
								  bool bRenderingPreTessPatchMesh )
{
	...
	else if ( g_config.nDrawResolution > 0 &&
		(IsFlag2Set( params, MATERIAL_VAR2_LIGHTING_VERTEX_LIT ) ||
			IsFlag2Set( params, MATERIAL_VAR2_LIGHTING_LIGHTMAP ) ||
			IsFlag2Set( params, MATERIAL_VAR2_LIGHTING_BUMPED_LIGHTMAP ) ||
			IsFlag2Set( params, MATERIAL_VAR2_DIFFUSE_BUMPMAPPED_MODEL ) ||
			IsFlagSet( params, MATERIAL_VAR_TRANSLUCENT ))
		)
	{
		DrawResolutionColor( pShader, params, vertexCompression, g_config.nDrawResolution );
	}
...
// DrawResolutionColor
//-----------------------------------------------------------------------------
int CShaderSystem::GetTotalMaterialTexels( IShader *pShader, IMaterialVar **ppParams, bool bLargestOnly /* = false */ )
{
	int nMaxX = 0;
	int nMaxY = 0;
	int nTotalPixels = 0;
	for ( int i = pShader->GetParamCount(); --i >= 0; )
	{
		if ( ppParams[i]->IsDefined() && ppParams[i]->IsTexture() )
		{
			if ( !Q_stristr( ppParams[i]->GetName(), "flashlight" ) && !Q_stristr( ppParams[i]->GetName(), "envmap" ) )
			{
				ITexture *pTexture = ppParams[i]->GetTextureValue();
				if ( pTexture )
				{
					int nX = pTexture->GetActualWidth();
					int nY = pTexture->GetActualHeight();
...
static ConVar mat_draw_resolution_threshold( "mat_draw_resolution_threshold", "3072", FCVAR_CHEAT );
void CShaderSystem::DrawResolutionColor( IShader *pShader, IMaterialVar **ppParams, VertexCompressionType_t vertexCompression, int nMode )
{
	int nTotalPixels = GetTotalMaterialTexels( pShader, ppParams, nMode == 2 );

	IMaterialInternal *pMaterial = m_pDebugMaterials[MATERIAL_DEBUG_RESOLUTION];

	bool bFound;
	IMaterialVar *pMaterialVar = pMaterial->FindVar( "$passcount", &bFound );

	pMaterialVar->SetFloatValue( sqrt( ((float)nTotalPixels) ) / mat_draw_resolution_threshold.GetFloat() );
