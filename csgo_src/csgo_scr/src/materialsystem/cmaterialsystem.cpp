...
#if defined( USE_SDL )
#if !defined( LINUX ) && ! ( defined (DX_TO_GL_ABSTRACTION ) && defined ( _WIN32 ) )
...
#define  COCOAMGR_INTERFACE_VERSION "CocoaMgrInterface006"
	...
	gameuifuncs = (IGameUIFuncs *)factory( VENGINE_GAMEUIFUNCS_VERSION, 0 );
#endif
...
static ConVar mat_draw_resolution( "mat_draw_resolution", "0", FCVAR_CHEAT );
...
//	pConfig->bAllowCheats = false; // hack
	...
	pConfig->nDrawResolution = mat_draw_resolution.GetInt();
...
//	config.bAllowCheats = false; // hack
	...
	mat_draw_resolution.SetValue( config.nDrawResolution );
...
#		endif
	...
	if ( !IsGameConsole() )
	{
		if ( bResetAnisotropy || recomputeSnapshots || bRedownloadLightmaps ||
			...
			bSetStandardVertexShaderConstants || bResetTextureFilter || bReloadMaterials )
		{
			Unlock( hLock );
			ForceSingleThreaded();
			hLock = Lock();
...
ITexture *CMaterialSystem::CreatePanoramaAlphaTexture( const char *pTextureName, const char *pTextureGroupName, int w, int h, int nFlags )
{
	return CreateProceduralTexture( pTextureName, pTextureGroupName, w, h, IMAGE_FORMAT_A8, nFlags );
}

bool CMaterialSystem::UpdatePanoramaAlphaTexture( ITexture *pTexture, int xOffset, int yOffset, int width, int height, void *pImageData )
{
	Assert( pTexture != NULL );
	Assert( pTexture->GetImageFormat() == IMAGE_FORMAT_A8 );

	g_pShaderAPI->ModifyTexture( ( (ITextureInternal*)pTexture )->GetTextureHandle( 0 ) );
	g_pShaderAPI->TexSubImage2D( 0, 0, xOffset, yOffset, 0, width, height, IMAGE_FORMAT_A8, width, false, pImageData );
...
void CMaterialSystem::GetShaderApiDynamicState( uint32 **ppRenderStates, SamplerStateCopy_t ** ppSamplerSates, bool *srgbWrite )
{
	g_pShaderAPI->GetShaderApiDynamicState( ppRenderStates, ppSamplerSates, srgbWrite );
};
...
IMaterial* CMaterialSystem::FindMaterial( char const *pMaterialName, const char *pTextureGroupName, bool bComplain, const char *pComplainPrefix )
{
	...
	// We need lower-case symbols for this to work
	int nLen = Q_strlen( pMaterialName ) + 1;
	// If the string is too long, we'll blow up in here. Let's just skip that part and return the error material here.
	if ( nLen > 2 * _MAX_PATH )
	{
		Assert( !"Found a weirdly long material name. Greater than 520 characters? Really?!" );
		return g_pErrorMaterial->GetQueueFriendlyVersion();
	}
...
void CMaterialSystem::BeginFrame( float frameTime )
{
	...
	g_nDebugVarsSignature = ( 
		...
		( mat_fullbright.GetInt() << 2 ) + ( mat_fastnobump.GetInt() << 4 ) + ( mat_fastspecular.GetInt() << 5 ) + (mat_draw_resolution.GetInt() << 3) ) << 24;
...
#if defined( INCLUDE_SCALEFORM )
			if ( g_pScaleformUI) g_pScaleformUI->SetSingleThreadedMode(true);
#endif
			break;
...
#endif
#if defined( INCLUDE_SCALEFORM )
				if ( g_pScaleformUI) g_pScaleformUI->SetSingleThreadedMode(false);
#endif
			}
			break;
		}
