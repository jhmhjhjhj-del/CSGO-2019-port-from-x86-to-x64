...
// ------------ End ----------------------------
	...
	void									PopScissorRect();
	DELEGATE_TO_OBJECT_0V(					ForceCommitScissorRect, g_pShaderAPI );
	...
	void									DrawScreenSpaceRectangle( IMaterial *pMaterial, float flDestX, float flDestY, float flW, float flH, float flU0, float flV0, float flU1, float flV1, void *pClientRenderable = NULL );
...
//	DELEGATE_TO_OBJECT_1V(					EnableSinglePassFlashlightMode, bool, g_pShaderAPI );
	...
	DELEGATE_TO_OBJECT_3V(					SetClientRenderStateBufferState, ClientRenderStateBuffer_t, void const *, int, g_pShaderAPI );
...
// #if defined( INCLUDE_SCALEFORM_ )
	...
	void SetScaleformSlotViewport( int slot, int x, int y, int w, int h );
	void RenderScaleformSlot( int slot );
	void ForkRenderScaleformSlot( int slot );
	void JoinRenderScaleformSlot( int slot );

	void SetScaleformCursorViewport( int x, int y, int w, int h );
	void RenderScaleformCursor( void );
	...
	{ 
#ifdef PANORAMA_ENABLE
		gameuifuncs->PanoramaRunFrame(nSlot); 
#endif
	}
	...
	{ 
#ifdef PANORAMA_ENABLE
		gameuifuncs->PanoramaRenderFrame(nSlot); 
#endif
	}
...
inline void CMatRenderContext::SetScaleformSlotViewport( int slot, int x, int y, int w, int h ) 
{ 
#ifdef INCLUDE_SCALEFORM
	IScaleformUI *pSfui = ScaleformUI();
	if ( pSfui )
	{
		pSfui->SetSlotViewport( slot, x, y, w, h ); 
	}
#endif	// INCLUDE_SCALEFORM
}

inline void CMatRenderContext::RenderScaleformSlot( int slot ) 
{ 
#ifdef INCLUDE_SCALEFORM
	IScaleformUI *pSfui = ScaleformUI();
	if ( pSfui )
	{
		ScaleformUI()->RenderSlot( slot ); 
	}
#endif	// INCLUDE_SCALEFORM
}

inline void CMatRenderContext::ForkRenderScaleformSlot( int slot ) 
{ 
#ifdef INCLUDE_SCALEFORM
	IScaleformUI *pSfui = ScaleformUI();
	if ( pSfui )
	{
		ScaleformUI()->ForkRenderSlot( slot ); 
	}
#endif	// INCLUDE_SCALEFORM
}

inline void CMatRenderContext::JoinRenderScaleformSlot( int slot ) 
{ 	
#ifdef INCLUDE_SCALEFORM
	IScaleformUI *pSfui = ScaleformUI();
	if ( pSfui )
	{
		ScaleformUI()->JoinRenderSlot( slot ); 
	}
#endif	// INCLUDE_SCALEFORM
}

inline void CMatRenderContext::SetScaleformCursorViewport( int x, int y, int w, int h ) 
{ 
#ifdef INCLUDE_SCALEFORM
	IScaleformUI *pSfui = ScaleformUI();
	if ( pSfui )
	{
		ScaleformUI()->SetCursorViewport( x, y, w, h ); 
	}
#endif	// INCLUDE_SCALEFORM
}

inline void CMatRenderContext::RenderScaleformCursor( void ) 
{ 
#ifdef INCLUDE_SCALEFORM
	IScaleformUI *pSfui = ScaleformUI();
	if ( pSfui )
	{
		ScaleformUI()->RenderCursor(); 
	}
#endif	// INCLUDE_SCALEFORM
}

inline void CMatRenderContext::AdvanceAndRenderScaleformSlot( int slot ) 
{ 
	/*VPROF_BUDGET( "Scaleform Advance And Render Slot", VPROF_BUDGETGROUP_SCALEFORM ); */
...
inline void CMatRenderContext::AdvanceAndRenderScaleformCursor() 
{ 
	/*VPROF_BUDGET( "Scaleform Advance And Render Cursor", VPROF_BUDGETGROUP_SCALEFORM ); */
	
#ifdef INCLUDE_SCALEFORM
	IScaleformUI *pSfui = ScaleformUI();
	if ( pSfui )
	{
		ScaleformUI()->AdvanceCursor(); ScaleformUI()->RenderCursor(); 
	}
#endif	// INCLUDE_SCALEFORM
}
