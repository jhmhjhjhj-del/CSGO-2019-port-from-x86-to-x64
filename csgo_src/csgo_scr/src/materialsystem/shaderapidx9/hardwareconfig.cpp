...
int CHardwareConfig::GetFirstVertexSampler() const
{
	return m_Caps.m_FirstVertexSampler;
}

bool CHardwareConfig::HasSharedVertexSamplers() const
{
	return m_Caps.m_bHasSharedVertexSamplers;
}
...
bool CHardwareConfig::SupportsNPO2Textures() const
{
	return m_Caps.m_bSupportsNPO2Textures;
}
...
int CHardwareConfig::GetActualTotalSamplerCount() const
{
#ifdef DX_TO_GL_ABSTRACTION
	return ( m_ActualCaps.m_bHasSharedVertexSamplers ? m_ActualCaps.m_NumSamplers : ( m_ActualCaps.m_NumSamplers + m_ActualCaps.m_NumVertexSamplers ) );
#else
	return m_ActualCaps.m_NumSamplers;
#endif
}
...
bool CHardwareConfig::SupportsResolveDepth( void ) const
{
	...
	// Note: (wills) I am removing the gpu_level check here because the survival dander zone effect
	// (and potentially other gameplay-relevant depth-sampling features) need to sample the depth
	// buffer and render correctly strictly if the hardware supports it, regardless of graphics quality level.
