...
static ConVar mat_using_d3d9ex( "mat_using_d3d9ex", "0", FCVAR_HIDDEN );
...
bool CShaderDeviceMgrDx8::Connect( CreateInterfaceFn factory )
{
	...
	mat_using_d3d9ex.SetValue( 0 );

#if defined( IS_WINDOWS_PC ) && defined( SHADERAPIDX9 ) && !defined( RECORDING ) && !defined( DX_TO_GL_ABSTRACTION )
	// Attempt to create a D3D9Ex device
	bool bTryToEnableD3D9Ex = ( CommandLine()->HasParm( "-d3d9ex" ) ) &&
							  //( !CommandLine()->HasParm( "-nod3d9ex" ) ) &&
							  ( CommandLine()->ParmValue( "-dxlevel", 95 ) >= 90 );

	if ( bTryToEnableD3D9Ex )
	{
		...
		if ( GetVersionEx( ( OSVERSIONINFO * )&osvi ) )
		{
			...
			{
				// Windows XP, so disable
				bTryToEnableD3D9Ex = false;
				Warning( "Disabling D3D9Ex because OS is less than Win7\n" );
			}
		}
	}
	...
	Warning( "Created %s device successfully\n", g_ShaderDeviceUsingD3D9Ex ? "D3D9Ex" : "D3D9" );
...
// BIN truth (lobby_ok / lobby_stable shaderapidx9.dll): no late DeviceReset
// notify and no Panorama_GetCreatedD3DDevice export. Use base registration only.
...
bool CShaderDeviceMgrDx8::ComputeCapsFromD3D( HardwareCaps_t *pCaps, int nAdapter )
{
	...
	{
		pCaps->m_bSupportsNPO2Textures = ( !( caps.TextureCaps & D3DPTEXTURECAPS_POW2 ) ) && ( !( caps.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL ) ); // false if only conditional support for NPO2 textures, || ( caps.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL );
	}
	else
	{
		pCaps->m_bSupportsNPO2Textures = true;

		// It's a requirement of GL 2.0+ to support NPO2 textures, (and FillD3DCaps just ORs in D3DPTEXTURECAPS_NONPOW2CONDITIONAL), but...
		// According to https://www.khronos.org/opengl/wiki/NPOT_TextureApparently, 
		// * R300/R400 (Radoen 9500+, X500+) lacking unless no mips
		// * NV30 (GeForce FX) don't support NPO2 at all.
		// TODO - Do we care about these devices? 
		// NOTE - Scaleform assumes full and conditional support for NPO2 under GL
	}
	...
	if ( CommandLine()->CheckParm( "-nosrgb" ) )
	{
...
#ifdef DX_TO_GL_ABSTRACTION
		pCaps->m_bSupportsVertexTextures  = pCaps->m_NumVertexSamplers > 0;
		pCaps->m_NumVertexSamplers		  = caps.NumVertexSamplers;
		pCaps->m_FirstVertexSampler		  = caps.FirstVertexSampler;
		pCaps->m_bHasSharedVertexSamplers = caps.MaxCombinedTIU <= caps.MaxVertexTIU;
#else
		...
		pCaps->m_bHasSharedVertexSamplers = false;
		pCaps->m_FirstVertexSampler		  = D3DVERTEXTEXTURESAMPLER0;
#endif
...
void CShaderDeviceDx8::SpewDriverInfo() const
{
	...
	Warning( "m_bSupportsNPO2Textures: %s\n", g_pHardwareConfig->Caps().m_bSupportsNPO2Textures ? "yes" : "no" );
...
bool CShaderDeviceDx8::TryDeviceReset()
{
	...
	if ( g_ShaderDeviceUsingD3D9Ex && bResetSuccess )
	{
...
#ifdef WIN32
	// Every time we device reset (or create a game window), we want to go ahead and 
	// scrub the nvidia settings for AO again.
	// There's still technically an exploit here. Someone could place a break point on
	// this function, execute it and then turn the setting on in their console. 
	// If we see that in the wild, we can check the settings after we restore the objects
	// and make sure that they're still what we think they should be.
	// or we could check and make sure we did set them all to off, verify they went off
	// and ban the user if we see them change again.
	extern NvAPI_Status ScanAndFixNvDriverProfiles();
	...
	if ( m_DeviceState == DEVICE_STATE_NEEDS_RESET )
	{
		...
		else
		{
			...
			if ( bResetSucceeded )
			{
				...
				{
					m_DeviceState = DEVICE_STATE_OK;
					// Note m_bQueuedDeviceLost was set to false in this function (just before calling "if ( m_DeviceState == DEVICE_STATE_OK )")
					// However by calling "g_pShaderUtil->UncacheUnusedMaterials( true );" before the device reset, m_bQueuedDeviceLost will be
					// set to true again (UncacheUnusedMaterials( true ) will try to reset render states and try to set render targets that will eventually
					// fail as the device has not been reseted yet)
					// At this point, we successfully reseted the d3d device, and can therefore set m_bQueuedDeviceLost back to false.
					// (Omitting to set m_bQueuedDeviceLost back to false here means that  m_bQueuedDeviceLost will only be set back to false 
					// the next time CheckDeviceLost() is called (ie the next frame) 
					m_bQueuedDeviceLost = false;
