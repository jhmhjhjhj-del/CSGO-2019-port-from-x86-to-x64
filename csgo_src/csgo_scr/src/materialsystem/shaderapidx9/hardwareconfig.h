...
struct HardwareCaps_t : public MaterialAdapterInfo_t
{
	...
	bool m_bSupportsNPO2Textures : 1;				// Unconditional support for Non-Power of 2 textures. Only used by Panorama. Some HW/drivers only support NPO2 conditionally, this is false in that case as it's restrictive for Panorama (conditional support - no mip-maps, no WRAP addressing modes, not compressed, etc).	
	bool m_bHasSharedVertexSamplers : 1;			// GL only, => VS texture slots shared with PS textures (some HW/drivers report VTF supported via GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS=16 say, but also report GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS=16).
...
public:
	...
	virtual bool SupportsNPO2Textures() const;
	...
	virtual int  GetFirstVertexSampler() const;
	virtual bool HasSharedVertexSamplers() const;
	...
	int GetActualTotalSamplerCount() const;
