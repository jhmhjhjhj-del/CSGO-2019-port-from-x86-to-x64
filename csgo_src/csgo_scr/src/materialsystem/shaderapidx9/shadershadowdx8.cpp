...
void CShaderShadowDX8::Init( )
{
	...
	for ( int i = 0; i < MAX_VERTEX_SAMPLERS; ++i )
	{
		m_VertexSamplerState[ i ].m_TextureEnable = false;
	}
}
...
// Pixel and vertex shader methods
//-----------------------------------------------------------------------------
static inline bool BRequiredShaderFile( const char* pFileName )
{
#define CSGO_OFFICIAL_SHADER_NAME( basefilename ) if ( !V_stricmp( pFileName, #basefilename ) ) return true;
#include "csgo_official_shader_list.inc"
#undef CSGO_OFFICIAL_SHADER_NAME
	return false;	// if random community-server VMT specifies a shader that we don't know about, then it's OK for that not to render
}
static void OnShaderFailedToLoad( const char* pFileName, const char *szType, int nIndex )
{
	if ( !pFileName || !*pFileName || !BRequiredShaderFile( pFileName ) )
		return;

	// Ignore this when running hammer
	if ( ShaderUtil()->InEditorMode() )
		return;

	char chMessage[256];
	V_sprintf_safe( chMessage, "Fatal error loading %s shader %s (%08X)\n", szType, pFileName, nIndex );
	LoggingSystem_LogDirect( LOG_GENERAL, LS_ERROR, Color( 255, 64, 64, 255 ), chMessage );
}
...
void CShaderShadowDX8::SetVertexShader( const char* pFileName, int nStaticVshIndex )
{
	...
	if ( m_ShadowShaderState.m_VertexShader == INVALID_SHADER )
		OnShaderFailedToLoad( pFileName, "vertex", nStaticVshIndex );
}
...
void CShaderShadowDX8::SetPixelShader( const char* pFileName, int nStaticPshIndex )
{
	...
	if ( m_ShadowShaderState.m_PixelShader == INVALID_SHADER )
		OnShaderFailedToLoad( pFileName, "pixel", nStaticPshIndex );
}
