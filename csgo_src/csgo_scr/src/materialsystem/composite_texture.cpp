...
ConVar mat_composite_creates_per_frame( "mat_composite_creates_per_frame", "2", FCVAR_RELEASE,
	"Max new composite textures enqueued per client frame (0=unlimited)." );
ConVar mat_composite_generate_sleep_ms( "mat_composite_generate_sleep_ms", "2", FCVAR_RELEASE,
	"Worker sleep between composite generation steps (ms)." );
...
int GetRTIndex( int nSize )
{
	...
	// saving some mem on these OS's 
	if ( ( Plat_GetOSVersion() == PLAT_OS_VERSION_XP ) ||
		 ( gpu_level.GetInt() == GPU_LEVEL_LOW ) ||
		 ( gpu_mem_level.GetInt() == GPU_MEM_LEVEL_LOW )/* ||
		 ( Plat_GetOSVersion() == PLAT_OS_VERSION_WIN2003 ) ||
		 ( Plat_GetOSVersion() == PLAT_OS_VERSION_VISTA ) ||
		 ( Plat_GetOSVersion() == PLAT_OS_VERSION_WIN2K )*/ )
	{
		nClosestBigger = 1;
		i = 1;
	}
	...
	for ( ; i < COMPOSITE_TEXTURE_RT_COUNT; i++ )
	{
...
CCompositeTexture::CCompositeTexture( const CUtlBuffer &compareBlob, KeyValues *pCompositingMaterialKeyValues, CompositeTextureSize_t size, CompositeTextureFormat_t format, int nMaterialParamNameId, bool bSRGB, bool bIgnorePicMip )
	...
	, m_nUniqueCompositeMaterialId( UINT16_MAX )
	, m_nUniqueCompositeTextureId( UINT16_MAX )
{
	...
	m_nUniqueCompositeTextureId = MaterialSystem()->GetCompositeTextureGenerator()->GetUnusedCompositeTextureNameId();

	V_sprintf_safe( m_szTextureName, "composite_texture_%d_%d_%d_%d", m_nMaterialParamNameId, m_nActualSize, m_format, m_nUniqueCompositeTextureId );
...
CCompositeTexture::~CCompositeTexture()
{
	...
	MaterialSystem()->GetCompositeTextureGenerator()->MarkCompositeMaterialNameIdUnused( m_nUniqueCompositeMaterialId );
	MaterialSystem()->GetCompositeTextureGenerator()->MarkCompositeTextureNameIdUnused( m_nUniqueCompositeTextureId );
}
...
void CCompositeTexture::GenerationStep( void )
{
	...
	if ( m_nRegenerateStage == COMPOSITE_TEXTURE_STATE_WAITING_FOR_ASYNC_TEXTURE_LOAD_FINISH )
	{
		...
		for ( int i = 0; i < NUM_PRELOAD_TEXTURES; i++ )
		{
			if ( m_pPreLoadTextures[ i ] != NULL )
			{
				if ( !m_pPreLoadTextures[ i ]->IsAsyncDone() )
				{
					bDone = false;
					break;
				}
				else
				{
					...
					{						
						DevMsg( "DoAsyncTextureLoad: Failed to preload texture: %s (%d)\n", m_pPreLoadTextures[ i ]->GetName(), i );
						m_pPreLoadTextures[ i ]->DecrementReferenceCount();
						m_pPreLoadTextures[ i ] = NULL;
					}
				}
...
void CCompositeTexture::DoAsyncTextureLoad( void )
{
	...
	// Debug glove composite path resolution for missing/invalid paint textures.
	static int s_iGloveCompositePathLog = 0;
	const char *pszPattern = m_pCompositingMaterialKeyValues->GetString( "$pattern" );
	const char *pszBaseTextureForTrack = m_pCompositingMaterialKeyValues->GetString( "$basetexture" );
	// Pattern can arrive as a short token (e.g. "solid"), not a full anodized_multi path.
	// Track any composite that carries a pattern or a weapon-customization basetexture.
	const bool bHasPattern = ( pszPattern && pszPattern[0] != 0 );
	const bool bLooksLikeCustomizationComposite = ( pszBaseTextureForTrack && V_stristr( pszBaseTextureForTrack, "customization" ) != NULL );
	const bool bTrackGlovePattern = ( bHasPattern || bLooksLikeCustomizationComposite );
	// Offline AutoMM: glove path Msg flooded console + tanked FPS (was 80+ lines per skin).
	const bool bGlovePathLog = false;
	if ( bGlovePathLog && bTrackGlovePattern && s_iGloveCompositePathLog < 8 )
	{
		++s_iGloveCompositePathLog;
		const char *pszBaseTexture = m_pCompositingMaterialKeyValues->GetString( "$basetexture" );
		const char *pszBumpMap = m_pCompositingMaterialKeyValues->GetString( "$bumpmap" );
		Msg( "GLOVE_COMP_PATH_BEGIN: pattern=%s basetexture=%s bumpmap=%s\n",
			pszPattern ? pszPattern : "",
			pszBaseTexture ? pszBaseTexture : "",
			pszBumpMap ? pszBumpMap : "" );
	}
	...
	for ( int i = 0; i < NUM_PRELOAD_TEXTURES; i++ )
	{
		...
		if ( pszTexture && pszTexture[0] != 0 )
		{
			if ( bGlovePathLog && bTrackGlovePattern && s_iGloveCompositePathLog <= 8 )
			{
				char szTextureVtf[MAX_PATH];
				char szMaterialsTextureVtf[MAX_PATH];
				V_snprintf( szTextureVtf, sizeof( szTextureVtf ), "%s.vtf", pszTexture );
				V_snprintf( szMaterialsTextureVtf, sizeof( szMaterialsTextureVtf ), "materials/%s.vtf", pszTexture );

				const bool bExistsRawVtf = ( g_pFullFileSystem && g_pFullFileSystem->FileExists( szTextureVtf, "GAME" ) );
				const bool bExistsMaterialsVtf = ( g_pFullFileSystem && g_pFullFileSystem->FileExists( szMaterialsTextureVtf, "GAME" ) );
				Msg( "GLOVE_COMP_PATH: param=%s tex=%s rawVtf=%s existsRaw=%d existsMaterials=%d\n",
					materialParams[ i ].m_pParamName,
					pszTexture,
					szTextureVtf,
					bExistsRawVtf ? 1 : 0,
					bExistsMaterialsVtf ? 1 : 0 );
			}
...
{
	{
		...
		m_nUniqueCompositeMaterialId = MaterialSystem()->GetCompositeTextureGenerator()->GetUnusedCompositeMaterialNameId();

		V_sprintf_safe( szCompositeMaterialName, "compositing_material_%d", m_nUniqueCompositeMaterialId );
...
CCompositeTextureGenerator::CCompositeTextureGenerator( void )
	...
	, m_nNextCompositeMaterialNameUnusedId( 0 )
	, m_nNextCompositeTextureNameUnusedId( 0 )
{
#ifndef DEDICATED
...
bool CCompositeTextureGenerator::Init( void )
{
...
#ifdef PLATFORM_WINDOWS_PC
	// saving some extra mem on these older win OS's 
	if ( ( Plat_GetOSVersion() == PLAT_OS_VERSION_XP )/* ||
		 ( Plat_GetOSVersion() == PLAT_OS_VERSION_WIN2003 ) ||
		 ( Plat_GetOSVersion() == PLAT_OS_VERSION_VISTA ) ||
		 ( Plat_GetOSVersion() == PLAT_OS_VERSION_WIN2K )*/ )
	...
	m_CompositeMaterialNameUnusedIdFreeList.RemoveAll();
	m_CompositeTextureNameUnusedIdFreeList.RemoveAll();
	m_nNextCompositeMaterialNameUnusedId = 0;
	m_nNextCompositeTextureNameUnusedId = 0;
...
ICompositeTexture *CCompositeTextureGenerator::GetCompositeTexture( IVisualsDataProcessor *pVisualsDataProcessor, int nMaterialParamNameId, CompositeTextureSize_t size, CompositeTextureFormat_t format, bool bSRGB, bool bIgnorePicMip, bool bAllowCreate )
{
	...
	{
		// x64: don't stampede the generate queue with hundreds of textures in one frame.
		{
			static int s_nCreatesThisFrame = 0;
			static uint32 s_nCreateFrame = ~0u;
			const uint32 nFrame = MaterialSystem() ? MaterialSystem()->GetCurrentFrameCount() : 0;
			if ( nFrame != s_nCreateFrame )
			{
				s_nCreateFrame = nFrame;
				s_nCreatesThisFrame = 0;
			}
			const int nCap = mat_composite_creates_per_frame.GetInt();
			if ( nCap > 0 && s_nCreatesThisFrame >= nCap )
				return NULL;
			++s_nCreatesThisFrame;
		}
...
void CCompositeTextureGenerator::GenerateThread()
{
	while ( !m_bGenerateThreadExit )
	{
		...
		if ( pTexture )
		{
			const int nSleep = MAX( 1, mat_composite_generate_sleep_ms.GetInt() );
		...
		{
			ThreadSleep( MAX( 1, mat_composite_generate_sleep_ms.GetInt() ) );
		}
	}
...
uint16 CCompositeTextureGenerator::GetUnusedCompositeMaterialNameId()
{
	uint16 id = m_CompositeMaterialNameUnusedIdFreeList.Count() ? m_CompositeMaterialNameUnusedIdFreeList.RemoveAtTail() : ( ++m_nNextCompositeMaterialNameUnusedId );
	AssertMsg( id != UINT16_MAX, "CCompositeTextureGenerator - unique material identifier overflow" );
	return id;
}

void CCompositeTextureGenerator::MarkCompositeMaterialNameIdUnused( uint16 nId )
{
	if ( nId != UINT16_MAX )
	{
		// Recycle id
		m_CompositeMaterialNameUnusedIdFreeList.Insert( nId );
	}
}

uint16 CCompositeTextureGenerator::GetUnusedCompositeTextureNameId()
{
	uint16 id = m_CompositeTextureNameUnusedIdFreeList.Count() ? m_CompositeTextureNameUnusedIdFreeList.RemoveAtTail() : ( ++m_nNextCompositeTextureNameUnusedId );
	AssertMsg( id != UINT16_MAX, "CCompositeTextureGenerator - unique texture identifier overflow" );
	return id;
}

void CCompositeTextureGenerator::MarkCompositeTextureNameIdUnused( uint16 nId )
{
	if ( nId != UINT16_MAX )
	{
		// Recycle id
		m_CompositeTextureNameUnusedIdFreeList.Insert( nId );
	}
}
