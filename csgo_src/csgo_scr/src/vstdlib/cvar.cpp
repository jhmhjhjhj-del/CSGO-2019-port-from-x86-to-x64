...
{
	const int MAX_SEARCH_ARGS = 20;
	const char* search[MAX_SEARCH_ARGS];

	int searchCount = args.ArgC() - 1;
	if ( searchCount < 1 )
	{
	...
	if ( searchCount > MAX_SEARCH_ARGS )
	{
		ConMsg( "Too many arguments, sorry\n" );
		return;
	}
		...
		i = m_CommandHash.Next(i) )
	{
		...
		// check matches all arguments
		bool ok = true;
		for ( int iSearch = 0; iSearch < searchCount; ++iSearch )
		{
			if ( !Q_stristr( var->GetName(), search[iSearch] ) &&
				!Q_stristr( var->GetHelpText(), search[iSearch] ) )
			{
				ok = false;
				break;
			}
