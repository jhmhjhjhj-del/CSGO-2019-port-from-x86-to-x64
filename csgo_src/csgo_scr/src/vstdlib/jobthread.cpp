...
inline void ServiceJobAndRelease( CJob *pJob, int iThread = -1 )
{
	...
	if ( !pJob->IsFinished() && pJob->TryLock() )
	{
		...
		pJob->ExecuteInternal(); // we own a reference (that we release at the end of this function), and the job is locked, so we can call ExecuteInternal()
		pJob->Unlock();
	}
	pJob->Release();
}
...
private:
	enum
	{
		...
		COMPUTATION_STACKSIZE = 1572864, // threads that compute network packets of size 512kb need 1.5Mb depth; the Windows default is 1Mb
	};
...
int CThreadPool::YieldWaitPerFrameJobs( )
{
	...
	int nRetVal = 0;
	CUtlVectorFixedGrowable< CJob*, 256 > remainingJobs; // using a vector here because (a) we don't know in advance how many jobs we'll be waiting for, and (b) using stackalloc inside an infinite loop causes an /analyze warning and a potential for stack overflow and crash.
	while ( true )
	{
		...
		remainingJobs.SetCount( nCount );
		V_memcpy( remainingJobs.Base(), m_PerFrameJobs.Base(), nCount * sizeof( CJob* ) );
		m_PerFrameJobs.RemoveAll();
		m_PerFrameJobListMutex.Unlock();

		nRetVal = YieldWait( remainingJobs.Base(), nCount );

		for ( int i = 0; i < nCount; ++i )
		{
			remainingJobs[i]->Release();
		}
	}
	return nRetVal;
}
...
// magic number to avoid adding another member to this queue
// This call time means the slot has been released
const double g_flFreeFunctionSlotMarker = -1e24f;

// this class manages a single thread and schedules a list of functions to run async with the main loop
// NOTE: This is not designed to scale to large numbers of calls.  We would have to stop scanning the entire list each
// iteration if we want to support hundreds of calls.  Tens of calls is probably fine.  Currently there are only 3-4 expected clients
...
public:
	...
	CAsyncScheduledFunctionManager() : m_scheduledCalls(0, 16), m_hCallThread(nullptr), m_bJoin(false) {}

	// IAsyncScheduledFunctions
	// NOTE: Will automatically init when the first call is scheduled
	void Init()
	{
		if (!m_hCallThread)
		{
			m_bJoin = false;
			m_hCallThread = CreateSimpleThread(StaticRunScheduledCalls, this, 64U * 1024); //, "AsyncScheduledFunctions");
			if (!m_hCallThread)
			{
				Plat_FatalError("Unable to create thread in %s\n", __FUNCTION__);
			}
		}
	}
	...
	void Shutdown()
	{
		if (m_hCallThread)
		{
			m_bJoin = true;
			m_wakeup.Set();
			ThreadJoin(m_hCallThread);
			ReleaseThreadHandle(m_hCallThread);
			m_hCallThread = nullptr;
		}
	}
	virtual AsyncFunctionHandle_t RegisterScheduledFunction(const CUtlDelegate<float()> &fn, double flTimeToCall) OVERRIDE
	{
		if (m_bJoin)
		{
			return AsyncFunctionHandle_t(-1);
		}
		else
		{
			AUTO_LOCK(m_callMutex);
			Init();
			int nIndex = -1;
			for (int i = 0; i < m_scheduledCalls.Count(); i++)
			{
				if (m_scheduledCalls[i].m_flTimeToCall == g_flFreeFunctionSlotMarker)
				{
					nIndex = i;
					break;
				}
			...
			if (nIndex < 0)
			{
				nIndex = m_scheduledCalls.AddToTail();
			}
			m_scheduledCalls[nIndex].m_fn = fn;
			m_scheduledCalls[nIndex].m_flTimeToCall = flTimeToCall;
			m_wakeup.Set();
			return AsyncFunctionHandle_t(nIndex);
		}
	}

	bool RescheduleFunction(AsyncFunctionHandle_t nHandle, double flTimeToCall)
	{
		if (m_bJoin)
			return false;
		m_pending.PushItem(call_time_change_t(nHandle, flTimeToCall));
		m_wakeup.Set();
		return true;
	}

	void UnregisterScheduledFunction(AsyncFunctionHandle_t nHandle)
	{
		RescheduleFunction(nHandle, g_flFreeFunctionSlotMarker);
	}

private:
	void RunScheduledCalls();
	static uintp StaticRunScheduledCalls(void *_pManager)
	{
		auto pManager = (CAsyncScheduledFunctionManager*)_pManager;
		pManager->RunScheduledCalls();
		return 0;
	}

	struct scheduled_call_t
	{
		double	m_flTimeToCall;
		CUtlDelegate<float()> m_fn;

		scheduled_call_t() {}
		scheduled_call_t(double flTime, const CUtlDelegate<float()> &fn)
		{
			m_flTimeToCall = flTime;
			m_fn = fn;
		}
	};
	...
	{
		AsyncFunctionHandle_t m_nHandle;
		double m_flTime;
		call_time_change_t() {}
		call_time_change_t(AsyncFunctionHandle_t nHandle, double flTimeToCall)
		{
			m_nHandle = nHandle;
			m_flTime = flTimeToCall;
		}
	};


	ThreadHandle_t m_hCallThread;
	volatile bool m_bJoin;
	CThreadEvent m_wakeup;
	double		m_flNextTime;

	CThreadMutex m_callMutex;
	CUtlVector<scheduled_call_t> m_scheduledCalls;
	CTSList<call_time_change_t> m_pending;
};

// this is the base thread function, it exits when the thread exits (Shutdown() triggers this)
void CAsyncScheduledFunctionManager::RunScheduledCalls()
{
	uint nWaitMs = TT_INFINITE;
	while (!m_bJoin)
	{
		if (nWaitMs)
		{
			m_wakeup.Wait(nWaitMs);
		}

		double flCurrentTime = Plat_FloatTime();
		call_time_change_t item;
		if (m_pending.PopItem(&item))
		{
			AUTO_LOCK(m_callMutex);
			do
			{
				m_scheduledCalls[item.m_nHandle.GetRaw()].m_flTimeToCall = item.m_flTime;
			} while (m_pending.PopItem(&item));
		}

		nWaitMs = TT_INFINITE;
		// UNDONE: PERF: Don't iterate everything, pre-sort these into a priority queue?
		// NOTE: This was the initial implementation but I reverted to an array to reduce complexity 
		// while handling calls that wanted to change their time due to some event arriving.
		if (m_scheduledCalls.Count())
		{
			double flMin = -1;
			int nMin = -1;
			for (int i = 0; i < m_scheduledCalls.Count(); i++)
			{
				if (m_scheduledCalls[i].m_flTimeToCall > 0)
				{
					if (flMin < 0 || m_scheduledCalls[i].m_flTimeToCall < flMin)
					{
						flMin = m_scheduledCalls[i].m_flTimeToCall;
						nMin = i;
					}
				}
			}
			...
			if (nMin >= 0)
			{
				float flDelta = (float)(flMin - flCurrentTime);
				// <1ms or time passed, call now
				if (flDelta < 0.001f)
				{
					float flInterval = m_scheduledCalls[nMin].m_fn();
					double flNextThink = -1.0;
					if (flInterval > 0)
					{
						flNextThink = flCurrentTime + flInterval;
					}
					m_scheduledCalls[nMin].m_flTimeToCall = flNextThink;
					nWaitMs = 0;
				}
				else
				{
					nWaitMs = int(1000.0f * flDelta);
					nWaitMs = MAX(1, nWaitMs);
				}
			}
		}
	}
...
// the global manager creates a single thread to execute the async functions on
CAsyncScheduledFunctionManager g_ScheduledFunctions;
IAsyncScheduledFunctions *g_pScheduledFunctions = &g_ScheduledFunctions;
