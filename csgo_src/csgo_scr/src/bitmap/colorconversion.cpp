						 ...
						 int width, int height, int srcStride, int dstStride )
{
	...
	if ( ( ( dstImageFormat == IMAGE_FORMAT_DXT1 || dstImageFormat == IMAGE_FORMAT_LINEAR_DXT1 || dstImageFormat == IMAGE_FORMAT_DXT1_RUNTIME || 
		     dstImageFormat == IMAGE_FORMAT_DXT3 || dstImageFormat == IMAGE_FORMAT_LINEAR_DXT3 || dstImageFormat == IMAGE_FORMAT_DXT3_RUNTIME ||
		 ...
		 (dstImageFormat == IMAGE_FORMAT_DXT3 && srcImageFormat == IMAGE_FORMAT_DXT3_RUNTIME) ||
		 ...
		 (dstImageFormat == IMAGE_FORMAT_DXT3_RUNTIME && srcImageFormat == IMAGE_FORMAT_DXT3) ||
...
//		Assert( srcStride == 0 && dstStride == 0 );
	...
	else if ( ( srcImageFormat == IMAGE_FORMAT_RGB888 ||
				srcImageFormat == IMAGE_FORMAT_BGR888 ) &&
			  dstImageFormat == IMAGE_FORMAT_DXT5 )
	{
		return ConvertRGBToDXT5YCoCg( src, srcImageFormat, dst, width, height );
...
{
	if ( !( ( srcImageFormat ==  IMAGE_FORMAT_RGB888 ) || 
		    ( srcImageFormat ==  IMAGE_FORMAT_BGR888 ) ) )
	{
		return false;
	}
	...
	// convert [RGB] (24bpp) to [CoCgY] (24bpp) before compressing to DXT5
	YCoCg::ConvertRGBToCoCgY( (byte *)src, srcImageFormat, width, height );

	// compress to DXT5 YCoCg format
	YCoCg::CompressYCoCgDXT5( src, dst, width, height, 3, outputBytes );
...
{
	if ( !( (srcImageFormat == IMAGE_FORMAT_RGBA8888) ||
		    (srcImageFormat == IMAGE_FORMAT_ABGR8888) ||
			(srcImageFormat == IMAGE_FORMAT_ARGB8888) ||
			(srcImageFormat == IMAGE_FORMAT_BGRA8888) ) )
	{
		return false;
	}
	...
	// convert [RGBA] (32bpp) to [CoCg_Y] (32bpp) before compressing to DXT5
	YCoCg::ConvertRGBAToCoCg_Y( (byte *)src, srcImageFormat, width, height );

	// compress to DXT5 YCoCg format
	YCoCg::CompressYCoCgDXT5( src, dst, width, height, 4, outputBytes );
