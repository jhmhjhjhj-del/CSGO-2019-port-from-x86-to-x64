...
void GLMContext::ForceFlushStates()
{
	...
	for ( int i = 0; i < Caps().m_nTotalSamplerCount; i++ )
	{
...
{
	m_nNumDirtySamplers = Caps().m_nTotalSamplerCount;
	for (uint i = 0; i < Caps().m_nTotalSamplerCount; i++)
	{
...
void GLMContext::DebugDump( GLMDebugHookInfo *info, uint options, uint vertDumpMode )
{
	...
	if ( (options & (1<<eDumpTextureSetup)) && is_draw )
	{
		...
		GLMPRINTF(( "-D- TODO" ));
#if 0
		for( int i=0; i<GLM_TOTAL_SAMPLER_COUNT; i++ )
		{
