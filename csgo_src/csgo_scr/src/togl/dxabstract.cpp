...
static void FillD3DCaps9( const GLMRendererInfoFields &glmRendererInfo, D3DCAPS9* pCaps )
{
	...
	pCaps->NumVertexSamplers = glmRendererInfo.m_nNumVertexSamplers;
	pCaps->MaxVertexTIU = glmRendererInfo.m_nMaxVertexTIU;
	pCaps->MaxCombinedTIU = glmRendererInfo.m_nMaxCombinedTIU;
	pCaps->FirstVertexSampler = glmRendererInfo.m_nFirstVertexSampler;
	pCaps->TotalSamplerCount = glmRendererInfo.m_nTotalSamplerCount;
#endif
}
...
{
	memset( &m_RsShadow, 0, sizeof( m_RsShadow ) );
	memset( &m_SamplerStateShadow, 0, sizeof( m_SamplerStateShadow ) );
	m_nCaptureMode = RS_CAPTURE_MODE_GAME;
}
...
// This totally sucks, but this information can't be gleaned any
// other way when translating from D3D to GL at this level
static int ShadowDepthSamplerMaskFromName( const char *pName, int &nExclusiveShadowDepthSampler )
{
	// assume we don't use sampler 0, used if shader can use one or another sampler for shadow sampling, only use the one defined here
	nExclusiveShadowDepthSampler = 0;
	...
	{
		nExclusiveShadowDepthSampler = 11;
		return (1 << 8 ) | (1 << 11);
	}
...
HRESULT IDirect3DDevice9::CreatePixelShader(CONST DWORD* pFunction,IDirect3DPixelShader9** ppShader, const char *pShaderName, char *pDebugLabel, const uint32 *pCentroidMask )
{
	...
	*ppShader = NULL;
	...
	int nExclusiveShadowDepthSampler = 0;
	int nShadowDepthSamplerMask = ShadowDepthSamplerMaskFromName( pShaderName, nExclusiveShadowDepthSampler );
...

	{
		...
		g_D3DToOpenGLTranslatorGLSL.TranslateShader( (uint32 *) pFunction, &tempbuf, &bVertexShader, glslPixelShaderOptions, nShadowDepthSamplerMask, nExclusiveShadowDepthSampler, nCentroidMask, pDebugLabel );
...
HRESULT IDirect3DDevice9::CreateVertexShader(CONST DWORD* pFunction, IDirect3DVertexShader9** ppShader, const char *pShaderName, char *pDebugLabel)
{
	...
	*ppShader = NULL;
			...
			
	{
		...
		g_D3DToOpenGLTranslatorGLSL.TranslateShader( (uint32 *) pFunction, &tempbuf, &bVertexShader, glslVertexShaderOptions, 0, -1, nCentroidMask, pDebugLabel );
...
void IDirect3DDevice9::ReleasedTexture( IDirect3DBaseTexture9 *baseTex )
{
	...
	for( int i=0; i< GetTotalSamplerCount(); i++)
	{
...
{
	pRect->left = gl.m_ScissorBox.x;
	pRect->top = gl.m_ScissorBox.y;
	pRect->right = gl.m_ScissorBox.x + gl.m_ScissorBox.width;
	pRect->bottom = gl.m_ScissorBox.y + gl.m_ScissorBox.height;
	return S_OK;
}
...
int TOGLMETHODCALLTYPE IDirect3DDevice9::GetTotalSamplerCount()
{
	return m_ctx->GetTotalSamplerCount();
}
