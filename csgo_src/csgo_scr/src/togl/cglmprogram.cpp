...
CGLMProgram::CGLMProgram( GLMContext *ctx, EGLMProgramType type )
{
	...
	m_maxSamplers      = m_ctx->Caps().m_nTotalSamplerCount;
	m_nNumUsedSamplers = m_ctx->Caps().m_nTotalSamplerCount;
...
bool CGLMShaderPair::ValidateProgramPair()
{
	...
	if ( !m_valid && m_bCheckLinkStatus )
	{
		...
		if (m_valid)
		{
			...
			for ( int sampler = 0; sampler < GLM_MAX_PIXEL_SAMPLERS; sampler++ )
			{
				char tmp[ 16 ];
			...
			int nMaxVertexTIU, nMaxCombinedTIU;
			nMaxVertexTIU = m_ctx->Caps().m_nMaxVertexTIU;
			nMaxCombinedTIU = m_ctx->Caps().m_nMaxCombinedTIU;

			bool bSupportsVTF = nMaxVertexTIU > 0;
			bool bOverlappingVTFTIU = nMaxCombinedTIU <= nMaxVertexTIU;

			int nFirstVertexSamplerIndex = bOverlappingVTFTIU ? 0 : GLM_MAX_PIXEL_SAMPLERS;

			for ( int sampler = 0; sampler < GLM_MAX_VERTEX_SAMPLERS; sampler++ )
			{
				char tmp[ 16 ];
				sprintf( tmp, "vsampler%d", sampler );	// sampler0 .. sampler1.. etc

				GLint nLoc = gGL->glGetUniformLocationARB( m_program, tmp );

				if ( nLoc >= 0 )
				{
					int nVertexSamplerIndex = nFirstVertexSamplerIndex + sampler;

					if ( !bSupportsVTF )
					{
						Warning( "VS uses Vertex Texture Fetch, but not supported!" );
						return false;
					}

					if ( ( m_locSamplers[ nVertexSamplerIndex ] != 0xFF ) && bOverlappingVTFTIU )
					{
						// driver reports that VTF is supported but TIU's are shared/overlap between PS and VS.
						// if shader slots overlap, we have a problem, so if possible, rewrite shader pair to
						// use different, non overlapping sampler slots between PS and VS.
						Warning( "Vertex and Pixel shaders have overlapping sampler usage, expect drawing errors using this shader!\n" );
					}

					m_locSamplers[ nVertexSamplerIndex ] = nLoc;
					gGL->glUniform1iARB( nLoc, nVertexSamplerIndex );
				}
			}
