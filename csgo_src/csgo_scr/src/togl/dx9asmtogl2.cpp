...
//			strcat_s( pRegisterName, nBufLen, "_centroid" );
	...
	switch ( dwRegType )
	{
		...
		case D3DSPR_DEPTHOUT:
			...
			break;
		case D3DSPR_SAMPLER:
			V_snprintf( buff, sizeof( buff ), m_bVertexShader ? "vsampler%d" : "sampler%d", dwRegNum );
			strcat_s( pRegisterName, nBufLen, buff );
			break;
		case D3DSPR_CONST2:
			TranslationError();
		...
		if ( nRegType == D3DSPR_SAMPLER )
		{
			...
			switch ( TextureType( dwToken ) )
			{
				...
				case D3DSTT_2D:
					...
					// re-wire shadow depth sampler mask if we have the exclusive flag set (only one shadow depth sampler, and explicitly assume only the higher numbered sampler is used)
					if ( ( ( 1 << nRegNum ) & m_nShadowDepthSamplerMask ) && ( nRegNum == m_nExclusiveShadowDepthSampler ) )
					{
						m_nShadowDepthSamplerMask = ( 1 << nRegNum );
					}
					break;
				case D3DSTT_CUBE:
					m_dwSamplerTypes[nRegNum] = SAMPLER_TYPE_CUBE;
...
{
	const char* pPrefix = m_bVertexShader ? "vsampler" : "sampler";
	...
	for ( int i=0; i < ARRAYSIZE( m_dwSamplerTypes ); i++ )
	{
		if ( m_dwSamplerTypes[i] == SAMPLER_TYPE_2D )
		{
			...
			{
				PrintToBuf( *m_pBufHeaderCode, "uniform sampler2DShadow %s%d;\n", pPrefix, i );
			}
			else
			{
				PrintToBuf( *m_pBufHeaderCode, "uniform sampler2D %s%d;\n", pPrefix, i );
			}
			++nSamplersWritten;
		}
		else if ( m_dwSamplerTypes[i] == SAMPLER_TYPE_3D )
		{
			PrintToBuf( *m_pBufHeaderCode, "uniform sampler3D %s%d;\n", pPrefix, i );
			++nSamplersWritten;
		}
		else if ( m_dwSamplerTypes[i] == SAMPLER_TYPE_CUBE )
		{
			PrintToBuf( *m_pBufHeaderCode, "uniform samplerCube %s%d;\n", pPrefix, i );
			++nSamplersWritten;
		}
...
int D3DToGL::TranslateShader( uint32* code, CUtlBuffer *pBufDisassembledCode, bool *bVertexShader, uint32 options, int32 nShadowDepthSamplerMask, int32 nExclusiveShadowDepthSampler, uint32 nCentroidMask, char *debugLabel )
{
	...
	m_nExclusiveShadowDepthSampler = nExclusiveShadowDepthSampler;
	...
	*bVertexShader = m_bVertexShader;
	...
	// might have changed (due to exclusive shadow sampler use), write back
	nShadowDepthSamplerMask = m_nShadowDepthSamplerMask;
