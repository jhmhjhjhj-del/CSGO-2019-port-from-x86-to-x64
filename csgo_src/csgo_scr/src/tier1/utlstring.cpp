...
int CUtlString::UnicodeCaseConvert( int nStringCaseFlags, EStringConvertErrorPolicy ePolicy /* = STRINGCONVERT_ASSERT_REPLACE */ )
{
	...
	char stackbuf[2000];
	int nChars = V_UnicodeCaseConvert( pSrc, stackbuf, V_ARRAYSIZE( stackbuf ), nStringCaseFlags, (EStringConvertErrorPolicy)(ePolicy | _STRINGCONVERTFLAG_TOTALSIZE) );
	if( nChars <= V_ARRAYSIZE( stackbuf ) )
	{
		Set( stackbuf );
		return nChars;
	}
...
{
	const int nLen = V_strnlen( pchAddition, nMaxChars );
	...
	{
		char* buf = new char[ nMaxChars + 1 ];
		V_memcpy( buf, pchAddition, nMaxChars );
		buf[nMaxChars] = '\0';
		Append( buf );
		delete[] buf;
	}
