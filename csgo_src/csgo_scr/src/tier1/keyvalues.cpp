...
{
public:
	CKeyValuesErrorStack() : m_pFilename("NULL"), m_nLine(1), m_errorIndex(0), m_maxErrorIndex(0), m_bEncounteredErrors(false) {}
...
//		m_Buffer.EatWhiteSpace();
		...
		if ( m_Buffer.IsText() && m_Buffer.IsValid() )
		{
			while( m_Buffer.GetBytesRemaining() > 0 && V_isspace( *(const unsigned char*)m_Buffer.PeekGet() ) )
			{
				char c = m_Buffer.GetChar();
				if ( c == '\n' )
				{
...
// Read from a buffer...
//-----------------------------------------------------------------------------
bool KeyValues::LoadFromBuffer( char const *resourceName, CUtlBuffer &buf, IBaseFileSystem* pFileSystem, const char *pPathID, GetSymbolProc_t pfnEvaluateSymbolProc, bool bForceTypeString /*= false*/ )
{
	...
	do 
	{
		...
		if ( s && *s == '{' && !wasQuoted )
		{
			...
			pCurrentKey->RecursiveLoadFromBuffer( resourceName, tokenReader, pfnEvaluateSymbolProc, bForceTypeString );
		}
		else
		{
...
// Read from a buffer...
//-----------------------------------------------------------------------------
bool KeyValues::LoadFromBuffer( char const *resourceName, const char *pBuffer, IBaseFileSystem* pFileSystem, const char *pPathID, GetSymbolProc_t pfnEvaluateSymbolProc, bool bForceTypeString /*= false*/ )
{
	if ( !pBuffer )
		return true;
	...
	return LoadFromBuffer( resourceName, buf, pFileSystem, pPathID, pfnEvaluateSymbolProc, bForceTypeString );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void KeyValues::RecursiveLoadFromBuffer( char const *resourceName, CKeyValuesTokenReader &tokenReader, GetSymbolProc_t pfnEvaluateSymbolProc, bool bForceTypeString /*= false*/ )
{
	...
	while ( 1 )
	{
		...
		if ( *value == '{' && !wasQuoted )
		{
			...
			// sub value list
			dat->RecursiveLoadFromBuffer( resourceName, tokenReader, pfnEvaluateSymbolProc, bForceTypeString );
		}
		else 
		{
			if ( wasConditional )
		...
		case TYPE_WSTRING:
			{
				...
				if ( nLength < 0 || nLength * static_cast<int>( sizeof( wchar_t ) ) > buffer.GetBytesRemaining() )
				{
					Assert( !"Someone gave us a bogus negative size for KeyValues::ReadAsBinary, cannot continue." );
					return false;
				}
...
bool KeyValues::ReadAsBinaryFiltered( CUtlBuffer &buffer, int nStackDepth )
{
	...
	while ( true )
	{
		...
		if ( type == TYPE_NONE )
		{
			...
			if ( !newKey->ReadAsBinaryFiltered( buffer, nStackDepth + 1 ) )
				return false;
		}
		else
		{
			...
			case TYPE_WSTRING:
				{
					...
					if ( nLength < 0 || nLength * static_cast<int>( sizeof( wchar_t ) ) > buffer.GetBytesRemaining() )
					{
						Assert( !"Someone gave us a bogus negative size for KeyValues::ReadAsBinaryFiltered, cannot continue." );
						return false;
					}
