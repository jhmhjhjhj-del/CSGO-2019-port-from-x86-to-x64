...
// Helpers for V_BasicHtmlEntityEncode

// CUTFTrailingChar<sizeof(c)>::Test(c) outputs whether c is a trailing code-point
// in a multi-code-point sequence in UTF-encoded text.
template <int sizeOfChar>
struct CUTFTrailingChar;

// UTF-8
template <>
struct CUTFTrailingChar<1> {
	static FORCEINLINE constexpr bool Test( char c )
	{
		// UTF-8 bit patterns:
		//  0xxxxxxx - ASCII
		//	11xxxxxx - multibyte leading char
		//  10xxxxxx - multibyte trailing char
		return ( uint8( c ) & 0xc0 ) == 0x80;
	}
};

// UTF-16
template <>
struct CUTFTrailingChar<2> {
	static FORCEINLINE constexpr bool Test( wchar_t c )
	{
		// UTF-16 characters
		//  1101 10xx xxxx xxxx: surrogate initial character
		//  1101 11xx xxxx xxxx: surrogate final character
		//  (other)            : raw codepoint
		//
		// so only 1101 11xx xxxx xxxx are trailing characters.
		return ( uint16( c ) & 0xfc00 ) == 0xdc00;
	}
};

// UTF-32
template<>
struct CUTFTrailingChar<4> {
	static FORCEINLINE constexpr bool Test( wchar_t c )
	{
		// UTF-32 has no multi-codepoint characters.
		return false;
	}
};

#define WRITE_CHAR_TO_DEST(c) do {	\
	if( iOutput == iOutputLast )	\
	{								\
		pDest[iValidOutput] = 0;	\
		pDest = nullptr;			\
		if ( !pnDstRequiredChars )	\
			return false;			\
	}								\
	if ( pDest )					\
	{								\
		pDest[iOutput] = TChar( c );\
	}								\
	++iOutput;						\
 } while(0)

#define VALIDATE_DEST_SIZE( n ) do {		\
	if(	iOutput <= iOutputLast				\
		&& (iOutput + (n)) > iOutputLast)	\
	{										\
		pDest[iValidOutput] = 0;			\
		pDest = nullptr;					\
		if ( !pnDstRequiredChars )			\
...
bool _V_BasicHtmlEntityEncode( TChar *pDest, int nDestChars, TChar const *pIn, int nInChars, int* pnDstRequiredChars, bool bPreserveWhitespace )
{
	Assert( nDestChars == 0 || pDest != nullptr );

	// Make it easy to check for valid pDest
	if ( nDestChars <= 0 )
	{
		nDestChars = 0;
		pDest = nullptr;
	...
	// where in the string are we writing to?
	// Invariant: (iOutput < nDestChars || pDest == nullptr)
	//
	// On success, right after writing the final terminating \0,
	// we relax this to (iOutput <= nDestChars || pDest == nullptr)
	// since it is allowed to write exactly nDestChars characters.
	int iOutput = 0;

	// where in the string could we terminate the string
	// Invariant: (iValidOutput <= iOutput)
	int iValidOutput = 0;

	int iOutputLast = nDestChars - 1;

	for ( int iInput = 0; iInput < nInChars; ++iInput )
	{
		TChar cIn = pIn[iInput];
		if ( !cIn )
			break;

		// If we aren't inside a multi-byte sequence, this is a valid
		// point to overwrite with \0 if we run out of space.
		if ( !CUTFTrailingChar<sizeof(TChar)>::Test( cIn ) )
			iValidOutput = iOutput;
		...
		{
			if ( cIn == TChar( g_BasicHTMLEntities[i].uCharCode ) )
			{
				bReplacementDone = true;
				VALIDATE_DEST_SIZE( g_BasicHTMLEntities[i].nEntityLength );
				for ( int j = 0; j < g_BasicHTMLEntities[i].nEntityLength; ++j )
				{
					if ( pDest )
						pDest[iOutput] = TChar( g_BasicHTMLEntities[i].pchEntity[j] );
		...
		{
			// See if the current char matches any whitespace entities
			for ( int i = 0; g_WhitespaceEntities[ i ].uCharCode != 0; ++i )
			{
				if ( cIn == TChar( g_WhitespaceEntities[i].uCharCode ) )
				{
					bReplacementDone = true;
					VALIDATE_DEST_SIZE( g_WhitespaceEntities[i].nEntityLength );
					for ( int j = 0; j < g_WhitespaceEntities[i].nEntityLength; ++j )
					{
						if ( pDest )
							pDest[iOutput] = TChar( g_WhitespaceEntities[i].pchEntity[j] );
	...
	if ( pDest )
	{
		Assert( iOutput <= iOutputLast );
		pDest[iOutput] = 0;
	}
	++iOutput;

	if ( pnDstRequiredChars )
		*pnDstRequiredChars = iOutput;

	return ( pDest != nullptr );
}

#undef WRITE_CHAR_TO_DEST

// Helpers for calling template inline versions of this code
bool V_BasicHtmlEntityEncode( char *pDest, const int nDestChars, char const *pIn, const int nInChars, int* pnDstRequiredChars /*= nullptr */, bool bPreserveWhitespace /*= false*/ )
{
	return _V_BasicHtmlEntityEncode( pDest, nDestChars, pIn, nInChars, pnDstRequiredChars, bPreserveWhitespace );
}

bool V_BasicHtmlEntityEncode( wchar_t *pDest, const int nDestChars, wchar_t const *pIn, const int nInChars, int* pnDstRequiredChars /*= nullptr */, bool bPreserveWhitespace /*= false*/ )
{
	return _V_BasicHtmlEntityEncode( pDest, nDestChars, pIn, nInChars, pnDstRequiredChars, bPreserveWhitespace );
}
