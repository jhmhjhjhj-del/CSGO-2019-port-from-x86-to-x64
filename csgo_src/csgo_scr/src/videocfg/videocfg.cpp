...
struct VideoConfigSetting_t
{
	...
	bool		m_bUserConfigured; // never reset to default if the user has changed
};
...
//	{ "setting.mat_monitorgamma",								true,		true,		true },
	...
	// Settings.
	{ "setting.fullscreen",										true,		true,		false,  false, true },
	{ "setting.nowindowborder",									true,		true,		false,  false, true },
...
bool CreateDefaultVideoKeyValues( VidMatConfigData_t &configData )
{
	...
	configData.pConfigKeys->SetInt( "setting.fullscreen", 0 );
	configData.pConfigKeys->SetInt( "setting.nowindowborder", 1 );
	...
	configData.pConfigKeys->SetInt( "setting.mat_forceaniso", 1 );
...
{
	static char const * arrIgnoredSettings[] = {
		"setting.mem_level",
		"setting.fullscreen",
...
bool VerifyDefaultVideoConfig( VidMatConfigData_t &configData )
{
	...
	if ( !bEqual )
	{
		...
		if ( kvPreviousVideoCfg->LoadFromFile( g_pFullFileSystem, VIDEOCONFIG_FILENAME, VIDEOCONFIG_PATHID ) )
		{
			...
			for ( int k = 0; k < Q_ARRAYSIZE( s_pVideoConfigSettingsWhitelist ); ++ k )
			{
				if ( !s_pVideoConfigSettingsWhitelist[k].m_bUseAutoOption && !s_pVideoConfigSettingsWhitelist[k].m_bUserConfigured ) continue;
				...
				else if ( s_pVideoConfigSettingsWhitelist[k].m_bUseAutoOption )
				{
