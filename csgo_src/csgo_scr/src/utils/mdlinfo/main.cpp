...
public:
	...
	void DumpKeyvalues( CStudioHdr &cStudioHdr ) const;
};
...
void CMdlInfoApp::DumpKeyvalues( CStudioHdr &cStudioHdr ) const
{
	Msg( "\n\n\n// KeyValueText:\n\n" );

	const studiohdr_t *pHdr = cStudioHdr.GetRenderHdr();
	Msg( "%s", pHdr->KeyValueText() );
...
void CMdlInfoApp::DumpSequenceList( CStudioHdr &cStudioHdr ) const
{
	...
	for ( int ii = 0; ii < nSeqCount; ++ii )
	{
		...
		{
			Msg( "//     %3d autolayers:\n", seqDesc.numautolayers );
			for ( int n=0; n<seqDesc.numautolayers; n++ )
			{
				if ( n != 0 )
					Msg( "\n" );

				const mstudioseqdesc_t &AutoLayerSeqDesc = cStudioHdr.pSeqdesc( seqDesc.pAutolayer(n)->iSequence );
				Msg( "//          %s", AutoLayerSeqDesc.pszLabel() );
			}
			Msg( "\n" );
		}
		...
		{
			Msg( "//     %3d activitymodifiers: ", seqDesc.numactivitymodifiers );
			for ( int n=0; n<seqDesc.numactivitymodifiers; n++ )
			{
				if ( n != 0 )
					Msg( ", " );

				Msg( "%s", seqDesc.pActivityModifier(n)->pszName() );
			}
			Msg( "\n" );
		}
		...
		{
			Msg( "//     %3d events:\n", seqDesc.numevents );
			for ( int n=0; n<seqDesc.numevents; n++ )
			{
				if ( n != 0 )
					Msg( "\n" );
				Msg( "//          %s cycle: %f options: %s", seqDesc.pEvent(n)->pszEventName(), seqDesc.pEvent(n)->cycle, seqDesc.pEvent(n)->pszOptions() );
			}
			Msg( "\n" );
		}

		if ( seqDesc.numanimtags )
		{
			Msg( "//     %3d animtags:\n", seqDesc.numanimtags );
			for ( int n=0; n<seqDesc.numanimtags; n++ )
			{
				if ( n != 0 )
					Msg( "\n" );
				Msg( "//          %s value: %f", seqDesc.pAnimTag(n)->pszTagName(), seqDesc.pAnimTag(n)->cycle );
			}
			Msg( "\n" );
		}
