...
*/
		...
		fprintf( fp, "\n");
		fprintf( fp, "DOCKER_EXEC :=\n" );
		fprintf( fp, "ifeq (${USE_DOCKER},1)\n" );
    	fprintf( fp, "    # We want to use docker. Are we already in the container? If so, nothing more to do.\n" );
    	fprintf( fp, "    ifneq ($(IN_DOCKER_BUILD),1)\n" );
        fprintf( fp, "        DOCKER_ENV_LIST := CFG MAKE_JOBS MAKE_VERBOSE P4CONFIG P4PORT P4USER P4CLIENT\n" );
        fprintf( fp, "        DOCKER_ENV_VARS := $(foreach var,$(DOCKER_ENV_LIST), -e $(var)=$($(var)))\n" );
        fprintf( fp, "        DOCKER_EXEC := $(shell echo docker run -i --rm -v `echo ~`:`echo ~` -v $(PWD)/..:$(PWD)/.. $(DOCKER_ENV_VARS) -w $$PWD -v /etc/group:/etc/group:ro -v /etc/passwd:/etc/passwd:ro -u=$$UID:$$(id -g $$USER) -h=`hostname` build_csgo_%s:latest) dumb-init\n", g_pVPC->HasCommandLineParameter( "/linux64" ) ? "client" : "server" );
    	fprintf( fp, "    endif\n" );
		fprintf( fp, "endif\n" );
		fprintf( fp, "\n" );
		...
		fprintf( fp, "\t$(DOCKER_EXEC) $(MAKE) -f $(lastword $(MAKEFILE_LIST)) -j$(MAKE_JOBS) all-targets\n\n" );
		...
		for ( int i=0; i < projects.Count(); i++ )
		{
			...
			fprintf( fp, "\n\t@+$(DOCKER_EXEC) $(MAKE) -C %s -f %s $(CLEANPARAM) SHELL=$(SHELL)", fileDir.Get(), pFilename );
		...
		fprintf( fp, "\nclean: \n" );
		fprintf( fp, "\t@$(DOCKER_EXEC) $(MAKE) -f $(lastword $(MAKEFILE_LIST)) -j$(MAKE_JOBS) all-targets CLEANPARAM=clean\n\n\n" );

		fprintf( fp, "\n# clean targets, so we re-link next time.\n" );
		fprintf( fp, "\ncleantargets: \n" );
		fprintf( fp, "\t@$(DOCKER_EXEC) $(MAKE) -f $(lastword $(MAKEFILE_LIST)) -j$(MAKE_JOBS) all-targets CLEANPARAM=cleantargets\n\n\n" );
	
		fprintf( fp, "\n# p4 edit and remove targets, so we get an entirely clean build.\n" );
		fprintf( fp, "\ncleanandremove: \n" );
		fprintf( fp, "\t@$(DOCKER_EXEC) $(MAKE) -f $(lastword $(MAKEFILE_LIST)) -j$(MAKE_JOBS) all-targets CLEANPARAM=cleanandremove\n\n\n" );
		...
		fprintf( fp, "ifdef CHROOT_DEPENDENCY\n"
				...
				"$(CHROOT_DEPENDENCY): SHELL = /bin/bash\n"
				"$(CHROOT_TIMESTAMP_FILE): $(CHROOT_TARBALL)\n"
				"\t@echo \"chroot ${CHROOT_NAME} at $(CHROOT_DIR) is out of date\"\n"
				"\t@echo You need to re-run sudo tools/runtime/linux/configure_runtime.sh ${CHROOT_NAME} ${RUNTIME_NAME}\n"
				"endif\n");
