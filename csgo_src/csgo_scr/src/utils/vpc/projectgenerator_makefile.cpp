...
//--------------------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------------------
void CheckDefinesSyntax( const char *pVPCPropertyName, const char *pStr )
{
    if ( strchr( pStr, ' ' ) != NULL ||
         strchr( pStr, '\t' ) != NULL )
    {
        g_pVPC->VPCError( "%s contains whitespace: '%s'", pVPCPropertyName, pStr );
    }
    if ( strchr( pStr, ',' ) != NULL )
    {
        g_pVPC->VPCError( "%s contains commas, use semicolons as separators: '%s'", pVPCPropertyName, pStr );
    }
}
                                               ...
                                               CProjectFile *pProjectFile )
{
	...
	CheckDefinesSyntax( g_pOption_PreprocessorDefinitions, preprocessorDefinitionsString );

    const char *undefinePreprocessorDefinitionsString = m_pVCProjGenerator->GetPropertyValueAsString( pProjectFile, pConfig->m_Name.Get(), KEYWORD_COMPILER, g_pOption_UndefinePreprocessorDefinitions );
	CheckDefinesSyntax( g_pOption_UndefinePreprocessorDefinitions, undefinePreprocessorDefinitionsString );

	// N x M loop to eliminate any macros defined and then undefined
    ...
    {
		bool bMacroUndefined = false;
		char *pMacro = outStrings.GetNext();

		CUtlStringBuilder *pStrUndefBuf = g_pVPC->GetTempStringBuffer2();
		pStrUndefBuf->Set( undefinePreprocessorDefinitionsString );
		CSplitStringInPlace outUndefStrings( pStrUndefBuf->Access(), ';' );
		{
			while ( outUndefStrings.HasNext() )
			{
				char *pUndefMacro = outUndefStrings.GetNext();

				if ( !V_strcmp( pMacro, pUndefMacro ) )
				{
					bMacroUndefined = true;
					break;
				}
			}
		}
