...
static void GetChangelistFilenames( CUtlVector<int> &changelists, CUtlVector<CUtlString> &changelistFilenames )
{
	...
	{
		g_p4->GetOpenedFileList( fileList, false );
	}
	...
	else
	{
		...
		{
			g_p4->GetFileListInChangelist(changelists[i], partialFileList);
	...
	for ( int i=0; i < fileList.Count(); i++ )
	{
		...
		{
			const char *pFilename = g_p4->String(fileList[i].m_sLocalFile);
