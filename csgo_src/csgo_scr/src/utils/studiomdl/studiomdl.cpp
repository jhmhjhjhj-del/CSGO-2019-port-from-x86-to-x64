...
void MdlError( const char *fmt, ... )
{
	...
	if (g_quiet)
	{
		if (g_bFirstWarning)
		{
			...
			if (g_p4)
			{
				CUtlVector<P4Revision_t> &revisions = g_p4->GetRevisionList( g_fullpath, false );
				if (revisions.Count() > 0)
				{
					int i = 0;
					printf( "%\t%s - ", g_p4->String( revisions[i].m_sUser ) );
...
void Cmd_RenameMaterialSubstr( void )
{
	char szSearch[256];
	char szReplace[256];
	char szOut[256];
	...
	GetToken( false );
	strcpy( szSearch, token );

	GetToken( false );
	strcpy( szReplace, token );

	int i;

	for ( i = 0; i < g_numtextures; i++ )
	{
		if ( V_StrSubst( g_texture[i].name, szSearch, szReplace, szOut, sizeof( szOut ) ) )
		{
			Msg( "$renamematerialsubstr renamed %s to %s\n", g_texture[i].name, szOut );
			strcpy( g_texture[i].name, szOut );
		}
	}

	if ( g_numtextures == 0 )
	{
		Msg( " There aren't any known materials yet. Did you try to rename a material before specifying any meshes?\n" );
	}
...
void Cmd_RenameMaterial( void )
{
	...
	if ( g_numtextures > 0 )
	{
		Msg( " Available materials to rename:\n" );
		for (i = 0; i < g_numtextures; i++) 
		{
			Msg( " %s\n", g_texture[i].name );
		}
...
void Cmd_StripMaterialPaths( void )
{
	int i;
	for (i = 0; i < g_numtextures; i++) 
	{
		Msg( "Replacing material reference %s with ", g_texture[i].name );
		V_FileBase( strdup( g_texture[i].name ), g_texture[i].name, sizeof( g_texture[i].name ) );
		Msg( "%s.\n", g_texture[i].name );
	}
}
...
void ClampMaxVerticesPerModel( s_source_t *pOrigSource )
{
	...
	MdlWarning( "model has too many verts (%i/%i), cutting into multiple models\n", pOrigSource->numvertices, g_maxVertexLimit );
	...
	for (int n = 0; n < newSource.Count(); n++)
	{
		...
		memcpy( newSource[n].m_meshindex, pOrigSource->meshindex, sizeof( newSource[n].m_meshindex ) );
	}
...
int ParseCmdlistToken( int &numcmds, s_animcmd_t *cmds )
{
	...
	else if (stricmp("forceboneposrot", token ) == 0)
	{
		...
		if (TokenAvailable())
		{
			...
			pcmd->u.forceboneposrot.bRangeSpecified = false;
			...
			if ( TokenAvailable() )
			{
				...
				if ( stricmp( token, "range" ) == 0 )
				{
					pcmd->u.forceboneposrot.bRangeSpecified = true;

					GetToken( false );
					pcmd->u.forceboneposrot.nFrameStart = verify_atoi( token );

					GetToken( false );
					pcmd->u.forceboneposrot.nFrameEnd = verify_atoi( token );
				}
				else
				{
					UnGetToken();
			...
			pcmd->u.forceboneposrot.bDoCopyFrame = false;
			pcmd->u.forceboneposrot.nCopyFrame = 0;

			if ( TokenAvailable() )
			{
				GetToken( false );
				if ( stricmp( token, "copyframe" ) == 0 )
				{
					pcmd->u.forceboneposrot.bDoCopyFrame = true;

					GetToken( false );
					pcmd->u.forceboneposrot.nCopyFrame = verify_atoi( token );
				}
				else
				{
					UnGetToken();
	...
	else if ( stricmp( "scalebonelen", token ) == 0 )
	{
		pcmd->cmd = CMD_SCALEBONELENGTH;

		GetToken( false );
		pcmd->u.forcebonelength.pBonename = strdup( token );
		pcmd->u.forcebonelength.flLengthScale = 1.0f;

		if ( TokenAvailable() )
		{
			GetToken( false );
			pcmd->u.forcebonelength.flLengthScale = verify_atof_with_null( token );
		}
...
void Cmd_ScreenAlign( void )
{
	if ( GetToken( false ) )
	{
		...
		if ( GetToken( false ) )
		{
			...
			else if ( !stricmp( "billboard", token ) )
			{
				g_screenalignedbone[g_numscreenalignedbones].flags = BONE_SCREEN_ALIGN_BILLBOARD;
			}
		}
...
void Cmd_KeepUpright( void )
{
	if ( GetToken( false ) )
	{
		Assert( g_numkeepuprightbones < MAXSTUDIOSRCBONES );

		strcpyn( g_keepuprightbone[g_numkeepuprightbones].name, token );
		g_keepuprightbone[g_numkeepuprightbones].flags = BONE_KEEP_UPRIGHT;

		g_numkeepuprightbones++;

	}
	else
	{
		TokenError( "$keepupright: expected bone name\n" );
	}
}
...
bool GetGlobalFilePath( const char *pSrc, char *pFullPath, int nMaxLen )
{
	...
	if ( g_bContentRootRelative && !g_pFullFileSystem->FileExists( pSrc ) )
		g_pFullFileSystem->RelativePathToFullPath( pSrc, "CONTENT", pFileName, sizeof( pFileName ) );
...
void Cmd_AddUVMapChannelTo()
{
	char szExistingBodySrcName[256];
	char szSrcNameOfUVDataToAdd[256];
	int nDestUVChannel = 0;

	if ( !GetToken( false ) )
	{
		MdlError( "$AddUVMapChannelTo: Expected existing body name!\n" );
		return;
	}

	strcpy( szExistingBodySrcName, token );

	if ( !GetToken( false ) )
	{
		MdlError( "$AddUVMapChannelTo: Expected name of src holding UV data to add!\n" );
		return;
	}

	strcpy( szSrcNameOfUVDataToAdd, token );

	if ( !GetToken( false ) )
	{
		MdlError( "$AddUVMapChannelTo: Expected integer UV map channel!\n" );
		return;
	}

	nDestUVChannel = atoi( token );

	//syntax:
	//$adduvmapchannelto existingsrc newsrc channelidx

	for ( int i = 0; i < g_nummodels; i++ )
	{
		if ( !V_strcmp( szExistingBodySrcName, g_model[i]->filename ) )
		{
			s_source_t *pSrcOfUVData = Load_Source( szSrcNameOfUVDataToAdd, "" );
			if ( pSrcOfUVData )
			{
				s_source_t *pExistingSrc = g_model[i]->source;

				bool bVertsMatch = pSrcOfUVData->numvertices == pExistingSrc->numvertices;
				bool bFacesMatch = pSrcOfUVData->numfaces == pExistingSrc->numfaces;

				if ( !bFacesMatch )
				{
					MdlError( "$AddUVMapChannelTo: Source does not have identical face count! %s!\n", szSrcNameOfUVDataToAdd );
					return;
				}

				if ( bVertsMatch )
				{
					for ( int nVert = 0; nVert < pExistingSrc->numvertices; nVert++ )
					{
						pExistingSrc->vertex[nVert].numTexcoord = MAX( pExistingSrc->vertex[nVert].numTexcoord, nDestUVChannel + 1 );
						pExistingSrc->vertex[nVert].texcoord[nDestUVChannel] = pSrcOfUVData->vertex[nVert].texcoord[0];
					}
				}
				else
				{
					for ( int nFace = 0; nFace < pExistingSrc->numfaces; nFace++ )
					{
						int nVertexA = pExistingSrc->face[nFace].a;
						int nVertexB = pSrcOfUVData->face[nFace].a;
						pExistingSrc->vertex[nVertexA].numTexcoord = MAX( pExistingSrc->vertex[nVertexA].numTexcoord, nDestUVChannel + 1 );
						pExistingSrc->vertex[nVertexA].texcoord[nDestUVChannel] = pSrcOfUVData->vertex[nVertexB].texcoord[0];

						nVertexA = pExistingSrc->face[nFace].b;
						nVertexB = pSrcOfUVData->face[nFace].b;
						pExistingSrc->vertex[nVertexA].numTexcoord = MAX( pExistingSrc->vertex[nVertexA].numTexcoord, nDestUVChannel + 1 );
						pExistingSrc->vertex[nVertexA].texcoord[nDestUVChannel] = pSrcOfUVData->vertex[nVertexB].texcoord[0];

						nVertexA = pExistingSrc->face[nFace].c;
						nVertexB = pSrcOfUVData->face[nFace].c;
						pExistingSrc->vertex[nVertexA].numTexcoord = MAX( pExistingSrc->vertex[nVertexA].numTexcoord, nDestUVChannel + 1 );
						pExistingSrc->vertex[nVertexA].texcoord[nDestUVChannel] = pSrcOfUVData->vertex[nVertexB].texcoord[0];

						nVertexA = pExistingSrc->face[nFace].d;
						nVertexB = pSrcOfUVData->face[nFace].d;
						pExistingSrc->vertex[nVertexA].numTexcoord = MAX( pExistingSrc->vertex[nVertexA].numTexcoord, nDestUVChannel + 1 );
						pExistingSrc->vertex[nVertexA].texcoord[nDestUVChannel] = pSrcOfUVData->vertex[nVertexB].texcoord[0];
					}
				}
			...
			{
				MdlError( "$AddUVMapChannelTo: Failed to load %s!\n", szSrcNameOfUVDataToAdd );
				return;
			}
		}
	}
...
bool EatClothUint( const char *pName, uint &dst )
{
	if ( !GetToken( true ) )
	{
		TokenError( "Cloth value %s is missing\n", pName );
...
static bool s_bClothEnableInU = false;
QAngle s_angClothPrerotate(0,0,0);
static float g_flWeldVertices = -1.0f;
...
void ParseClothKeyvalues()
{
	...
	BoolVal_t boolVals[] = {
		...
		{ "cloth_enable_in_u", &s_bClothEnableInU },
		{ "follow_the_lead", &GetAuthPhysFx()->m_bFollowTheLead }
	};
	FloatVal_t floatVals[] = {
		...
		{ "teleport_distance_multiplier", &GetAuthPhysFx()->m_flTeleportDistanceMultiplier },
		{ "weld_vertices", &g_flWeldVertices }
	};
	UIntVal_t uintVals[] = {
		{ "extra_iterations", &GetAuthPhysFx()->m_nExtraIterations }
	};
		...
		if ( *pToken )
		{
			...
			for ( int i = 0; !bFound && i < ARRAYSIZE( boolVals ); ++i )
			{
				if ( !V_stricmp( pToken, boolVals[ i].pKey ) )
				{
					...
					{
						//TokenError( "Cloth keyvalue %s: bool expected\n", pToken );
						return;
					}
				}
			}
			...
			for ( int i = 0; !bFound && i < ARRAYSIZE( floatVals ); ++i )
			{
				if ( !V_stricmp( pToken, floatVals[ i ].pKey ) )
				{
					...
					{
						//TokenError( "Cloth keyvalue %s : float expected\n", pToken );
						return;
					}
				}
			}
			
			for ( int i = 0; !bFound && i < ARRAYSIZE( uintVals ); ++i )
			{
				if ( !V_stricmp( pToken, uintVals[ i ].pKey ) )
				{
					if ( EatClothUint( uintVals[ i ].pKey, *uintVals[ i ].pUInt ) )
					{
						bFound = true;
						break;
					}
			...
			if ( !bFound )
			{
				...
				else if ( !V_stricmp( pToken, "follow_bone" ) )
				{
					if ( !GetToken( true ) )
					{
						TokenError( "follow_bone must be followed by bone name\n" );
						return;
					}
					if ( CAuthPhysFx::CBone *pFollowBone = GetAuthPhysFx()->GetOrCreateBone( token ) )
					{
						if ( EatClothFloat( "follow_bone", GetAuthPhysFx()->m_flFollowSimCtrl ) )
						{
							pFollowBone->m_bSimulated = false;
							GetAuthPhysFx()->m_FollowSimCtrl = pFollowBone->m_Name;
							bFound = true;
						}
					}
					if ( !bFound )
...
CDmElement *LoadClothSource( const char *pSourcePath )
{
	// append cloth piece to the cloth builder
	// use the full search tree, including mod hierarchy to find the file
	char pFullPath[ MAX_PATH ];
	if ( !GetGlobalFilePath( pSourcePath, pFullPath, sizeof( pFullPath ) ) )
	{
		TokenError( "Cannot find file %s", pSourcePath );
		return NULL;
	}
	const char *pFileExtension = V_GetFileExtension( pFullPath );
	if ( !pFileExtension )
	{
		TokenError( "$cloth command must always specify file extension; dmx or fbx are supported (%s )", pSourcePath );
		return NULL;
	}

	CDmElement *pRoot = NULL;
	if ( !V_stricmp( pFileExtension, "fbx" ) )
	{
		...
		dmFbxSerializer.m_bIgnoreMaterialErrors = true;
		pRoot = dmFbxSerializer.ReadFBX( pFullPath );
		if ( !pRoot )
		{
			TokenError( "Unexpected error when converting cloth fbx\n" );
		}
	}
	else
	{
...
uint WeldVertices( CDmeMesh * pMesh, float flTolerance )
{
	CDmeVertexData *pBindState = pMesh->GetBindBaseState();
	if ( !pBindState )
		return 0;
	FieldIndex_t nPositionField = pBindState->FindFieldIndex( CDmeVertexDataBase::FIELD_POSITION );
	if ( nPositionField < 0 )
		return 0;
	CDmrArray< Vector > vertexData( pBindState->GetVertexData( nPositionField ) );
	CDmrArray< int > indexData( pBindState->GetIndexData( nPositionField ) );
	CUtlVector< int > mapToNew;
	mapToNew.SetCount( vertexData.Count() );
	
	Vector vTolerance( flTolerance, flTolerance, flTolerance );

	uint nRemoved = 0;
	CDynamicTree tree;
	for ( int i = 0; i < vertexData.Count(); ++i )
	{
		const Vector &vert = vertexData[ i ];
		CProxyVector others;
		tree.Query( others, vertexData[ i ], flTolerance);
		if ( others.IsEmpty() )
		{
			mapToNew[ i ] = i - nRemoved;
			if ( nRemoved )
				vertexData.Set( i - nRemoved, vertexData[ i ] );
			tree.CreateProxy( AABB_t( vert, vert ), ( void* )( i - nRemoved ) );
		}
		else
		{
			nRemoved++;
			// this vertex has others... find the one with minimal index
			int nMin = INT_MAX;
			for ( int nOtherProxy : others )
			{
				intp nOtherVert = ( intp )tree.GetUserData( nOtherProxy );
				if ( nOtherVert < nMin )
					nMin = nOtherVert;
				mapToNew[ i ] = nMin;
			}
	...
	vertexData.RemoveMultiple( vertexData.Count() - nRemoved, nRemoved );

	// by now, we have each vertex doppelganger's index in the mapToNew array
	for ( int i = 0; i < indexData.Count(); ++i )
	{
		indexData.Set( i, mapToNew[ indexData[ i ] ] );
	}
	return nRemoved;
}


uint WeldVertices( CDmeDag * pDag, float flTolerance )
{
	uint nRemoved = 0;
	if ( CDmeMesh *pMesh = CastElement< CDmeMesh >( pDag->GetShape() ) )
	{
		nRemoved += WeldVertices( pMesh, flTolerance );
	}
	...
	for ( int nChild = 0; nChild < nChildCount; ++nChild )
	{
		...
		nRemoved += WeldVertices( pChild, flTolerance );
	}

	return nRemoved;
}

uint WeldVertices( CDmeModel * pModel, float flTolerance )
{
...
void Cmd_Cloth()
{
	...
	const char *pSourcePath = token;
	if ( CDmElement *pRoot = LoadClothSource( pSourcePath ) )
	{
		...
		{
			if ( g_flWeldVertices >= 0 )
			{
				uint nRemoved = WeldVertices( pModel, g_flWeldVertices );
				Msg( "Removed %u vertices, threshold %g\n", nRemoved, g_flWeldVertices );
			}

			CVClothProxyMesh meshOptions;
			meshOptions.m_bFlexClothBorders = s_bFlexClothBorderJoints;
			meshOptions.m_bClothEnableInU = s_bClothEnableInU;
...
void Cmd_ClothPlaneCollision()
{
	...
	if ( CDmElement *pRoot = LoadClothSource( pSourcePath ) )
	{
		...
		{
			TokenError( "File %s has no DmeModel in it\n", pSourcePath );
		}
	}
}
...
void Cmd_NoErrorOnSeqRemapFail()
{
	g_bErrorOnSeqRemapFail = false;
}
...
MDLCommand_t g_Commands[] =
{
	...
	{ "$keepupright",	Cmd_KeepUpright, MC_CURRENT_VERSION },
	...
	{ "$renamematerialsubstr", Cmd_RenameMaterialSubstr, MC_CURRENT_VERSION },
	{ "$overridematerial", Cmd_OverrideMaterial, MC_CURRENT_VERSION },
	{ "$stripmaterialpaths", Cmd_StripMaterialPaths, MC_CURRENT_VERSION },
	...
	{ "$erroronsequenceremappingfailure_disable", Cmd_NoErrorOnSeqRemapFail, MC_CURRENT_VERSION },
	...
	{ "$adduvmapchannelto", Cmd_AddUVMapChannelTo, MC_CURRENT_VERSION },
};
...
int CStudioMDLApp::Main()
{
	...
	if ( !bLoadingPreprocessedFile )
	{
		...
		// ensure that the model isn't being compiled with an out-of-date qc
		if ( g_p4 )
		{
			P4File_t pFileInfo;
			if ( g_p4->GetFileInfo( g_path, &pFileInfo ) && pFileInfo.m_iHeadRevision != pFileInfo.m_iHaveRevision )
			{
				if ( g_p4->SyncFile( g_path ) )
				{
					MdlWarning( "Syncing out-of-date dependency file '%s'.\n", g_path );
				}
				else
				{
					MdlError( "Failed to sync out-of-date dependency file '%s'.\n", g_path );
				}
			}
		}
