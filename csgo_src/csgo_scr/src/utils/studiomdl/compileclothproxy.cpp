...
void CClothProxyCompiler::AppendPlaneCollisionMesh( CModelContext &modelContext, CDmeMesh *pMesh, const matrix3x4_t &tm )
{
	...
	for ( int nNode = 0; nNode < bestPlanes.Count(); ++nNode ) // new nodes/bones may have been added since we created bestPlanes array, but they're all static
	{
...
bool CClothProxyCompiler::CopyFirstFloat( CDmeVertexData *pBindState, FieldIndex_t nField, CUtlVector< float > &attr )
{
	if ( nField < 0 )
	{
		Msg( "Cloth has no cloth_enable attribute!\n" );
		attr.FillWithValue( 0 );
		return false;
	}
	CDmrArrayConst< int > indexData( pBindState->GetIndexData( nField ) );
	if ( !indexData.IsValid() )
		return false;
	if ( indexData.Count() < attr.Count() )
		return false;
	const CDmAttribute *pVertexData = pBindState->GetVertexData( nField );
	switch ( pVertexData->GetType() )
	{
	case AT_COLOR_ARRAY:
		{
			const CDmrArrayConst< Color > vertexData = pVertexData;
			if ( !vertexData.IsValid() )
				return false;
			for ( int i = 0; i < attr.Count(); ++i )
			{
				const Color& src = vertexData[ indexData[ i ] ];
				attr[ i ] = src.r() * ( 1.0f / 255.0f );
			}
		}
		return true;
	...
	case AT_VECTOR2_ARRAY:
		{
			const CDmrArrayConst< Vector2D > vertexData = pVertexData;
			if ( !vertexData.IsValid() )
				return false;
			for ( int i = 0; i < attr.Count(); ++i )
			{
				const Vector2D& src = vertexData[ indexData[ i ] ];
				attr[ i ] = fabsf( src.x );
			}
		}
		return true;
	...
	case AT_VECTOR3_ARRAY:
		{
			const CDmrArrayConst< Vector > vertexData = pVertexData;
			if ( !vertexData.IsValid() )
				return false;
			for ( int i = 0; i < attr.Count(); ++i )
			{
				const Vector& src = vertexData[ indexData[ i ] ];
				attr[ i ] = fabsf( src.x );
			}
		}
		return true;
	...
	case AT_VECTOR4_ARRAY:
		{
			const CDmrArrayConst< Vector4D > vertexData = pVertexData;
			if ( !vertexData.IsValid() )
				return false;
			for ( int i = 0; i < attr.Count(); ++i )
			{
				const Vector4D& src = vertexData[ indexData[ i ] ];
				attr[ i ] = fabsf( src.x );
			}
		}
		return true;
...
void CClothProxyCompiler::CreateClothBones( CMeshContext &context )
{
	...
	CUtlVector< float > attrCloth;
	attrCloth.SetCount( context.m_AttrPos.GetElementCount() );
	
	if ( CopyFirstFloat( context.m_pBindState, context.m_nClothEnableField, attrCloth ) )
	{
		int nDataCount = context.m_AttrPos.GetDataCount();
		CVarBitVec dmePosEnable( nDataCount );
		for ( int nIndex = 0; nIndex < attrCloth.Count(); ++nIndex )
		{
...
int CClothProxyCompiler::CMeshContext::FindMostBoundJoint( int nDmePos, float flBonusForExisting )
{
	...
	for ( int i = 0; i < nBindCount; ++i )
	{
		...
		int nIndex = pIndices[ i ];
		if ( flWeight > 0 && uint( nIndex ) < uint( m_pModelContext->m_JointToFxBone.Count() ) )
		{
			if ( m_pModelContext->m_JointToFxBone[ nIndex ] >= 0 )
			{
...
void CClothProxyCompiler::AddFitWeights( CMeshContext &context )
{
			...
			for ( int nIndex = 0; nIndex < attrCloth.Count(); ++nIndex )
			{
				...
				if ( flEnable < context.m_pModelContext->m_flClothEnableThreshold ) // only create dynamic nodes at this time, we may or may not need the static nodes
				{
					...
					for ( int nBinding = 0; nBinding < nBindCount; ++nBinding )
					{
						...
						{
							int nIndex = pIndices[ nBinding ]; // some indices are sometimes invalid, protect against bad data
							if ( nIndex >= 0 && nIndex < lockedJoints.GetNumBits() )
							{
								lockedJoints.Set( nIndex );
							}
						}
					}
				}
	...
	for ( int nPos = 0; nPos < context.m_AttrPos.m_Data.Count(); ++nPos )
	{
		...
		for ( int nBinding = 0; nBinding < nBindCount; ++nBinding )
		{
			...
			if ( inf.flWeight >= flWeightThreshold && nJoint >= 0)
			{
...
CClothProxyCompiler::CMeshContext::CMeshContext( CClothProxyCompiler *pCompiler, CModelContext *pModelContext, CDmeMesh *pMesh, const matrix3x4_t &tm, int nDmeMesh )
{
	...
	m_nClothEnableField = m_pBindState->FindFieldIndex( CDmeVertexDataBase::FIELD_CLOTH_ENABLE ); // default
	if ( m_nClothEnableField < 0 )
	{
		m_nClothEnableField = m_pBindState->FindFieldIndex( CDmeVertexDataBase::FIELD_COLOR );
		if ( m_nClothEnableField < 0 )
			m_nClothEnableField = m_pBindState->FindFieldIndex( "color$0" ); // Source2 variant of the name, produced by fbx converter
		if ( m_nClothEnableField >= 0 )
		{
			Msg( "Mesh %s has no cloth_enable attribute, but has vertex color. Cloth will use vertex color as cloth_enable attribute.\n", pMesh->GetName() );
		}
	}
...
void CClothProxyCompiler::AppendMesh( CModelContext &modelContext, CDmeMesh *pMesh, const matrix3x4_t &tm )
{
	...
	if ( modelContext.m_Proxy.m_bClothEnableInU )
	{
		context.m_nClothEnableField = context.m_pBindState->FindFieldIndex( CDmeVertexDataBase::FIELD_TEXCOORD );
	}
