			...
			case CMD_FORCEBONEPOSROT:
				{
					...
					if (bone != -1)
					{
						...
						int nStartFrame = 0;
						int nEndFrame = panim->numframes;

						if ( pcmd->u.forceboneposrot.bRangeSpecified )
						{
							nStartFrame = clamp( pcmd->u.forceboneposrot.nFrameStart, 0, panim->numframes );
							nEndFrame = clamp( pcmd->u.forceboneposrot.nFrameEnd, 0, panim->numframes );
						}

						int nCopyFrame = clamp( pcmd->u.forceboneposrot.nCopyFrame, 0, panim->numframes );

						for ( int i=nStartFrame; i<nEndFrame; i++ )
						{
							if ( pcmd->u.forceboneposrot.bDoCopyFrame )
							{
								panim->sanim[i][bone].pos = panim->sanim[nCopyFrame][bone].pos;
								panim->sanim[i][bone].rot = panim->sanim[nCopyFrame][bone].rot;
							}
				...
				break;
			case CMD_SCALEBONELENGTH:
				{
					int bone = findGlobalBone( pcmd->u.forcebonelength.pBonename );
					if (bone != -1)
					{
						for ( int i=0; i<panim->numframes; i++ )
						{
							panim->sanim[i][bone].pos *= pcmd->u.forcebonelength.flLengthScale;
						}
					}
					else
					{
						MdlError("unable to find bone %s to scalebonelen\n", pcmd->u.forcebonelength.pBonename );
					}
				}
				break;
			case CMD_BONEDRIVER:
...
static void TagKeepUprightBones()
{
	for ( int i = 0; i < g_numkeepuprightbones; i++ )
	{
		int j = findGlobalBone( g_keepuprightbone[i].name );
		if ( j == -1 )
		{
			MdlError( "unknown g_keepuprightbone link '%s'\n", g_keepuprightbone[i].name );
		}

		g_bonetable[j].flags |= g_keepuprightbone[i].flags;
		printf( "tagging bone: %s as world aligned (index %i, flags:%x)\n", g_bonetable[j].name, j, g_bonetable[j].flags );
	}
}
...
static void LinkAttachments()
{
	...
	for (i = 0; i < g_numattachments; i++)
	{
		...
		for (k = 0; k < g_numbones; k++)
		{
			if ( !stricmp( g_attachment[i].bonename, g_bonetable[k].name ) || !stricmp( g_attachment[i].bonename, "" ) )
			{
