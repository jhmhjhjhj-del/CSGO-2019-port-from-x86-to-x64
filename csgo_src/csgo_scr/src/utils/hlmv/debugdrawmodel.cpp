...
int DebugDrawModelTexCoord( IStudioRender *pStudioRender, const char *pMaterialName, const DrawModelInfo_t& info, matrix3x4_t *pBoneToWorld, float w, float h )
{
	...
	float flwBorder = (w * 0.5f) * g_flUVZoom;
	float flhBorder = (h * 0.5f) * g_flUVZoom;
	float flXRangeMax = 1.0f;
	float flYRangeMax = 1.0f;
	float flRestrictedH = MAX( 0, h - 290 );
	float flAspectRatio = w / flRestrictedH;

	if ( w > flRestrictedH )
	{
		flXRangeMax *= flAspectRatio;
	}
	else
	{
		flYRangeMax *= 1.0f / flAspectRatio;
	}
	...
	// now render a single quad with the base texture on it
	{
		...
		{
			Vector p = *pUV[i];
			p.x = RemapVal( p.x, 0, flXRangeMax, flwBorder, w - flwBorder );
			p.y = RemapVal( p.y, 0, flYRangeMax, flhBorder, h - flhBorder );
	...
	// use the stencil buffer to accumulate overdraw
	ShaderStencilState_t stencilStateOverdrawPrep;
	stencilStateOverdrawPrep.m_bEnable = true;
	stencilStateOverdrawPrep.m_nReferenceValue = 0x0;
	stencilStateOverdrawPrep.m_nTestMask = 0xFF;
	stencilStateOverdrawPrep.m_nWriteMask = 0xFF;
	stencilStateOverdrawPrep.m_CompareFunc = SHADER_STENCILFUNC_ALWAYS;
	stencilStateOverdrawPrep.m_PassOp = SHADER_STENCILOP_INCREMENT_CLAMP;
	stencilStateOverdrawPrep.m_FailOp = SHADER_STENCILOP_INCREMENT_CLAMP;
	stencilStateOverdrawPrep.m_ZFailOp = SHADER_STENCILOP_INCREMENT_CLAMP;
	pRenderContext->SetStencilState( stencilStateOverdrawPrep );

	// clear the stencil buffer
	pRenderContext->ClearBuffers( false, false, true );

	// draw the UV polys invisibly, overlap is captured in the stencil buffer. FIXME: Reuse these verts instead of recomputing
	for ( int i = 0; i < batchList.Count(); i++ )
	{
		GetTriangles_MaterialBatch_t &materialBatch = tris.m_MaterialBatches[batchList[i]];
		
		pRenderContext->Bind( g_materialAccumulateOverdraw );
		...
		for ( indexStart = 0; indexStart < materialBatch.m_TriListIndices.Count(); )
		{
			...
			for ( vertID = 0; vertID < materialBatch.m_Verts.Count(); vertID++ )
			{
				...
				Vector p;
				p.x = RemapVal( vert.m_TexCoord.x, 0, flXRangeMax, flwBorder, w - flwBorder );
				p.y = RemapVal( vert.m_TexCoord.y, 0, flYRangeMax, flhBorder, h - flhBorder );
			...
			{
				meshBuilder.FastIndex( materialBatch.m_TriListIndices[j + indexStart] );
			}
			meshBuilder.End();
	...
	const color32 uvWireframeColor = {0,255,255,0};
	const color32 uvWireframeOutOfBoundsColor = { 255,128,0,0 };
	// now draw all batches with the matching material in wireframe over the render of the base texture. FIXME: re-use the verts computed above
	for( int i = 0; i < batchList.Count(); i++ )
	{
		GetTriangles_MaterialBatch_t &materialBatch = tris.m_MaterialBatches[batchList[i]];
		...
		for ( indexStart = 0; indexStart < materialBatch.m_TriListIndices.Count(); )
		{
			...
			for ( vertID = 0; vertID < materialBatch.m_Verts.Count(); vertID++ )
			{
				...
				bool bInBounds = vert.m_TexCoord.x >= 0 && vert.m_TexCoord.x <= 1 && vert.m_TexCoord.y >= 0 && vert.m_TexCoord.y <= 1;
				...
				if ( bInBounds )
				{
					meshBuilder.Color4ubv( &uvWireframeColor.r );
				}
				else
				{
					meshBuilder.Color4ubv( &uvWireframeOutOfBoundsColor.r );
				}
	...
	// use the stencil buffer to read back overdraw
	ShaderStencilState_t stencilStateOverdrawShow;
	stencilStateOverdrawShow.m_bEnable = true;
	stencilStateOverdrawShow.m_nReferenceValue = 0x2;
	stencilStateOverdrawShow.m_nTestMask = 0xFF;
	stencilStateOverdrawShow.m_nWriteMask = 0x0;
	stencilStateOverdrawShow.m_CompareFunc = SHADER_STENCILFUNC_LEQUAL;
	stencilStateOverdrawShow.m_PassOp = SHADER_STENCILOP_KEEP;
	stencilStateOverdrawShow.m_FailOp = SHADER_STENCILOP_KEEP;
	stencilStateOverdrawShow.m_ZFailOp = SHADER_STENCILOP_KEEP;
	pRenderContext->SetStencilState( stencilStateOverdrawShow );

	//float flPulse = abs( sin( Plat_FloatTime() * 2.0f ) * 0.5 + 0.5f );

	// now render a single quad to reveal overdraw
	{
		pRenderContext->Bind( g_materialDrawUVOverlap );
		...
		for ( int i = 0; i < 4; i++ )
		{
			...
			meshBuilder.Color4f( 1.0f, 1.0f, 0.0f, 0.6f );
			meshBuilder.AdvanceVertex();
		}
		meshBuilder.FastIndex( 0 );
		meshBuilder.FastIndex( 1 );
