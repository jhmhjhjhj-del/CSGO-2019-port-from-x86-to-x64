...
CUtlVector< KeyValues* > g_kvMVScriptSubKeys;
KeyValues* g_MVScriptTopKey;
...
: mxWindow (0, 0, 0, 0, 0, g_appTitle, mxWindow::Normal)
{
	...
	menuFile->addSeparator();
	menuFile->add( "Copy Sequence List to Clipboard", IDC_DUMP_ANIM_SEQUENCE_INDICES );

	menuFile->addSeparator();
	menuFile->add( "Extract posed SMD", IDC_DUMP_SMD );
	menuFile->add( "Extract posed OBJ", IDC_DUMP_OBJ );
	...
	menuView->add( "Orbit selected bone", IDC_VIEW_ORBIT_BONE );
...
void MDLViewer::Refresh( void )
{
	...
	// save current animation frame selections so they can be 
	// restored when the model is reloaded, this will prevent the
	// model from 'popping' back to the default pose when refreshed.
	float flSavedLayerFrames[MAX_SEQUENCES];
	for ( int i=0; i<ARRAYSIZE(flSavedLayerFrames); i++ )
	{
		flSavedLayerFrames[i] = g_pStudioModel->GetFrame( i );
	}
	...
	// restore the saved animation frames on all layers.
	bool bNeedToRefreshLayerAnimUI = false;
	for ( int i=0; i<ARRAYSIZE(flSavedLayerFrames); i++ )
	{
		if ( g_pStudioModel->GetFrame( i ) != flSavedLayerFrames[i] )
		{
			g_pStudioModel->SetFrame( i, flSavedLayerFrames[i] );
			bNeedToRefreshLayerAnimUI = true;
		}
	}
	// and update the UI if necessary
	if ( bNeedToRefreshLayerAnimUI )
	{
		d_cpl->updateFrameSelection();
		d_cpl->updateFrameSlider();
	}
...
void MDLViewer::ExecuteMVScriptKey( KeyValues *kvSub )
{
	if ( !kvSub )
		return;
...
{
	if ( g_MVScriptTopKey != NULL )
	{
		delete g_MVScriptTopKey;
		g_MVScriptTopKey = NULL;
	}

	g_MVScriptTopKey = new KeyValues("ModelViewerScript");

	if (g_MVScriptTopKey->LoadFromFile(g_pFullFileSystem, p_szScriptPath))
	{
		Msg("Executing mvscript: %s\n", p_szScriptPath);

		g_kvMVScriptSubKeys.RemoveAll();

		for (KeyValues *kvSub = g_MVScriptTopKey->GetFirstSubKey(); kvSub; kvSub = kvSub->GetNextKey())
		{
			g_kvMVScriptSubKeys.AddToTail( kvSub );
		}
	}
}
...
MDLViewer::handleEvent (mxEvent *event)
{
	...
	case mxEvent::Action:
	{
		...
		case IDC_DUMP_ANIM_SEQUENCE_INDICES:
		{
			CStudioHdr *pStudioHdr = g_pStudioModel->GetStudioHdr();
			if ( pStudioHdr )
			{
				...
				int nSequenceCount = pStudioHdr->GetNumSeq();
				...
				if ( out.GetSize() )
				{
					...
					mxMessageBox( this, "Sequence indices copied to clipboard.", g_appTitle, MX_MB_OK | MX_MB_INFORMATION );
				}
		...
		case IDC_VIEW_ORBIT_BONE:
			d_cpl->OrbitSelectedBone();
			break;
...
#define HEIGHT 140
	...
	case mxEvent::Timer:
	{
		if ( g_MVScriptTopKey != NULL && !g_kvMVScriptSubKeys.Count() )
		{
			delete g_MVScriptTopKey;
			g_MVScriptTopKey = NULL;
		}

		if ( g_kvMVScriptSubKeys.Count() )
		{
			ExecuteMVScriptKey( g_kvMVScriptSubKeys[0] );
			g_kvMVScriptSubKeys.Remove(0);
			setTimer( 10 );
		}
		else if ( g_HlmvIpcServer.HasCommands() )
		{
		...
		g_ControlPanel->UpdateSelectedBoneAngle();
