...
public:
	...
	mxLabel* m_lblSelectedBoneAngle;

	int GetSelectedBone( void ) const { return m_Bone; }
...
void CBoneControlWindow::Init( )
{
	...
	// show selected bone angles:
	m_lblSelectedBoneAngle = new mxLabel (this, left + 110, top + 4, 160, 15, "");
...
void ControlPanel::OrbitSelectedBone( void )
{
	if ( !m_pBoneWindow )
		return;

	int nSelectedBone = m_pBoneWindow->GetSelectedBone();

	CStudioHdr *pHdr = g_pStudioModel->GetStudioHdr();
	if ( !pHdr )
		return;

	if ( clamp( nSelectedBone, 0, pHdr->numbones() ) != nSelectedBone )
		return;

	matrix3x4_t matLocalBone;
	ConcatTransforms( g_viewtransform.InverseTR(), *g_pStudioModel->BoneToWorld( nSelectedBone ), matLocalBone );
	g_SelectedBoneAngle = matLocalBone.ToQAngle();
	g_SelectedBonePos = matLocalBone.GetOrigin();
}
...
void ControlPanel::SetupMatVarWindow( mxTab* pTab )
{
	...
	lblMatrixErrorMsg = new mxLabel (wMatVars, 405, 25, 600, 18, "Failed to locate this material at any of the following available search paths:");
	lblMatrixErrorMsg->setVisible(false);

	cMaterialErrorPaths = new mxListBox( wMatVars, 405, 43, 600, 145 );
	cMaterialErrorPaths->setEnabled(true);
	cMaterialErrorPaths->setVisible(false);
...
void ControlPanel::UpdateSelectedBoneAngle( void )
{
	if ( !g_pStudioModel || (g_viewerSettings.highlightBone < 0 ) )
	{
		m_pBoneWindow->m_lblSelectedBoneAngle->setLabel( "" );
	}
	else
	{
		char tmp[128];
		sprintf( tmp, "Angle: %.3f, %.3f, %.3f", g_SelectedBoneAngle.x, g_SelectedBoneAngle.y, g_SelectedBoneAngle.z );
		m_pBoneWindow->m_lblSelectedBoneAngle->setLabel( tmp );
	}
}
...
void ControlPanel::CompileTimerUpdate( void )
{
	...
	if ( (GetTickCount() - nCompileLastUpdateTick) < 1000 || !g_pStudioModel || !g_pStudioModel->GetFileName() )
		return;
	nCompileLastUpdateTick = GetTickCount();
...
void ControlPanel::SetupBodyWindow( mxTab* pTab )
{
	...
	new mxLabel( wBody, 105, 142, 100, 18, "UV Zoom:" );
	slUVZoom = new mxSlider( wBody, 155, 142, 100, 18, IDC_UVZOOM );
	slUVZoom->setRange( 0.0f, 0.9f );
	slUVZoom->setValue( g_flUVZoom );
	...
	switch (event->action)
	{
		...
		case IDC_MATERIALVARMATS:
		{
			...
			if ( pStudioR )
			{
				...
				{
					int nMatListIndex = cMaterialList->getSelectedIndex();
					if ( nMatListIndex >= 0 && nMatListIndex <= pStudioR->numcdtextures )
					{
						cMaterialErrorPaths->removeAll();

						for ( int n=0; n<pStudioR->numcdtextures; n++ )
						{
							const char *pCdTexture = pStudioR->pCdtexture(n);
							if ( pCdTexture[0] == CORRECT_PATH_SEPARATOR || pCdTexture[0] == INCORRECT_PATH_SEPARATOR )
								++pCdTexture;

							char temp[255];
							V_ComposeFileName( pCdTexture, pStudioR->pTexture( nMatListIndex )->pszName(), temp, sizeof( temp ) );
							V_SetExtension( temp, ".vmt", sizeof(temp) );
							cMaterialErrorPaths->add( temp );
						}

						lblMatrixErrorMsg->setVisible(true);
						cMaterialErrorPaths->setVisible(true);
					}
				}
			}
			break;
...
void ControlPanel::dumpSMD()
{
	// naively write a plaintext SMD of the loaded model

	CStudioHdr *hdr = g_pStudioModel->GetStudioHdr();
	if ( !hdr )
		return;

	char *ptr = (char *)mxGetSaveFileName( this, "", "*.smd" );
	if ( !ptr )
		return;
	...
	{
		matBoneToWorld[i] = ConcatTransforms( rootxform, *g_pStudioModel->BoneToWorld( i ) );
	}

	CTextBuffer out;
	char tmp[128];
	out.WriteText( "version 1\n" );

	{
		...
		for ( int i = 0; i < hdr->numbones(); i++ )
		{
			sprintf( tmp, " %i \"%s\" %i\n", i, hdr->pBone( i )->pszName(), hdr->pBone( i )->parent );
			out.WriteText( tmp );
		}
		out.WriteText( "end\n" );
	}
	...
	{
		out.WriteText( "skeleton\ntime 0\n" );
		const mstudiobone_t *pbones = hdr->pBone( 0 );
		for ( int i = 0; i < hdr->numbones(); i++ )
		{
			matrix3x4_t pBoneMat = matBoneToWorld[i];

			int nParent = hdr->pBone( i )->parent;
			if ( nParent >= 0 )
			{
				matrix3x4a_t pBoneMatParentInv;
				MatrixInvert( matBoneToWorld[nParent], pBoneMatParentInv );

				ConcatTransforms( pBoneMatParentInv, pBoneMat, pBoneMat );
			}

			Vector vecBoneOrigin = pBoneMat.GetOrigin();
			RadianEuler eulAngle;
			MatrixAngles( pBoneMat, eulAngle );
			sprintf( tmp, " %i %.5f %.5f %.5f %.5f %.5f %.5f\n", i,
				isnan( vecBoneOrigin.x ) ? 0 : vecBoneOrigin.x,
				isnan( vecBoneOrigin.y ) ? 0 : vecBoneOrigin.y,
				isnan( vecBoneOrigin.z ) ? 0 : vecBoneOrigin.z,
				isnan( eulAngle.x ) ? 0 : eulAngle.x,
				isnan( eulAngle.y ) ? 0 : eulAngle.y,
				isnan( eulAngle.z ) ? 0 : eulAngle.z );
			out.WriteText( tmp );
		}
		out.WriteText( "end\n" );
	}
	...
	{
		out.WriteText( "triangles\n" );

		static GetTriangles_Output_t tris;
		g_pStudioRender->GetTriangles( g_DrawModelInfo, matBoneToWorld, tris );
		...
		for ( batchID = 0; batchID < tris.m_MaterialBatches.Count(); batchID++ )
		{
			...
			for ( int nTri = 0; nTri < materialBatch.m_TriListIndices.Count(); nTri += 3 )
			{
				sprintf( tmp, "%s\n", materialBatch.m_pMaterial->GetName() );
				out.WriteText( tmp );

				for ( int nVert = 0; nVert < 3; nVert++ )
				{
					GetTriangles_Vertex_t &vert = materialBatch.m_Verts[materialBatch.m_TriListIndices[nTri + nVert]];
					...
					sprintf( tmp, " %i  %.5f %.5f %.5f  %.5f %.5f %.5f  %.5f %.5f ",
						vert.m_BoneIndex[0],
						skinnedPos.x,
						skinnedPos.y,
						skinnedPos.z,
						...
						skinnedNormal.z,
						vert.m_TexCoord.x,
						1.0f - vert.m_TexCoord.y ); // fixme: why flip?
					out.WriteText( tmp );

					if ( vert.m_NumBones > 0 )
					{
						sprintf( tmp, " %i ", vert.m_NumBones );
						out.WriteText( tmp );

						for ( k = 0; k < vert.m_NumBones; k++ )
						{
							sprintf( tmp, " %i %.5f ", vert.m_BoneIndex[k], vert.m_BoneWeight[k] );
							out.WriteText( tmp );
						}
					}
...
void ControlPanel::dumpOBJ()
{
	// naively write a plaintext OBJ of the loaded model

	CStudioHdr *hdr = g_pStudioModel->GetStudioHdr();
	if ( !hdr )
		return;

	char *ptr = (char *)mxGetSaveFileName( this, "", "*.obj" );
	if ( !ptr )
		return;
	...
	int nFaceIndex = 1; // obj face indices are 1-based
	...
	for ( batchID = 0; batchID < tris.m_MaterialBatches.Count(); batchID++ )
	{
		...
		for ( int nVert = 0; nVert < materialBatch.m_Verts.Count(); nVert++ )
		{
			GetTriangles_Vertex_t &vert = materialBatch.m_Verts[nVert];
			const Vector &pos = vert.m_Position;
			Vector skinnedPos( 0.0f, 0.0f, 0.0f );
		...
		{
			GetTriangles_Vertex_t &vert = materialBatch.m_Verts[nVert];
			sprintf( tmp, "vt %.5f %.5f 0.0\n",
				vert.m_TexCoord.x,
				1.0f - vert.m_TexCoord.y );
			out.WriteText( tmp );
		}

		char szMaterial[128];
		V_FileBase( materialBatch.m_pMaterial->GetName(), szMaterial, sizeof( szMaterial ) );
		...
		for ( int nTri = 0; nTri < materialBatch.m_TriListIndices.Count(); nTri += 3 )
		{
			sprintf( tmp, "f %i/%i/%i %i/%i/%i %i/%i/%i\n",
				materialBatch.m_TriListIndices[nTri + 0] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 0] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 0] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 1] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 1] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 1] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 2] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 2] + nFaceIndex,
				materialBatch.m_TriListIndices[nTri + 2] + nFaceIndex
			);
			out.WriteText( tmp );
		...
		nFaceIndex += materialBatch.m_Verts.Count();
	}
...
{
	g_pStudioModel->m_origin = vec3_origin;
	QAngle angSaved = g_pStudioModel->m_angles;
	g_pStudioModel->m_angles = vec3_angle;

	Vector vecMin, vecMax;
	g_pStudioModel->ExtractVertExtents( vecMin, vecMax );
	VectorITransform( vecMin, g_viewtransform, vecMin );
	VectorITransform( vecMax, g_viewtransform, vecMax );
	AABB_t vertBounds = AABB_t( vecMin, vecMax );

	g_SelectedBonePos = vertBounds.GetCenter();
	
	g_pStudioModel->m_origin = vec3_origin;
	g_pStudioModel->m_angles = angSaved;

	AngleMatrix( g_pStudioModel->m_angles, g_viewtransform );
	PositionMatrix( -g_pStudioModel->m_origin, g_viewtransform );

	g_pStudioModel->ExtractVertExtents( vecMin, vecMax );
	vertBounds = AABB_t( vecMin, vecMax );

	g_pStudioModel->m_origin = vertBounds.GetCenter();
	g_pStudioModel->m_origin.x += vertBounds.LengthOfLargestDimension() * 1.2f;
