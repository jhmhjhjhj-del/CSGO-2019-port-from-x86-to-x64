...
// Draws the skeleton
//-----------------------------------------------------------------------------
QAngle g_SelectedBoneAngle = QAngle( 0, 0, 0 );
Vector g_SelectedBonePos = Vector( 0, 0, 0 );
...
void StudioModel::DrawBones( )
{
	...
	for (int i = 0; i < pStudioHdr->numbones(); i++)
	{
		...
		if ( pbones[i].parent >= 0 )
		{
			...
			{
				if (j == g_viewerSettings.highlightBone)
				{
					meshBuilder.Color3ub( 255, 255, 0 );
				}
				else
				{
					meshBuilder.Color3ub( 100, 100, 100 );
				}
			}
		...
		if (g_viewerSettings.highlightBone >= 0)
		{
			...
			// remember selected bone angle for display in UI
			matrix3x4_t matLocalBone;
			ConcatTransforms( g_viewtransform.InverseTR(), m_pBoneToWorld[i], matLocalBone );
...
static Vector4D hullcolor[] =
{
	Vector4D( 1.0, 1.0, 1.0, 1.0 ),
	Vector4D( 1.0, 0.5, 0.5, 1.0 ),
	Vector4D( 0.5, 1.0, 0.5, 1.0 ),
	Vector4D( 1.0, 1.0, 0.5, 1.0 ),
	Vector4D( 0.5, 0.5, 1.0, 1.0 ),
	Vector4D( 1.0, 0.5, 1.0, 1.0 ),
	Vector4D( 0.5, 1.0, 1.0, 1.0 ),
	Vector4D( 0.5, 0.75, 1.0, 1.0 ),
	Vector4D( 1.0, 0.7, 0.5, 1.0 ),
};
...
void StudioModel::DrawHitboxes( )
{
	...
	if (g_viewerSettings.showHitBoxes || (g_viewerSettings.highlightHitbox >= 0))
	{
		...
		for (unsigned short j = list.Head(); j != list.InvalidIndex(); j = list.Next(j) )
		{
			...
			int c = pBBox->group % V_ARRAYSIZE( hullcolor );
			...
			else if ( (g_viewerSettings.highlightHitbox >= 0) && (g_viewerSettings.highlightHitbox == j) )
			{
				...
				if ( pBBox->flCapsuleRadius > 0 )
				{
					drawCapsule( bbMinPreview, bbMaxPreview, pBBox->flCapsuleRadius, hitboxMatrixPlusWidgetRotation, interiorcolor, hullcolor[c].Base() );
				}
				else
				{
					drawTransparentBox( bbMinPreview, bbMaxPreview, hitboxMatrixPlusWidgetRotation, interiorcolor, hullcolor[ c ].Base() );
				}
			...
			else
			{
				if ( pBBox->flCapsuleRadius > 0 )
				{
					drawCapsule( pBBox->bbmin, pBBox->bbmax, pBBox->flCapsuleRadius, hitboxMatrix, interiorcolor, hullcolor[ c ].Base() );
				}
				else
				{
					drawTransparentBox( pBBox->bbmin, pBBox->bbmax, hitboxMatrix, interiorcolor, hullcolor[ c ].Base() );
				}
			}
...
void StudioModel::DrawPhysicsModel( )
{
	...
	else
	{
		for (int i = 0; i < m_pPhysics->Count(); i++)
		{
			...
			{
				float interior[] = { 0.0, 1.0, 1.0, 0.4 };
				if ( g_pAlpha )
					DrawPhysmesh( pmesh, -1, g_pAlpha, interior );
				DrawPhysmesh( pmesh, -1, g_materialBones, interior );
			}
		}
	}
}
...
int StudioModel::DrawModel( bool mergeBones, int nRenderPassMode )
{
	...
	if ( g_viewerSettings.renderMode == RM_TEXCOORDS ) // yes, the model is rendering normally under the uv chart
	{
		const char *pMatName = "";
