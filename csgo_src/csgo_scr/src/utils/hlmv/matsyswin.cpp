...
IMaterial *g_materialAccumulateOverdraw = NULL;
IMaterial *g_materialDrawUVOverlap = NULL;
...
//	config.m_VideoMode.m_Width = config.m_VideoMode.m_Height = 0;
	...
	}
	{
		...
		pVMTKeyValues->SetInt( "$ignorez", 1 );
		pVMTKeyValues->SetInt( "$nocull", 1 );
		pVMTKeyValues->SetInt( "$color", 0 ); // black
		pVMTKeyValues->SetInt( "$additive", 1 ); // and additive, so it doesn't actually change the dest pixel
		g_materialAccumulateOverdraw = g_pMaterialSystem->CreateMaterial( "debug/accumoverdraw", pVMTKeyValues );
	}
	{
		KeyValues *pVMTKeyValues = new KeyValues( "UnlitGeneric" );
		pVMTKeyValues->SetInt( "$vertexcolor", 1 );
		...
		g_materialDrawUVOverlap = g_pMaterialSystem->CreateMaterial( "debug/drawuvoverlap", pVMTKeyValues );
	}
...
MatSysWindow::handleEvent (mxEvent *event)
{
	...
	case mxEvent::MouseDrag:
	{
		if (event->buttons & mxEvent::MouseLeftButton)
		{
			...
			{
				Vector vecOffset = g_SelectedBonePos;
				Vector vecDelta;
				...
				if (!g_viewerSettings.rotating)
				{
					...
					movement = QAngle( 0, rx, 0 );
					AngleMatrix( g_pStudioModel->m_angles, tmp1 );
					vecDelta = VectorTransform( vecOffset, tmp1 );
					AngleMatrix( movement, tmp2 );
					ConcatTransforms( tmp1, tmp2, tmp3 );
					MatrixAngles( tmp3, g_pStudioModel->m_angles );
					...
					AngleMatrix( g_pStudioModel->m_angles, tmp1 );
					vecDelta -= VectorTransform( vecOffset, tmp1 );
					g_pStudioModel->m_origin -= vecDelta;
				}
				else
				{
...
void MatSysWindow::dumpViewportWithLabel(const char *filename, const char *label)
{
	...
	if ( hImage != NULL) {
		...
		if ((pDataBits = new BYTE [uiBytesPerAllRows]) != NULL)
		{
			...
			HDC hDC_img = GetDC ((HWND) getHandle ());
			...
			GetDIBits ( hDC_img, hImage, 0, client.bottom, pDataBits, (BITMAPINFO*) &bmi, DIB_RGB_COLORS);

			ReleaseDC ((HWND) getHandle (), hDC_img );
