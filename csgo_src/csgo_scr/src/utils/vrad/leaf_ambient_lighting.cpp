...
void CompressAmbientSampleList( CUtlVector<ambientsample_t> &list, int nThreshold )
{
	Vector testCube[6];
	for ( int i = 0; i < list.Count(); i++ )
	{
		if ( list.Count() > 1 )
		{
			...
			if ( CubeDeltaGammaSpace(testCube, list[i].cube) < nThreshold )
			{
				list.FastRemove(i);
				i--;
			}
...
void ComputeAmbientForLeaf( int iThread, int leafID, CUtlVector<ambientsample_t> &list )
{
	...
	if ( g_bFastAmbient || g_bAmbientFromLeafCenters )
	{
	...
	for ( int i = 0; i < sampleCount; i++ )
	{
		...
		if ( g_bAmbientFromLeafCenters )
		{
			samplePosition = (Vector( dleafs[leafID].mins[0], dleafs[leafID].mins[1], dleafs[leafID].mins[2] ) + Vector( dleafs[leafID].maxs[0], dleafs[leafID].maxs[1], dleafs[leafID].maxs[2] )) * 0.5f;
		}
		else
		{
	...
	int nThreshold = 3;
	CompressAmbientSampleList( list, nThreshold );

	// cull more aggressively to try and hit the desired reduction
	if ( list.Count() > 1 && g_flLeafAmbientSampleReduction < 1.0f )
	{
		int nListCountTarget = MAX( 1, RoundFloatToInt( (float)list.Count() * g_flLeafAmbientSampleReduction ) );

		while ( list.Count() > nListCountTarget && nThreshold < 48 )
		{
			nThreshold++;
			CompressAmbientSampleList( list, nThreshold );
		}
