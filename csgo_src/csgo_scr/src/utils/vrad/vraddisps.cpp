...
void CVRadDispMgr::UnserializeDisps( void ) 
{
	...
	// FIXME: These params are derived at level load time by querying the displacement material.
	// The surface prop alpha feature is not enabled at this point, but that means that VRAD
	// will erroneously use the pre-surfacepropalpha-modified surface property for any calculations,
	// unless that same material query is performed here.
	FDispCollTreeParams dispTreeParams;
	dispTreeParams.bSurfacePropAlphaSpecified = false;
	dispTreeParams.fSurfacePropAlphaFilter = 0.0f;

	for( int iDisp = 0; iDisp < g_dispinfo.Count(); iDisp++ )
	{
		pDispTrees[iDisp].Create( builderDisps[iDisp], dispTreeParams );
