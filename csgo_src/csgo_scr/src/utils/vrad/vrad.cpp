...
bool		g_bAmbientFromLeafCenters = false;
...
float g_flLeafAmbientSampleReduction = 1.0f;
...
int ParseCommandLine( int argc, char **argv, bool *onlydetail )
{
	*onlydetail = false;
	...
	for( i=1 ; i<argc ; i++ )
	{
		...
		else if ( !Q_stricmp( argv[ i ], "-StaticPropLightingOld" ) ) // same behavior as -StaticPropLighting before baked direct lighting was fixed (removed), temporary so mappers can avoid re-tweaking lighting for props with fixed lighting
		{
			g_bStaticPropLighting = true;
			extern int g_numVradStaticPropsLightingStreams;
			g_numVradStaticPropsLightingStreams = 3;
			extern bool g_bVradStaticPropsLightingOld;
			g_bVradStaticPropsLightingOld = true;
		}
		...
		else if ( !Q_stricmp( argv[i], "-unlitdetail" ) )
		{
			g_bUnlitDetail = true;
		}
		...
		else if ( !Q_stricmp( argv[i], "-LeafAmbientSampleReduction" ) )
		{
			if ( ++i < argc && *argv[i] )
			{
				g_flLeafAmbientSampleReduction = atof( argv[i] );
			}
			else
			{
				Warning( "Error: expected a scale factor after '-LeafAmbientSampleReduction'\n" );
				return 1;
			}
		}
		...
		else if ( !Q_stricmp( argv[i], "-ambientfromleafcenters" ) )
		{
			g_bAmbientFromLeafCenters = true;
		}
