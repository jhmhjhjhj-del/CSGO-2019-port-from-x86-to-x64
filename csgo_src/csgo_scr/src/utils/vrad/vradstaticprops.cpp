...
bool g_bVradStaticPropsLightingOld = false;
...
void CVradStaticPropMgr::UnserializeModels( CUtlBuffer& buf )
{
	...
	for ( int i = 0; i < count; ++i )				  
	{
		...
		m_StaticProps[i].m_flScale = lump.m_flScale;
...
//-----------------------------------------------------------------------------
void ComputeDirectLightingAtPoint( Vector &position, Vector *normals, Vector *outColors, float *outSunAmount, int numNormals, bool bSkipDynamicLights, int iThread,
...
//			flEpsilon = 1.0;
		...
		for ( int k = 0; k < numNormals; ++k )
		{
			// now that we don't accumulate direct lighting for static props (as of CL#3838343)
			// We must skip ALL lights that will be handled as dynamic lights at runtime here.
			// emit_skyambient fpr example should not be skipped here since it won't be handled as a dynamic light at runtime

			bool bSkipBakingDirectLight;

			if ( g_bVradStaticPropsLightingOld ) // temp measure to allow support of baking some direct lighting before this bug was fixed (we no longer support baking of direct illumination on static props)
			{
				bSkipBakingDirectLight = ( dl->light.type == emit_skylight ) && bSkipDynamicLights;
			}
			else
			{
				bSkipBakingDirectLight = ( ( dl->light.type == emit_skylight ) ||
									       ( dl->light.type == emit_point ) ||
									       ( dl->light.type == emit_spotlight ) ) && bSkipDynamicLights;
			}

			if ( !bSkipBakingDirectLight )
			{
		...
		for ( int modelID = 0; modelID < pBodyPart->nummodels; ++modelID )
		{
			...
			for ( int meshID = 0; meshID < pStudioModel->nummeshes; ++meshID )
			{
				...
				for ( int vertexID = 0; vertexID < pStudioMesh->numvertices; ++vertexID )
				{
					...
					matrix.ScaleUpper3x3Matrix( prop.m_flScale );
	...
	for ( int nProp = 0; nProp < count; ++nProp )
	{
		...
		if ( !g_bStaticPropPolys )
		{
			if ( dict.m_pModel )
			{
				...
				xform.As3x4().ScaleUpper3x3Matrix( prop.m_flScale );
...
void CVradStaticPropMgr::BuildTriList( CStaticProp &prop )
{
	...
	matrix3x4_t	matrixNormals;
	AngleMatrix( prop.m_Angles, matrixNormals );
				...
				for ( int nGroup = 0; nGroup < pVtxMesh->numStripGroups; ++nGroup )
				{
					...
					for ( nStrip = 0; nStrip < pStripGroup->numStrips; nStrip++ )
					{
						...
						for ( int i = 0; i < pStrip->numIndices; i += 3 )
						{
							...
							Vector normal2;
							Vector normal3;
							VectorTransform( *vertData->Normal( vertex1 ), matrixNormals, normal1 );
							VectorTransform( *vertData->Normal( vertex2 ), matrixNormals, normal2 );
							VectorTransform( *vertData->Normal( vertex3 ), matrixNormals, normal3 );
