...
//-----------------------------------------------------------------------------
// Purpose: IZipWriteStream Wrapper for filesystem I/O methods
//-----------------------------------------------------------------------------
class CFilesystemZipStream : public IZipWriteStream
{
public:

	CFilesystemZipStream( FileHandle_t filehandle ) : IZipWriteStream(), m_filehandle( filehandle )
	{
		m_nStartingOffset = g_pFileSystem->Tell( m_filehandle );
	}

	// Implementing IZipWriteStream method
	virtual void Put( const void* pMem, int size )
	{ 
		if ( g_pFileSystem->Write( pMem, size, m_filehandle ) != size )
		{
	...
	// Implementing IZipWriteStream method
	virtual unsigned int Tell( void ) { return ( g_pFileSystem->Tell( m_filehandle ) - m_nStartingOffset ); }

private:

	FileHandle_t m_filehandle;
	unsigned int m_nStartingOffset;
};
...
static void WritePakFileLump( void )
{
	...
	// Set up the pakfile lump dictionary entry
	g_Lumps.size[LUMP_PAKFILE] = 0;	// mark it written

	lump_t* lump = &g_pBSPHeader->lumps[LUMP_PAKFILE];

	// pre-align to desired write position
	// this may be increased alignment as needed by lumps
	AlignFilePosition( g_hBSPFile, 1 );
	lump->fileofs = g_pFileSystem->Tell( g_hBSPFile );

	// Writes zip file to disk
	CFilesystemZipStream stream( g_hBSPFile );
	GetPakFile()->ActivateByteSwapping( IsX360() );
	GetPakFile()->SaveToCustomStream( stream );

	lump->filelen = g_pFileSystem->Tell( g_hBSPFile ) - lump->fileofs;
	lump->version = 0;
...
void CGameLump::ParseGameLump( BSPHeader_t* pHeader )
{
	...
	if (length > 0)
	{
		...
		for (int i = 0; i < pGameLumpHeader->lumpCount; ++i )
		{
			...
			int nLumpLength = pGameLump[i].filelen;
			GameLumpHandle_t lump = g_GameLumps.CreateGameLump( pGameLump[i].id, nLumpLength, pGameLump[i].flags, pGameLump[i].version );
			if ( g_bSwapOnLoad )
			{
				SwapGameLump( pGameLump[i].id, pGameLump[i].version, (byte*)g_GameLumps.GetGameLump(lump), (byte *)pHeader + pGameLump[i].fileofs, nLumpLength );
			}
			else
			{
				memcpy( g_GameLumps.GetGameLump(lump), (byte *)pHeader + pGameLump[i].fileofs, nLumpLength );
			}
		}
	}
}
	...
	for ( int i = 0; i < HEADER_LUMPS; ++i )
	{
		...
		else
		{
			...
			*((unsigned int *)pOutBSPHeader->lumps[lumpNum].fourCC) = 0;
			...
			CUtlBuffer lumpBuffer;
			lumpBuffer.SetExternalBuffer( ((byte *)pInBSPHeader) + pSortedLump->pLump->fileofs, pSortedLump->pLump->filelen, pSortedLump->pLump->filelen );
			...
			else if ( lumpNum == LUMP_PAKFILE )
			{
				...
				outputBuffer.Put( lumpBuffer.Base(), lumpBuffer.TellPut() );
			}
			else
			{
				CUtlBuffer compressedBuffer;
				bool bCompressed = pCompressFunc( lumpBuffer, compressedBuffer );
				if ( bCompressed )
				{
					// placing the uncompressed size in the unused fourCC, will decode at runtime
					*((unsigned int *)pOutBSPHeader->lumps[lumpNum].fourCC) = BigLong( lumpBuffer.TellPut() );
...
bool GetBSPDependants( const char *pBSPFilename, CUtlVector< CUtlString > *pList )
{
	...
	if ( g_PhysLevelSize && pPhysLevelData && pPhysLevelData->dataVersion == dphyslevelV0_t::DATA_VERSION )
	{
		...
		for( int i = 0; i < pPhysLevelData->levelStaticModels.size; ++i )
		{
			...
			V_strncpy( szPhzFilename, pPhz, sizeof( szPhzFilename ) );		
			V_SetExtension( szPhzFilename, ".phz", sizeof( szPhzFilename ) );

			// add with prefix exclude token
			pList->AddToTail( CFmtStr( "*%s", szPhzFilename ).Access() );
		}
	}
