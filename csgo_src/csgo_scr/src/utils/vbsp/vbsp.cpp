...
bool		g_bColorizeCombinedProps = false;
bool		g_bPrintAutoCombineRules = false;
...
int RunVBSP( int argc, char **argv )
{
	...
	for (i=1 ; i<argc ; i++)
	{
		...
		else if ( !Q_stricmp( argv[ i ], "-staticpropcombine_colorinstances" ) )
		{
			g_bColorizeCombinedProps = true;
		}
		else if ( !Q_stricmp( argv[ i ], "-staticpropcombine_printautocombinerules" ) )
		{
			g_bPrintAutoCombineRules = true;
		}
