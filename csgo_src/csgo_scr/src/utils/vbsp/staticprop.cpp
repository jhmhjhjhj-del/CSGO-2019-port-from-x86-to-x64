...
static CUniformRandomStream s_ClusterRandomStream;
...
struct StaticPropBuild_t
{
	...
	CCopyableUtlVector<unsigned short> m_leafList;
};
...
{
	CUtlSymbol m_Name;
	float m_flScale; // part of the key for the cache entry
...
//-----------------------------------------------------------------------------
static CPhysConvex* ComputeConvexHull( studiohdr_t* pStudioHdr, mstudiomesh_t* pMesh, float flScale = 1.0f )
{
...
//-----------------------------------------------------------------------------
CPhysCollide* ComputeConvexHull( studiohdr_t* pStudioHdr, float flScale = 1.0f )
{
...
//-----------------------------------------------------------------------------
static CPhysCollide* GetStaticPropCollisionModel( char const* pModelName, float flScale = 1.0f )
{
...
static void AddStaticPropToLump( StaticPropBuild_t const& build )
{
	...
	CPhysCollide* pConvexHull = GetStaticPropCollisionModel( build.m_pModelName, build.m_flTransformScale );
	if (!pConvexHull)
		return;

	// Compute the leaves the static prop's convex hull hits
	CUtlVector< unsigned short > leafList;
	if ( !build.m_bHasLeafList )
	{
		ComputeStaticPropLeaves( pConvexHull, build.m_Origin, build.m_Angles, leafList );
	}
	else
	{
		leafList.AddVectorToTail( build.m_leafList );
	}
	...
	propLump.m_flScale = build.m_flTransformScale;
...
bool StaticPropsAreGroupPeers( StaticPropBuild_t* prop1, StaticPropBuild_t* prop2, int nCombineRule )
{
	...
	CUtlString prop1SurfaceProp;
	CUtlString prop2SurfaceProp;
	...
	FOR_EACH_VEC( g_vecCombineRules[nCombineRule].m_vecCombinePeers, j )
	{
		...
		{
			bProp1Present = true;
			prop1SurfaceProp = pPeer->m_surfaceProp;
		...
		{
			bProp2Present = true;
			prop2SurfaceProp = pPeer->m_surfaceProp;
		...
		if ( bProp1Present && bProp2Present && prop1SurfaceProp == prop2SurfaceProp )	// Also make sure surface props match
			return true;
	}
...
void CombineStaticProps( propBuildVector &vecGroup, int &nCombineIndex )
{
	...
	{
		// Build union leaf list for each prop. This should be smaller than the leaf list created from the
		// convex hull of the combined prop.
		CUtlVector< unsigned short > combinedLeafList;
		CUtlVector< unsigned short > leafList;
		for ( int i = 0; i < vecGroup.Count(); i++ )
		{
			// Get the collision model
			CPhysCollide* pConvexHull = GetStaticPropCollisionModel( vecGroup[ i ].m_pModelName, vecGroup[ i ] .m_flTransformScale );
			if ( !pConvexHull )
			{
				continue;
			}

			// Compute the leaves the static prop's convex hull hits. Quadratic complexity, but the problem is small enough for this not to be a perf issue.
			leafList.RemoveAll();
			ComputeStaticPropLeaves( pConvexHull, vecGroup[ i ].m_Origin, vecGroup[ i ].m_Angles, leafList );
			for ( int j = 0; j < leafList.Count(); j++ )
			{
				if ( combinedLeafList.Find( leafList[ j ] ) < 0 )
				{
					combinedLeafList.AddToTail( leafList[ j ] );
				}
			}
		}
		vecGroup[ 0 ].m_leafList.AddVectorToTail( combinedLeafList );
		vecGroup[ 0 ].m_bHasLeafList = true;
	}
	...
	if ( g_pFullFileSystem->WriteFile( szGeneratedQCpath, "CONTENT", bufQC ) )
	{
		...
		if ( CompileQC( szTempAbsPath ) )
		{
			...
			if ( !bBSPZipSuccess )
			{
				Warning( "Failed to bspzip autogenerated model into the map: %s!\n", szModelNameModelPrefix );
			}
...
{
	// Get the collision model
	CPhysCollide* pConvexHull = GetStaticPropCollisionModel( entry1->m_pModelName, entry1->m_flTransformScale );
	if ( !pConvexHull )
		return false;
...
bool StaticPropLocationsMatch( const StaticPropBuild_t* entry1, const StaticPropBuild_t* entry2, int nCombineRule )
{
	if ( staticpropcombine_considervis )
	{
		...
		pConvexHull = GetStaticPropCollisionModel( entry2->m_pModelName, entry2->m_flTransformScale );
		if ( !pConvexHull )
			return false;
	...
	{
		// Just let them merge together no matter the distance. The clustering will sort out distance limits later.		
		return true;
	}
}
...
bool GroupProp( propBuildVector &vecSearch, propBuildVector &vecMoveTo )
{
	...
	if ( !vecMoveTo[ 0 ].m_bAllowCombine )
	{
		return false;	// Prevent combining.
	}
	if ( nCombineRule < 0 )
	{
	...
	if ( vecMoveTo.Count() > g_vecCombineRules[nCombineRule].m_nClusterLimit )
	{
		...
		// Ignore limits here. The k-means clustering code will sort it out.
	}
	...
	{
		if ( vecSearch[i].m_bAllowCombine == true &&	// candidate must be allowed to combine
			PropDiffuseModulationsMatch( &vecSearch[i], &vecMoveTo[0] ) && // candidate must have the same diffuse modulation
...
void UpdateCentroids( CUtlVector< int > &clusterMembership, const propBuildVector &vecGroup, int nK,
					  CUtlVector< ClusterInfo_t > &newClusterCentroidsOut )
{
	for ( int i = 0; i < nK; i++ )
	{
		newClusterCentroidsOut[ i ].vCentroid = vec3_origin;
		newClusterCentroidsOut[ i ].nMemberCount = 0;
		newClusterCentroidsOut[ i ].flRadius = 0.0f;
	}

	for ( int i = 0; i < clusterMembership.Count(); i++ )
	{
		int nCluster = clusterMembership[ i ];
		newClusterCentroidsOut[ nCluster ].vCentroid += vecGroup[ i ].m_Origin;
		newClusterCentroidsOut[ nCluster ].nMemberCount++;
	}

	for ( int i = 0; i < nK; i++ )
	{
		newClusterCentroidsOut[ i ].vCentroid /= Max( float( newClusterCentroidsOut[ i ].nMemberCount ), 1.0f );
	}

	for ( int i = 0; i < clusterMembership.Count(); i++ )
	{
		int nCluster = clusterMembership[ i ];
		float flDist = newClusterCentroidsOut[ nCluster ].vCentroid.DistTo( vecGroup[ i ].m_Origin );
		newClusterCentroidsOut[ nCluster ].flRadius = Max( flDist, newClusterCentroidsOut[ nCluster ].flRadius );
	}
}

void Classify( CUtlVector< int > &clusterMembership, const propBuildVector & vecGroup, const CUtlVector< ClusterInfo_t > &clusterCentroids )
{
	const int nK = clusterCentroids.Count();
	for ( int i = 0; i < clusterMembership.Count(); i++ )
	{
		int nCluster = -1;
		...
		for ( int j = 0; j < nK; j++ )
		{
			float flDist = clusterCentroids[ j ].vCentroid.DistToSqr( vecGroup[ i ].m_Origin );
			if ( flDist < flMinDist )
			{
				flMinDist = flDist;
				nCluster = j;
		...
		clusterMembership[ i ] = nCluster;
	}
}

int ComputeClustersFromGroup( propBuildVector &vecGroup, propBuildVector **pOutClusters )
{
	s_ClusterRandomStream.SetSeed( 0x32424242 );	// Totally arbitrary seed

	bool bRetry = true;
	int nK = 1;
	CUtlVector< ClusterInfo_t > clusterCentroids;
	CUtlVector< int > clusterMembership;
	clusterCentroids.SetCount( nK );
	clusterMembership.SetCount( vecGroup.Count() );
	for ( int i = 0; i < clusterMembership.Count(); i++ )
	{
		clusterMembership[ i ] = s_ClusterRandomStream.RandomInt( 0, nK - 1 );
	}
	...
	while ( bRetry )
	{
		...
		UpdateCentroids( clusterMembership, vecGroup, nK, clusterCentroids );

		// cluster + refine
		int nIter = 0;
		CUtlVector< ClusterInfo_t > newClusterCentroids;
		for ( nIter = 0; nIter < 1000; nIter++ )
		{
			CUtlVector<int> oldClusterMembership;
			oldClusterMembership.AddVectorToTail( clusterMembership );
			Classify( clusterMembership, vecGroup, clusterCentroids );

			if ( V_memcmp( clusterMembership.Base(), oldClusterMembership.Base(), sizeof( int ) * oldClusterMembership.Count() ) == 0 )
			{
				// Found a stable clustering.
				break;
			}
...
#ifdef _DEBUG
		Msg( "Found %d-clustering for %d props after %d iterations. Cluster sizes: (", nK, vecGroup.Count(), nIter + 1 );
		for ( int i = 0; i < nK; i++ )
		{
			Msg( "[%d,%d] ", clusterCentroids[ i ].nMemberCount, int( clusterCentroids[ i ].flRadius + 0.5f ) );
		}
		Msg( ")\n" );
#endif

		int nCombineRule = GetCombineRuleForProp( vecGroup[ 0 ].m_pModelName );

		// Check for valid max number of instances per cluster
		int nOldK = nK;
		for ( int i = 0; i < nOldK; i++ )
		{
			if ( clusterCentroids[ i ].nMemberCount > g_vecCombineRules[ nCombineRule ].m_nClusterLimit )
			{
				//Msg( "Subdividing due to large cluster count (%d)\n", clusterCentroids[ i ].nMemberCount );
				clusterCentroids.AddToTail();
				// randomly assign members of the subdivided cluster to the new one.
				for ( int j = 0; j < clusterMembership.Count(); j++ )
				{
					if ( clusterMembership[ j ] == i && s_ClusterRandomStream.RandomInt( 0, 1 ) )
					{
						clusterMembership[ j ] = nK;
					}
				}
				nK++;
				bRetry = true;
		...
		if ( !bRetry )
		{
			// We didn't already subdivide due to cluster counts, check cluster sizes too

			// Check for valid max cluster size
			const float flMaxSize = g_vecCombineRules[ nCombineRule ].m_flDistanceLimit * g_vecCombineRules[ nCombineRule ].m_flDistanceLimit;
			for ( int i = 0; i < nOldK; i++ )
			{
				for ( int j = 0; j < clusterMembership.Count(); j++ )
				{
					if ( clusterMembership[ j ] != i )
						continue;

					float flDist = vecGroup[ j ].m_Origin.DistToSqr( clusterCentroids[ i ].vCentroid );
					if ( flDist > flMaxSize )
					{
						//Msg( "Subdividing due to large cluster bounds (%.2f)\n", sqrtf( flDist ) );
						clusterCentroids.AddToTail();
						// randomly assign members of the subdivided cluster to the new one.
						for ( int k = 0; k < clusterMembership.Count(); k++ )
						{
							if ( clusterMembership[ k ] == i && s_ClusterRandomStream.RandomInt( 0, 1 ) )
							{
								clusterMembership[ k ] = nK;
							}
						}
						nK++;
	...
	// Create clusters
	*pOutClusters = new propBuildVector[ nK ];
	for ( int j = 0; j < clusterMembership.Count(); j++ )
	{
		int nCluster = clusterMembership[ j ];
		(*pOutClusters)[ nCluster ].AddToTail( vecGroup[ j ] );
	}
...
void AutoCreatePropCombineRules( propBuildVector& vecBuilds )
{
	...
	if ( staticpropcombine_suggestcombinerules )
	{
		...
		for ( const MaterialUsageInfo_t &e : sortedMaterials )
		{
			if ( e.nMaterialCount == 1 || e.nInstanceCount < 5 )
			{
				continue;
			}
			Msg( "%4d %s\n", e.nInstanceCount, e.matName.Get() );
...
void EmitStaticProps()
{
	...
	for ( i = 0; i < num_entities; ++i)
	{
		...
		if (!strcmp(pEntity, "static_prop") || !strcmp(pEntity, "prop_static"))
		{
			...
			build.m_bHasLeafList = false;

			build.m_flTransformScale = FloatForKey( &entities[i], "uniformscale" );
			if ( build.m_flTransformScale <= 0 )
				build.m_flTransformScale = 1.0f;
			...
			build.m_bAllowCombine = true;
			if ( IntForKey( &entities[ i ], "preventpropcombine" ) )
			{
				build.m_bAllowCombine = false;
			}
	...
	if ( bDoStaticPropCombine )
	{
		...
		{
			propBuildVector vecGroupRaw;
			while ( GroupProp( vecBuilds, vecGroupRaw ) ) {}
			// vecGroup now contains all compatible props of a type, ignoring cluster limits and distance limits
			// Now run clustering on this to see if we can get reasonable clusters
			propBuildVector *pClusters = nullptr;
			int nClusterCount = 0;
			if ( vecGroupRaw.Count() > 1 )
			{
				nClusterCount = ComputeClustersFromGroup( vecGroupRaw, &pClusters );
			}
			else
			{
				nClusterCount = 1;
				pClusters = new propBuildVector[ 1 ];
				pClusters[ 0 ].AddVectorToTail( vecGroupRaw );
			}

			for ( int j = 0; j < nClusterCount; j++ )
			{
				propBuildVector &vecGroup = pClusters[ j ];
...
#undef HelperSetFlags_IF_ANY
						...
						if ( g_bColorizeCombinedProps )
						{
							float t = RandomFloat();
							Vector randCol;
							randCol.x = cos( 2.0f * M_PI_F * t );
							randCol.y = cos( 2.0f * M_PI_F * ( t + 0.33f ) );
							randCol.z = cos( 2.0f * M_PI_F * ( t + 0.67f ) );
							randCol = Vector( 0.5f, 0.5f, 0.5f ) + 0.5f * randCol;
							vecGroup[ 0 ].m_DiffuseModulation.r = byte( Clamp( randCol.x, 0.0f, 1.0f ) * 255.0f );
							vecGroup[ 0 ].m_DiffuseModulation.g = byte( Clamp( randCol.y, 0.0f, 1.0f ) * 255.0f );
							vecGroup[ 0 ].m_DiffuseModulation.b = byte( Clamp( randCol.z, 0.0f, 1.0f ) * 255.0f );
						}
					...
					{
						for ( int k = 0; k < vecGroup.Count(); k++ )
						{
							vecCombinedStaticProps.AddToTail( vecGroup[ k ] );
						}
					}
				}
				else
		...
		FOR_EACH_VEC( g_vecCombineRules, k )
		{
			if ( g_vecCombineRules[k].m_vecStatsMemberCounts.Count() )
			{
				int nSum = 0;
				FOR_EACH_VEC( g_vecCombineRules[k].m_vecStatsMemberCounts, j )
				{
					nSum += g_vecCombineRules[k].m_vecStatsMemberCounts[j];
				}
				float flAvg = ((float)(nSum)) / ((float)(g_vecCombineRules[k].m_vecStatsMemberCounts.Count()));

				Msg( "%d clusters of group \"%s\",\taverage %.1f models\n",
					g_vecCombineRules[k].m_vecStatsMemberCounts.Count(),
					g_vecCombineRules[k].m_szGroupName.Get(),
					flAvg );
			}
		...
		FOR_EACH_VEC( vecCombinedStaticProps, m )
		{
			AddStaticPropToLump( vecCombinedStaticProps[m] );
		}
	}
