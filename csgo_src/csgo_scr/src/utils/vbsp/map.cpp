...
void CMapFile::MergeBrushSides( entity_t *pInstanceEntity, CMapFile *Instance, Vector &InstanceOrigin, QAngle &InstanceAngle, matrix3x4_t &InstanceMatrix )
{
	...
	{
		if ( nummapbrushsides + i >= MAX_MAP_BRUSHSIDES )
			Error( "MAX_MAP_BRUSHSIDES" );
		...
		if ( side->pMapDisp )
		{
			...
			// need to rotate these, too
			int size = (((1 << disp->power) + 1) * ((1 << disp->power) + 1));
			for ( int j = 0; j < size; j++ )
			{
				Vector vecTemp = disp->vectorDisps[j];
				VectorRotate( vecTemp, InstanceAngle, disp->vectorDisps[j] );
			}
...
ChunkFileResult_t CMapFile::LoadSideCallback(CChunkFile *pFile, LoadSide_t *pSideInfo)
{
	...
	if (eResult == ChunkFile_Ok)
	{
		...
		if (side->contents & (CONTENTS_PLAYERCLIP|CONTENTS_MONSTERCLIP|CONTENTS_GRENADECLIP|CONTENTS_DRONECLIP) )
		{
		...
		if (!(side->contents & (ALL_VISIBLE_CONTENTS | CONTENTS_PLAYERCLIP|CONTENTS_MONSTERCLIP|CONTENTS_BLOCKLOS|CONTENTS_OPAQUE|CONTENTS_GRENADECLIP|CONTENTS_DRONECLIP)  ) )
		{
