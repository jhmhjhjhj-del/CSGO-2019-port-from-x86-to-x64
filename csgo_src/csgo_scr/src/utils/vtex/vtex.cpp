...
static bool g_bPanorama = false; 

static int g_nSecondaryTargetStage = 0;
static int g_nSecondaryTargetReduceFactor = -1;
static char g_szSecondaryTargetPathReplacements[1024];
...
void VTexConfigInfo_t::ParseOptionKey( const char *pKeyName,  const char *pKeyValue )
{
	...
	else if ( !stricmp( pKeyName, "secondary_target_pathsubs" ) )
	{
		V_strcpy_safe( g_szSecondaryTargetPathReplacements, pKeyValue );
	}
	else if ( !stricmp( pKeyName, "secondary_target_reduce" ) )
	{
		g_nSecondaryTargetReduceFactor = atoi( pKeyValue );
	}
	else
	{
...
static ImageFormat ComputeDesiredImageFormat( IVTFTexture *pTexture, VTexConfigInfo_t &info )
{
	...
	if ( g_bPanorama &&
		 ( ( pTexture->Format() == IMAGE_FORMAT_RGB888 ) || ( pTexture->Format() == IMAGE_FORMAT_BGR888 ) ) && 
		...
		(pTexture->Width() < 4) || (pTexture->Height() < 4) )
	{
		...
		else
		{
			if ( g_bPanorama )
			{
				// allows fast copy of texture data in the S1 Wrapper (BGRX8888 is the destination format for BGR888 anyway)
...
void MakeSrcFileName( VTexConfigInfo_t &info, const char *pFullNameWithoutExtension, int frameID, int faceID, int mipLevel, int z )
{
	...
	if( info.m_bNormalToDuDv )
	{
		...
		if( pNormalString )
		{
			...
			pNormalString = Q_stristr( normalTempBuf, "_dudv" );
								  ...
								  const VTexConfigInfo_t &info )
{
	...
	ImageFormat textureFormat = ( g_eMode == BITMAP_FILE_TYPE_PFM ) ? info.m_bDisplacementMap ? dMapFormat : IMAGE_FORMAT_RGB323232F : ( g_bPanorama ? sourceFormat : IMAGE_FORMAT_RGBA8888 ); 
	// Force mipcount to 1 if using -panorama (assuming we don't want them for UI/panorama) to work around num mipmap calculations therein. 
	if ( !pTexture->Init( nWidth, nHeight, nDepth, textureFormat, info.m_nFlags, nFrameCount, g_bPanorama ? 1 : -1 ) ) 
	{
...
//-----------------------------------------------------------------------------
bool ProcessFiles( const char *pFullNameWithoutExtension, const char *pOutputDir, const char *pBaseName, const char *pSuffix, VTexConfigInfo_t &info )
{
	...
	// For multi-res generation, avoid compressed formats where required
	if ( g_bPanorama &&
		 ( ( pVTFTexture->Width() % 4 ) || ( pVTFTexture->Height() % 4 ) ) )
	{
		info.m_vtfProcOptions.flags0 |= VtfProcessingOptions::OPT_NOCOMPRESS;
	}
	...
	bool bDXT5YCoCg = g_bPanorama && ( vtfImageFormat == IMAGE_FORMAT_DXT5 ) && ( ( pVTFTexture.Get()->Format() == IMAGE_FORMAT_RGB888 ) || ( pVTFTexture.Get()->Format() == IMAGE_FORMAT_BGR888 )) ;
	pVTFTexture->ConvertImageFormat( vtfImageFormat, info.m_bNormalToDuDv, info.m_bNormalToDXT5GA, bDXT5YCoCg );
...
static bool LoadConfigFile( const char *pFileBaseName, VTexConfigInfo_t &info )
{
	...
	if ( g_eMode == BITMAP_FILE_TYPE_PFM )
	{
		if ( g_bUsedAsLaunchableDLL && !( info.m_vtfProcOptions.flags0 & VtfProcessingOptions::OPT_NOCOMPRESS ) )
		{
			info.m_nFlags |= TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD;
		}
...
bool GetOutputDir( const char *inputName, char *outputDir )
{
	...
	if ( g_nSecondaryTargetReduceFactor != -1 )
	{
		if ( g_nSecondaryTargetStage == 1 ) // we've already saved the full-res original
		{
			if ( !g_Quiet )
				VTexMsg( "Patching secondary target path.\n" );

			CUtlVector<char *> vecSzTargetPathReplacementList;
			V_SplitString( g_szSecondaryTargetPathReplacements, " ", vecSzTargetPathReplacementList );
			
			if ( !vecSzTargetPathReplacementList.Count() || vecSzTargetPathReplacementList.Count() % 2 == 1 )
			{
				return false; // if we're not changing the path or we have an odd mapping of replacements
			}

			Q_FixSlashes( outputDir );

			// do replacements
			FOR_EACH_VEC( vecSzTargetPathReplacementList, n )
			{
				if ( n % 2 == 1 )
					continue; // by pairs

				char temp[1024];
				Q_StrSubst( outputDir, vecSzTargetPathReplacementList[n], vecSzTargetPathReplacementList[n+1], temp, sizeof( temp ) );
				strncpy( outputDir, temp, sizeof( temp ) );
			}
		}
	}
...
bool Process_File( char *pInputBaseName, int maxlen )
{
	...
	if ( g_nSecondaryTargetReduceFactor != -1 )
	{
		g_nSecondaryTargetStage++;

		ret = Process_File_Internal( pInputBaseName, maxlen, false, false );
		if ( ret == false )
			return ret;
	}
...
bool Process_File_Internal( char *pInputBaseName, int maxlen, bool bOutputPwlColorConversion, bool bConvertTo360PwlSrgb )
{
	...
	if ( strstr( outputDir, "panorama" ) || CommandLine()->FindParm( "-panorama" ) )
	{
		// use different dst format rules
		g_bPanorama = true;

		// if source file is .vtf then modify texture format to YCoCg if current format is RGB888

		char chFileNameConvert[512];
		sprintf( chFileNameConvert, "%s.vtf", pInputBaseName );

		FILE *fp = fopen( chFileNameConvert, "rb" );

		if ( fp )
		{
			fclose( fp );

			// vtf file, convert texture format to YCoCg if source format is RGB888
			VTexMsg( "Attempting YCoCg conversion of '%s'...\n", pInputBaseName );
			...
			int lenBaseName = strlen( pInputBaseName );
			char *pFileName = (char *)stackalloc( lenBaseName + strlen( ".txt" ) + 1 );
			strcpy( pFileName, pInputBaseName );
			memcpy( pFileName + lenBaseName, ".txt", 5 );
			bool bConfigExists = FileExistsAbsolute( pFileName );

			if ( bConfigExists )
			{
				VTexConfigInfo_t tmpInfo;
				LoadConfigFile( pInputBaseName, tmpInfo );
				tmpInfo.m_nFlags |= TEXTUREFLAGS_NOMIP;
				bNoCompress = (tmpInfo.m_vtfProcOptions.flags0 & VtfProcessingOptions::OPT_NOCOMPRESS) ? true : false;
			}
			...
			{
				VTexMsg( "Converting to YCoCg...\n", pInputBaseName );

				pVtf->ConvertImageFormat( IMAGE_FORMAT_DXT5, false, false, true );

				CUtlBuffer dstData;

				pVtf->Serialize( dstData );

				fp = fopen( chFileNameConvert, "wb" );
	...
	if ( g_bPanorama )
	{
		// force mips off for panorama textures, use TEXTUREFLAGS_ALLMIPS in a config file if they are required
		info.m_nFlags |= TEXTUREFLAGS_NOMIP;
	}
	...
	if ( g_nSecondaryTargetReduceFactor != -1 )
	{
		...
		{
			if ( !g_Quiet )
				VTexMsg( "Setting secondary target reduction factor: x%i.\n", g_nSecondaryTargetReduceFactor );

			// apply targeted reduction
			info.m_nReduceX = info.m_nReduceY = g_nSecondaryTargetReduceFactor;
		}
	}
	...
	// multi-res
	if ( g_bPanorama && !CommandLine()->FindParm( "-nomultires" ) )
	{
		// _medium
		info.m_nReduceX *= 2;
		info.m_nReduceY *= 2;
		bProcessedFilesOK = ProcessFiles( pInputBaseName, outputDir, pBaseName, "_medium", info );
		if ( !bProcessedFilesOK )
			return FALSE;
		...
		info.m_nReduceX *= 2;
		info.m_nReduceY *= 2;
		bProcessedFilesOK = ProcessFiles( pInputBaseName, outputDir, pBaseName, "_small", info );
		if ( !bProcessedFilesOK )
			return FALSE;
	}
...
int CVTex::VTex( int argc, char **argv )
{
	...
	if ( g_bUsedAsLaunchableDLL && !CommandLine()->FindParm( "-nop4" ) && bP4DLLExists )
	{
		...
		g_p4 = (IP4 *)fn( P4_INTERFACE_VERSION, NULL );
		if ( !g_p4 )
		{
		...
		{
			g_p4->Connect( FileSystem_GetFactory() );
			g_p4->Init();
		}
	}
	else
	...
	// Shutdown P4
	if ( g_bUsedAsLaunchableDLL && g_p4 )
	{
		g_p4->Shutdown();
		g_p4->Disconnect();
	}
