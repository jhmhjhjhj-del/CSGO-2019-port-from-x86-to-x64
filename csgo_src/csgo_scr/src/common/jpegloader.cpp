...
bool ConvertJpegToRGB( const byte *pubData, int cubData, CUtlVector<byte> &buf, int &width, int &height, int *pcubUsed )
{
	...
	// allocate the memory to read the cdnfile data into. Do this by creating a second
	// tmp vector, because setting size on the passed one is likely to double in size and 
	// if we need something like 69M we may well end up with 132M or such which isn't awesome.
	CUtlVector<byte> vecTmp( 0, mem_required );
	vecTmp.SetSize( mem_required );
	...
	{
		row_pointer[0] = &(vecTmp[cur_row * row_stride]);
...
bool ConvertJpegToRawInternal( const byte *pubJpegData, int cubJpegData, CUtlBuffer &bufOutput, int &width, int &height, int *pcubUsed, bool bMakeRGBA )
{
	...
	{
		VPROF_BUDGET( "ConvertJpegToRGB", VPROF_BUDGETGROUP_STEAMUI );
	...
	if ( bConverted )
	{
		...
		if ( bMakeRGBA )
		{
			VPROF_BUDGET( "ConvertRGBToRGBAImage", VPROF_BUDGETGROUP_STEAMUI );
...
bool ConvertRGBToJpegInternal( CJpegDestMgr &destMgr, int quality, int width, int height, CUtlBuffer &bufRGB )
{
	...
	return true;
#else
    // HACK: those jpeg lib methods arn't linking when built in osx.
