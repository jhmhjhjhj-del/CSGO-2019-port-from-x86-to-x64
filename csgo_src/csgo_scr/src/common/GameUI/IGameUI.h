...
public:
	...
	virtual void CreateCommandMsgBox( const char* pszTitle, const char* pszMessage, bool showOk = true, bool showCancel = false, const char* okCommand = NULL, const char* cancelCommand = NULL, const char* closedCommand = NULL, const char* pszLegend = NULL, const char* szPanelID = nullptr ) = 0;
...
*/	
	...
	virtual void SetBackgroundMusicDesired(bool bPlayMusic, bool bAllowFinishCurrentTrack=false) = 0;

	// Used when loading community maps to require loading screen to be shown 
	virtual void ShowCustomProgress( void ) = 0;

	virtual bool IsInGameUiFrontEndInteractiveState() = 0; // returns true if we want most of main menu functionality (like friends updating, etc.)
