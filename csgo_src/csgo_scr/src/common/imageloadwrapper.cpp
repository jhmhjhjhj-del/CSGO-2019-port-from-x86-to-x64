...
{
	if ( !pubImageData || cubImageData == 0 )
		return false;
	...
	else if( GetSVGDimensions( pubImageData, cubImageData, (uint32&)width, (uint32&)height ) )
	{
		if( ConvertSVGToRGBA( pubImageData, cubImageData, bufOutput, width, height ) )
			bSuccess = BConvertRGBAToRGB( bufOutput, width, height );
	}
...
bool ConvertImageToRGBA( const byte *pubImageData, int cubImageData, CUtlBuffer &bufOutput, int &width, int &height )
{
	...
	{
		bSuccess = ConvertSVGToRGBA( pubImageData, cubImageData, bufOutput, width, height );
	}
	else if ( GetTGADimensions( cubImageData, (char*)pubImageData, &width, &height ) )
	{
		byte *pubRGB = NULL;
