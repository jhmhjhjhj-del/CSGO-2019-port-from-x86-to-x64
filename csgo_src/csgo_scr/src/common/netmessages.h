...
{
public:
	CNetMessageRatelimitPolicyBase()
		: m_dblLastMessageProcessingTime( 0 )
		, m_numMessagesProcessedSinceTick( 0 )
		, m_numTotalBytesProcessedSinceTick( 0 )
		, m_prev_dblLastMessageProcessingTime( 0 )
		, m_prev_numMessagesProcessedSinceTick( 0 )
		, m_prev_numTotalBytesProcessedSinceTick( 0 )
		, m_lifetime_numMessagesProcessed( 0 )
		, m_lifetime_numTotalBytesProcessed( 0 )
	{
	}

	inline void OnIncomingMessage( double dblNetTime, int numBytes, bool bRolloverCurrent )
	{
		++m_lifetime_numMessagesProcessed;
		m_lifetime_numTotalBytesProcessed += numBytes;

		if ( bRolloverCurrent )
		{
			m_prev_dblLastMessageProcessingTime = m_dblLastMessageProcessingTime;
			m_prev_numMessagesProcessedSinceTick = m_numMessagesProcessedSinceTick;
			m_prev_numTotalBytesProcessedSinceTick = m_numTotalBytesProcessedSinceTick;

			// Reset current
			m_dblLastMessageProcessingTime = dblNetTime;
			m_numMessagesProcessedSinceTick = 0;
			m_numTotalBytesProcessedSinceTick = 0;
		}

		++m_numMessagesProcessedSinceTick;
		m_numTotalBytesProcessedSinceTick += numBytes;
	}

protected:
	// Lifetime processing state
	int m_lifetime_numMessagesProcessed;
	int m_lifetime_numTotalBytesProcessed;

	// Previous processing state
	double m_prev_dblLastMessageProcessingTime;
	int m_prev_numMessagesProcessedSinceTick;
	int m_prev_numTotalBytesProcessedSinceTick;

	// Current processing state
	double m_dblLastMessageProcessingTime;
	int m_numMessagesProcessedSinceTick;
	int m_numTotalBytesProcessedSinceTick;
};

class CNetMessageRatelimitPolicyNone : public CNetMessageRatelimitPolicyBase
{
public:
	inline bool BIncomingMessageForProcessing( double dblNetTime, int numBytes )
	{
		bool bRolloverCurrent = ( m_dblLastMessageProcessingTime != dblNetTime );
		OnIncomingMessage( dblNetTime, numBytes, bRolloverCurrent );
		return true;
	}
};
...
{
public:
	CNetMessageRatelimitPolicy( double dblWindowTime, int numWindowMessages, int numWindowBytes )
		: m_dblRatelimitWindowTime( dblWindowTime )
		, m_numRatelimitWindowMessages( numWindowMessages )
		, m_numRatelimitWindowBytes( numWindowBytes )
	{
		Assert( dblWindowTime > 0 );
		Assert( numWindowMessages > 0 || numWindowBytes > 0 );
	}

	inline bool BIncomingMessageForProcessing( double dblNetTime, int numBytes )
	{
		bool bRolloverCurrent = ( !m_dblLastMessageProcessingTime
			|| ( ( dblNetTime - m_dblLastMessageProcessingTime ) > m_dblRatelimitWindowTime )
			|| ( ( dblNetTime - m_dblLastMessageProcessingTime ) < -m_dblRatelimitWindowTime ) );
		OnIncomingMessage( dblNetTime, numBytes, bRolloverCurrent );

		if ( ( m_numRatelimitWindowMessages > 0 ) && ( m_numMessagesProcessedSinceTick > m_numRatelimitWindowMessages ) )
			return false;

		if ( ( m_numRatelimitWindowBytes > 0 ) && ( m_numTotalBytesProcessedSinceTick > m_numRatelimitWindowBytes ) )
			return false;
...
protected:
	...
	const double m_dblRatelimitWindowTime;
	const int m_numRatelimitWindowMessages;
	const int m_numRatelimitWindowBytes;
};
...
public:
	...
	{
		delete m_pBind;
		m_pBind = new BindParams<_N, CNetMessageRatelimitPolicyNone>( pNetChannel, handler, CNetMessageRatelimitPolicyNone() );
	}

	template< class _N >
	void Bind( INetChannel *pNetChannel, CUtlDelegate< bool( const typename _N::PBType_t & obj ) > handler, CNetMessageRatelimitPolicy const &msgRatelimits )
	{
		delete m_pBind;
		m_pBind = new BindParams<_N, CNetMessageRatelimitPolicy>( pNetChannel, handler, msgRatelimits );
	}
...
private:
	template < class _N, class _TRatelimitPolicy >
	struct BindParams : public INetMessageBinder
	{
		BindParams( INetChannel *pNetChannel, CUtlDelegate< bool ( const typename _N::PBType_t & obj ) > handler, _TRatelimitPolicy const &msgRatelimits )
			: m_NetChannel( pNetChannel )
			, m_handler( handler )
			, m_msgRatelimits( msgRatelimits )
		{
			if ( m_NetChannel )
			{
		...
		virtual bool BIncomingMessageForProcessing( double dblNetTime, int numBytes )
		{
			return m_msgRatelimits.BIncomingMessageForProcessing( dblNetTime, numBytes );
		}

		INetChannel	*m_NetChannel;	// netchannel this message is from/for
		CUtlDelegate< bool ( const typename _N::PBType_t & obj ) > m_handler;
		_TRatelimitPolicy m_msgRatelimits;
	};
...
typedef CNetMessagePB< clc_HltvFixupOperatorTick, CCLCMsg_HltvFixupOperatorTick >					CCLCMsg_HltvFixupOperatorTick_t;
...
typedef CNetMessagePB< svc_HltvFixupOperatorStatus, CSVCMsg_HltvFixupOperatorStatus, INetChannelInfo::ENTITIES > CSVCMsg_HltvFixupOperatorStatus_t;
...
// ------------------------------------------------------------------------------------------------
inline void SetMsgVector( CMsgVector* pOutVec, const Vector& inVec )
{
	Assert( pOutVec );
...
inline Vector AsVector( const CMsgVector &vec )
{
	return Vector( vec.x(), vec.y(), vec.z() );
}

// ------------------------------------------------------------------------------------------------
inline void SetMsgQAngle( CMsgQAngle* pOutAng, const QAngle& inAng )
{
	Assert( pOutAng );
...
inline QAngle AsQAngle( const CMsgQAngle &ang )
{
