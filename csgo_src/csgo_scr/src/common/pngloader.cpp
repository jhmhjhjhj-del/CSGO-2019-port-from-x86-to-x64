...
bool ConvertPNGToRGBA( const byte *pubPNGData, int cubPNGData, CUtlBuffer &bufOutput, int &width, int &height )
{
	...
	// Required for interlaced PNGs — without this png_read_image warns and can corrupt.
	png_set_interlace_handling( png_ptr );

	// check channels. for 3 byte RGB channels or single channel palette/gray images, we want
	// to expand to 4 (RGBA), so we specify a constant opaque alpha.
