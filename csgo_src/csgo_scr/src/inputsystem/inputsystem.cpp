...
{
#if ( defined( PLATFORM_WINDOWS_PC ) && defined( PANORAMA_ENABLE ) )
	{ "imemanager", IMEMANAGER_INTERFACE_VERSION },
#endif
	{ NULL, NULL }
};

const AppSystemInfo_t* CInputSystem::GetDependencies()
{
...
#ifdef PANORAMA_ENABLE
#define ShouldGenerateIMEEvents() ( g_pIMEManager == NULL && m_bIMEAllowed )
#else
#define ShouldGenerateIMEEvents() ( m_bIMEAllowed )
#endif
...
LRESULT CInputSystem::WindowProc( HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	if ( ShouldGenerateIMEEvents() && ShouldGenerateUIEvents() && (hwnd != m_hLastIMEHWnd) )
	{
		m_hLastIMEHWnd = hwnd;
	...
	case WM_SYSKEYDOWN:
		{
			...
			if ( ShouldGenerateUIEvents() )
			{
				...
				{
					PostEvent( IE_KeyCodeTyped, m_nLastSampleTick, virtualCode, GetKeyModifierFlags() );
				}
			}
		}
		break;
	...
	case WM_INPUTLANGCHANGE:
		...
		if ( g_pIMEManager )
		{
			g_pIMEManager->HandleIMEEvent( (PlatWindow_t)hwnd, uMsg, wParam, lParam );
		}
#endif
#if defined( INCLUDE_SCALEFORM )
		if ( g_pScaleformUI )
...
#ifdef PANORAMA_ENABLE
		if ( g_pIMEManager && g_pIMEManager->HandleIMEEvent( (PlatWindow_t)hwnd, uMsg, wParam, lParam ) )
			return 0;
#endif
#if defined( INCLUDE_SCALEFORM )
	...
	case WM_IME_SETCONTEXT:
		...
		if ( g_pIMEManager )
		{
			...
			// MUST ALWAYS BE PASSED ON regardless, otherwise Windows IME internals get botched.
			// We draw all IME windows ourselves. Per SF IME Docs section 4.2.
			lParam = 0;
		}
#endif
#if defined( INCLUDE_SCALEFORM )
...
#endif
		else if ( ShouldGenerateUIEvents() )
		{
...
void CInputSystem::InitCursors()
{
	...
	m_pDefaultCursors[INPUT_CURSOR_NONE]		= INPUT_CURSOR_HANDLE_INVALID;
	m_pDefaultCursors[INPUT_CURSOR_ARROW]		= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);
	m_pDefaultCursors[INPUT_CURSOR_IBEAM]		= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_IBEAM);
	m_pDefaultCursors[INPUT_CURSOR_HOURGLASS]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_WAIT);
	m_pDefaultCursors[INPUT_CURSOR_CROSSHAIR]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_CROSSHAIR);
	m_pDefaultCursors[INPUT_CURSOR_WAITARROW]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_WAITARROW);
	m_pDefaultCursors[INPUT_CURSOR_UP]			= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);
	m_pDefaultCursors[INPUT_CURSOR_SIZE_NW_SE]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZENWSE);
	m_pDefaultCursors[INPUT_CURSOR_SIZE_NE_SW]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZENESW);
	m_pDefaultCursors[INPUT_CURSOR_SIZE_W_E]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZEWE);
	m_pDefaultCursors[INPUT_CURSOR_SIZE_N_S]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZENS);
	m_pDefaultCursors[INPUT_CURSOR_SIZE_ALL]	= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZEALL);
	m_pDefaultCursors[INPUT_CURSOR_NO]			= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_NO);
	m_pDefaultCursors[INPUT_CURSOR_HAND]		= (InputCursorHandle_t)SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_HAND);
#endif
}
...
void CInputSystem::SetCursorIcon( InputCursorHandle_t hCursor )
{
	...
	m_pLauncherMgr->SetMouseCursor((SDL_Cursor*)hCursor);
#endif
}
...
bool CInputSystem::IsIMEAllowed() const
{
...
void CInputSystem::SetIMEAllowed( bool bAllowed )
{
	...
	if ( CommandLine()->HasParm( "-noime" ) )
	{
		Assert( m_bIMEAllowed == false );
		return;
	}

	if ( !bAllowed )
	{
		// Make sure to dismiss before disallowing
		// so that the dismiss goes through.
		DismissIME();
	}
	...
	{
		g_pPanoramaUIEngine->SetIMEAllowed( m_bIMEAllowed );
	}
	if ( g_pIMEManager )
	{
		g_pIMEManager->SetIMEEnabled( m_bIMEAllowed );
	}
#endif
}
...
void CInputSystem::DismissIME()
{
	...
	if ( g_pIMEManager )
	{
		// We need to pass bActive as true for this
		// to set nothing active.
		g_pIMEManager->SetActiveUIView( nullptr, true );
	}
#endif
}
