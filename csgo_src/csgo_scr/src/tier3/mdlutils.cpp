...
CMDL::CMDL()
{
	...
	m_DecalHandle = STUDIORENDER_DECAL_INVALID;
...
//-----------------------------------------------------------------------------
// Remove all decals on a CMDL
//-----------------------------------------------------------------------------
void CMDL::RemoveAllDecals()
{
	if ( m_DecalHandle != STUDIORENDER_DECAL_INVALID )
	{
		g_pStudioRender->DestroyDecalList( m_DecalHandle );
		m_DecalHandle = STUDIORENDER_DECAL_INVALID;
	}
}
...
void CMDL::SetUpBones( const matrix3x4_t& rootToWorld, int nMaxBoneCount, matrix3x4_t *pBoneToWorld, const float *pPoseParameters, MDLSquenceLayer_t *pSequenceLayers, int nNumSequenceLayers )
{
	...
	// FIXME: We're always wrapping; may want to determine if we should clamp
	...
	if ( pSequenceLayers )
	{
		...
		for ( int i = 0; i < nNumSequenceLayers; ++i )
		{
			...
			{				
				...
				int nCurFrameCount = MAX( 1, Studio_MaxFrame( &studioHdr, nSeqIndex, pPoseParameters ) );

				float flLayerPlaybackRate = Studio_FPS( &studioHdr, nSeqIndex, pPoseParameters );
				float flLayerAdjustedTime = ( pSequenceLayers[ i ].m_flStartTime > -1.0f ) ? ( m_flTime - pSequenceLayers[ i ].m_flStartTime ) : flAdjustedTime;
				float flLayerCycle = ( flLayerAdjustedTime * flLayerPlaybackRate ) / nCurFrameCount;

				// FIXME: We're always wrapping; may want to determine if we should clamp
				flLayerCycle = SubtractIntegerPart( flLayerCycle );

				boneSetup.AccumulatePose( pos, q, nSeqIndex, flLayerCycle, flWeight, flLayerAdjustedTime, NULL );
			}
		}
	}
	...
	for ( int i = 0; i < nMaxBoneCount; i++ ) 
	{
		...
		if ( ( studioHdr.boneFlags( i ) & BONE_ALWAYS_PROCEDURAL ) &&
			 ( studioHdr.pBone( i )->proctype & STUDIO_PROC_JIGGLE ) &&
			 m_isJiggleBonesEnabled )
		{
			matrix3x4a_t goalMatrix = pBoneToWorld[ i ];

			// get jiggle properties from QC data
			mstudiojigglebone_t *jiggleInfo = (mstudiojigglebone_t *)studioHdr.pBone( i )->pProcedure();
			...
			// do jiggle physics
			m_pJiggleBones->BuildJiggleTransformations( i, flAdjustedTime, jiggleInfo, goalMatrix, pBoneToWorld[ i ], false );
		}
									...
									const matrix3x4_t &matModelToWorld )
{
	...
	for ( int iMergeBone = 0; iMergeBone < pMergeHdr->numbones(); ++iMergeBone )
	{
		...
		if ( iParentBoneIndex >= 0 )
		{
			...
			{
				ConcatTransforms( pFollowBoneToWorld[iParentBoneIndex], matModelToWorld, pMergeBoneToWorld[iMergeBone] );
			}
			else
			{
		...
		if ( !bMerged )
		{
			...
			if ( ( pMergeHdr->boneFlags( iMergeBone ) & BONE_ALWAYS_PROCEDURAL ) &&
				 ( pMergeHdr->pBone( iMergeBone )->proctype & STUDIO_PROC_JIGGLE ) &&
				 m_isJiggleBonesEnabled )
			{
				matrix3x4a_t goalMatrix = pMergeBoneToWorld[ iMergeBone ];

				// get jiggle properties from QC data
				mstudiojigglebone_t *jiggleInfo = (mstudiojigglebone_t *)pMergeHdr->pBone( iMergeBone )->pProcedure();
				...
				// do jiggle physics
				m_pJiggleBones->BuildJiggleTransformations( iMergeBone, flAdjustedTime, jiggleInfo, goalMatrix, pMergeBoneToWorld[ iMergeBone ], false );
			}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CMDL::EnableJiggleBones( void )
{
	m_isJiggleBonesEnabled = true;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CMDL::DisableJiggleBones( void )
{
...
//	m_nNumSequenceLayers = 0;
	m_aSequenceLayers.EnsureCapacity( MAX_SEQUENCE_LAYERS );
}

CMergedMDL::~CMergedMDL()
{
	m_aSequenceLayers.Purge();
	m_aMergeMDLs.Purge();
}
...
void CMergedMDL::SetupBonesForAttachmentQueries( void )
{
	...
	m_RootMDL.m_MDL.SetUpBones( m_RootMDL.m_MDLToWorld, pRootStudioHdr->numbones(), rdBoneToWorld.Base(), m_PoseParameters, m_aSequenceLayers.Base(), m_aSequenceLayers.Count() );
...
void CMergedMDL::Draw()
{
	...
	CStudioHdr *pRootStudioHdr = new CStudioHdr( g_pMDLCache->GetStudioHdr( m_RootMDL.m_MDL.GetMDL() ), g_pMDLCache );
	CMatRenderData< matrix3x4_t > rdBoneToWorld( pRenderContext, pRootStudioHdr->numbones() );
	// don't use pRootMergeHdrModelToWorld, use each merge mdl's m_MDLToWorld transform instead, see notes below, this is an experimental change
	...
	for ( int iMerge = 0; iMerge < nMergeCount; ++iMerge )
	{
		...
		// use merge mdl's own transform instead of previous merged mdl, TODO - sanity check this is okay
		...
		m_aMergeMDLs[iMerge].m_MDL.SetupBonesWithBoneMerge( pMergeHdr, pMergeBoneToWorld, pRootStudioHdr, pFollowBoneToWorld, m_aMergeMDLs[iMerge].m_MDLToWorld );
		...
		if ( m_aMergeMDLs[iMerge].m_bRequestBoneMergeTakeover && ( iMerge + 1 < nMergeCount ) )
		{
			...
			// TODO - check, if it's valid/okay to allow each merge mdl to have it's own mdltoworld transform instead of using the previous merged mdl transform as defined here 
...
void CMergedMDL::SetSequenceLayers( const MDLSquenceLayer_t *pSequenceLayers, int nCount )
{
	if ( pSequenceLayers )
	{
		nCount = MIN( MAX_SEQUENCE_LAYERS, nCount );
		m_aSequenceLayers.EnsureCount( nCount );
		for ( int iLayer = 0; iLayer < nCount; ++iLayer )
		{
			m_aSequenceLayers[ iLayer ] = pSequenceLayers[ iLayer ];
		}
	}
	else
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CMergedMDL::SetLocalTransform( MDLHandle_t handle, matrix3x4_t &matLocalTransform )
{
	int nMergeCount = m_aMergeMDLs.Count();
	...
	for ( int iMerge = 0; iMerge < nMergeCount; ++iMerge )
	{
		if ( m_aMergeMDLs[iMerge].m_MDL.GetMDL() == handle )
		{
			m_aMergeMDLs[iMerge].m_MDLToWorld = matLocalTransform;
			return;
		}
	}
}
