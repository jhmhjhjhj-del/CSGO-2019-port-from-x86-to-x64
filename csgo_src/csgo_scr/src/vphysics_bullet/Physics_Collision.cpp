...
// Deep-copy a collide for DuplicateAndScale. CollideSize/CollideWrite are stubs
// in vphysics_bullet, so serialize/unserialize path cannot be used.
static btCollisionShape *CloneConvexChild( const btCollisionShape *pChild )
{
	if ( !pChild )
		return NULL;

	const int shapeType = pChild->getShapeType();
	if ( shapeType == CONVEX_HULL_SHAPE_PROXYTYPE )
	{
		const btConvexHullShape *pHull = (const btConvexHullShape *)pChild;
		btConvexHullShape *pNewHull = new btConvexHullShape;
		pNewHull->setMargin( pHull->getMargin() );
		const int nPoints = pHull->getNumPoints();
		const btVector3 *pPoints = pHull->getUnscaledPoints();
		for ( int j = 0; j < nPoints; j++ )
		{
			pNewHull->addPoint( pPoints[j], false );
		}
		pNewHull->recalcLocalAabb();
		pNewHull->setUserPointer( pHull->getUserPointer() );
		return pNewHull;
	}

	if ( shapeType == CONVEX_TRIANGLEMESH_SHAPE_PROXYTYPE )
	{
		// Fall back to hull from mesh vertices (triangle meshes are rare for static props).
		const btConvexTriangleMeshShape *pTri = (const btConvexTriangleMeshShape *)pChild;
		btConvexHullShape *pNewHull = new btConvexHullShape;
		pNewHull->setMargin( pTri->getMargin() );
		const int nVerts = pTri->getNumVertices();
		for ( int j = 0; j < nVerts; j++ )
		{
			btVector3 v;
			pTri->getVertex( j, v );
			pNewHull->addPoint( v, false );
		}
		pNewHull->recalcLocalAabb();
		return pNewHull;
	}

	Warning( "DuplicateAndScale: unsupported child shape type %d, skipped\n", shapeType );
	return NULL;
}

static CPhysCollide *ClonePhysCollide( const CPhysCollide *pSrc )
{
	if ( !pSrc || !pSrc->GetCollisionShape() )
		return NULL;

	const btCollisionShape *pShape = pSrc->GetCollisionShape();
	if ( pShape->isCompound() )
	{
		const btCompoundShape *pCompound = (const btCompoundShape *)pShape;
		btCompoundShape *pNewCompound = new btCompoundShape( false );
		pNewCompound->setMargin( pCompound->getMargin() );

		for ( int i = 0; i < pCompound->getNumChildShapes(); i++ )
		{
			btCollisionShape *pNewChild = CloneConvexChild( pCompound->getChildShape( i ) );
			if ( !pNewChild )
				continue;
			pNewCompound->addChildShape( pCompound->getChildTransform( i ), pNewChild );
		}

		if ( pNewCompound->getNumChildShapes() <= 0 )
		{
			delete pNewCompound;
			return NULL;
		}

		CPhysCollide *pOut = new CPhysCollide( pNewCompound );
		pOut->SetMassCenter( pSrc->GetMassCenter() );
		pOut->SetRotationInertia( pSrc->GetRotationInertia() );
		return pOut;
	}

	if ( pShape->isConvex() )
	{
		btCollisionShape *pNewChild = CloneConvexChild( pShape );
		if ( !pNewChild )
			return NULL;

		btCompoundShape *pNewCompound = new btCompoundShape( false );
		pNewCompound->setMargin( pShape->getMargin() );
		pNewCompound->addChildShape( btTransform::getIdentity(), pNewChild );
	...
	Warning( "DuplicateAndScale: unsupported root shape type %d\n", pShape->getShapeType() );
	return NULL;
}
...
void CPhysicsCollision::DuplicateAndScale( vcollide_t *pOut, const vcollide_t *pIn, float flScale )
{
	...
	// pOut may be freshly allocated and uninitialized — only unload if it looks valid.
	if ( pOut->solids )
	{
		VCollideUnload( pOut );
	}
	...
	pOut->descSize = 0;
	pOut->pKeyValues = NULL;
	pOut->pUserData = NULL;
	pOut->solids = NULL;

	if ( pIn->solidCount <= 0 || !pIn->solids )
		return;

	pOut->solids = new CPhysCollide*[pIn->solidCount];
	pOut->solidCount = pIn->solidCount;

	for ( int i = 0; i < pIn->solidCount; i++ )
	{
		pOut->solids[i] = ClonePhysCollide( pIn->solids[i] );
		if ( pOut->solids[i] && flScale != 1.0f )
		{
			CollideSetScale( pOut->solids[i], Vector( flScale, flScale, flScale ) );
			pOut->solids[i]->SetMassCenter( pOut->solids[i]->GetMassCenter() * flScale );
		}
	}

	// Keyvalues text is needed by CreateVPhysics ParseSolid; VCollideLoad may leave descSize=0.
	if ( pIn->pKeyValues )
	{
		const int nBytes = ( pIn->descSize > 0 ) ? (int)pIn->descSize : ( (int)strlen( pIn->pKeyValues ) + 1 );
		if ( nBytes > 0 )
		{
			pOut->pKeyValues = new char[nBytes];
			memcpy( pOut->pKeyValues, pIn->pKeyValues, nBytes );
			pOut->descSize = (unsigned short)nBytes;
		}
	}
}
