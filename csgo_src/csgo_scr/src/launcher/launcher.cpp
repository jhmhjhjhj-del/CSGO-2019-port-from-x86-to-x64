...
#define LAUNCHER_APPSYSTEM( name ) name DLL_EXT_STRING
	...
	// See if we're running panorama
	bool bPanorama = false;
	bool bScaleform = true;

	//
	// August 2018 Panorama default mode, changing opt-in to scaleform
	//
#if !defined( CSTRIKE_STAGING_BUILD )
	// VACnet needs scaleform until the end of the London 2018 major so we can integrate the correct but riskier fix
	// in CL# 4705588.
	CommandLine()->RemoveParm( "-scaleform" );				// No longer allow actual "-scaleform" on the command line
#endif
	CommandLine()->RemoveParm( "-legacyscaleformui" );		// No longer allow "-legacyscaleformui" on the command line

#ifdef PANORAMA_ENABLE
	bScaleform = !!CommandLine()->FindParm( "-scaleform" );
	bPanorama = !bScaleform;

	if ( bPanorama )
	{
		AppSystemInfo_t panoramaInfo[] =
		{
			{ LAUNCHER_APPSYSTEM( "panoramauiclient" ), PANORAMAUI_CLIENT_INTERFACE_VERSION },
			{ LAUNCHER_APPSYSTEM( "panorama" ), PANORAMAUI_ENGINE_INTERFACE_VERSION },
			{ "", "" }
		};

#ifdef WIN32

		// Load XP version of v8, if required. This code can be deleted after XP support ends on 1/1/19		
		SetErrorMode( SEM_FAILCRITICALERRORS );
		
		// Allow to force v8_winxp using command line too
		bool bForceV8Winxp = CommandLine()->HasParm( "-v8_winxp" );
		if ( bForceV8Winxp	||	( LoadModule( "v8.dll" ) == APP_MODULE_INVALID ) )
		{
			LoadModule( "v8_winxp\\v8.dll" );
			LoadModule( "v8_winxp\\v8_libbase.dll" );
			LoadModule( "v8_winxp\\v8_libplatform.dll" );
		}
		else if ( !bForceV8Winxp )
		{
			// We successfully loaded v8.dll, unload it now and let the game pull it
			// in via import lib dependencies. This makes the test load a no-op if it succeeds.
			UnloadModule( "v8.dll");
		}
		
		// Hacky. Reset error mode to 0. GetErrorMode is not supported on Win XP, so can't call it to get
		// the previous error mode, but the previous error mode value is known to be 0 at this point. This is just
                // a temp fix while we maintian XP support till 1/1/19 
		SetErrorMode( 0 );

#endif
		if ( !AddSystems( panoramaInfo ) )
		{
			return false;
		}
	}
	...
	// Load Scaleform either if not running Panorama, or running hybrid client for debug
	if ( bScaleform )
	{
		// SF4 TODO
...
//-----------------------------------------------------------------------------
#ifdef WIN32
extern "C" __declspec(dllexport) int LauncherMain( int nSecure, HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
#elif defined( _PS3 )
...
#ifdef WIN32
	if ( nSecure == 0 )
	{	// If the .exe wants to start insecure then add a param
#if !defined( NO_STEAM )
		if ( !CommandLine()->FindParm( "-insecure" ) )
		{
			CommandLine()->AppendParm( "-insecure", "" );

			char chSecureCode[64] = {};
			sprintf( chSecureCode, "%d", nSecure );
			CommandLine()->AppendParm( "-insecure_forced_by_launcher", chSecureCode );
		}
#endif
	}
#endif
