...
static IMaterial *s_DebugMaterials[kDebugMaterial_Count];
...
void InitializeStandardMaterials()
{
	...
	s_DebugMaterials[kDebugMaterialWireframe] = g_pMaterialSystem->CreateMaterial( "__utilWireframe", pVMTKeyValues );
	s_DebugMaterials[kDebugMaterialWireframe]->IncrementReferenceCount();

	pVMTKeyValues = new KeyValues( "wireframe" );
	pVMTKeyValues->SetInt( "$vertexcolor", 1 );
	pVMTKeyValues->SetInt( "$vertexalpha", 1 );
	pVMTKeyValues->SetInt( "$ignorez", 1 );
	s_DebugMaterials[kDebugMaterialWireframeIgnoreZ] = g_pMaterialSystem->CreateMaterial( "__utilWireframeIgnoreZ", pVMTKeyValues );
	s_DebugMaterials[kDebugMaterialWireframeIgnoreZ]->IncrementReferenceCount();

	pVMTKeyValues = new KeyValues( "unlitgeneric" );
	pVMTKeyValues->SetInt( "$vertexcolor", 1 );
	pVMTKeyValues->SetInt( "$vertexalpha", 1 );
	s_DebugMaterials[kDebugMaterialVertexColor] = g_pMaterialSystem->CreateMaterial( "__utilVertexColor", pVMTKeyValues );
	s_DebugMaterials[kDebugMaterialVertexColor]->IncrementReferenceCount();

	pVMTKeyValues = new KeyValues( "unlitgeneric" );
	pVMTKeyValues->SetInt( "$vertexcolor", 1 );
	pVMTKeyValues->SetInt( "$vertexalpha", 1 );
	pVMTKeyValues->SetInt( "$ignorez", 1 );
	s_DebugMaterials[kDebugMaterialVertexColorIgnoreZ] = g_pMaterialSystem->CreateMaterial( "__utilVertexColorIgnoreZ", pVMTKeyValues );
	s_DebugMaterials[kDebugMaterialVertexColorIgnoreZ]->IncrementReferenceCount();
}
...
void ShutdownStandardMaterials()
{
	...
	for ( int i = 0; i < kDebugMaterial_Count; ++i )
	{
		if ( s_DebugMaterials[i] )
		{
			s_DebugMaterials[i]->DecrementReferenceCount();
			s_DebugMaterials[i] = NULL;
		}
	}
}

//-----------------------------------------------------------------------------
// Gets one of the standard debug materials
//-----------------------------------------------------------------------------
IMaterial* RenderUtil_GetDebugMaterial( EDebugMaterial material )
{
	InitializeStandardMaterials();

	if ( material >= 0 && material < kDebugMaterial_Count )
		return s_DebugMaterials[material];
...
void RenderWireframeSphere( const Vector &vCenter, float flRadius, int nTheta, int nPhi, Color c, bool bZBuffer )
{
	...
	pRenderContext->Bind( bZBuffer ? s_DebugMaterials[kDebugMaterialWireframe] : s_DebugMaterials[kDebugMaterialWireframeIgnoreZ] );
...
void RenderSphere( const Vector &vCenter, float flRadius, int nTheta, int nPhi, Color c, bool bZBuffer, bool bInsideOut )
{
	...
	IMaterial *pMaterial = bZBuffer ? s_DebugMaterials[kDebugMaterialVertexColor] : s_DebugMaterials[kDebugMaterialVertexColorIgnoreZ];
...
void DrawScreenSpaceRectangle( IMaterial *pMaterial, float flDestX, float flDestY, float flW, float flH, float flU0, float flV0, float flU1, float flV1, void *pClientRenderable )
{
	CMatRenderContextPtr pRenderContext( g_pMaterialSystem );
	...
	flDestX = RemapVal( flDestX, 0.0f, 1.0f, -1.0f, 1.0f );
	flDestY = RemapVal( flDestY + flH, 0.0f, 1.0f, 1.0f, -1.0f );
	...
	meshBuilder.TexCoord2f( 0, flU0, flV1 );
	meshBuilder.Position3f( flDestX, flDestY, 0 );
	meshBuilder.Normal3f( 0.0f, 0.0f, 1.0f );
	meshBuilder.TangentS3f( 0.0f, 1.0f, 0.0f );
	meshBuilder.TangentT3f( 1.0f, 0.0f, 0.0f );
	meshBuilder.AdvanceVertex();

	meshBuilder.TexCoord2f( 0, flU0, flV0 );
	meshBuilder.Position3f( flDestX, flDestY + flH, 0 );
	meshBuilder.Normal3f( 0.0f, 0.0f, 1.0f );
	meshBuilder.TangentS3f( 0.0f, 1.0f, 0.0f );
	meshBuilder.TangentT3f( 1.0f, 0.0f, 0.0f );
	meshBuilder.AdvanceVertex();

	meshBuilder.TexCoord2f( 0, flU1, flV0 );
	meshBuilder.Position3f( flDestX + flW, flDestY + flH, 0 );
	meshBuilder.Normal3f( 0.0f, 0.0f, 1.0f );
	meshBuilder.TangentS3f( 0.0f, 1.0f, 0.0f );
	meshBuilder.TangentT3f( 1.0f, 0.0f, 0.0f );
	meshBuilder.AdvanceVertex();

	meshBuilder.TexCoord2f( 0, flU1, flV1 );
	meshBuilder.Position3f( flDestX + flW, flDestY, 0 );
	meshBuilder.Normal3f( 0.0f, 0.0f, 1.0f );
	meshBuilder.TangentS3f( 0.0f, 1.0f, 0.0f );
	meshBuilder.TangentT3f( 1.0f, 0.0f, 0.0f );
	meshBuilder.AdvanceVertex();
