	...
	else
	{
		...
		else
		{
			...
			else if ( m_pRC->m_nForcedMaterialType == OVERRIDE_DEPTH_WRITE || m_pRC->m_nForcedMaterialType == OVERRIDE_SSAO_DEPTH_WRITE )
			{
				...
				static unsigned int nNoCSMWriteVarCache = 0;
				IMaterialVar *pNoCSMWriteVar = ppMaterials[index]->FindVarFast( "$nocsmwrite", &nNoCSMWriteVarCache );
				if ( pNoCSMWriteVar && pNoCSMWriteVar->GetIntValue() == 1 )
					return NULL;
...
// world.
//-----------------------------------------------------------------------------
static void ScreenAlignBone( matrix3x4_t *pPoseToWorld, const mstudiobone_t *pCurBone, 
	const Vector& vecViewOrigin, const Vector& vecViewNormal, const matrix3x4_t &boneToWorld )
{
	...
	else if ( pCurBone->flags & BONE_SCREEN_ALIGN_BILLBOARD )
	{
		vX = vecViewNormal;
		VectorNormalize( vX );
		vZ = Vector( 0, 0, 1 );
...
//-----------------------------------------------------------------------------
void ComputePoseToWorld( matrix3x4_t *pPoseToWorld, studiohdr_t *pStudioHdr, int boneMask, const StudioRenderContext_t* pRC, const matrix3x4_t *pBoneToWorld )
{ 
	...
	if ( !pStudioHdr->pLinearBones() )
	{
		...
		for (int i = 0; i < pStudioHdr->numbones; i++)
		{
			...
			if( pCurBone->flags & (BONE_SCREEN_ALIGN_SPHERE | BONE_SCREEN_ALIGN_CYLINDER | BONE_SCREEN_ALIGN_BILLBOARD) )
			{
				ScreenAlignBone( &pPoseToWorld[i], pCurBone, pRC->m_ViewOrigin, pRC->m_ViewPlaneNormal, pBoneToWorld[i] );
			}
			else
			{
	...
	else
	{
		...
		for (int i = 0; i < pStudioHdr->numbones; i++)
		{
			...
			if( pLinearBones->flags(i) & (BONE_SCREEN_ALIGN_SPHERE | BONE_SCREEN_ALIGN_CYLINDER | BONE_SCREEN_ALIGN_BILLBOARD) )
			{
				const mstudiobone_t *pCurBone = pStudioHdr->pBone( i );
				ScreenAlignBone( &pPoseToWorld[i], pCurBone, pRC->m_ViewOrigin, pRC->m_ViewPlaneNormal, pBoneToWorld[i] );
			}
