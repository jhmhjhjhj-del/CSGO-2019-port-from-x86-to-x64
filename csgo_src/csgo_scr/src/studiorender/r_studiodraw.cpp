...
bool CStudioRender::R_StudioProcessFlexedMesh_StreamOffset( mstudiomesh_t* pmesh, int lod )
{
	...
	if ( !m_VertexCache.SetupComputation( pmesh, true ) )
	{
		ExecuteNTimes( 10, DevWarning( "Could not process flex for model '%s' (it has > 65536 vertices)\n", m_pStudioHdr->pszName() ) );
		return false;
	}
				...
				float r_blend, IMaterial* pMaterial, int lod, ColorMeshInfo_t *pColorMeshes  )
{
	...
	if ( (pGroup->m_Flags & MESHGROUP_IS_DELTA_FLEXED) && m_pRC->m_Config.bFlex )
	{
		...
		if ( bUseSOFlex )
		{
			if ( R_StudioProcessFlexedMesh_StreamOffset( pmesh, lod ) )
			{
	...
	StudioModelLighting_t lighting, IMaterial *pMaterial, int lod )
{
	...
	pRenderContext->SetNumBoneWeights( 0 );
	if ( !m_VertexCache.SetupComputation( pmesh ) )
	{
		ExecuteNTimes( 10, DevWarning( "Could not render eyeball for model '%s' (it has > 65536 vertices)\n", m_pStudioHdr->pszName() ) );
		return 0;
	}
