	...
	OptimizedModel::Vertex_t* pVertex, mstudiomesh_t* pMesh, const mstudio_meshvertexdata_t *vertData, bool hwSkin, bool bExtraUV )
{
	...
	*/
	...
	if (bExtraUV)
	{
		float* uv = GetTexcoord1( vertData, idx );
		
		Vector2D vUV;
		vUV.x = *uv;
		vUV.y = *++uv;

		bool bBadX = !IsFinite( vUV.x );
		bool bBadY = !IsFinite( vUV.y );

		if ( bBadX || bBadY )
		{
			AssertMsgOnce( false, "Bad UV data on static mesh! Zeroing." );
		...
		meshBuilder.TexCoord2fv( 1, vUV.Base() );
	}
	...
	if (vertData->HasTangentData())
	{
		...
		*/
		...
		float flNormalLen = vert.m_vecNormal.Length();
		float flTangentLen = vecTangentS.AsVector3D().Length();
		if ( abs( flNormalLen - flTangentLen ) > 0.05f )
		{
			//AssertMsgOnce( false, "Bad tangent data on mesh! Matching normal." );

			Vector vecNewTangent = vecTangentS.AsVector3D().Normalized() * flNormalLen;
			vecTangentS.x = vecNewTangent.x;
			vecTangentS.y = vecNewTangent.y;
			vecTangentS.z = vecNewTangent.z;
		}
...
void CStudioRenderContext::InvokeBindProxies( IMatRenderContext *pRenderContext, ICallQueue *pCallQueue, const DrawModelInfo_t &info )
{
	...
	for ( int i=0 ; i < info.m_pStudioHdr->numbodyparts; ++i ) 
	{
		...
		for ( int somethingOtherThanI = 0; somethingOtherThanI < pModel->nummeshes; ++somethingOtherThanI)
		{
			...
			for ( int ix = 0; ix < m_RC.m_nForcedMaterialIndexCount; ix++ )
			{
				if ( m_RC.m_nForcedMaterialIndex[ ix ] == nMaterialIndex )
				{
					nOverrideIndex = ix;
					break;
				}
