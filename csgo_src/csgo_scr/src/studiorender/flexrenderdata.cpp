...
bool CCachedRenderData::SetupComputation( mstudiomesh_t *pMesh, bool flexComputation )
{
	Assert((m_Model >= 0) && (m_Body >= 0) && (m_Mesh >= 0));
//	Assert( !m_pFirstIndex );
	...
	if (dict.m_Tag != m_CurrentTag)
	{
		if ( (m_IndexCount + pMesh->numvertices) > MAXSTUDIOVERTS )
		{
			// This model has too many vertices - continuing would overflow the fixed-size vertex cache arrays and cause memory corruption. The caller should emit spew identifying the offending model.
			return false;
		}
