...
#include "tier0/tier1_tables.h"

const uint32 g_Tier1_CRC32Table[NUM_CRC32_TABLE_ENTRIES] =
{
...
const uint64 g_Tier1_CRC64Table[ NUM_CRC64_TABLE_ENTRIES ] =
{
	/*
...
uint32 g_Tier1_BitWriteMasks[32][33];

#if !PLAT_LITTLE_ENDIAN
uint32 g_Tier1_LittleBits[32];
#endif

// (1 << i) - 1
uint32 g_Tier1_ExtraMasks[33];

inline int TableBitForBitnum( int bitNum )
{
    return 1 << ( bitNum & 31 );
}
	...
	CBitWriteMasksInit()
	{
		for( unsigned int startbit=0; startbit < 32; startbit++ )
		{
			for( unsigned int nBitsLeft=0; nBitsLeft < 33; nBitsLeft++ )
			{
				...
				g_Tier1_BitWriteMasks[startbit][nBitsLeft] = TableBitForBitnum(startbit) - 1;
				if(endbit < 32)
					g_Tier1_BitWriteMasks[startbit][nBitsLeft] |= ~(TableBitForBitnum(endbit) - 1);
			}
		}

		for ( unsigned int maskBit=0; maskBit < 32; maskBit++ )
			g_Tier1_ExtraMasks[maskBit] = TableBitForBitnum(maskBit) - 1;
		g_Tier1_ExtraMasks[32] = ~0u;

#if !PLAT_LITTLE_ENDIAN
		for ( unsigned int littleBit=0; littleBit < 32; littleBit++ )
			StoreLittleDWord( (uint32*) &g_Tier1_LittleBits[littleBit], 0, 1u<<littleBit );
#endif
	}
};
