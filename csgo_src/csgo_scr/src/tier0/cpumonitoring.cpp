...
public:
	...
	{
		// On GC we don't want a ton of unnecessary threads with no graceful shutdown code
		const char *pchCmdLineA = Plat_GetCommandLineA();
		if ( strstr( pchCmdLineA, "gc.exe" ) && strstr( pchCmdLineA, "gc.dll" ) && strstr( pchCmdLineA, "-gc" ) )
		{
			if ( !strstr( pchCmdLineA, "-cpumonitoringstart" ) )
				return;
		}
