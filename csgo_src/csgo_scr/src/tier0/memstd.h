...
// CSGO is using around 214 MB SBH when loading into dz_blacksite and releases some memory later
// You can test different SBH sizes by running with -forcesbhsizemb 224, playing a few games
// and making sure that you don't get DRASTIC MEMORY OVERFLOW message in debug output / file
// We give some additional slack for accessing inventory and party systems while in a map and hope
// to avoid expensive compact activity upon failed allocations
#define MBYTES_PRIMARY_SBH 224
// Disabling fallback heap since we tend to run out of address space on 32-bit systems, this saves us 32MB of memory address ranges
#define MEMALLOC_NO_FALLBACK
#endif
