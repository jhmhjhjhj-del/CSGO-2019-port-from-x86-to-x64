...
void ThreadSetDebugName( ThreadHandle_t hThread, const char *pszName )
{
	...
	const uint cThreadId = LookupThreadIDFromHandle( hThread );

#ifdef RAD_TELEMETRY_ENABLED
	// Give Telemetry a chance to have pretty names (currently windows only)
	TelemetryThreadSetDebugName( cThreadId, pszName );
#endif
...
//////////////////////////////////////////////////////////////
   ...
   pthread_mutex_lock( &m_Mutex );
    ...
    int ret = m_bManualReset ? pthread_cond_broadcast( &m_Condition ) : pthread_cond_signal( &m_Condition );
   pthread_mutex_unlock( &m_Mutex );
   return ret == 0;
#endif
...
CThreadSemaphore::CThreadSemaphore( int32 initialValue, int32 maxValue )
{
	...
	if ( iErr != 0 )
	{
		AssertMsg1( false, "%s", strerror( iErr ) );
		return;
	}
...
bool CThreadSemaphore::Wait( uint32 dwTimeout )
{
	...
	int rc;
	if ( ( rc = pthread_mutex_lock( &m_Mutex ) ) )
	{
		Assert( 0 );
		return false;
	}
	...
	if ( rc )
	{
		// Three different possible reasons to return false. Only one of them
	...
	if ( ( rc = pthread_mutex_unlock( &m_Mutex ) ) )
	{
		Assert( 0 );
		return false;
	}
...
bool CThreadSemaphore::Release( int32 releaseCount, int32 *pPreviousCount )
{
   ...
   {
	   if ( ( rc = pthread_cond_signal( &m_Condition ) ) )
	   {
		   Assert( 0 );
		   return false;
	   }
...
bool ThreadInitOnce( ThreadInitOnce_t *pMarker )
{
	for ( ;; )
	{
		// Check the current init state.  If it needs init we
		// swap in the in-init flag to block later callers.
		ThreadInitOnce_t nState = ThreadInterlockedCompareExchange( pMarker, THREAD_INIT_ONCE_IN_INIT, THREAD_INIT_ONCE_NEED_INIT );
		if ( nState == THREAD_INIT_ONCE_NEED_INIT )
		{
			// Caller is the initializer.
			return true;
		}
		else if ( nState == THREAD_INIT_ONCE_IN_INIT )
		{
			// Another thread is currently initializing so wait.
			// Make sure our busy-wait isn't too busy.
			ThreadYield();
		}
		else
		{
			Assert( nState == THREAD_INIT_ONCE_INIT_DONE );
			return false;
		}
	}
}

void ThreadFinishInitOnce( ThreadInitOnce_t *pMarker )
{
	ThreadInterlockedCompareExchange( pMarker, THREAD_INIT_ONCE_INIT_DONE, THREAD_INIT_ONCE_IN_INIT );
}

void ThreadInitOnceCall( ThreadInitOnce_t *pMarker, bool( *InitFunc )(ThreadInitOnce_t*) )
{
	if ( ThreadInitOnce( pMarker ) )
	{
		if ( !InitFunc( pMarker ) )
		{
			Plat_FatalError( "ThreadInitOnceCall initialization failed\n" );
		}

		ThreadFinishInitOnce( pMarker );
	}
}
