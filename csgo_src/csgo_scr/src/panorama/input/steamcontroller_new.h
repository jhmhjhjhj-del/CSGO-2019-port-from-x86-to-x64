...
#ifndef PANORAMASTEAMCONTROLLERNEW_H
#define PANORAMASTEAMCONTROLLERNEW_H
...
#if defined( PANORAMA_PUBLIC_STEAM_SDK ) || defined( SOURCE2_PANORAMA )
...
//
class CSteamGameControllerNew
{
public:
	CSteamGameControllerNew( IUIInput *pInputParent );
	~CSteamGameControllerNew();
	void RunFrame();
	void Shutdown();
	...
	void SetActiveActionSet( const char *pchActionSet );
	const char * GetCurrentActionSet() const;
...
private:
	int InsertEmptyActionSet( const char *pchActionSet );

	bool BIsFingerDownAnalog( GamePadCode code ) const;
	...
	{
		int iAction;
		ControllerDigitalActionHandle_t hAction;
	};

	struct analogData_t
	{
		int iAction;
		ControllerAnalogActionHandle_t hAction;
	};

#define MAX_ANALOG 5

	struct ActionSetData_t
	{
		ControllerActionSetHandle_t m_hActionSet;
		buttonData_t m_buttonData[ XK_MAX_KEYS ];
		analogData_t m_analogData[ MAX_ANALOG ]; // BUGBUG
	};
	...
	struct buttonState_t
	{
		...
		float flFirstInput;
		float flLastInput;
		bool bSuppressUntilUp;
	};

	struct analogState_t
	{
		int nRepeats;
		float flFirstInput;
		float flLastInput;			
	...
	buttonState_t m_buttonState[ XK_MAX_KEYS ];
	analogState_t m_analogState[ MAX_ANALOG ]; // BUGBUG

	CUtlHashMap< CUtlString, ActionSetData_t > m_mapNameToActionSetData;
	int m_iCurrentActionSet;
	...
	int m_unConnectedControllers;


	buttonData_t *GetButtonDataForCurrentActionSet();
};
} // namespace panorama

#endif // PANORAMASTEAMCONTROLLERNEW_H
