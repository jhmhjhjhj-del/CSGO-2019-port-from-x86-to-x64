...
void CMatchEventsSubscription::BroadcastEvent( KeyValues *pEvent )
{
	...
	if ( m_bBroadcasting )
	{
		if ( !m_bAllowNestedBroadcasts )
		{
			...
			// Add diagnostic info when event is delayed
			g_pMMF->DiagnosticInfoAdd( CFmtStr( "mmevent (delaying@%d) %s", m_arrQueuedEvents.Count(), pEvent->GetName() ) );
			return;
		}
	}
	...
	// Add diagnostic info when event is sent to listeners
	g_pMMF->DiagnosticInfoAdd( CFmtStr( "mmevent (bcast@%d) %s", m_arrSentEvents.Count(), pEvent->GetName() ) );
	...
	// Add diagnostic info when event with nested bcasts finishes
	if ( m_arrSentEvents.Count() > 1 )
	{
		g_pMMF->DiagnosticInfoAdd( CFmtStr( "mmevent (finished@%d) %s", m_arrSentEvents.Count(), m_arrSentEvents.Head()->GetName() ) );
	}
