...
#include "../game/shared/cstrike15/cstrike15_gcconstants.h" // $$$REI HACK allow cross-project inclusion of this file
...
static bool UTIL_GenerateMMGame( const char* pchGameMode, EMsgGCCStrike15_v2_MatchmakingGame_t &eMatchmakingGame ) // Copy of UTIL_GenerateMMGame from cstrike15_matchmaking_utils.cpp
{
	if ( !pchGameMode ) {
		return false;
	}
...
#include "../game/shared/cstrike15/cstrike15_gcgamemodes.inc"
#undef GAMEMODEENUM
	else
	{
		return false;
...
void CMatchTitleGameSettingsMgr::ExtendGameSettingsForLobbyTransition( KeyValues *pSettings, KeyValues *pSettingsUpdate, bool bEndGame )
{
	...
	if ( !V_stricmp( "offline", pSettings->GetString( "system/network" ) ) )
	{
		// This is the conversion from offline session into online session
		// we have to also make sure that correct slots are configured on the lobby
		pSettingsUpdate->SetString( "system/network", "LIVE" );
		pSettingsUpdate->SetInt( "members/numSlots", 10 ); // all CS:GO online lobbies are 10-slots
	}
...
void CMatchTitleGameSettingsMgr::InitializeGameSettings( KeyValues *pSettings, const char *szReason )
{
	...
	if ( !Q_stricmp( "searchempty", pSettings->GetString( "options/createreason" ) ) &&
		!pSettings->GetBool( "game/hosted" ) && g_pGameTypes )
	{
	...
	if ( !IsX360() )
	{
		// Add search key as a filter (INetSupport may be unset early / offline)
		static ConVarRef sv_search_key( "sv_search_key" );
		int nBuild = 0;
		if ( g_pMatchExtensions && g_pMatchExtensions->GetINetSupport() )
			nBuild = g_pMatchExtensions->GetINetSupport()->GetEngineBuildNumber();
	...
	if ( KeyValues *kvLocalPlayer = pSettings->FindKey( "members/machine0/player0" ) )
	{
		...
		if ( pFriends )
		{
			...
			{
				if ( pFriends->GetClanByIndex( k ).GetAccountID() == iPlayerClanID )
				{
		...
		{
			g_pMatchExtensions->GetIBaseClientDLL()->DetermineSubscriptionKvToAdvertise( pSettings, szReason );
		}
		...
		// If local player has prime, then initialize the session with it
		// if ( kvLocalPlayer->GetInt( "game/prime" ) && !pSettings->FindKey( "game/prime" ) )
		// Always initialize session with prime even if we boot before we connect to GC
		pSettings->SetInt( "game/prime", 1 );
...
void UpdateAggregateMembersSettings( KeyValues *pFullGameSettings, KeyValues *pUpdate )
{
...
#if defined( NO_STEAM )
	static CSteamID s_mysteamid;
#else
	static CSteamID s_mysteamid = steamapicontext->SteamUser()->GetSteamID();
#endif

	char const *szGameMode = pFullGameSettings->GetString( "game/mode" );
	EMsgGCCStrike15_v2_MatchmakingGame_t eMMGame;
	EPlayerRankTypeID_t eLobbyRankType = k_EPlayerRankTypeID_Glicko_ClassicCompetitive;
	if ( UTIL_GenerateMMGame( szGameMode, eMMGame ) )
	{
		EPlayerRankTypeID_t eProposedRankType = MatchmakingGameToRankTypeID( eMMGame );
		if ( MatchmakingRankTypeIsUserVisible( eProposedRankType ) )
			eLobbyRankType = eProposedRankType;
	}
	...
	for ( int iMachine = 0, numMachines = pFullGameSettings->GetInt( "members/numMachines" ); iMachine < numMachines; ++iMachine )
	{
		...
		for ( int iPlayer = 0, numPlayers = pMachine->GetInt( "numPlayers" ); iPlayer < numPlayers; ++iPlayer )
		{
			...
			EPlayerRankTypeID_t eRankType = ( EPlayerRankTypeID_t )pPlayer->GetInt( "game/ranktype" );
			if ( eRankType == eLobbyRankType && nRanking != 0 )
			{
				nAvgRank += nRanking;
				++ nHaveRank;

				if ( nRanking > nHighestRank )
					nHighestRank = nRanking;
			}
			++ nTotalPlayers;
	...
	if ( !nHaveRank )
	{
		nAvgRank = 0; // Match the logic when player is all by himself: if all players don't have a rank then the entire lobby will not have a rank
	}
	...
	int numSlots = 5;
	if ( !V_stricmp( szGameMode, "cooperative" ) )
		numSlots = 2;
	if ( nTotalPlayers >= numSlots )
		nAPR |= 0x4;

	if ( MatchmakingRankTypeToRankTable( eLobbyRankType ) == k_EPlayerRankTable_PlayerRankPips )
		nAvgRank = nHighestRank * 10;
