...
#define TD_ENTRY( szName, nTD, eDataType, numBytesOffset ) \
	{ \
	int tdEntryNumBytesOffset = (int)numBytesOffset; /* explicit cast to avoid size_t -> int warning */ \
	TitleDataFieldsDescription_t aTDFD = { szName, TitleDataFieldsDescription_t::nTD, TitleDataFieldsDescription_t::eDataType, tdEntryNumBytesOffset }; \
	s_tdfd.AddToTail( aTDFD ); \
	if ( tdEntryNumBytesOffset >= XPROFILE_SETTING_MAX_SIZE ) \
		Warning( "\nnumBytesOffset %d is > XPROFILE_SETTING_MAX_SIZE in TD_ENTRY for %s\n\n", tdEntryNumBytesOffset, szName ); \
	}
