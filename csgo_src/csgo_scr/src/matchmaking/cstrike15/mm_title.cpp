...
InitReturnVal_t CMatchTitle::Init()
{
	if ( IGameEventManager2 *mgr = g_pMatchExtensions->GetIGameEventManager2() )
	{
		...
		mgr->AddListener( this, "player_death", false );
		mgr->AddListener( this, "hltv_replay", false );
...
uint64 CMatchTitle::GetTitleSettingsFlags()
{
	return MATCHTITLE_SETTING_MULTIPLAYER
		...
		| MATCHTITLE_SESSIONS_PERSIST_MAPS
#endif // !CSTRIKE15
	;
}
...
// CS:GO doesn't have developer's commentary
// 	if ( !Q_stricmp( "commentary", pSettings->GetString( "options/play", "" ) ) )
// 		pGameDllReserve->SetString( "map/mapcommand", "map_commentary" );

	if ( !Q_stricmp( "dontmodifylocalsession", pSettings->GetString( "options/play", "" ) ) )
		pGameDllReserve->SetString( "map/mapcommand", "dontmodifylocalsession" );
...
void CMatchTitle::FireGameEvent( IGameEvent *pIGameEvent )
{
	...
	if ( !Q_stricmp( "round_start", szGameEvent ) ||
		...
		!Q_stricmp( "player_death", szGameEvent ) ||
		!Q_stricmp( "hltv_replay", szGameEvent ) ||
	...
	KeyValues *kvSessionSystem = pMatchSession->GetSessionSystemData();
	char const *szSessionType = kvSessionSystem ? pMatchSession->GetSessionSystemData()->GetString( "type", NULL ) : NULL;
	if ( szSessionType && !Q_stricmp( szSessionType, "client" ) )
		return;

	// Don't run this when playing warmup training maps
	if ( g_pMatchExtensions->GetIVEngineClient()->IsHLTV() )
		return;
	bool bModifyLocalSession = true;
	if ( g_pMatchExtensions->GetIVEngineClient()->IsClientLocalToActiveServer() )
	{
		if ( KeyValues *mode = g_pMatchExtensions->GetIVEngineServer()->GetLaunchOptions() )
		{
			...
			{
				if ( !V_stricmp( arg->GetString(), "dontmodifylocalsession" ) )
					bModifyLocalSession = false;
			}
		}
	}
	if ( !bModifyLocalSession )
		return;
