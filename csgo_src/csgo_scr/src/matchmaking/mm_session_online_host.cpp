...
{
	g_pMMF->DiagnosticInfoAdd( CFmtStr( "session_host(%p) created", this ) );
...
CMatchSessionOnlineHost::CMatchSessionOnlineHost( CSysSessionClient *pSysSession, KeyValues *pExtendedSettings ) :
	...
	m_pMatchSearcher( NULL )
{
	g_pMMF->DiagnosticInfoAdd( CFmtStr( "session_host(%p) migration", this ) );
	Assert( m_pSettings );
	...
	MigrateGameSettings( !Q_stricmp( pExtendedSettings->GetString( "migrate" ), "fullpostinit" ) );
...
{
	g_pMMF->DiagnosticInfoAdd( CFmtStr( "session_host(%p) closed", this ) );
...
void CMatchSessionOnlineHost::Command( KeyValues *pCommand )
{
	...
	g_pMMF->DiagnosticInfoAdd( CFmtStr( "session_host(%p) command %s (%s)", this, szCommand, szRun ) );
...
void CMatchSessionOnlineHost::OnRunCommand( KeyValues *pCommand )
{
	...
	if ( !Q_stricmp( "StartListenServer", szCommand ) )
	{
...
void CMatchSessionOnlineHost::OnEndGameToLobby( int nEndValue /* = 0 */ )
{
	m_eState = STATE_ENDING;

	// Update session settings
	KeyValues *kv = KeyValues::FromString(
		"updatedelete",
		" update { "
			" system { "
				" lock endgame "
	...
	if ( nEndValue )
	{
		kv->SetInt( "update/options/endvalue", nEndValue );
	}

	// Remove server information
	UpdateSessionSettings( KeyValues::AutoDeleteInline( kv ) );
...
void CMatchSessionOnlineHost::OnEvent( KeyValues *pEvent )
{
	...
	if ( !Q_stricmp( "OnEngineClientSignonStateChange", szEvent ) )
	{
			...
			iNewState  < SIGNONSTATE_CONNECTED )
		{
			...
			else if ( m_eState == STATE_ENDING )
			{
				...
				int nEndValue = m_pSettings->GetInt( "options/endvalue" );
				...
				KeyValues *kvTransitionNotificationEvent = new KeyValues( "OnMatchSessionUpdate", "state", "ready", "transition", "hostendgame" );
				if ( nEndValue )
					kvTransitionNotificationEvent->SetInt( "endvalue", nEndValue );
				g_pMatchEventsSubscription->BroadcastEvent( kvTransitionNotificationEvent );
			}
		}
	}
	...
	else if ( !Q_stricmp( "OnEngineEndGame", szEvent ) )
	{
		if ( m_eState == STATE_LOADING || m_eState == STATE_GAME )
		{
			OnEndGameToLobby( pEvent->GetInt( "endvalue" ) );
		}
	}
...
void CMatchSessionOnlineHost::MigrateGameSettings( bool bFullPostInit )
{
	...
	case STATE_LOBBY:
		{
			...
			KeyValues *kvFix =
				bFullPostInit
				? // smooth upconversion offline>live, don't affect MM queue states
				KeyValues::FromString(
				"updatedelete",
				" update { "
				" } "
				)
				: // real migration client>host, make us no longer matchmaking
				KeyValues::FromString(
				"updatedelete",
				" update { "
					" system { "
...
void CMatchSessionOnlineHost::InitializeGameSettings()
{
	...
	if ( KeyValues *pMembers = m_pSettings->FindKey( "members", true ) )
	{
		...
		if ( KeyValues *pMachine = pMembers->FindKey( "machine0", true ) )
		{
			...
			pMachine->SetUint64( "dlcmask", g_pMatchFramework->GetMatchSystem()->GetDlcManager()->GetDLCMask() );
