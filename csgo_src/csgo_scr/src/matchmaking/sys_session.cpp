...
#if !defined( NO_STEAM )
static bool Helper_UserOwnsFullMultiplayerLicense()
{
	...
	if ( !bPerfectWorld ) // CS:GO F2P -- always enabled, license is free
		return true;

	bool bMultiplayerLicenseFound = steamapicontext->SteamApps()
		&& ( steamapicontext->SteamApps()->BIsSubscribedApp( nPublicSteamApp ) ||
		( bPerfectWorld && steamapicontext->SteamApps()->BIsSubscribedApp( nPublicPwApp ) ) );

	return bMultiplayerLicenseFound;
}
#endif
	...
	m_ullCrypt( 0 )
{
	...
	{	// Offline>online upconversion should not send a notification about leader changed
		// Send a notification
		KeyValues *kvEvent = new KeyValues( "OnPlayerLeaderChanged" );
		kvEvent->SetString( "state", "host" );
		kvEvent->SetUint64( "xuid", m_xuidMachineId );
...
void CSysSessionHost::UpdateStateInit()
{
	...
	if ( Helper_UserOwnsFullMultiplayerLicense() )
	{
	...
	{
		LobbyCreated_t cbck;
		V_memset( &cbck, 0, sizeof( cbck ) );
		cbck.m_eResult = k_EResultFail;
		Steam_OnLobbyCreated( &cbck, true );
	}
#endif
}
...
void CSysSessionHost::Migrate( KeyValues *pCommand )
{
	...
	if ( Q_stricmp( pCommand->GetString( "event" ), "suppressed" ) )
	{
		// Prepare the update notification
		KeyValues *kv = new KeyValues( "mmF->SysSessionUpdate" );
		kv->SetPtr( "syssession", this );
...
void CSysSessionClient::UpdateStateToFailedAndNotify()
{
	m_eState = STATE_FAIL;
...
bool CSysSessionClient::Update()
{
	...
	case STATE_INIT:
		...
		if ( Plat_FloatTime() > m_flInitializeTimestamp + mm_session_sys_connect_timeout.GetFloat() )
		{
			UpdateStateToFailedAndNotify();
		}
		else if (
#if !defined( NO_STEAM )
			( IsSteamRankingConfigured() || ( Plat_FloatTime() >= m_flInitializeTimestamp + mm_session_sys_ranking_timeout.GetFloat() ) ) &&
	...
	case STATE_JOIN_LOBBY:
		...
		else
		{
			LobbyEnter_t cbck;
			V_memset( &cbck, 0, sizeof( cbck ) );
			cbck.m_ulSteamIDLobby = m_lobby.m_uiLobbyID;
			cbck.m_EChatRoomEnterResponse = k_EChatRoomEnterResponseLimited;
			Steam_OnLobbyEntered( &cbck );
		}
		break;
#endif
...
void CSysSessionClient::UpdateStateInit()
{
	...
	if ( m_lobby.m_uiLobbyID )
	{
		// Only transition into state to join lobby if we actually have a reasonable lobby ID
