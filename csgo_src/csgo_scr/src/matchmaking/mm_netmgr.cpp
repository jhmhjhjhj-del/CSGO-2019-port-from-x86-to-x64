...
KeyValues * CConnectionlessLanMgr::UnpackPacket( netpacket_t *packet )
{
	...
	if ( !packet->message.ReadBytes( m_buffer.Base(), nDataLen ) )
		return NULL;
	...
	{
		// Remove binary keys in case they are malicious
		KeyValues *kvPtr = pMsg->FindKey( "binary" );
		int nBinarySizeInKv = kvPtr ? kvPtr->GetInt( "size" ) : 0;
		if ( kvPtr )
		{
			pMsg->RemoveSubKey( kvPtr );
			kvPtr->deleteThis();
		}

		// Read binary data as well
		int nBinarySize = packet->message.ReadLong();
		if ( ( nBinarySize != nBinarySizeInKv ) || ( nBinarySize < 0 ) || ( nBinarySize >= 1024 ) )
		{
			pMsg->deleteThis();
			return NULL;
		}
		if ( nBinarySize > 0 )
		{
			...
			if ( !packet->message.ReadBytes( m_buffer.Base(), nBinarySize ) )
			{
				pMsg->deleteThis();
				return NULL;
			}
			...
			pMsg->SetInt( "binary/size", nBinarySize );
		}
...
void CConnectionlessLanMgr::SendPacket( KeyValues *pMsg, char const *szAddress /*= NULL*/, INetSupport::NetworkSocket_t eSock )
{
	...
	g_pMatchExtensions->GetINetSupport()->SendPacketToNsAddress( eSock,
