...
static uint32 s_nDiagTrackRunFrame;
void CMatchFramework::RunFrame()
{
	++ s_nDiagTrackRunFrame;
...
bool CMatchFramework::ApplySettings( KeyValues* keyValues )
{
	return g_pMatchExtensions->GetIServerGameDLL()->ApplyGameSettings( keyValues );
}
...
// Diagnostic tracking of various things
struct MmDiagInfoEntry_t
{
	int m_unSize;
	uint32 m_unFrameCounter;
	uint32 m_msTimeReceived;
	char *m_pszInfo;
	MmDiagInfoEntry_t *m_pNext;
};
struct MmDiagInfoAll_t
{
	int m_numEntries;
	int m_unSizeTotal;
	MmDiagInfoEntry_t *m_pHead;
	MmDiagInfoEntry_t *m_pTail;
}
g_mmdiaginfo;

static char g_szDiagnosticInfoBuffer[6*1024];

DEVELOPMENT_ONLY_CONVAR( mm_diag_enable, 0 );
void CMatchFramework::DiagnosticInfoAdd( char const *szReportLine )
{
	MmDiagInfoEntry_t *pNew = new MmDiagInfoEntry_t;
	pNew->m_unFrameCounter = s_nDiagTrackRunFrame;
	pNew->m_msTimeReceived = Plat_MSTime();
	pNew->m_unSize = V_strlen( szReportLine );
	pNew->m_pszInfo = strdup( szReportLine );
	pNew->m_pNext = NULL;

	if ( g_mmdiaginfo.m_pTail )
		g_mmdiaginfo.m_pTail->m_pNext = pNew;
	else
		g_mmdiaginfo.m_pHead = pNew;

	g_mmdiaginfo.m_pTail = pNew;
	g_mmdiaginfo.m_unSizeTotal += pNew->m_unSize;
	++ g_mmdiaginfo.m_numEntries;

	while ( ( g_mmdiaginfo.m_pHead != g_mmdiaginfo.m_pTail ) && ( ( g_mmdiaginfo.m_unSizeTotal > 6 * 1024 ) || ( g_mmdiaginfo.m_numEntries > 100 ) ) )
	{
		MmDiagInfoEntry_t *pOld = g_mmdiaginfo.m_pHead;
		g_mmdiaginfo.m_pHead = pOld->m_pNext;

		free( pOld->m_pszInfo );
		-- g_mmdiaginfo.m_numEntries;
		g_mmdiaginfo.m_unSizeTotal -= pOld->m_unSize;
		delete pOld;
	}

	if ( mm_diag_enable.GetBool() )
	{
		Msg( "F%u@%u\n%s\n", s_nDiagTrackRunFrame, Plat_MSTime(), szReportLine );
	}
}

char const * CMatchFramework::DiagnosticInfoGet()
{
	uint32 msTimeNow = Plat_MSTime();
	
	g_szDiagnosticInfoBuffer[0] = 0;
	CUtlBuffer buf( g_szDiagnosticInfoBuffer, sizeof( g_szDiagnosticInfoBuffer ), CUtlBuffer::TEXT_BUFFER );

	for ( MmDiagInfoEntry_t *p = g_mmdiaginfo.m_pHead; p; p = p->m_pNext )
	{
		buf.Printf( "F-%u@-%u\n%s\n", s_nDiagTrackRunFrame - p->m_unFrameCounter, msTimeNow - p->m_msTimeReceived, p->m_pszInfo );
	}
	
	g_szDiagnosticInfoBuffer[ V_ARRAYSIZE( g_szDiagnosticInfoBuffer ) - 1 ] = 0;
	return g_szDiagnosticInfoBuffer;
}
