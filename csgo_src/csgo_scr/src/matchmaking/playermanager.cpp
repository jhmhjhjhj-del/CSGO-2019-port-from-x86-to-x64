...
#ifdef PLATFORM_OSX
// amortizing the cost of player friend updates on OSX - with a couple of hundred friends we end up locking the game client for a couple of seconds on OSX by updating all at once
static ConVar mm_player_search_update_interval( "mm_player_search_update_interval", "2", FCVAR_DEVELOPMENTONLY, "Interval between players searches." );
static ConVar mm_player_search_update_limit( "mm_player_search_update_limit", "20", FCVAR_DEVELOPMENTONLY, "How many friend updates to perform per update interval (round robin)." );
#else
...
#ifdef PLATFORM_OSX
	m_nLastFriendQueried( 0 ),
	m_flStartFriendListScanTimestamp( 0.0f ),
#endif
	m_searchesPending( 0 ),
...
#ifdef PLATFORM_OSX
		if ( player.GetUpdateTimestamp() < m_flStartFriendListScanTimestamp )
		{
...
#ifdef PLATFORM_OSX
						int nFirstFriendToQuery = ( m_nLastFriendQueried >= numFriends ) ? 0 : m_nLastFriendQueried;
						int nLastFriendToQuery = MIN( numFriends, m_nLastFriendQueried + mm_player_search_update_limit.GetInt() );
						m_nLastFriendQueried = ( nLastFriendToQuery == numFriends ) ? 0 : nLastFriendToQuery;

						float flTime = Plat_FloatTime();
						if ( nFirstFriendToQuery == 0 )
						{
							MarkOldFriends();
							RemoveOldFriends();
							m_flStartFriendListScanTimestamp = flTime;
						}
#else
 						int nFirstFriendToQuery = 0;
 						int nLastFriendToQuery = numFriends;
#endif

						for ( int index = nFirstFriendToQuery; index < nLastFriendToQuery; ++index )
						{
...
#ifdef PLATFORM_OSX
								player->SetUpdateTimestamp( flTime );
#endif
...
#endif
		( !IsLocalClientConnectedToServer() || ( g_pMatchExtensions->GetIGameUI() && g_pMatchExtensions->GetIGameUI()->IsInGameUiFrontEndInteractiveState() ) ) )
	{
#ifndef PLATFORM_OSX
		MarkOldFriends();
#endif
