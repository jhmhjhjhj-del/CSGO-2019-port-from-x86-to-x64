...
{
	KeyValues* kvSessionData = g_pMatchFramework->GetMatchSession()->GetSessionSystemData();
	char chSession[64];
	sprintf( chSession, "$%llx",
		uiReservationCookieOverride ? uiReservationCookieOverride :
		kvSessionData ? kvSessionData->GetUint64( "xuidReserve", 0ull ) :
		0ull );
