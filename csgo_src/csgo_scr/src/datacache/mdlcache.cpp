...
struct studiodata_t
{
	...
	// additional scaled vcollide models
	CUtlVector<CStudioVCollide*> m_vecScaledVCollides;
...
public:
	...
	virtual vcollide_t *GetVCollide( MDLHandle_t handle, float flScale ) { return GetVCollideEx( handle, true, flScale ); }
	virtual vcollide_t *GetVCollideEx( MDLHandle_t handle, bool synchronousLoad = true, float flScale = 1.0f );
...
void CMDLCache::DestroyVCollide( studiodata_t *pStudioData )
{
	if ( pStudioData->m_nFlags & STUDIODATA_FLAGS_VCOLLISION_LOADED )
	{
		...
		if ( pStudioData->m_pVCollide )
		{
			...
			FOR_EACH_VEC_BACK( pStudioData->m_vecScaledVCollides, i )
			{
				pStudioData->m_vecScaledVCollides[i]->Release();
				pStudioData->m_vecScaledVCollides[i] = NULL;
			}
			pStudioData->m_vecScaledVCollides.RemoveAll();
		}
	}
}
...
// Unserializes the PHY file associated w/ models (the vphysics representation)
//-----------------------------------------------------------------------------
vcollide_t *CMDLCache::GetVCollideEx( MDLHandle_t handle, bool synchronousLoad /*= true*/, float flScale /* = 1.0f */ )
{
	...
	if ( flScale > 0 && flScale != 1.0f )
	{
		
		FOR_EACH_VEC( pStudioData->m_vecScaledVCollides, i )
		{
			// if a vcollide for this scale already exists, return it.
			if ( pStudioData->m_vecScaledVCollides[i]->m_flScale == flScale )
			{
				return pStudioData->m_vecScaledVCollides[i]->GetVCollide();
			}
		}
		
		// A vcollide for this scale does not exist; create it now.

		CStudioVCollide *pNewVCollide = new CStudioVCollide;
		pNewVCollide->m_flScale = flScale;

		g_pPhysicsCollision->DuplicateAndScale( pNewVCollide->GetVCollide(), pStudioData->m_pVCollide->GetVCollide(), pNewVCollide->m_flScale );

		pStudioData->m_vecScaledVCollides[pStudioData->m_vecScaledVCollides.AddToTail()] = pNewVCollide;

		return pNewVCollide->GetVCollide();
	}
...
bool CMDLCache::GetVCollideSize( MDLHandle_t handle, int *pVCollideSize )
{
	...
	*pVCollideSize += pCollide->descSize;
	...
	// add size of scaled vcollides
	FOR_EACH_VEC( pStudioData->m_vecScaledVCollides, i )
	{
		vcollide_t *pScaledCollide = pStudioData->m_vecScaledVCollides[i]->GetVCollide();
		for ( int j = 0; j < pScaledCollide->solidCount; j++ )
		{
			*pVCollideSize += g_pPhysicsCollision->CollideSize( pScaledCollide->solids[j] );
		}
		*pVCollideSize += pScaledCollide->descSize;
	}
	...
	case MDLCACHE_VCOLLIDE:
		{
			...
			if ( cacheData.Data() )
			{
				...
				if ( ( header.size == sizeof( header ) ) && header.solidCount > 0 )
				{
					...
					pStudioDataCurrent->m_pVCollide->m_flScale = 1.0f;
...
int CMDLCache::ProcessPendingAsync( intp iAsync )
{
	...
	// [JIRA CSGO-2382] Threading issue warning
	// ----------------------------------------
	// It is possible for ProcessPendingAsync and ClearAsync to run concurrently for the same 
	// MDLHandle_t & MDLCacheDataType_t which could lead to a double free of pData (returned
	// by g_pFullFileSystem->AsyncGetResult) (Note that only m_PendingAsyncs is protected by
	// a mutex)
	//
	// Scenario (In the same frame):
	//   1 - Main Thread: call to g_pMDLCache->Release( hMDL ), ref count will go from 1 to 0
	//       => hMDL pushed onto m_UnloadHandles (in CMDLCache::ShutdownStudioData and processed
	//          on the render thread)
	//   2 - Main Thread: g_pMDLCache->FindMDL(...) returning the same MDL handle
	//       => ref count going from 0 to 1
	//   3 - Concurrently:
	//         a - Main Thread: CMDLCache::GetHardwareData->...->CMDLCache::LoadVertexData->CMDLCache::ProcessPendingAsync
	//         b - Render Thread: Processing m_UnloadHandles list (in CMDLCache::UnloadQueuedHardwareData)
	//             eventually calling ClearAsync with the abort flag set to true
	//       => Double free of pData returned by g_pFullFileSystem->AsyncGetResult
